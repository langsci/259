...
\eal
\ex[]{
We exected [all students] [to understand].
}
\ex[]{
We expected [to understand] [all those who attended the class]. \label{HNPS}
}
\ex[]{ 
We expected [that [all those who atended the class] understand].
}
\ex[*]{
...
\inlinetodostefan{to be revised, is this true?}
%\eal
%\label{rsg-two}
...
the following description. The complement of \word{pans} must have an \xarg coindexed with the subject of \word{pans}, but its SUBJ list is not constrained: it can be a clausal complement (whose \subjl value is the empty list) or a VP complement (whose \subjl value is not the empty list).
...
\eal
\ex \type{subj-rsg-verb} \impl \begin{avm} \[arg-st & \@1 \append \<\[x-arg & \@1\]\>\] \end{avm} 
\ex \type{subj-cont-verb} \impl \begin{avm} \[arg-st & \<NP$_{\@i}$, ...\[x-arg & \<\[ind & \@i\]\>\]\>\] \end{avm}
\zl
...
prd & $+$\]\>\] \end{avm}
\ex \type{obj-pred-v} \impl \begin{avm} \[arg-st & \<\NP\> \append \@1 \append \<\[subj & \@1\\
prd & $+$\]\>\] \end{avm}
\end{exe}
...
Subject inversion is handled by a subtype of head-subject-complement phrase, which is independently needed for verb initial languages like \ili{Welsh} \parencites[]{Borsley}[]{SWB2003a}. It is a specific (non"=binary) construction, of which other constructions such as \type{polar-interrogative-clause} are subtypes, and whose head must be [\textsc{inv} $+$].  
