...
\begin{exe}
\ex \begin{xlist}
\ex \gll Marie dit {ne pas} \^etre convaincue. [\ili{French}] \\
...
As observed by \citet{PollardandSag1994}\addpages, the putative correspondence between source and target for raising structures is not systematic: \word{seem} may take a sentential complement (with an expletive subject) (\ref{seem4}) but the other subject-raising verbs (aspectual and modal verbs) do not. 
...
\eal
\ex 
\gll Mig vantar peninga. (\ili{Icelandic}, \citealp[\page 138--139]{PollardandSag1994})\\
...
complement, and not with a sentential complement \parencites[\page 423]{Bresnan1982}{PollardandSag1994}\addpages: this shows that \word{expect} has two complements in (\ref{ex-we-expect-paul-to-unterstand}) and only one in (\ref{ex-we-expect-that-paul-understands}).
...
\label{german2}
\ex 
\gll Der Wächter  sah den Einbrecher     und seinen Helfer            einen       / *  einer nach dem anderen weglaufen [\ili{German}]\\
...
\ex
\label{subj-cont-lx}
\type{subj-cont-lx}	\impl \argst
\avmtmp{<NP$_{\tag{i}}$, \ldots[subj & <[ind & \tag{i}]>]>}
\ex
\type{obj-cont-lx} \impl  \argst
\avmtmp{<\0, \1 [ind & \tag{i}], [subj & <[ind &\tag{i}]>]>}
\zl
...
\ex \begin{xlist}
\ex 
\gll Janek chce byé miły. [\ili{Polish}]\\
...
\eal
\ex[]{ 
\gll To pou kontign ou aret bwar? [\ili{Mauritian}] \\
...
\ili{Balinese} also has subject-raising verbs like \word{ngenah} `seem':
...
\emph{look like}: \argst \avmtmp{<\1, S[xarg & \1]>}
...
The types for subject-raising and subject-control verbs in (\ref{rais-1}) and (\ref{subj-cont-lx})\inlinetodostefan{Stefan: cont-1 was not defined, replaced it. Check. Remove this
  bubble, if OK. rais-1 and cont-1 were subj-cont-lx and are now subj-cont-verb. Should it be -lx
  here as well?} will thus be revised as follows:\\
\eal
\ex \type{subj-rsg-verb} \impl \argst
\avmtmp{
	\1 \+ <[x-arg & \1]>
}
\ex \type{subj-cont-verb} \impl \argst
\avmtmp{
	<NP$_{\tag{i}}$, \ldots [x-arg &	<[ind & \tag{i}]> ] >
}
\zl
...
\ex 
\gll Ihm wurde schlecht [\ili{German}] \citep[\page 72]{Mueller2002b}\label{german3}\\
     him.\DAT{} got bad\\
\glt `He got sick.'
\zl
...
\begin{exe}
\ex  {}[\sub{NP} e] be [\sub{S} John sick] $\leadsto$  [\sub{NP} John ] is  [\sub{S} $e_{i}$ sick] \label{rais2}
...
\begin{exe} 
\ex	\type{subj-pred-v}	\impl \argst
\avmtmp{
	\1 \+ <[subj & \1 \\
			prd & $+$]>
}
\ex \type{obj-pred-v} \impl \argst
\avmtmp{
	<NP> \+ \1 \+	<[subj & \1 \\
					prd & $+$]>
}
...
\footnote{See \citew{KimandSag2002} for a comparison of \ili{French} and \ili{English} auxilaries, \citew{AG2002b-u} for a thorough analysis of \ili{French} auxiliaries as ``generalized'' raising verbs, inheriting not only the subject but also any complement from the past participle; such generalized raising was first suggested by \citet{HN89a,HN94a} for \ili{German} and has been adopted since in various analyses of verbal complexes in \ili{German} \citep{Kiss95a,Meurers2000b,Kathol2001a,Mueller99a,Mueller2002b}, \ili{Dutch} \citep{BvN98a} and \ili{Persian} \citep[Section~4]{MuellerPersian}. See also \crossrefchaptert{complex-predicates}.}
