...
Mauritian is a \ili{French}-based creole, which has raising and control verbs, belonging roughly to the same semantic classes as in English or \ili{French}. Verbs marking aspect or
...
Unlike in \ili{French}, its superstrate, in Mauritian,  verbs neither inflect for tense, mood and aspect nor for person, number, and
...
\end{exe}
 ...
 See also \citew[\page 408--409]{Sag2007a} and \citew{KaySag2009} for the obligatory control of possessive determiners in English expressions such as \emph{keep one's cool}, \emph{lose one's temper}, with an \xarg feature on nouns and NPs:
\begin{exe}
\ex \begin{xlist}
...
A copular verb thus takes any subject (or object) allowed by the predicate: \emph{be} can take a PP subject in English (\ref{under2}), \emph{werden} takes no subject when combined with a subjectless predicate like \emph{schlecht} (bad) in \ili{German} (\ref{german3}):
...
case in \ili{African American Vernacular English} (AAVE)\il{English!African American Vernacular} \citep{Bender2001a}, in \ili{French} \citet{Laurens2008} and creole languages
...
\footnote{See \citew{KimandSag2002} for a comparison of \ili{French} and English auxilaries, \citew{AG2002b-u} for a thorough analysis of \ili{French} auxiliaries as ``generalized'' raising verbs, inheriting not only the subject but also any complement from the past participle; such generalized raising was first suggested by \citet{HN89a,HN94a} for \ili{German} and has been adopted since in various analyses of verbal complexes in \ili{German} \citep{Kiss95a,Meurers2000b,Kathol2001a,Mueller99a,Mueller2002b}, \ili{Dutch} \citep{BvN98a} and \ili{Persian} \citep[Section~4]{MuellerPersian}. See also \crossrefchaptert{complex-predicates}.}
