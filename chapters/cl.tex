...
(for an overview, see \crossrefchapteralt{chap-evolution}).
...
(\eg\ the \isi{shuffle operator} \citep{Reape94a}),
...
(see Section~\ref{cl:resources} below for some examples).\footnote{%
	There are implementations of Minimalism, notably \citealt{Stabler97a-u} and \citealt{Herring:16}. (See also \citealt{Tor:Sta:Ste:19} for a recent broad-coverage, treebank-trained\is{treebank} parser in this framework.)
...
A second important property of HPSG that supports a strong connection between theoretical and computational work is an interest in both so-called ``\isi{core}'' and so-called ``\isi{peripheral}'' phenomena. Most implemented grammars are built with the goal of handling naturally occurring text.\footnote{It is possible, but less common, to do implementation work strictly against test suites of sentences constructed specifically to focus on phenomena of interest.} This means that they will need to handle a wide variety of linguistic phenomena not always treated in theoretical syntactic work \citep{Baldwin-et-al-05}. A syntactic framework that excludes research on ``peripheral'' phenomena as uninteresting provides less support for implementational work than does one, like HPSG or \isi{Construction Grammar}, that values such topics (for a comparison of HPSG and Construction Grammar, see \crossrefchapteralt{cxg}).

Finally, the \isi{type hierarchy} characteristic of HPSG lends itself well to developing broad-coverage grammars which are maintainable\is{maintability!of grammars} over time (see \citealt{Syg:Win:11}). The use of the type hierarchy to manage complexity\is{complexity!of grammars} at scale comes out of the work of \citet{Flickinger87} and others at \is{HP labs} in the project where HPSG was originally developed. The core idea is that any given constraint is (ideally) expressed only once on types which serve as supertypes to all entities that bear that constraint.\footnote{Originally this only applied to \isi{lexical entries} in Flickinger's work. Now it also applies to \isi{phrase structure rules}, \isi{lexical rules}, and types below the level of the sign which are used in the definition of all of these. See \crossrefchapteralt{chap-evolution} for further discussion.} Such constraints might represent broad generalizations that apply to many entities or relatively narrow, idiosyncratic properties that apply to only a few. By isolating any given constraint on one type (as opposed to repeating it in multiple places), we build grammars that are easier to update and adapt in light of new data that require refinements to constraints. Having a single locus for each constraint also makes the types a very useful target for documentation\is{documentation!of grammars} \citep{Hashimoto-etal:07} and grammar exploration \citep{Letcher:18}. 
...
memory\is{complexity!memory} and how much computational time\is{complexity!time} 
...
in technical terminology, the formalism is \textit{Turing-complete}\is{Turing-complete}
...
such as Combinatory Categorial Grammar (CCG),\footnote{%
	...
	For a comparison with HPSG, see \crossrefchapteralt{cg}.
}
...
For \is{\delphin\ Consortium}\delphin\ grammars (see Section \ref{cl:delphin}),
the answer is yes.
...
A \isi{lists}list is a simple example,\footnote{%
...
For example, selecting \isi{heads} might place constraints on their \is{complements}complements' \isi{subjects}
(\eg\ in raising/control constructions),\isi{raising}\isi{control}
...
The only lists that constrain grammaticality are \isi{valence lists},
but in practical grammars these are never greater than length four or five.\footnote{%
	...
	adopt proposals like the \textsc{deps}\isfeat{deps} list of \citet*{BMS2001a-unlinked}.
	Furthermore, in many \is{\delphin\ Consortium}\delphin\ grammars, including the ERG,\is{English Resource Grammmar (ERG)}
...
it turns out that the average-case complexity\isi{complexity!average-case} is much better than might be expected \citep{Carroll94}.
...
and in fact, the 2018 version of the \is{English Resource Grammar (ERG)}English Resource Grammar \citep[ERG;][]{Flickinger2000a,Flickinger2011a-u}
...
First, a corpus is \textit{treebanked}:\is{treebanks}
...
based on many features\footnote{%
	In the \isi{machine-learning} sense of ``feature'',
...
choosing the correct lexical entry\is{lexical entries} brings in rich information
...
To allow \isi{multi-word expressions} (such as \textit{by and large}),
...
we need to first annotate\is{annotation} a treebank.\is{treebanks}
...
it can be helpful to use \textit{\isi{discriminants}},
properties which hold for some parses but not for others \citep{Carter:97}.
For example, \isi{discriminants} might include
...
It is therefore often desirable to extract a concise semantic representation.\is{semantic representations}
...
as this enables the use of graph-based algorithms.\footnote{%
	In this section, we are concerned with \emph{semantic} dependencies.\is{dependencies!semantic}
	For \emph{syntactic} dependencies,\is{dependencies!syntactic} see \crossrefchapteralt{dg}.
...
\textit{\isi{intrinsic argument}}, an MRS can be converted to a variable-free semantic representation,\is{semantic representations!variable free}
...
which is fully interconvertible with MRS.\footnote{%
	...
	every predicate (except for \isi{quantifiers}) must have an \isi{intrinsic argument},
...
Finally, \delphin\ MRS Dependencies \citep[DM;][]{ivanova2012dm}\is{\delphin\ MRS Dependencies (DM)}
express \isi{predicate-argument structure} purely in terms of the surface tokens,
without introducing any abstract predicates.\is{predicates!abstract}
...
By convention, \is{\delphin\ Consortium}\delphin\ predicates beginning with an underscore correspond to a \isi{lexical item},\is{predicates!lexical}
and have a three-part format,
consisting of a lemma, a part-of-speech tag, and (optionally) a sense.
Predicates without an initial underscore are abstract predicates.\is{predicates!abstract}
The \textit{qeq}\is{QEQ} constraints (equality modulo quantifiers)\is{equal modulo quantifiers}
are scopal\is{scope} relationships,
where \isi{quantifiers} may possibly intervene
...
The corresponding \isi{Dependency Minimal Recursion Semantics (DMRS)}
...
Predicates\is{predicates} are represented as nodes,
while \isi{semantic roles} and scopal\is{scope} constraints are represented as directed edges,
...
Finally, the corresponding \delphin\ MRS Dependencies (DM) representation
is shown in~(\ref{cl:fig:dm}).\is{\delphin\ MRS Dependencies (DM)}
...
as well as software packages to manipulate such graphs (\eg\ \citealt{ivanova2012dm}, \citealt{copestake2016pydmrs}, \citealt{Her:Kuh:Oep:19}, or \isi{PyDelphin}\footnote{\url{https://github.com/delph-in/pydelphin/}, accessed 16 August 2019}),
...
such resources can be used both in \isi{linguistic hypothesis testing} as well as in various \isi{practical applications}.
...
\ili{Danish} \citep{MOeDanish-language},
\ili{Persian} \citep{MG2010a},
\ili{Maltese} \citep{MuellerMalteseSketch},
and \ili{Mandarin} \citep{ML2013a}.
...
phonologically \isi{empty elements},
\isi{relational constraints},
implications with \isi{complex antecedents},
and \isi{cyclic feature structures}.
It also accomodates \isi{macros} and an expressive morphological component.\is{morphology}
...
``bottom-up\is{linguistic universals!bottom-up exploration of} approach with cheating'' (p.\,43)\,---\,the aim is
...
To measure progress, grammars are evaluated against \isi{test suites},
collections of sentences each annotated with a grammaticality judgment\is{grammaticality judgments} \citep{ONK97a,Mueller2004f}.
...
and the graphical debugging tool \isi{Kahina} \citep{DER2010a-u,DER2013a}.
...
English Resource Grammar\is{English Resource Grammar (ERG)}\il{English}
\citep[ERG;][]{Flickinger2000a,Flickinger2011a-u} had been under
development already for 8 years and the \isi{Verbmobil} project
\citep{Wahlster2000a-ed} had also spurred the development of grammars for
\ili{German}\is{GG} \citep[GG;][]{MK2000a-cr,Crysmann2003b} and \ili{Japanese}\is{Jacy}
\citep*[Jacy;][]{SBB2016a}. \isi{Project DeepThought}
\citep*{Callmeier-etal:2004} was exploring methodologies for combining
deep\is{deep processing} and \isi{shallow processing} in \isi{practical applications} across multiple
...
extended to include ``libraries''\is{Grammar Matrix!libraries} of analyses of cross-linguistically
...
is, both grammars and \isi{meta-grammars} (including the Matrix and \isi{CLIMB},
...
\citep[e.g.,][]{Hashimoto-etal:07}, software for creating \isi{treebanks}
(\citealt{OFTM2004a-u,Packard:15}; see also Section~\ref{cl:lang-doc:treebanks} below), 
and \isi{parse ranking} models trained on them
...
\citep{Copestake:02:CLE}, a formalism for the \isi{semantic representations}
...
as well as the results of treebanking\is{treebanks} \citep{Oepen:01,OFTM2004a-u}).
...
(including, so far,
the \isi{LKB} \citep{Copestake2002a},
...
\isi{art}\footnote{\url{https://sweaglesw.org/linguistics/libtsdb/art.html}, accessed 16 August 2019}, and \isi{PyDelphin}\footnote{\url{https://github.com/delph-in/pydelphin/}, accessed 16 August 2019})
...
\isi{linguistic generalizations} and model \isi{grammaticality}. This, in turn,
...
well-formed strings in \isi{generation}. The focus on \isi{practical applications}
...
Processing large quantities of text motivates performance innovations, such as \isi{supertagging} or \isi{ubertagging} (\eg\ \citealp{matsuzaki2007supertag,dridan2013ubertag}, see also Section~\ref{cl:prac:rank}) to speed
up processing times.\is{processing time} Naturally occurring text can include very long 
sentences which can run up against processing limits. Supertagging helps some here, too, but other strategies include \textit{\is{sentence chunking}}, or the task of breaking a long sentence into smaller ones without loss of meaning \citep{muszynska:2016:ACL-SRW}.
...
the integration of external components such as \isi{morphological analyzers} 
\citep[e.g.,][]{Marimon2013a-u} and \isi{named entity recognizers} \citep[e.g.,][]{L06-1115,Sch:Usz:Fed:08}. As described in Section~\ref{cl:prac:rank}, working with real-world applications requires \isi{parse ranking} \citep[e.g.,][]{Tou:Man:Fli:Oep:05}, and similarly ranking of generator outputs (known as \textit{\isi{realization ranking}}; \citealt[e.g.,][]{Velldal:09}). Finally, research on embedding \isi{broad-coverage grammars} in \isi{practical applications} inspires work  towards making sure that the \isi{semantic representations}
...
is a broad-coverage grammar\is{broad-coverage grammars} of \ili{Dutch}
...
lexical information (such as \isi{subcategorisation frames}) has also been extracted from two existing lexicons,
\isi{Celex} \citep{baayen1995celex}
and \isi{Parole} \citep{kruyt1997parole}.

Alpino produces syntactic dependency graphs,\is{dependencies!syntactic}
following the annotation format of the \isi{Spoken Dutch Corpus} \citep{oostdijk2000corpus}.
These dependencies are constructed directly in the feature-structure formalism,
exploiting the fact that a feature structure can be formalised as a \isi{directed acyclic graph}.
...
Although these dependencies differ from the semantic dependencies\is{dependencies!semantic} discussed in Section~\ref{cl:prac:dep},
a common motivation is to make the representations easier to use in \isi{practical applications}.
...
Alpino uses a \isi{statistical model} trained on a dependency \isi{treebank},
and in fact the same statistical model can be used in both \isi{parsing} and \isi{generation} \citep{dekok2011reversible}.
...
including subject-\isi{control} verbs, \isi{auxiliary verbs}, \isi{coordination}, and extracted arguments.\is{extraction}
Each converted tree can then be combined with a small set of hand-written HPSG schemata,
to induce a lexical entry\is{lexical entries!induction of} for each word in the sentence.
...
it makes extensive use of \isi{discontinuous constituents} \citep{Mueller2004b}.
...
documentation.\footnote{Grammar engineering\is{grammar engineering} isn't specific to HPSG and
in fact has a history going back to at least the early 1960s \citep{Kay:63,ZFHW65a,Petrick65a-u,FBDPM71a-u}
and modern work in Lexical Functional Grammar\is{Lexical Functional Grammar (LFG} \citep{BKNS99a-ed}, Combinatory Categorial
...
allows a language to have \isi{V2 word order}
(as exemplified by \ili{Germanic languages}),
but rules out \isi{verb-penultimate word order},
as discussed by \citet{MuellerCoreGram}
(see also \crossrefchapteralt{order}).
...
consists of a shared \isi{core grammar} and a series of ``libraries''\is{Grammar Matrix!libraries} of
...
the other hand, the Matrix libraries\is{Gramamr Matrix!libraries} represent an additional kind of
...
Furthermore, \isi{treebanks} constructed from implemented grammars can be
...
a multilingual grammar engineering project like \isi{CoreGram}, \isi{ParGram} \citep{BDKMR02a-u,Kin:For:Kuh:But:05}, the \isi{GF Resource Grammar Library} \cite{Ranta:09}, or the
\isi{Grammar Matrix}.
...
high-level \isi{typological properties} or specific constraints on \isi{lexical
classes} and \isi{lexical rules}.  The goal of AGGREGATION is to
...
linguistic subsystems. For example, it learns \isi{morphotactics} by
observing morpheme order in the training data, and the grouping
of affixes together into \isi{position classes} based on measures of overlap of
...
AGGREGATION system can detect \isi{case frames} for verbs, \isi{word order}
...
% GE 2019-02-09 - TODO link to WeSearch?
% http://wesearch.delph-in.net/deepbank/search.jsp
% EMB 2019-02-21 Kou:Oep:14 is WeSearch isn't it? 
% GE 2019-02-25 I meant the above url
% could be useful if readers are likely to want to use it
% EMB 2019-02-25 I see -- added!
In the context of language documentation and description, searchable
treebanks\is{treebanks!searchable} can also be a valuable addition, helping readers connect
...
stochastic\is{parsing!stochastic} and neural parsers\is{parsing!neural} (see Section~\ref{cl:downstream:data}).

Traditional treebanks are created by doing a certain amount of
preprocessing on data, including possibly \isi{chunking} or \isi{CFG} parsing, and
...
annotators\is{annotation} select among the parses. The selection process is
facilitated by allowing the annotators to mark potential \isi{lexical entries} 
for words in the sentence at hand as correct, possibly
correct, or wrong and to pre-mark some \isi{constituent boundaries}. These
...
\eg\ misspellings helps increase grammar coverage.  Disambiguation\is{ambiguity}\is{disambiguation} is
handled with the aid of \textit{\isi{discriminants}} \ie\ properties true of
some but not all trees in the \isi{parse forest} \citep{Carter:97}. Finally,
...
\isi{dependency representations} instead.  The Alpino parser was similarly
...
decisions the annotator\is{annotation} made about each discriminant.\is{discriminants} Thus when the
...
done to handle any additional \isi{ambguity} introduced. The activity of
\isi{treebanking} in turn provides useful insight into grammatical analyses,
...
covers 55m sentences of \ili{English} (900m tokens).
It was produced by \citet{flickinger2010wikiwoods} and \citet{solberg2012wikiwoods}
from the July 2008 dump of the full English \isi{Wikipedia},
...
a grammar can be extended with so-called \textit{\isi{mal-rules}}.
...
Given a semantic representation,\is{semantic representations}
...
\textit{\isi{Information retrieval}} is the task of finding relevant documents for a given query.
...
\textit{\isi{Information extraction}} is the task of identifying useful facts in a collection of documents.
...
such as \isi{negation} and hedging\is{hedges}
(including \isi{adverbs} like \textit{possibly}, \isi{modals} like \textit{may}, and verbs of speculation\is{verbs!of speculation} like \textit{suggest}).
...
evaluating on the \isi{*SEM 2012 Shared Task} \citep{morante2012task}.
They find that converting the output of the ERG\is{English Resource Grammar (ERG)} with a relatively simple set of rules
achieves high performance on this \ili{English} dataset,
and combining this approach with a purely statistical system
outperforms previous approaches.
\citet{zamaraeva2018pathology} use the ERG for \is{negation} detection and then
use that information to refine the (machine-learning) features in a 
system that classifies \ili{English} pathology reports and improve system performance.\is{biomedical NLP}
...
recent work on \isi{robust parsing} may help to close this coverage gap.
...
and this shared task aims to evaluate the use of \isi{dependency representations} (both semantic and syntactic),
...
Some participating teams use DM\is{\delphin\ MRS Dependencies (DM)} dependencies\footnote{DM stands for \delphin\ MRS dependencies; see (\ref{cl:fig:dm}).} \citep{schuster2017dep,chen2017dep}.
...
Most existing work on this task focuses on so-called \textit{extractive summarization}:\is{summariziation!extractive}
...
\citet{fang2016summarise} show how a wide-coverage grammar like the ERG\is{English Resource Grammar (ERG)}
...
including both DM dependencies\is{\delphin\ MRS Dependencies (DM)} and \isi{Enju} \isi{predicate-argument structures}.
...
Similarly, \citet{buys2017parse} develop a parser for EDS\is{Elementary Dependency Structures (EDS)} and DMRS\isi{Dependency Minimal Recursion Semantics (DMRS)}
...
including logical structure, as well as phenomena like \isi{polysemy}.
...
This approach relies on the \isi{semantic analyses} given by a grammar,
as well as the infrastructure to parse a large amount of text.

Finally, there are also applications using grammars not to parse, but to \isi{generate}.
\citet{kuhnle2018shapeworld} consider the task of \textit{visual question answering},\is{visual question answering (VQA)}
where a system is presented with an image and a question about the image,
and must answer the question.
This task requires \isi{language understanding}, \isi{reference resolution}, and \isi{grounded reasoning},
...
%TODO references
For this reason, there is increasing interest in \isi{artificial datasets},
...
A case in point is the range of \isi{raising} and \isi{control} \isi{valence frames}
found in the ERG\is{English Resource Grammar (ERG)} \citep{Flickinger2000a,Flickinger2011a-u}.
As of the 2018 release, the ERG includes over 60 types for raising and control predicates,
including \isi{verbs}, \isi{adjectives}, and \isi{nouns},
...
and an \isi{infinitival VP} complement,
...
\isi{meta-grammar engineering} can be used to address this problem: using
