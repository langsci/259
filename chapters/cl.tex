...
(see Section~\ref{cl:resources} below for some examples).\footnote{%
	There are implementations of Minimalism, notably \citet{Stabler97a-u} and \citet{Herring:16}. (See also \citealt{Tor:Sta:Ste:19} for a recent broad-coverage, \isi{treebank}-trained parser in this framework.)
\inlinetodoopt{\citegen{Torr2019a-u} dissertation is also great.}
...
is to consider the \textit{computational complexity}\footnote{%
	...
	the complexity hierarchy of language classes in \isi{formal language theory}.\inlinetodoobl{Stefan: Cite somebody. eg \citet{Chomsky63a-u}}
...
means that the formalism is stable, even as the theory develops.\inlinetodoopt{Stefan: \citet{Mueller2004b} also argues that in reality some parsing algorithm with terrible worst-case turns out better than other parsing algorithms since they require the use of empty elements or equivalent devices, which basically overwhelms every parser even if theoretically well-behaved. You cite this paper later. Maybe this is ok. But I somehow thought about this point here \ldots}
...
For a feature-structure grammar, each span must be analyzed as a set of feature structures,\footnote{%
	...
	distinguishes between fully resolved feature structures and possibly underspecified feature structure descriptions.\is{underspecification}
...
but in practical grammars these are never greater than length four or five.\footnote{%
	...
	(such as to model \isi{cross-serial dependencies}),\inlinetodoobl{Stefan: Cross-serial dependencies are done with \comps not with \slasch. Crossref the \crossrefchaptert{complex-predicates}. The udc chapter has \slasch examples with two elements in \slasch. But it is still bounded. In \citet[\page 220]{Mueller2004b}, I argued that restricting \comps in length is not warranted if one believes in a competence/performance distinction.}
...
\item Visiting relatives can be annoying.\inlinetodoobl{Stefan: Shouldn't it be \emph{boring}? With \emph{annoying} I get a strong preference for the person reading. Who is responsible for this example? A Freudian mistake? =;-)}
\end{xlist}
\end{exe}
...
or the top~$N$ parses.\inlinetodoopt{Stefan: Why capital $n$?}
...
\inlinetodoopt{Stefan: Just out of curiosity: What about nominalizations? Do they keep the \_v in
  the relation name?}
\begin{exe}
...
\inlinetodoopt{Stefan: Just a remark: Shouldn't the compound relation introduce a new variable to be
  able to deal with non-compositional compounds? The headed compounds could identify this variable
  with the one of the head.}
\begin{exe}
...
\inlinetodoobl{Stefan: Hm, this sounds as if the TRALE syntax would change every week \ldots. I
  think the difference is that TRALE uses a complex inference machine that you get (or rather got)
  with SICStus-Prolog. To redo this in C++ or whatever would involve a lot of work.}
...
\inlinetodoopt{Stefan: Well, I am not a native and two natives wrote this text and one native proofread, but \emph{such as for cross-serial dependencies}
...
\inlinetodoopt{Stefan: Insert \emph{The}?}
...
\inlinetodoopt{JP: Shouldn't it be morph order to avoid commitment to a morphemic analysis?}
...
\inlinetodoopt{Stefan: What is the difference between ``automatically annotated'' and ``without any
  post-editing of analyses'' mentioned above?}
...
\inlinetodoopt{Stefan: I do not know whether this can be fixed with a few words, but I am somehow
  uneasy with the previous paragraph. The CxG people tell us that the core/periphery distinction does not
  make any sense and I guess that we all agree on this that the Chomskyan core/periphery distinction
  does not make sense since it is related to UG-driven vs.\ not UG-driven acquisition. Since we do
  not assume (a rich) UG in the first place, the core/periphery distinction crumbles away. BUT: As
  CLers we know that we get 90\,\% coverage with \emph{John loves Mary} structures and that the rest
  is where the fun starts. So Core for us is something like prototpicality or frequency. What I did
  in this paper was to do this cross-linguistically. I think this is totally different from the
  Chomskyan notion since Chomsky's notion refers to the periphery of a single language, doesn't it? What my
  method gives you is that there is a constraint set for \emph{student after student} constructions,
  which exist in a lot of languages. In this sense the NPN construction has a certain degree of
  ``coriness'' (as I call it) although nobody in the Chomskyan camp would count this among the Core. Actually they have
  to ignore this construction since it breaks their theories (as I show in the \crossrefchaptert[Secion~4.2.3]{minimalism}).}
...
\inlinetodoopt{Stefan: Isn't this \emph{it} extraposition? If \emph{To speak plainly is incumbent on
  you.} is impossible it may still be a lexeme with obligatory \emph{it} extraposition. Something
like \emph{rumored}.}
\begin{exe}
