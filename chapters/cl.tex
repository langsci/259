...
(see Section~\ref{cl:resources} below for some examples).\footnote{%
	There are implementations of Minimalism, notably \citealt{Stabler97a-u} and \citealt{Herring:16}. (See also \citealt{Tor:Sta:Ste:19} for a recent broad-coverage, \isi{treebank}-trained parser in this framework.)
...
but in practical grammars these are never greater than length four or five.\footnote{%
	...
	Furthermore, in many \is{\delphin\ Consortium}\delphin\ grammars, including the ERG,\is{English Resource Grammar (ERG)}
...
it can be helpful to use a test suite composed of a representative sample of sentences \citep{OF98}.\is{complexity!computational|)}
...
\textit{\isi{intrinsic argument}}, an MRS can be converted to a variable-free semantic representation,\is{semantic representations!variable-free}\is{logical variable}
...
By convention, \is{\delphin\ Consortium}\delphin\ predicates beginning with an underscore correspond to a \isi{lexical item},\is{predicate!lexical}
...
where \isi{quantifier}s may possibly intervene
...
as we will discuss in Section~\ref{cl:downstream}.\is{dependencies!semantic|)}
...
as well as the results of \isi{treebank}ing \citep{Oepen:01,OFTM2004a-u}).
...
is a broad-coverage grammar\is{broad-coverage grammar}s of \ili{Dutch}
...
the other hand, the Matrix libraries\is{Grammar Matrix!libraries} represent an additional kind of
...
would be too complex for exploration.\is{Grammar Matrix|)}\is{\delphin\ Consortium|)}
...
patterns, etc.\ \citep{Ben:Goo:Cro:Xia:13,Zam:How:Ben:19}.\footnote{The \isi{TypeGram} project \citep{Hel:Bee:14} is in a similar spirit. TypeGram provides methods of creating HPSG grammars by encoding specifications of valence and inflection in particularly rich IGT and then creating grammars based on those specifications.}\is{AGGREGATION|)}
...
used to construct the \isi{Lassy treebanks} of written Dutch
...
done to handle any additional \isi{ambiguity} introduced. The activity of
...
\textit{Information retrieval}\isi{information retrieval}
...
\textit{Information extraction}\isi{information extraction}
