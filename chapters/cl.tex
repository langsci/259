...
it is possible to adapt the standard \textit{parsing} algorithm \cite{kay:1973} for context-free grammars.\is{context-free grammar (CFG)}\is{parsing!chart parsing}
...
\isi{parse ranking} models trained on them
...
including possibly \isi{chunking} or CFG\is{context-free grammar (CFG)} parsing, and
...
\eg\ misspellings helps increase grammar \isi{coverage}.  Disambiguation\is{ambiguity}\is{disambiguation} is
...
including both DM dependencies\is{\delphin\ MRS Dependencies (DM)}
