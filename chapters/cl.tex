...
\largerpage 
Keeping processing complexity manageable is handled differently in other computationally-friendly frameworks, such as \isi{Combinatory Categorial Grammar (CCG)},\footnote{%
...
\isi{practical application}s.  The intended purpose of the resources influences the form that they
...
\isi{cyclic feature structure}s.  It also accomodates \isi{macro}s and an expressive morphological
...
the \isi{test suite} tool \itsdb\is{\itsdb} (\citealp{Oepen:01}; see also Section~\ref{cl:delphin}),
...
development of grammars for \ili{German}\is{GG} \citep[GG;][]{MK2000a,Crysmann2003b} and
...
\isi{shallow processing} in \isi{practical application}s across multiple languages. This inspired
...
like \itsdb\ \citep{Oepen:01},\is{\itsdb} \is{art
...
which capture \isi{linguistic generalization}s and model \isi{grammaticality}. This, in turn, leads
...
\isi{practical application}s leads to several kinds of additional research goals. Practical
...
\isi{practical application}s inspires work towards making sure that the \isi{semantic
...
\is{constituent boundary}constituent boundaries and to mark \is{lexical entry}lexical entries as
...
and calcuating the discriminants for each parse forest.  After annotation, the treebanking software
