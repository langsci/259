...
is to represent the meaning of a sentence as a \textit{dependency graph},
...
For example, English Resource Grammar (ERG)
...
By convention, \delphin\ predicates beginning with an underscore correspond to a lexical item,
...
where quantifiers may possibly intervene
(for details, see \citealt{CFPS2005a} or \crossrefchaptert{semantics}).
...
The corresponding Dependency Minimal Recursion Semantics (DMRS)
representation is shown in~(\ref{cl:fig:dmrs}).
This captures all of the information
in the MRS in Figure~\ref{cl:fig:mrs-abs}.
Predicates are represented as nodes,
while semantic roles and scopal constraints are represented as dependencies.
Each dependency has two labels.
The first is an argument label,
such as \textsc{arg1}, \textsc{arg2},
or \textsc{rstr} (the restriction of a quantifier).
...
This is a simplified version of MRS,
...
The label \textsc{bv} stands for the ``bound variable'' of a quantifier,
equivalent to the \textsc{rstr/qeq} of DMRS.
