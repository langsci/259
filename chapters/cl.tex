...
% EMB 2019-02-21 Is this not a tautology? That is, if something doesn't involve well-defined data structures, is it actually a formalism?
...
One way to measure how easy or difficult it is to use a syntactic
theory in practical computational applications is to consider the
\textit{computational complexity} of parsing and generation algorithms
\citep{gazdar1985complexity}.  For example, consider how much
computational time a parsing algorithm needs to process a particular
sentence.\footnote{% 
In this section, we only consider parsing
  algorithms, but a similar analysis can be done for generation
  \citep[\eg][]{carroll1999generate}.  } Longer sentences should
always take longer to process, but the more complex the algorithm is,
the more quickly the amount of processing time increases.  Parsing
complexity can thus be judged by considering sentences containing
$n$~tokens, finding the average amount of time taken or the longest
amount of time taken, and then increasing~$n$ and seeing how the
amount of time changes, both in the average case and in the worst
case.
% EMB 2019-02-21 Edits just above to work around pedantic mathy `we'.
...
rules.\footnote{In fact, in earlier work, this task was undertaken by
  ...
  determining appropriate weights was a available and the system was
...
restricting attention to a smaller set of parses 
 can help the parser
find the correct parse, provided
that the algorithm for selecting the subset includes
the correct parse in the subset sufficiently frequently.
Carefully choosing how to restrict the parser's attention 
can also drastically reduce processing time without hurting parsing accuracy.
One method, called \textit{supertagging},\footnote{%
	The term \textit{supertagging}, due to \citet{bangalore1999supertag},  refers to \textit{part-of-speech tagging},
...
\citep{OFTM2004a-u,Fli:Oep:Ben:17}. For more on treebanking, see Section~\ref{cl:cl:lang-doc:treebanks}.
...
The CoreGram Project (Section~\ref{cl:coregram}) primarily targets linguistic hypothesis testing,
the Enju, Alpino and Babel parsers (Section~\ref{cl:othergrammars}) primarily target practical applications,
% EMB 2019-02-21 Added Babel to this list, but is that accurate?
...
leads to several kinds of research goals: Practical applications
require robust processing, which in turn requires methods for handling unknown words
\citep[\eg][]{chartmapping}, methods for managing extra-grammatical mark-up in
text (\eg\ Wikipedia pages) \citep[\eg][]{FOY2010a-u} and strategies
...
% EMB 2019-02-21 Bookmark ... editing here.
(2)
...
such as for cross-serial dependencies \citep{BvN98}.
As well as using hand-written rules,
...
to induce a lexical entry for each words in the sentence.
...
at scale, we meant both against large quantities of data and as
...
\crossrefchapterp[see also]{chap-order}.
It also includes constraints for argument structure and linking
\crossrefchapterp[see]{chap-arg-st},
as well as for information structure
\crossrefchapterp[see]{chap-information-structure}.
...
structure-based searches for phenomena of interest \citep{Rohde:05,Gho:Bir:10,Kou:Oep:14}.
...
(including adverbs like ``possibly'', modals like ``may'', and verbs of speculation like ``suggest'').
...
\citet{mackinlay2012biomed} also consider the biomedical domain,
...
Similarly, \citet{zamaraeva2018pathology} consider the scope of negation in medical reports,
outperforming a system specifically designed for this task.
...
recent work on parsing may help to close this coverage gap.
...
Because the summary is not directly extracted from the input document,
...
A more traditional grammar-based approach uses three grammars:
...
and also to convert syntactic constructions
...
within this paradigm of parsing, transferring, and generating.
...
is that the grammar is unlikely to cover all sentences in the data \citep{Baldwin-et-al-05,flickinger2012deepbank}.
...
Similarly, \citet{buys2017parse} develop a DMRS parser
...
\citet{chen2018parse} consider parsing to DMRS graphs,
...
\citep[for an overview, see:][]{erk2012vector,clark2015vector}.
...
One of the strengths of HPSG as a theoretical framework is that it allows for the analysis of both `core' and `peripheral' phenomena within a single, integrated model. When working with actual data and large scale gramamrs, it quickly becomes apparent just long the long-trail of `peripheral' phenomena are. Furthermore, the sustained development of broad-coverage linguistic resources makes it possibel to bring more and more low-frequency phenomena or low-frequency variations on relatively high-frequency phenomena into view. A case in point is the range of raising and control valence frames found in the ERG \cite{Flickinger2000a,Flickinger2011a-u}. As of the 2018 release, the ERG includes FIXME types for raising and control predicates, including verbs, adjectives, and nouns, many of which are not otherwise discussed in the syntactic literature.
% EMB 2019-02-07 I think I should ask Dan if it's okay to include this, given that this is something he and I are working on together but haven't published.
