...
For a feature-structure grammar, each span must be analysed as a set of feature structures,\footnote{Much theoretical work in HPSG, including \citet{ps2} distinguishes between fully-resolved feature structures and possibly underspecified feature structure descriptions. Much comutational work, by contrast, operates entirely with partially specified feature structures, at both the level of grammar and the level of analyses licensed by the grammar. In keeping with this tradition, we use the term ``feature structure'' to refer to both fully-specified and partially specified objects.}
...
Section \ref{cl:delphin}), the answer is yes. Assuming a system without
relational constraints, the potential for
unboundedness in the feature structures stems from the possibility
of recursion in feature paths, \eg\ the elements on the \textsc{comps}
list themselves include the feature \textsc{comps} or list-valued features, where lists are in fact recursive feature structures.\footnote{The typical
implementation of a list as a feature structure provides a type signature
where \emph{list} has two subtypes \emph{null} and \emph{non-empty-list}, and
\emph{non-empty-list} has the features {\sc first}
and {\sc rest}, where the value of {\sc rest} is of type \emph{list}.}
In practice, neither of these sources leads to an unbounded
...
\begin{subfigure}{\textwidth}
	...
	\caption{
		...
		representation that captures all of the information
		in the MRS in Figure~\ref{cl:fig:mrs}.
...
\begin{subfigure}{\textwidth}
	...
	\vspace*{2mm}
	\caption{
		A \delphin\ MRS Dependencies (DM) representation.
...
\citep{callmeier00},
ACE,\footnote{\url{http://sweaglesw.org/linguistics/ace/}, accessed 16 August 2019} and Agree
\citep{Slayden2012a-u}), of multiple software systems for processing
bulk grammar output (\itsdb\ \citep{Oepen:01}, art\footnote{%
\url{https://sweaglesw.org/linguistics/libtsdb/art.html}, accessed 16 August 2019}, PyDelphin\footnote{%
	\url{https://github.com/delph-in/pydelphin/}, accessed 16 August 2019
})
...
project.\footnote{\url{http://depts.washington.edu/uwcl/aggregation/}, accessed 16 August 2019}
...
In Section~\ref{cl:lang-doc} above, we described multiple ways in which computational methods can be used in the service of linguistic research, especially in testing linguistic hypotheses. Here, we highlight a few ways in which grammar eningineering work in HPSG has turned up linguistic insights that had not previously been discovered through non-computational means.\footnote{For similar reflections from the point of view of LFG, see \cite{King:16}.}
