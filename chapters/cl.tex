...
\begin{subfigure}{\textwidth}
	...
	\caption{
		...
		\textsc{rstr} stands for ``restriction'' (of a quantifier).
		\textsc{eq} indicates that the linked nodes share a label in the MRS
		(which is generally true for modifiers),
		while \textsc{neq} indicates that they don't share a label.
	}
\end{subfigure}
\begin{subfigure}{\textwidth}
	\centering
	...
	\caption{
		...
		\textsc{bv} stands for ``bound variable'' (of a quantifier).
	}
\end{subfigure}
\caption{%
	MRS-based dependency graphs for the sentence
	\textit{The cherry tree blossomed},
