...
Construction Grammar) and its characterization
of linguistic signs as sets of structures not necessarily linked by
attributes (\crossrefchaptert{cxg}).
...
at least syntax, semantics (\crossrefchaptert{semantics}), pragmatics (\crossrefchaptert{pragmatics}) and morphology (\crossrefchaptert{morphology}). The
...
({\ref{ex-n-full}) describes exhaustively. Finally, the disjunction
...
the two distinct attribute paths leading to its occurrences. In the Head Feature Principle, the tag notation
...
%    \end{avm}
    ...
    \type{word} $\Rightarrow$ $\left(LE_1 \vee LE_2 \vee \ldots  \vee LE_n \right)$
...
semantics chapter (example (10), \crossrefchaptert{semantics}, taken
...
(subset relation), and the set value \idx{1} of \attrib{qstore}
contains those elements of \idx{2} which are not on the
\attrib{retrieved} list (using set difference). In other words,
each element of \attrib{pool} is either in \attrib{qstore} or
...
idealizations \citep{Pollard99a} and the linguistic utterances they
are supposed to represent. The failure of achieving the intended
one-to-one correspondence
is due to technical properties of the structure of the respective models
and to imprecisions of actual HPSG grammar
specifications, where the two factors are partially
...
between structural configurations in the model and empirically
...
in HPSG with attributes \attrib{first} and
\attrib{rest}, and sorts \type{list}, \type{elist}, and
\type{nelist}, but of course the exact naming does not matter.
...
\type{nelist} and \type{elist}, respectively, and the reserved symbols
...
by including them in the extension of $S_{max}$, designated as $\Aug{S}_{max}$.
An AVM describing a chain with two \type{synsem} objects corresponding to the description
of a list with two \type{synsem} objects in (\ref{list-w2-eles}) now looks as
follows:
...
\begin{exe}
  ...
  $\left(\its\sim\mbox{\type{hd-ph}}\right)\rightarrow$\\ % \wedge \its\attrib{dtrs}\sim\mbox{\type{headed-struc}}
...
description in (\ref{synsem-rel}) to the theory of the grammar:
...
Consider a non-empty grammar model $\Inte$ containing words and
phrases. Since (\ref{synsem-rel}) is in the theory, and we look
at a \emph{model}, (\ref{synsem-rel}) describes every object in the
universe of $\Inte$.  By the bi-implication, every component object of every
object $u$ in $\Inte$ which is of sort \type{synsem} is in
$\Reli$(\texttt{synsem-rel}) (right to left), and every element of
$\Reli$(\texttt{synsem-rel}) is a \type{synsem} object (left to
right). But that means that $\exists x\,\texttt{synsem-rel}(x)$
describes every object in $\Inte$ which has a component that is in the
\texttt{synsem-rel} relation, and $\forall
x\,\texttt{synsem-rel}(x)$ describes every object in $\Inte$ all of
whose components are in that relation.\footnote{Of which there are
  none, given the usual structure of signs where synsem objects always
  have components of other sorts.}
...
expresses correctly all and only what there is to say about English.
Interestingly, a 
$\left< \Sigma, \theta \right>$ model of this perfect grammar
of English may consist of nothing but a structure
of the single sentence \emph{Elon is going to Mars}. This follows from
the definition of $\Gamma$ models, because any appropriate grammar of
English describes all objects of which this well-formed sentence consists. But
this one-sentence-model of the grammar of English is obviously too
small for counting as a good
candidate for the English language, because English contains much more
than this single sentence. We conclude that in arbitrarily chosen
models it cannot be detected if
...
their assumptions about the nature of the model. The decision what
...
types represent linguistic token expressions in the sense that an
object type conventionally corresponds to one grammatical expression
of the language $\mathcal{L}$ that $\left< \Sigma, \theta\right>$
...
node. Here a root node corresponds to the object $u$ in
$\CInt_{\Inte}^u$, the sets of components of an $u$ in an
interpretation $\Inte$ introduced in
Definition~\ref{def-components-of-u}.  The abstract feature structures
...
inacceptable and self-contradictory. However, \cite{Pollard99a} also
