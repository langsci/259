...
\type{plus} and \type{minus} are also called atomic, because they are species
...
a \type{phrase} with \attrib{dtrs} value \type{headed-struc}, it is
...
object of sort \type{synsem}, i.e.\ it is not a phrase. But as a
...
that the set value \idx{2} of \attrib{pool} is a subset of \idx{4}
(subset relation), and the set value \idx{1} of \attrib{qstore}
contains those elements of \idx{4} which are not on the
\attrib{retrieved} list (using set difference).\footnote{One
  ...
  value \idx{4}. However, the list \idx{3} and the set \idx{1} are
...
regular explicit encoding in HPSG, usually with attributes \attrib{first} and
...
A description of a list with two \type{synsem} objects can then be notated
