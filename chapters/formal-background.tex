...
\abstract{
  ...
  \citew{PollardSag1994} and explains its fundamental concepts. It pays
...
usages of terms. Similarly, it does not cover \added{the HPSG variant} Sign-Based
Construction Grammar (SBCG; \citealt{Sag2012a}; \crossrefchapteralt[Section~\ref{cxg:sec-sbcg}]{cxg}).
...
at least syntax, semantics \crossrefchapterp{semantics}, pragmatics \crossrefchapterp{pragmatics}
and morphology \crossrefchapterp{morphology}.\itdopt{add phonology with \citealp{BK94b}, \citealp{Hoehle99a-u}, and \citealp{Walther99a-u}?
  And order the levels: phonology, morphology, syntax, \ldots?} 
...
specific. For example, an object of sort \type{noun} has \feat{case} and \feat{prd} properties. The
object that is the \feat{case} value must be of sort \type{case} (because \type{case} is a species
...
Principle, shown in (\ref{ex-hfp}),
\itdopt{(\mex{1}) is very far. Maybe split or move here?} is a frequent example. Every
phrase whose syntax is a headed phrase (\type{\changed{headed-phrase}}) is such that its
...
a \type{\changed{headed-phrase}}, it is
...
is not described by \type{\changed{headed-phrase}} still satisfies
...
\type{\changed{headed-phrase}}. As a
...
\begin{exe}
  ...
  $\left(\its\sim\mbox{\type{\changed{headed-phrase}}}\right)\rightarrow$\\ % \wedge \its\feat{dtrs}\sim\mbox{\type{headed-struc}}
...
what they mean, we have to explain the meaning of signatures and of descriptions.
\itdobl{No academic plural. Two \emph{we} here. Check paper.}
...
\footnote{In RSRL syntax, (\ref{ex-word-order}) can be written as\\
  $\its\sim$\type{\changed{head-filler-phrase}} $\rightarrow$\\
...
% todo avm remove &
\label{ex-word-order}
  \type{\changed{head-filler-phrase}}  $\Rightarrow$ $\exists \idx{1}\; \exists \idx{2}\; \exists \idx{3}$ $\left(\mbox{
  \avm{
    [phon \3\\
