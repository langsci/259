...
fully specific AVM in (\ref{ex-n-full}): two \type{noun} structures
with the \attrib{case} property \type{case} and the \attrib{prd}
property \type{plus} or the \attrib{prd} value \type{minus}. But the
fully explicit statement of this fact can be underspecified in many
different ways. For \type{noun} structures (\ref{ex-n-n}), these are
the only two choices, as can be verified by inspecting
Figure~\ref{ex-hier-decl}. Of course it could be mentioned in addition
that they have a \attrib{case} property and leave its value
underspecified, but that does not make a difference
(\ref{ex-n-o}). Moreover, the only \type{object}s with \attrib{case}
(\ref{ex-o-o}) are nouns; since that leaves exactly the two possible
\attrib{prd} values \type{plus} and \type{minus}, it is yet another way to
