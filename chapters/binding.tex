...
arguments of the same predicate.\footnote{%
  See \citet{Riezler95a} for a way to formalize this in HPSG. See \citet{RR93a} for an approach to
...
\emph{believe}:\footnote{%
 ...
 (\mex{1}) is taken from \citet[\page 128]{MS98a}. See also \citet[\page 679]{RR93a} on Binding
 Theory and raising.%
}
\ea
...
thematic approaches to binding see \citet[Section~8]{PS92a} and \citet[Section~6.8.2]{ps2}.
}
...
are coindexed and Y c-commands Z. One precondition for being coindexed (in English) is that the
...
being the first element \citet[\page 266]{PS92a}:\footnote{
  While \citet[\page 120]{ps} use \citegen[]{KC77a} version of the Obliqueness Hierarchy in (i), they avoid the
  terms \emph{direct object} and \emph{indirect object} in \citet[\page 266, 280]{PS92a} and
  \citet[\page 24]{ps2}.
\ea
...
\citet{Wechsler95a-u}, \citet{Davis2001a-u}, \citet{DK2000b-u}, and \crossrefchapterw{arg-st} for this.
...
HPSG flavors like \sbcg that do not have daughters in linguistic signs (see \citet{Sag2012a} and
...
\citet{Mueller99b} on relative clauses. \citet{Sag97a} suggests an approach to relative clauses in
...
problematic as the following example (based on \citet[\page 412]{Mueller99a}) shows:
\ea
...
languages with freer constituent order like German, which causes problems for Binding Theories not
...
subject-object asymmetry seems to exist in German, as \citet[\page 148]{Grewendorf85a} pointed
...
%definition of c-command that can be found in \citet[\page 168]{SKS2013a-u}
...
\z
Since \emph{sich} is not local to \emph{ihm} and since reflexives are not excempt in German
...
Theory. See also \citet[\page 420]{Mueller99a} for attested examples.
...
Austronesian languages arguing that the order of the elements in the \argstl should not reflect the
grammatical functions of the \argst elements. Section~\ref{binding-sec-passive} deals with data from Russian and argues
...
developed for English can be applied to them without changes.
...
%(see \citet[Chapter~6]{Grewendorf88a} for such rules
%for German in GB).
}% too long

\section{Disentangeling \argst and grammatical functions}

So far we have discussed binding for English with some occasional reference to Mandarin Chinese,
Portuguese and German. The question is whether Binding Theory is universal, that is, whether it is a
...
\citet{MS98a} discuss data from \ili{Toba Batak}, a \ili{Western Austronesian} language. They assume that the
...
% Yes, the whole book
This would not make a difference for languages like English, but for languages like
...
relative clauses exist in English as well, but in addition, English has various types of relative clauses without
...
comparison to functor-based approaches like Categorial Grammar (see \citet{Pollard88a} for
...
%% } 
For further discussion of relative clauses see \citet{Sag97a},
\crossrefchaptert{relative-clauses} and \citet{Mueller99b}.
}
...
different.\footnote{
  ...
  agreement features of the verb \citep[Section~2.4.2]{ps2}. See \citet{WZ2003a} and \crossrefchapterw{agreement} on
...
\footnotetext{
        Goethe, \emph{Hermann und Dorothea}, Berliner Ausgabe, volume 3, p.\,609, Berlin: Aufbau, 1960. See also \citet[\page 139]{Jung67a}.
}
\ex 
...
% \citet{Kiss2001a} develops an account of quantifier scope determination for German arguing that scope
% is determined with respect to an unmarked order.\footnote{
%   See \citet{Hoehle82a} for a definition of \emph{normal order}.
% }
