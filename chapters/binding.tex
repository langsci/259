...
chapter discusses the advantages of this new view, some of the open issues, and the development to a
strongly lexical binding theory as a result of shortcomings of earlier approaches. The chapter also
addresses so-called exempt anaphors, that is, anaphors not bound inside of the clause. 
}
...
\itdopt{Anne: This is supposed to explain why HPSG develops its own binding theory
...
\emph{believe} \citep[\page 128]{MS98a}:\footnote{%
 See \citealp[Chapter~3.5]{ps2} and \crossrefchapteralp{control-raising} on raising. See also \citew[\page 679]{RR93a} on Binding
 Theory and raising.%
}
\ea
...
\itdopt{Anne: explain example: explain active/passive or skip it (it will be discussed later anyway)}
...
\end{forest}
\itdopt{Anne: Would be easier if you labeled the NPs as NP1, NP2, NP3.}
...
Now, the goal is to find restrictions that ensure that \added{English} reflexives are bound locally, personal
...
\itdopt{Anne: What do you mean? rel pro are anaphoric, not interrog ones;
...
conflicts with true non-local dependencies. 
\itdopt{
this is too long and too complicated, 
does not help the reader at all
all this should go ib section 10?
}
A possible way to fix this seems to be the introduction of
...
referential units come with a referential index in their semantic representation.\footnote{
 ...
 gender/""person:
\eal
\ex I told John that we \ldots
\ex Paul told Mary that they \ldots
\zl
See \crossrefchapterw[Section~\ref{coordination:sec-agreement-with-coordinate-phrases}]{coordination} for more on conjoined antecedents.
} (On referential
...
\itdopt{Anne: this cant be right: like is not an agentive verb
...
or an infinitival verb phrase into the valence list of the respective predicates (see also \crossrefchapteralt[Section~\ref{prop:sec-lex-rules}]{properties}). Since the
...
\ex[]{
\emph{likes}:\\
\argst \sliste{ NP$_i$, NP[\type{ppro}]$_i$ }
}
\zl
...
\eal
\ex[]{
John showed Mary$_j$ herself$_j$.\\
...
\itdopt{Add argst everywhere? Check text.}
}
\ex{
...
\itdopt{Anne: what’s this doing there ? should be in intro= traditional view. The last part with figure 5 should be in section 1 (how different is it from gov and binding theory?)\\
Stefan: Drop it?}
...
\inlinetodoopt{Stefan: Drop this section? 
...
\itdopt{Anne: this is old fashioned, skip it since we don not have traces now}
...
\zl
\itdopt{Anne: Refer to traceless analysis, what is the pb with ex 22  under a traceless analysis?
Knows <he, NP[gap]>
}
\itdopt{Anne: 22b can be an exempt anaphor, like […] This picture of himself was a disaster/ was depressing him.}
...
reach inside of the trace. There is just nothing below the trace. It is a terminal symbol without
daughters. \\
Anne: Take into account Bob’s comment= if the gap object is a synsem description, it has no daughter.
}
\ea
...
\zl
\itdopt{Discussing Pollard and Sag’s view on extraction seems obsolete, under current analyses, is there is a pb with (25b)?
...
\itdopt{Anne:
Instead of (\mex{1}), (\ref{def-local-o-command}), and (\ref{def-non-configurational-o-command}), please give us (\ref{def-non-configurational-o-command}) directly
Explain what it  does for you:
Paul likes this picture of himself
}
...
\end{enumerate}
\itdopt{Anne: having Z in its arg-st.\\
I’m not sure (\mex{0}) is a natural reformulation of (\mex{-1}), I would rather have
Y locally o-commands some X having Z on its arg-st list
}
\z
\citet[\page 303]{ps2} assume the following valence for a subject control verb like \emph{promise}
(\subcat is replaced by \subj):
\eal
\ex Kim promised (Sandy) to help Robin.
\ex \emph{promise}:
\argst \sliste{ NP$_i$(, NP), VP[\subj \sliste{ NP:\type{refl}$_i$ } ] }
\zl
...
\itdopt{Anne: I don't think the discussion of Visser’s generalization is worth it, this generalization does not hold (you say it later, see chapter raising and control); was it in Antonio’s part ? if not, skip it;
This is a handbook for people to read and understand what is good about HPSG, not obsolete examples and discussions
}
...
\ea
\emph{promised} (lexical item for the passive):\\
\argst \sliste{ NP$_i$, VP[\subj \sliste{ NP:\type{refl}$_i$ } ], PP[\type{by}]$_i$ }
\z
...
\ex 
\argstl of \emph{seem}:
\sliste{ \ibox{1} NP$_i$, PP[\type{to}, \type{ana}]$_i$, VP[\subj \sliste{ \ibox{1} ] } } 
\zl
The \argstl of \emph{seems} contains the subject NP (\emph{he}), the PP of the reflexive
(\emph{himself}) and the VP (\emph{to be observing a stone}). Since the
...
\itdopt{Anne: This recursive definition of o-command is really impressive and it can account for binding phenomena in approaches that do not have empty nodes for traces in the tree structures 
...
\itdopt{Anne: this may be ‘interesting’ but it is not for a handbook, it is for a paper.\\
...
\itdopt{Anne: The discussion about formal foundation starting p883 should be skiped, a handbook is for promoting solid published work, not for raising open issues, page 884 is not understandable;
...
\itdopt{Anne: There should be a subsection on adjuncts
...
\itdopt{‘conclusion’ is not clear=> should just be a sentence pointing to section 10}
...
\itdopt{Anne: this Section is a bit misleading, exempt anaphors are often used in indirect speech, Pollard and Sag refer to attested examples found by A Zribi Hertz, (\ref{ex-it-nothers-myself-that}) would be ok in such a context}
...
\itdobl{Anne: Turn this into a footnote and delete section. This section should be skiped and the problematic ex put in section 10 where a solution to them is given}
...
\itdopt{Section 7 should be called Binding and coordination (it is because the coord phrase is unheaded), and put as a subsection inside section 10}
...
\z
Since \emph{sich} is not local to \emph{ihm} and since reflexives are not exempt in \ili{German}
...
\itdopt{Anne: Section 9 should be called Binding and passive and should come before
...
\z
\itdopt{Anne: add an example to explain}
...
\itdopt{Anne: But sebia is a long distance anaphor isn’t it?\\
Stefan: This is MS's example. I am just discussing it.}
\ex
...
\itdopt{Anne: section should have subsections}
...
We thank Anne Abeillé and Bob Borsley for detailed comments on an earlier version of the paper and Bob Levine for discussion and Giuseppe Varaschin for comments.
