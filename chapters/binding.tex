...
\footnote{\citep[p.279]{polsag:hpsg94}.}
...
\ex
\begin{xlist}
\ex John_{i} said that Peter_{j} shaved himself_{*i/j}.
\ex John_{i} said that Peter_{j} shaved him_{i/*j}.
\end{xlist}
\end{exe}
...
\ex
\begin{xlist}
\ex[?]{The doctor_{i} told the patient_{j} [that the nurse_{k} would protect them_{i+j+k} during the storm].}
\ex[??]{The doctor_{i} said [that the patient_{j} told the nurse_{k} about them_{i+j+k}].}
\end{xlist}
\end{exe}
...
\begin{exe}
\ex John_{i} told Mary_{j} that Kim talked about them_{i+j}.
\end{exe}
...
\begin{exe}
\ex
		\gll O m\'{e}dico_{i} disse-me [que [o director do Pedro_{j}]_{k} ainda n\~{a}o reparou [que $\emptyset_{*i/*j/k}$ cometeu um erro]]. (Portuguese)\\
		the doctor told-me \mbox{ }that \mbox{ }the director of.the Pedro yet not noticed \mbox{ }that { } made a mistake.\\
		\trans 'The doctor_{i} told me [that [Pedro_{j}'s director]_{k} didn't notice yet [that he_{*i/*j/k} made a mistake]].'
...
\begin{exe}
\ex Mary_{i} thought the artist had done a bad job, and was sorry
that her parents
came all the way to Columbus just to see the portrait of herself_{i}.
\end{exe}
...
is encoded into its semantic representation, where r-mark_{w} is the reference marker
of {\it w}.
...
constraining effect of Principle A, \mbox{[r-mark_{John}, r-mark_{Mary}]} 
is the list of antecedent candidates of
{\it himself}, which will be narrowed down to [r-mark_{John}] when all the other
...
of this type, such as {\it John_{i} told Mary_{j} they_{i+j} would eventually get
...
with each other. That is, in addition to having marker {\em r_{a}} as an admissible antecedent
in its set of candidate antecedents, that anaphor N has also to eventually have marker {\em r_{b}} 
...
violations of Principle B are prevented, as illustrated with the help of the following example.
 ...
 \begin{exe}
\ex [*]{The captain_{i/j} thinks he_{i} loves him_{j}.}
\end{exe}
...
with an analogous pattern as above, but for non pronouns, are prevented:
 ...
 \begin{exe}
\ex [*]{When John_{i/j} will conclude his therapy, [the boy_{i} will stop believing [that the patient_{j} is a Martian]].}
\label{transitivityC}
\end{exe}
...
\trans 'When [the robot]_{i} concluded the task, the operator saw that [his_{i} wheel] was smashing [his_{i} power cord].'
\end{exe}
...
of the respective binding principle can be found for reflexives, as illustrated in the following example.
 ...
 \begin{exe}
\ex {The captain_{i} thinks he_{i/j} loves himself_{*i/j}.}
...
\begin{xlist}
 \label{nonquantOcom}
\ex[]{$[$The captain who knows this sailor]_{i} thinks Mary loves him_{i}.}
 \label{nonquantOcom}
\ex[]{$[$The captain who knows [this sailor]_{i}] thinks Mary loves him_{i}.}
...
introduced by \mbox{\textSigma-abstraction}, 
...
\begin{exe}
\ex[]{Every sailor_{i} has many girlfriends. They_{i}/He_{*i} travel(s) a lot.}
\end{exe}

And the next contrast illustrates that, in a bound-variable anaphoric link, the v-marker is singular:

\begin{exe}
\ex[]{Every sailor_{i} shaves themselves_{*i}/himself_{i}.}
\end{exe}
...
\ex\label{etype}
\begin{xlist}
\ex[*]{$[$Every captain who knows this sailor]_{i} thinks Mary loves them_{i}.}
\label{etypea}
\ex[]{$[$The captain who knows [every sailor]_{i}] thinks Mary loves them_{i}.}
\label{etypeb}
\end{xlist}
\end{exe}
...
\begin{xlist}
\ex[]{$[$Every captain who knows this sailor]_{i} thinks Mary loves him_{i}.}
\label{weakcrossovera}
\ex[*]{$[$The captain who knows [every sailor]_{i}] thinks Mary loves him_{i}.}
...
\sn
{$[$Every captain who knows [every sailor]_{i}]_{j} thinks Mary loves them_{i/*j}.}
\end{exe}
...
\sn
{$[$Every captain who knows [every sailor]_{i}]_{j} thinks Mary loves him_{*i/j}.}
\end{exe}
...
\begin{exe}
\ex[*]{$[$The captain who knows him_{i}] thinks Mary loves every sailor_{i}.}
\end{exe}
...
the possibility of binding in {\em Every man's_{i} mother loves him_{i}}
...
\end{exe}
\vspace{-7mm}
 \centerline{\includegraphics[width=18pc]{bindingSquareOpposition.pdf}}
...
\end{exe}
\vspace{-10mm}
\centerline{\includegraphics[width=13pc]{patternSquareOpposition.pdf}}
...
\end{exe}
\vspace{-7mm}
\centerline{\includegraphics[width=12pc]{patternSquareDuality.pdf}}
...
\end{exe}
\vspace{-7mm}
\centerline{\hspace{0 mm}\includegraphics[width=14pc]{classicalSquareDuality.pdf}}
...
via the application of the DRT Abstraction operator \textSigma, which
...
\mbox{\textSigma-abstraction}, in the {\sc conds} value of a determiner, 
...
\mbox{\textSigma-abstraction} is a plurality.
