...
reflexives in (\mex{1}) have to refer to the referent of an NP in the same clause and they
have to have the same \isi{gender} as the NP they are coreferent with:
\eal
...
tree. Accoring to one definition, a node Y is said to c-command another node Z, iff Y and Z
are sisters or if a sister of Y dominates Z.\footnote{\label{fn-c-command-GB}%
...
projection\is{projection!unary}), in my opinion an unwanted consequence of this proposal. 
...
(\ref{def-local-o-command}) and (\ref{def-o-command}) is necessary to deal with cases of \isi{raising}, in
...
\citet[\page 265]{ps2} point out an interesting consequence of the treatment of nonlocal dependencies in HPSG:
since nonlocal dependencies are introduced by traces that are lexical elements rather then by
deriving one structure from another one as is common in Transformational Grammar, there is no
way to reconstruct some phrase with all its internal structure into the position of the trace. Since traces do not have daughters,
...
Billionen DM) vorfristig zu erfüllen, [\ldots]\us\footnote{
...
as well. So, if one assumes that a central phenomenon like control is handled parallely in English
...
raised and different from Y.\footnote{%
  ...
  feature \feat{raised} \citep{Prez99,Meuerers99}. This would simplify things considerably.%
...
component in the definition is not formalizable and suggested the following revision:
\ea
...
objects without changing binding behavior (see also \citealp[\page 13]{Riezler95a}):\footnote{%
  ...
  bound anaphor must be bound by a co-argument that precedes it.'' (\citealp[]{AW96a-Workshop} for and
  \citealp[\page 44]{AMM2017a-u} for Moro). Rather than incorporating linear precedence into
...
canceled-off is not ordered there is too much freedom and the result is \isi{spurious ambiguities}. 
...
languages like English and German a (non-reduced) relative clause consists of a relative phrase and a sentence in
which this relative phrase is missing. The relative phrase has to contain a relative word. The
simplest case is of course a relative clause with just a relative pronoun in initial position but as
...
relative pronouns and their antecedent match in gender and number in languages like German. This is parallel to the
...
%
The example may still seem a bit artificial but there are attasted examples from newspapers \citep[\page 420]{Mueller99a}:
\eal
...
\zl
If one assumed transformational theories of coordination deriving (\mex{0}) from (\mex{1})\todostefan{ref}, the
problem would be solved, but as has been pointed out frequently in the literature such
transformation-based theories of coordinations have many problems
\parencites[\page 102]{BV72}[\page 192--193]{Jackendoff77a}[\page143]{Dowty79a}[\page 104--105]{denBesten83a}{Klein85}{Eisenberg94a}[]{Borsley} and nobody ever assumed something
parallel in HPSG (see \crossrefchaptert{coordination} on coordination in HPSG).
\ea
...
\section{\argstls with internal structure}
...
\citet{AGS1998a-u,Branco2002a-u,Branco:Marrafa:99,Branco98a-u,Pollard2005a-u,PX2001a-u,Riezler95a,XMcF98a-u,Xue:Pollard:ea:94,Buering2005a-u}
...
I thank Anne Abeillé for discussion and Bob Levine for discussion and for making \citet{HL95b} available to me.
