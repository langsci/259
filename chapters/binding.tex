...
\abstract{
This chapter is an introduction to the Binding Theory assumed within HPSG. While it was inspired
...
alone is not sufficient and reference to the syntactic level of argument structure is
...
chapter discusses the advantages of this new view and its development to a strongly lexical binding
...
\z
\added{So binding is not coreference.}
...
are divided into reflexives (\type{refl}) and reciprocals (\type{recp}).
...
o-commands}\is{o-command!local} Z in the case that Y is less oblique than Z.
\z
...
\eanoraggedright
\label{def-o-bind}
Y (\emph{locally}) \emph{o-binds}\is{o-bind} Z in the case that Y and Z are coindexed and Y (locally) o"=commands Z. If Z
...
\footnotetext{
From \citew[\page 349]{Fanselow86a}.
}
\ex[]{
...
assumes that an extraposed \emph{it} is inserted into the \argstl and the clause is appended to
this list:
\eal
\ex[]{
...
away from the topic at hand, I want to discuss Koenig's lexical\is{lexical rule!for body part nouns|(} rule for possessive nouns. He assumes a rule
...
Let Y and Z be \type{synsem} objects with distinct \localvs, Y referential. Then Y o-commands Z in the case that either:
...
clauses were analyzed as involving an empty relativizer (\citealp[Chapter~5]{ps2}; \crossrefchapteralp[Section~\ref{sec:rc-pollard--sag}]{relative-clauses}). One way to eliminate this empty element from
grammars is to assume a headless schema that directly combines the relative phrase and the clause from which
it is extracted \parencites[Section~2.7]{Mueller99b}[522]{Sag:10b}[\page 345]{MuellerCurrentApproaches}. In addition, there were proposals to analyze free
...
From \citew[\page 50]{Varaschin2021a-u}
}
\zl
