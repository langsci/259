...
$k$). \changed{Binding} of \emph{himself} \changed{to} \emph{Mary} is ruled out in (\mex{0}b), since \emph{himself}
has an incompatible gender. \added{Expressions like \emph{Mary}, \emph{the morning star}, \emph{Venus}, \emph{fear} are
...
\changed{semantic level with respect to thematic roles \parencites[Section~4.10]{Jackendoff72a-u}{Wilkins1988a-u}[Chapter~6]{Williams1994a-u}:}
%\citep{BP80a}: 
...
% p. 678
\added{\crossrefchaptert[Section~\ref{cg:binding}]{cg} discusses an approach to binding operating on
semantic formulae.}%
} For examples like (\ref{ex-binding-reflexives}), a theory assuming that reflexives and their antecedents have to
\changed{fill a semantic role} of the same head makes the right predictions,
...
person, number, and gender features of the involved items are compatible, \added{since these features are
part of the index}. 
...
pronouns are not bound locally and that \changed{r-expressions like proper names and full NPs are
...
binding theories can be explained in a configurational binding theory in HPSG. \added{So, while it ultimately may turn
...
\added{See \citew{Levine2010a} for special cases, for example, singular
...
fronted element and the missing object.\footnote{\added{\gisu pointed out to me that pronouns are possible
...
\added{Steve Wechsler (p.c. 2021) pointed out to me that the totally non-configurational binding theory
...
\added{Alternatively, one might not assume ``reconstruction'' and explain the effects by different means
...
reflexives do not apply and hence both a pronoun and an anaphor is possible:\footnote{
  \added{There are various conflicting judgements of examples like (\mex{1}) in the literature. For an
  overview and an experiment see \citew[Chapter~3]{Golde1999a-u}.}
}
...
\noindent
Later work in HPSG abolished traces alltogether (\citew*{BMS2001a}; \crossrefchapterw{udc} but
...
issues.\footnote{
\added{One was already mentioned in footnote~\ref{fn-non-configurational-BT-SW}.}
}\footnote{%
...
(\mex{0}c,d) can.
\added{In relation to the discussion of examples like (\mex{0}), \citet[\page 233]{Walker2011a} noted that whether binding of the subject pronoun is possible also
depends on the attachment position of the adjunct. While binding of a subject pronoun into a VP
adjunct is out (\mex{1}a), binding into a sentential adjunct is fine (\mex{1}b).
\ealnoraggedright
...
\citet{Wechsler99a-u} compares GB analyses with \argst-based HPSG analyses \added{and shows that the GB
analysis, which seems to be parallel to the HPSG analysis, does not extend to the Balinese facts
but results in an insoluble contradiction.}
...
\added{\gisu pointed out to me that many \iwithini violations may be due to semantic/pragmatic
...
\z
So may be large parts of the explanation of \iwithini effects can be found in semantics/""pragmatics.
}}
\ea
...
\ex[]{
der Besitzer$_i$ seines$_{*i}$ Bootes\\
...
\added{\ex {}That woman$_i$ listening to her$_i$ own voice on the radio is Barbra Streisand.\footnote{
\citew[\page 71]{Culicover1997a-u}
}}
\zl
...
situation. \added{Similarly, the subject of \emph{listening} is the antecedent of \emph{her}.}
...
Similar to the notion assumed in Minimal Recursion Semantics (MRS, \citealp*{CFPS2005a}, see also \crossrefchapteralt[Section~\ref{semantics:sec-mrs}]{semantics} for an introduction to MRS) there is an
