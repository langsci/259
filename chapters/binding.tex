...
\author{%
António Branco\affiliation{University of Lisbon} \lastand Stefan Müller\affiliation{Humboldt-Universität zu Berlin}
}
...
say what BT has to account for. And of course we have to have a BT. We will see on the way, where
...
\end{figure}
\citet{Chomsky81a,Chomsky86a}\addpages suggested that tree-configurational properties play a role in
...
depictive predication\is{predicate!depictive secondary} \citep[Section~2]{Mueller2008a}.
...
is nothing like the ``reconstruction'' known from GB.
...
analysis that treats the extraposition \emph{it} as a referential element \citep[\page 215, 232]{Mueller99a}.
...
get rid of the subject. This could have been done for the traceless analysis as well, but I think BT
was not discussed any longer.
}

\noindent
Later work in HPSG abolished traces alltogether (\citealp*{BMS2001a}; \crossrefchapteralp{udc} but
see \citew[Section~4.9]{MuellerCurrentApproaches} for a trace-based approach and Müller \citeyear{Mueller2004e}; \citeyear[Chapter~19]{MuellerGT-Eng4} on empty elements in general) and hence BT cannot rely on
...
Apart from the elimination of the model component in the defintion of vc-command, there is a further
...
non-local dependency (\ili{German} is a V2 language, see \cites\citealp[Chapter~2.4]{Erdmann1886a};
\citealp[\page 69, \page 77]{Paul1919a}[Section~3]{MuellerGermanHandbook}) and that the arguments are scrambled in
...
lexical rule demoting the subject argument and adding an optional PP.\footnote{
  See also \cites{AG}[\page 114, 116]{MS98a}{Mueller2003e,MOe2013a} for lexical rule-based analyses of the
\itdopt{Which paper shall I cite on French?}
  passive in \ili{French}, \ili{English}, \ili{German}, and \ili{Danish}. See \crossrefchapterw[Section~\ref{arg-st:sec-passives}]{arg-st} for an overview.
} 
\ea
...
\textsc{nonlocal} as the value of a newly introduced feature \feat{bind}. These binding lists differ from
