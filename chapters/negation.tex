...
%in question.\footnote{This chapter grows out of \citep{Kim:00} and \citep{Kim:18}.}
...
\gll Robin  n'aime  pas  Stacy. \\
     Robin  (n')likes  \textsc{neg} Stacy \\
\trans`Robin does not like Stacy.'
}
\ex[*]{
...
\trans `Not speaking French is a great disadvantage in this case.'
}
\ex[*]{
...
\ex[]{
\gll
[Ne           pas  [\sub{VP} parler  fran\c{c}ais]]  est  un grand d\'{e}savantage  en ce cas. \\
...
have taken this group of adverbs (Adv\jbsub{{\sc i}}) including the sentential negation {\it not} to function as the complement of a finite auxiliary verb via the following lexical rule:
...
\glt {`Robin does not like Stacy.'}\label{pas-good-a}}
 \ex[ ]
 {
 \gll Robin  (n')aime  pas  Stacy. \\
Robin  (n')likes  \textsc{neg}  Stacy \\
\glt{`Robin does not like Stacy.'}\label{pas-good-b}}
\zl
...
\begin{figure}
	...
	\end{forest}
	\caption{Negative-Infl Construction ($\uparrow$\textit{infl-cxt})}\label{fig:2}
\end{figure}	
\fi
...
\begin{figure}
	...
	\end{forest}
\caption{Partial structure of (\ref{basque-ex})}\label{basque}
\end{figure}
%
%
%
...
\gll Gianni non telefona a nessuno. \hfill (Italian) \\
     Gianni {\sc neg} telephones to no.one\\
...
%
  ...
  his linear-based approach assigns the negative expression in
...
that a negative element bearing the positive {\sc neg} feature is either in the second or the third field.  One advantage of this approach can come from a simple way to capture the differences between Italian and Welsh
by referring to the difference in the domain value of {\sc neg} elements. That is,
within the analysis, the Welsh {\sc neg} bearing negative, different
from Italian, is in the second or the third field as illustrated in the following domain for the sentence (\ref{position-1c}) (from
...
In addition to this work focusing on the distributional possibilities
 ...
 \citet{Prz:00} focuses on the non-local genitive of negation in Polish where the object argument is not accusative but genitive-marked with the presence of negative marker as in (\ref{genitive}b). %In Polish,  negation is realized as the prefix
...
\glt `No one is such that they love no one.' \hfil (double negation)
\glt `No one likes anyone.' \hfil  (negative concord)
\z
%
...
\ex[]{\begin{avm}
\[form  \;  \<\type{n'aime}\>\\
 arg-st \<NP[{\sc store} \{\avmbox{1}\}], NP[{\sc store} \{\avmbox{2}\}]\>\\
 quants \<\avmbox{1}, \avmbox{2}\>\]\end{avm}}
\ex[] {
\begin{avm}
\[form  \;  \<\type{n'aime}\>\\
  arg-st \<NP[{\sc store} \{\avmbox{1}\}], NP[{\sc store} \{\avmbox{2}\}]\>\\
  quants \<\@1\>\]\end{avm}}\label{quant}
\zl
%
%\; \; :
