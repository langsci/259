...
%\fi
\noindent
\changed{The lexical specification as premodifier (\textsc{pre-modifier}+) together with an LP rule requiring
...
\changed{Since the negator modifies a VP it follows} that the negator does not separate an infinitival verb
...
within the system of Minimal Recursion Semantics (MRS). On MRS see also \crossrefchapterw[Section~\ref{semantics:sec-mrs}]{semantics}.}
...
constituent negator, the sentential negator \textit{not} typically
 follows a finite auxiliary verb. \changed{\textit{Too}, \textit{so}, and \textit{indeed} also behave like this:}
\eal
...
follow a finite auxiliary verb. \changed{This suggests that \emph{too}, \emph{so}, \emph{indeed} and the sentential \emph{not} belong to
...
rule:\footnote{The symbol $\oplus$ stands for the \changed{relation} \emph{append}, i.e., a relation
  that concatenates two lists. \added{The rule adds the adverb to the \compsl. More recent variants
...
\ea
\changed{Adverb-Complement Lexical Rule}:\\
\changed{\avm{
[\type*{fin-aux}
...
adverb {\it pas}
as a finite verb's complement.\itdopt{for Elizabeth: Check English ``In that French ...''}}  This would then
...
%\fi
As shown in Figure~\ref{negation-fig:6}, the \changed{finite auxiliary
verb} \textit{could} combines with two complements, the negator
\textit{not} (Adv\jbsub{\textsc{i}}) and the VP \textit{approve the bill}.
This combination results in a well"=formed head"=complement \changed{phrase}.
...
behaves like a slashed expression in not mapping into the syntactic grammatical
 ...
 \argst to \comps). \added{The YP without structure sharing is a shorthand for carrying over all
...
previous work, \citet{AG:97} and \citet{Kim:00}\itdopt{Stefan: 1997 is before 2002. I would switch
  this but I cannot do it without your ok since the 2002 paper may be earlier than the 1997 one but
  delayed.}
allow the adverb \textit{pas} to function
as a syntactic complement of a finite verb in \ili{French}.\footnote{Following \citet{AG:2002}, one could assume \textit{ne} to
...
and \emph{pas} is the morphosyntactic value [\AUX\ $+$] \added{of the verb they combine with}, and this induces
...
\gll tabe-sase-na-i/*tabe-na-sase-i \\
     eat-\textsc{caus}-\NEG-\textsc{npst}/eat--\NEG-\textsc{caus}-\textsc{npst} \\\hfill\added{(\ili{Japanese})}
...
straightforwardly by \changed{the following lexical rule} (see \cites[\iaddpages]{Kim:00}[111--112\iaddpages]{Crowgey:12}):
\itdopt{Kim does not contain a lexical rule and Crowgey contains something but this is not even
  remotely similar to the rule below.}
\ea
...
\z
%
\changed{Following \citet[\page 10]{BK:12}, one} can treat these clitics as affixes and generate
...
\citep{Payne:85}\addpages. \ili{Finnish} also exhibits this property \citep[\page 376]{Mitchell:91}:
...
\gll ilk-ko/*-ci siph-ta. \\
     read-\textsc{conn}/\textsc{conn} would.like-\textsc{decl} \\\hfill\added{(Korean)}
...
\gll Mimi-nun          (yehathun)           tosi-lul          (yehathun)           ttena-ci            (*yehathun) anh-ass-ta. \\
     Mimi-\textsc{top} \hphantom{(}anyway city-\textsc{acc} \hphantom{(}anyway leave-\textsc{conn} \hphantom{(*}anyway \NEG-\textsc{pst}-\textsc{decl} \\\hfill\added{(Korean)}
\glt `Anyway, Mimi didn't leave the city.'
\z
%
Further, in an elliptical construction, the elements of a verb complex
 always occur together. Neither the lexical  verb \added{(\ref{negation-fragment}c)} nor the
 auxiliary verb alone \added{(\ref{negation-fragment}d)} can serve
...
\gll Kim-i hakkyo-eyse pelsse tolawa-ss-ni? \\
     Kim-\textsc{nom} school-\textsc{src} already return-\textsc{pst}-\textsc{que} \\\hfill\added{(Korean)}
...
\noindent
The \changed{lexical verb and the auxiliary must appear
together as in (\ref{negation-fragment}b)}. These constituenthood properties indicate that the
...
\gll Gianni  vuole  che  io  non  legga  articoli  di  sintassi. \\
     Gianni  wants  that  I   \NEG{}   read  articles  of  syntax \\\hfill\added{(\ili{Italian})}
...
languages. Such an analysis, however,
  ...
  in contrast to the morphological negative, the negative auxiliary, and the \changed{adverbial
  negative}.
...
\gll Gianni non    legge articoli di sintassi.\\
     Gianni \NEG{} reads articles of syntax\\\hfill\added{(\ili{Italian})}
\glt `Gianni doesn't read syntax articles.'
\end{exe}
...
The principle allows a \textsc{neg}+ verbal expression to assign
 the \textsc{case} value \textit{gen} to \changed{all
...
allows the \changed{verbal} complement of a raising verb like \type{seem} to optionally undergo lexical
...
\added{Representation for \emph{nie wydawał siȩ} `did not seem' when combined with \emph{lubić} `like':}\\
\changed{
\avm{
...
\changed{lexical construction-specific} case assignment (or satisfaction).
