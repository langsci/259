...
As we have noted earlier, it is a general fact of French that \textit{pas} must follow a finite verb, in which case the verb optionally bears negative morphology (\textit{ne}-marking) (\citealp[\page 361]{KS:02}):
...
to represent a key difference between English and French, in that
French has no restriction on the feature \AUX\ to introduce the negative
adverb {\it pas}
...
\citet{AG:2002} for French adverbs including
\textit{pas}.  Building upon this
previous work, \citet{AG:97} and \citet{Kim:00}
allow the adverb \textit{pas} to function
as a syntactic complement of a finite verb in French.\footnote{Following \citet{AG:2002}, we could assume \textit{ne} to
...
As noted earlier, languages like Turkish and Japanese employ morphological negation where the negative marker behaves like a suffix (\citealt[\page 171]{kelepir} for
...
\gll Gianni non telefona a nessuno.\\
     Gianni \textsc{neg} telephones to nobody\\ \hfill (Italian, \citealt[\page 62]{Borsley:06})
...
\gll Dw i ddim wedi gweld neb.\\
     am I \textsc{neg} \textsc{perf} see nobody\\ \hfill  (Welsh, \citealt[\page 108]{Borsley:05})
...
In addition to this work focusing on the distributional possibilities
 ...
 In Polish, negation is realized as the prefix \type{nie} to a verbal expression (see \citealt{PK:99, Prz:00, Prz:01}), and
...
\Citet{Swart:02} investigate  negative concord in French, where multiple occurrences
