...
\citet{KS:96, Kim:00, KS:02} develop analysis of sentential negation in English, French, Korean, and Italian within the framework of HPSG, showing that the postulation of Neg and its projection NegP creates more empirical and theoretical problems than it solves (see \citet{Newmyer:06} for this point).
...
of negation.  For the account of English negation, \citet{Warner:00},
further developing the analyses of \citet{KS:96, Kim:00},
characterizes negation within the English auxiliary system without the use of lexical rules, explores inheritance hierarchies in interpreting the distributional possibilities of negation in various environments. For instance, \citet{Warner:00} classifies auxiliaries into two subtypes with respect to negation and inversion, each of which is again subclassified in terms of being negated and inverted.
...
\citet{Borsley:06}, adopting \citet{Kathol:00}'s topological approach,
...
With the semantic assumption that the contribution of negation in a concord context is semantically empty, they formulate an HPSG analysis for negativem concord. This analysis is adopted to account for negative concord in Welsh by \citet{BJ:05}. Meantime, \citet{Warner:00} and  \citet{BL:13} explores an HPSG analysis with the mechanism of MRS (Minimal Recursion Semantics) to for the scopal interaction of negation with auxiliaries (modals) and quantifiers.
...
\begin{figure}
	...
	\end{forest}
\caption{An instance (construct) of the \hd-lite}
\end{figure}
%
...
emposed on by the \hd-lite\
in (\ref{hd-lex-cxt}). That is, the \hd-lite\ licenses
...
\hd-lite\ through the combination with the lexical  verb \textit{mek-ko} eat-\conn.
...
of each auxiliary verb and the \hd-lite.\footnote{
As hinted here, the present system does not allow the final negative
auxiliary {\it anh-ta} `\neg-\decl' to combine with the preceding auxiliary {\it ha-ci} `do-\conn' at first. This is because the resulting verb-complex {\it ha-ci anh-ta} `do-\conn \neg-\decl' then cannot have the information that its preceding verbal expression ({\it siph-e}) must have the specified form value, which is coming from the following auxiliary verb {\it ha-ci}.
...
{\it not} must cooccur with the particular coordinator {\it but} (e.g., {\it *Kim likes not beer}. vs. {\it Kim likes not beer but wine.})}
%
\ea
\label{c-neg}
\begin{avm} \avml
...
\ees[i]{\item They will obviously not have time to change.
\item You are usually not thinking about the right problem.}
%
The sentential negation {\it not} in such examples has a wide scope reading,
as seen from the possible tag question:

\ees[i]{\item They will obviously not have time to change, will/??won't they?
\item You are usually not thinking about the right problem, are/??aren't you?.}
%
The sentential negation with a wide scope reading can be separated
by a variety of intermediate constituents (see Warner 2000):

\ees[ii]{\item
\item They should, though, I suggest, not expect me to grant it immediately, should they?
\item They could, frankly, however, not have anticipated the problems that
...
specified by the construction in (\ref{vpe-cxt}):
%
%
%
\begin{figure}
