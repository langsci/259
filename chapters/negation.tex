...
%\inlinetodostefan{Page numbers are missing, have a look at chapter on order or chapter on %coordination to
 % see what is required. You cite books and it is unclear what you are referring to. }
...
differently from the negative in English and French. See \citet[Chapter~11]{MuellerGT-Eng1}
...
\z
 ...
 with the lexical rule in (\ref{lr-neg-word-formation}):\footnote{\citet{BK:12} note that
...
%  rule but it is two forms. This is very confusing. jb: this part is revised to make it  %simple\\
%Stefan: But this simplification is wrong. It does not make sense formally. Please change %into two
%separate lexical items and say that one is licensed by the lexical rule %(\ref{lr-neg-word-formation}).}
%
%
%
%
...
In addition to this work focusing on the distributional possibilities
 ...
 \citet{Prz:00} offers a HPSG analysis for the non-local genitive of negation in \ili{Polish}.
 ...
 %In Polish,  negation is realized as the prefix
  ...
  the unbounded relation as shown in (\ref{negation-genitive-1}) (data from \citealt[\page 120]{Prz:00}):
...
we often find in languages like French, Italian, Polish, and so forth.  \Citet{Swart:02} investigate  negative concord in French, where multiple occurrences
