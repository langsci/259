...
\inlinetodostefan{Page numbers are missing, Stefan will mark necessary additions in the final read. }
...
[ cat|head & [\type*{adv}
 ...
 %                        arg1 & \1 ] \} ] ]}
\z
\itdgreen{JP: Since at least the last chapter of P\&S (1994) and Davis (1996), many folks tend to treat relation labels such as “neg-rel” not as values of a PRED attribute but as the sort of the entire restriction (opr value of CONTENT or MAIN ... Are you sure you want it to be the value of PRED? Your choice}
...
specified by the following lexical rule \parencites{Kim:00}[\page 209]{kimmichaelis:2020}:
...
as a syntactic complement of a finite verb in \ili{French}.\footnote{Following \citet{AG:2002}, we could assume \textit{ne} to
...
\z
\itd{JP: no orphaned tags: \ibox{2}}
%
%
...
f\sub{\type{neg}} in Libyan Arabic\il{Arabic!Libyan} allows the attachment of the negative prefix
...
\citet{Koenig99a}, \citet{Sag:12}, \citet{Cryssman:Bonami:2016}, and  
\citet{Hilpert:16}.
\itdblue{JP: t is a little odd to contrast the lexical-rule based approach and constructional
...
\citep{Payne:85}. \ili{Finnish} also exhibits this property \citep{Mitchell:91}:
...
the auxiliary verb alone can serve as an independent fragment answer. The two verbs must appear
...
[\type*{head-light-phrase}
 ...
 dtrs     & < \3 [light & $+$\\
                  comps & \1 ], \2 [light & $+$\\
                                    comps & < \3 > ] > ]}
\z
\itdblue{JP: Since the syntactic schema already references the COMPS of the lexical complement (1), I don’t see the point of adding (1) to the head. In fact, it would be more strictly syntactic/constructional argument composition if (1) was not part of the COMPS value of the head-daughter. What do you think?}
...
creating a well-formed \textsc{head-light-phrase}.
...
an existing \textsc{head-light} phrase with the final auxiliary bearing an appropriate
  ...
  verbs we can add (see \citealt[\page 88]{Kim:16} for detailed discussion).
...
\itdblue{JP: And still the same remark as above, the argument attraction, (2) here, does nothing and, IMHO, should be dropped.}
\end{figure}
%
%
%
...
\citealt{Borsley:06}):\footnote{Different from \citet{Borsley:06}, \citet{BJ:00} offer  a selectional analysis of \ili{Welsh} negation.
...
\itdgreen{JP: Somewhere in this section reference should be made to Richter and Sailer’s work on negative concord and to the chapter on semantics in the Handbook which explicitly discusses this issue.}}
...
and offer HPSG analyses. Consider a \ili{Welsh} example from \citet{BJ:00}:
...
and syntactic categories.  By admitting morphological and syntactic categories, we have been able to
capture their idiosyncratic properties in a simple and natural manner. Furthermore, this theory has
been built upon the lexical integrity principle, the thesis that the principles that govern the
...
I thank reviewers of this chapter for detailed comments and suggestions, which
helped improve the quality of this chapter a lot. I also thank
Anne Abeill\'{e}, Bob Borsley, and Stefan M\"{u}ller for constructive comments on
an earlier version. My thanks also go to Okgi Kim, Rok Sim, and Jungsoo Kim for
helpful feedback.
