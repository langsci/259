...
\citet{KS:96, AG:97, Kim:00, KS:02} develop analysis of sentential negation in English, French, Korean, and Italian within the framework of HPSG, showing that the postulation of Neg and its projection NegP creates more empirical and theoretical problems than it solves (see \citep{Newmeyer:2006} for this point).
...
of negation (see, among others, \citep{PK:99, BJ:00, Prz:00, Kupsc:02, Swart:02, Borsley:05, Crysmann:10, Bender:13}.)
...
the theory of grammar.\footnote{This chapter grows out of \citep{Kim:00,18}.} We
...
grammar, e.g rather questionable subtheories (see \citep{Kim:00} and \citep{KS:02}
...
possibilities in an example like (\ref{not-two}) (see \citep{Klima:64, Baker:89}, and \citep{Warner2000a-u}).\footnote{\citet{Warner2000a-u} and \citet{BL:13}
...
has taken this group of adverbs (Adv\jbsub{{\sc i}}) including the sentential negation {\it not} to function as the complement of a finite auxiliary verb via the following lexical rule:
...
VP[\textit{bse}] is unrealized (see \citep{Kim:00, KS:08} for
...
straightforwardly by a lexical rule given in the following (see \citep{Kim:00,Crowgey:12}):
...
an existing Head-Light construct with the final auxiliary bearing an appropriate
  ...
  verbs we can add (see \citep{Kim:16} for detailed discussion).
...
feature {\sc neg}.\footnote{In Polish,  negation is realized as the prefix
  \type{nie} to a verbal expression (\citep{PK:99, Prz:00, Prz:01}).} The
...
for why {\it pas} does not induce a resumptive interpretation with a quantifier (from \citep{Swart:02}):
...
\z
%
In this standard French example, \citep{Swart:02}, accepting
...
reading for standard French, inducing double negation as in (\ref{qstore}):\footnote{See \citep{Swart:02} for cases where \textit{pas} induces negative concord.}
...
\citep{BJ:00} for detailed discussion.}
