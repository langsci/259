...
(reversing the truth value of a certain sentence), but employs
language particular grammatical strategies. Natural
languages adopt four main types of negative markers
in expressing negation in the languages: morphological negative,
auxiliary negative verb, adverbial negative, and clitic-like
preverbal negative. This chapter provides an overview
of HPSG analyses for these four strategies in expressing negation.}
%
%Change the  abstract in chapters/03.tex \lipsum[3]}
\maketitle
...
in expressing negation in
languages: morphological negative,
auxiliary negative verb, adverbial negative, and clitic-like
 preverbal negative (see \citep{Dahl:79, Payne:85, Dryer:05}).
...
verb by affixation. Meantime, languages like Korean
 ...
 verb here is marked with the basic verbal categories such as agreement, tense, aspect, and mood, while the lexical verb remains in an invariant, participle form. The third major way of expressing negation is to use an adverbial
negative. This type of negation, forming an independent word, is prevalent in English and French, as given in (\ref{1c}). In these languages, negative markers behave like adverbs in their ordering with respect to the verb. The fourth
...
This chapter reviews construction-based HPSG analyses of these four main types of negation,
...
For example, in \citegen{Pollock:89} proposal, all verbs in French
...
the literature (see \citep{Pollock:89,Pollock:94,Pollock97a-u,Pollock:97b, Vikner:94,Vikner97a-u}). In \citet{Pollock:89},
...
considerably in subsequent work (see, among others, \citep{Zanuttini:01, Bo:14,
Zeijlstra:15}).  As far as we are aware, there is no agreed upon movement-based analysis of
...
in Welsh. Welsh allows negation to occur in the preverbal position, as
shown in (\ref{NC-1a}). In addition, the language  has negative concord where
...
\gll Ni soniodd neb am y digwyddiad \\
     {\sc neg} mention.{\sc past}.3{\sc sg} noone about the event\\
...
the multiple realizations of negation, as in (\ref{position-1c}) (data from \citep{Borsley:06}).
...
%
 ...
 and assumes that a negative clause in these languages has a negative element (bearing the feature {\sc neg}) in the specified field.
...
negative {\it ez} prefixed to a finite verb stem (data from \citep{CB:11}):
...
feature {\sc negated} as well as a verb complex structure (similar
to the Head-Light Construction).
...
\gll Personne (n’)a  rien fait \\
     Noone {\sc ne}-has nothing done\\
\glt `No one had done anything.'
\zl

With the semantic assumption that the contribution of negation in a concord context is semantically empty, they formulate an HPSG analysis for negativem concord. This analysis is adopted to account for negative concord in Welsh by \citet{BJ:00}. Meantime, \citet{Warner2000a-u} and  \citet{BL:13} explores an HPSG analysis, assuming Minimal Recursion Semantics (MRS) to account for the scopal interaction of negation with auxiliaries (modals) and quantifiers.
...
Departing from the derivational view, the HPSG literature has offered
...
morphosyntactic and valence properties of syntactic heads, and constructional properties (see \citep{Kim:00, KS:02, Crowgey:12}).
...
(see \citep{Payne:85}). Finnish exhibits this property \citet{Mitchell:91}:
...
\[\type{hd-lex-cxt }\\
\COMPS\  \textit{L}
\LEX\  +\]    $\rightarrow$ \@1 \[\LEX\  +\\
                                  \COMPS\  \textit{L}\], H \[\LEX\  +\\
                                           \COMPS\  \q<\@1\q>\]
\end{myavm}
\z

\noindent   This construction rule means that a \LEX\ head
expression combines with a \LEX\ complement, yielding
...
forming a well-formed head-light construct.\footnote{The negative auxiliary
...
as well as the \textit{ae} \VFORM\ head feature from the auxiliary (see
\citep{AG:97} for similar behavior in French auxiliary constructions). Meanwhile,
...
The third main type of negation is to employ
an adverbial negative. There are two main factors
...
English is not exceptional in this respect \citet{Baker:89,Baker:91,Ernst:92}.
The negation \textit{not} precedes an infinitive verb, but cannot follow
a finite lexical  verb.
%\footnote{A similar contrast between finiteness and
...
the derivational view (exemplified by \citep{Pollock:89, Chomsky:91})
...
the combination of two lexical expressions such as verb and particle (e.g., \emph{figure out, give
  up}, etc), as well (see \citep{KM}). The construction, along with the assumption that the
sentential negator \emph{not} bears the \LEX\ as well as {\sc neg} features projects
a structure like in Figure~\ref{fig:6}.
...
head-light structure,  expressions like
...
\begin{figure}
	\begin{forest}
		[\begin{avm}
			...
			vform & fin\\
			neg & $+$\]\\
			arg-st & \<\@{1}\,XP{,} $\textnormal{Adv}$ \[light & $+$\\neg & $+$\]{,} \@{2}\,XP\>\]
		\end{avm}
			[\begin{avm}
				\[\tp{fin-aux}\\
				syn|head & \[aux & $+$\\
...
forming a well-formed head-light construct. This construct then can combine with a VP complement,
...
by a variety of intermediate constituents (see \citep{Warner2000a-u}):
...
\ex[*]{
Gianni ha non parlato. \ \ \ \citet[12]{Belletti:90}
}
\zl
...
that select a head expression \citet{Eynde:07,kimsells:2011,Sag:12}, the present analysis takes \emph{non} as a functor, as represented by
...
then licenses the combination of the negative \emph{non} with the following
...
\ex[*]{
Maria ha sempre non pagato le tasse.  \citet[123]{Zanuttini:91}}
\zl
...
Gianni   claims   to   have   \textsc{neg}  gone.out \ \ \ \citet[90]{Belletti:90} \\
}
%\ex
%\gll *Gianni  sostiene   di   essere   non   uscito. \\
...
the earlier version. My thanks also go to Sim Rok and Jungsoo Kim for
helpful feedback. An earlier version of this paper also appeared in
(\citet{kim:18}).
