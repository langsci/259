...
verb by affixation. Meanwhile, languages like \ili{Korean}
 ...
 for sentential negation. See \citet{Kim:00,Kim:16} and references therein for detail\add{s}.}
...
It starts with the HPSG analyses of adverbial negatives in \ili{English} and \ili{French}, which have been most extensively studied in Transformational Grammars,
...
\itdopt{Suggestion: Mention the other sections. Otherwise referring to the final section is strange.}
...
we find in \ili{English} and \ili{French}.
...
or a lexical  verb (see \citealt[Chapter~3]{Kim:00}, \citealt{KS:02}).\footnote{\ili{German} also
employs an adverbial negative \textit{nicht}, which behaves quite
differently from the negative in \ili{English} and \ili{French}. See \citet[Section~11.7.1]{MuellerGT-Eng1}
for a detailed review of the previous theoretical analyses of \ili{German} negation.}
...
in \ili{English} and \ili{French}, as noted earlier, the derivational view (exemplified by \citealt{Pollock:89} and \citealt{Chomsky:91})
...
between \ili{English} and \ili{French}. By simply assuming that the
two languages have different scopes of verb movement -- in \ili{English}
only auxiliary verbs move to a higher functional projection, whereas
all \ili{French} verbs undergo this process -- the derivational
view could explain why the \ili{French} negator \textit{pas} follows
a finite verb, unlike the \ili{English} negator \textit{not}.  In order for this system to succeed,
...
all lexical  verbs in \ili{English} to undergo verb movement, but his
theory blocks this. This contradictory outcome has forced him to adopt
an otherwise unmotivated mechanism, a dummy
nonlexical counterpart of \textit{do} in \ili{English} (which \citep{Chomsky:89} tries
...
\ili{English} and \ili{French} negators as well as adverb positioning in
...
\citet{Pollock:89}'s system for \ili{English} auxiliaries posits
...
all auxiliaries are generated under the head of AspP.} But for \ili{French},
all verbs, whether
auxiliary or lexical  verbs, are generated under V.
This contrast does not seem to be unreasonable, considering that in
modern \ili{French} no syntactic phenomenon clearly distinguishes auxiliary
...
%movement to Tense, prohibits affix movement in \ili{French} and turns {\it
%not} into a block for affix movement
%in \ili{English}, but [-fin] does not require verb movement, does not prohibit %affix
...
\gll [Ne           pas  [repeindre      sa    maison]] est une n\'{e}gligence. \\
     \spacebr{}\NEG{}  \NEG{}  \spacebr{}paint one's house    is  a   negligence \\
...
%abbreviates a non-finite VP whose CONTENT value is
...
[ cat|head & [\type*{adv}
 ...
 %                        arg1 & \1 ] \} ] ]}
\z
\itdgreen{JP: Since at least the last chapter of P\&S (1994) and Davis (1996), many folks tend to
  treat relation labels such as “neg-rel” not as values of a PRED attribute but as the sort of the
  entire restriction (opr value of CONTENT or MAIN ... Are you sure you want it to be the value of
  PRED? Your choice\\
Stefan: This was fixed but broken. I added the ARG1 back in again. It is not consistent with the
AVMs below. I will suggest fixes as we go along.}
...
[vform \type{nonfin}]}
					[\ldots]]]
	\end{forest}
\itdopt{Stefan: Mentioning the categories at both occurrences of \ibox{1} is redundant. And it is
  confusing since \type{nonfin} occurs on the right-hand side only. I suggest
  removing the ``VP'' after the \ibox{1} in MOD.}
\caption{Structure of constituent negation}\label{negation-not-vp-mod}
\end{figure}
%\fi
\noindent
This structure implies\itdopt{Stefan: Strictly speaking the structure does not imlpy anything. If we
  did not have any constraint at all in the grammar, this structure would be possible. Apart from
  this, there can be other structures in the grammar. So we cannot derive anything from the
  existence of this structure.} that
...
this in the chapter on order \crossrefchapterp{order}.}}  It also
...
\ex[]{
\gll [Ne           pas  [parler  fran\c{c}ais]]  est  un grand d\'{e}savantage  en ce cas. \\
     \spacebr\NEG{}  \NEG{}  \spacebr{}to.speak \ili{French}  is  a great disadvantage  in this case \\
} \label{negation-34a}
\ex[*]{
\gll [Ne  parler  pas  fran\c{c}ais]  est  un  grand d\'{e}savantage en ce cas.\\
\spacebr\NEG{}  to.speak \NEG{} \ili{French}   is  a great disadvantage  in this case\\
} \label{negation-34b}
\zl
...
\noindent And much the same is true for \ili{French}, as the
...
As we have noted earlier, it is a general fact of \ili{French} that \textit{pas} must follow a finite verb, in which case the verb optionally bears negative morphology (\textit{ne}-marking) (\citealp[\page 361]{KS:02}):

\eal
\ex[]{
\gll Dominique (n')   aime pas Alex.\\
     Dominique \hphantom{(}\NEG{} like  \NEG{} Alex\\
...
possibilities that we can observe in an example like (\ref{negation-not-two}) \parencites(see)(){Klima:64}{Baker:91}{Warner2000a-u}[\page 200]{kimmichaelis:2020}.\footnote{\citet{Warner2000a-u} and \citet{BL:13}
discuss scopal interactions of negation with auxiliaries (modals) and quantifiers
within the system of Minimal Recursion Semantics (MRS). \added{On MRS see also \crossrefchapterw[Section~\ref{semantics:sec-mrs}]{semantics}.}}
...
after a modal auxiliary, but not after a\del{n} non-modal auxiliary verb like {\it be, have}
...
%behave alike in non-finite contexts.
%
%\iffalse{
%
...
constituent negator, the sentential negator \textit{not} typically
 follows a finite auxiliary verb. In this respect,
   \textit{too}, \textit{so}, and \textit{indeed} also behave alike:\itdopt{Stefan: Suggestion for
     reformulation: ``\textit{too}, \textit{so}, and \textit{indeed} also behave like this:}
\eal
...
that the sentential \emph{not} in \ili{English}
form a group of adverbs\itdopt{Stefan: rephrase. You name just \emph{not} and continue in
  plural. Suggestion:
``This suggests that \emph{too}, \emph{so}, \emph{indeed} and the sentential \emph{not} belong to
a special class of adverbs''.}  (which \rep{we}{I} call Adv\jbsub{\textsc{i}}) that combine with a
preceding auxiliary verb (see \citealt{Kim:00}\addpages).
...
\itdopt{Stefan: The name seems not to be appropriate since the rule does apply to other adverbs as
  well, not just negative ones.}
\avm{
[\type*{fin-aux}
...
[\type*{adv-comp-fin-aux}
  synsem|loc|cat & [ head & [aux   & $+$\\
                     ...
                     comps & < Adv\sub{I} > \+ \1 ]] }
\z
\itdblue{JP: Why is the output sort so specific? Don’t you want a sort that would cover all ADV$_I$ rather than just negation. Or is the sort called
Neg-fin-aux When no negation is involved?}
\itdobl{Stefan: Mentioning HEAD in the output is redundant.\\
JP's comment seems to have been addressed already. But this was not done consistently in the rest of
the paper.}
%
...
that selects Adv\jbsub{\textsc{i}} (including the sentential negator) as an additional complement.\footnote{As discussed in the following, this type of lexical rule allows us
to represent a key difference between \ili{English} and \ili{French}, in that
\ili{French} has no restriction on the feature \AUX\ to introduce the negative
adverb {\it pas}
as a finite verb's complement.\itdopt{Stefan: Check English ``In that French ...''}}  This would then
...
\itdobl{Stefan: The type \type{neg-fin-v} seems to be wrong. Should be
  \type{adv-comp-fin-aux}.}
\itdopt{Stefan: Remove redundant NP, Adv, VP in lexical entry of \emph{could}. }
\end{figure}

%\fi
As shown in Figure~\ref{negation-fig:6}, the negative finite auxiliary
verb\itdopt{Stefan: Adapt text to new type name?} \textit{could} combines with two complements, the negator
...
specified by the following lexical rule \parencites[\iaddpages]{Kim:00}[\page 209]{kimmichaelis:2020}:
...
\avm{
[\type*{neg-fin-aux}
 arg-st &  < \1 XP, \2 Adv\sub{I}, \3 YP > ]}  $\mapsto$
\avm{
[\type*{aux-ellipsis-wd}
 comps  & < >\\
 arg-st & < \1 XP, \2 Adv\sub{I},  \3 ! YP[\type{pro}]! >]}
\itdobl{Stefan: Type \type{neg-fin-aux} wrong?\\
...
can be projected into an elided auxiliary verb (\type{aux-ellipsis-wd}) whose second
argument\itdobl{Stefan: It is the third argument.}
...
behaves like a slashed expression in not mapping into the syntactic grammatical
 function \comps \added{(see \crossrefchaptert[Section~\ref{properties:lexemes-and-words}]{properties} and \crossrefchaptert[Section~\ref{argst-valence-sec}]{arg-st} for mappings from
 \argst to \comps)}. This analysis would then license
	...
	\begin{forest}
		...
		[VP
			...
			[head|aux $+$\\
			 ...
			 arg-st < \1, \2, VP![\type{bse}\added{, \type{pro}}]! > ]}
					[could]]
			[\ibox{2} Adv\sub{I}\\
					[not]]]
	\end{forest}
...
\noindent
Unlike the \ili{English} negator \textit{not}, \textit{pas} must follow a
...
with a certain adverb like \textit{pas} as an additional complement.
 The idea of converting modifiers in \ili{French} into
complements has been independently proposed by \citet{Miller92d-u} and
\citet{AG:2002} for \ili{French} adverbs including
\textit{pas}.  Building upon this
previous work, \citet{AG:97} and \citet{Kim:00}\itdopt{Stefan: 1997 is before 2002.}
allow the adverb \textit{pas} to function
as a syntactic complement of a finite verb in \ili{French}.\footnote{Following \citet{AG:2002}, \rep{we}{one} could assume \textit{ne} to
be an inflectional affix which can be optionally realized
in the output of the lexical rule in Modern \ili{French}.}
...
and even present participle verb forms in \ili{French} are finite, we
can expect that \textit{pas} cannot precede any of these verb
forms, which the following examples confirm (\citealp[\page 142]{Kim:00}):

\eal
\ex[]{
\gll Si j'avais de l'argent, je n' ach\`{e}terais pas de voiture. \\
     if I.had   of money      I \NEG{} buy \NEG{} a car\\
...
\gll Il est important que vous ne r\'{e}pondiez pas. \\
     it  is important  that you \NEG{} answer \NEG{} \\
...
\gll Ne parlant pas Fran\c{c}ais, Stacy avait des difficult\'{e}s. \\
     \NEG{} speaking  \NEG{} \ili{French}    Stacy   had  of  difficulties\\
...
and \emph{pas} is the morphosyntactic value [\AUX\ $+$],\itdobl{Stefan: This is a strange
  formulation since AUX is a feature of the verbs that are input to the lexical rule and not a
  feature of \emph{not} and \emph{pas}.} and this induces
...
straightforwardly by \changed{the following lexical rule} (see \cites[\iaddpages]{Kim:00}[\iaddpages]{Crowgey:12}):
...
\itdgreen{JP: If ma- and -s are clitics, the appropriate boundary symbol is “=“. But given what you say below, I would simply call them prefix and suffix. Otherwise it’s confusing.}
...
\z
%
\changed{Following \citet{BK:12}, one} can treat these clitics as affixes and generate
a negative word. Given that the function
f\sub{\type{neg}}\itdobl{Stefan: Small or capital ``f''? Consistency.} in Libyan Arabic\il{Arabic!Libyan} allows the attachment of the negative prefix
...
\itdobl{Stefan: RESTR for verbs? I removed it. OK? I also removed PRED.}
\z

\iffalse{
In the
construction-based HPSG, we could take this as an inflectional
construction.  The negative marker, as we have seen in \ili{Turkish} and \ili{Japanese}, is realized as a suffix
...
construction in \ili{Turkish}.\footnote{See \citet{Sag:12} and \citet{Hilpert:16} for a construction-based approach to
inflectional as well as derivational processes.}


This inflectional construction ($\uparrow$\textit{infl-cxt}) allows us to generate a \ili{Turkish} inflection construct like \textit{ser-me} `like-\NEG' (in (\ref{negation-1a})) from the v-lexeme \textit{ser-} with the change in the root's meaning into a sentential negation. The morphological function \textit{F}\jbsub{\NEG{}} could ensure that the vowel of the negative affix \textit{me} is subject to phonological changes depending on its environment. If it is followed by a consonant-initial morpheme, it undergoes vowel harmony with the vowel in the preceding syllable (e.g., \textit{yika-n-ma-di} `wash-\REFL-\NEG-\PST'). If it is followed by a vowel-initial morpheme, its vowel drops (gel-m-iyor `come-\NEG-\PROG')  \citep[see]{kelepir}.\footnote{As
...
\citep{Payne:85}\addpages. \ili{Finnish} also exhibits this property \citep{Mitchell:91}\addpages:

\ea
\gll Min\"{a} e-n puhu-isi. \\
     I.\textsc{nom} \textsc{neg}-\textsc{1sg} speak-\textsc{cond} \\\hfill\added{(Finish)}
...
information (such as tense, agreement, and mood) carried by lexical items into various different phrase-structure nodes (see, among others, \citealt{Hagstrom:02}, \citealt{Han:07} for \ili{Korean}, and \citealt{Vasishth:00} for \ili{Hindi}).
This derivational view has
been appealing in that the configurational structure for \ili{English}-type
...
%for \ili{Finnish} negative auxiliary such as that of \citep{Mitchell:91}.}


%
%and an independently
%motivated construction for other types of auxiliary verbs.
The \ili{Korean} negative auxiliary displays all the key properties of auxiliary verbs in the language. For instance, both the canonical auxiliary verbs and
...
\itdobl{for Stefan: Give caption more space so that it can go into one line.}
\end{figure}
%\fi
...
\noindent
As shown in (\ref{negation-fragment}c) and (\ref{negation-fragment}d), neither the lexical verb nor
the auxiliary verb alone can serve as an independent fragment answer.\itdopt{Stefan: You already
  said this before the examples.} The two verbs must appear
together, as shown in (\ref{negation-fragment}b). These constituenthood properties indicate that the
negative auxiliary forms a syntactic unit with a preceding lexical  verb in \ili{Korean}.
...
[\type*{head-light-phrase}
 ...
 dtrs     & < \3 [light & $+$\\
                  comps & \1 ], \2 [light & $+$\\
                                    comps & < \3 > ] > ]}
\itdobl{Stefan: Where is LIGHT? Is it under SYNSEM? If so, it has to be ordered after COMPS. In any
  case this should be consistent.}
\z
...
\begin{forest}
		...
		\avm{
			...
			\avm{
				[\punk{head|vform}{ci}\\
				 light  & $+$\\
				 comps  & \2 < NP > ]}, edge label={node[midway,left,outer sep=1.5mm,]{Lexical arg.}}
				[ilk-ci\\read-\textsc{conn},tier=word]]
			[V\\
			\avm{
				[head  & \3\\
				 comps & < \1 V > \+ \2 < NP > ]}, edge label={node[midway,right,outer sep=1.5mm,]{H}}
					[anh-ass-ta\\ \textsc{neg-pst-decl},tier=word]]]
\end{forest}
\caption{An example structure licensed by the \hdlight}\label{kor-v-complex}
\itdobl{Stefan Having the \ibox{2} in the right daughter is not what is done in the schema.}
\end{figure}
...
inheriting the lexical verb's complement value through argument composition.\itdblue{JP: See my remark above.}
...
\itdobl{The last phrase has a dot. Please do this conistently.}
\end{xlist} \end{exe}
%
As seen from the bracketed structures, \rep{we can}{it is possible to} add one more auxiliary verb to
an existing \textsc{head-light} phrase with the final auxiliary bearing an appropriate
  ...
  verbs \rep{we}{one} can add (see \citealt[\page 88]{Kim:16} for detailed discussion).
...
negation and word order in \ili{Basque}. Consider their example:

\ea
\label{negation-basque-ex}
\gll ez-ditu irakurri liburuak. \\
     \NEG-3\textsc{plo}.\textsc{prs}.\textsc{3sgs} read.\PRF{} book.\ABS.\pl\\
\glt `has not read books'
\itd{Stefan: Is this a full clause? If so add subject in translation and dot at the end.}
\z
%
%
...
Figure~\ref{negation-basque} (adopted from \citealt{CB:11}\addpages):
...
\itdobl{Stefan: LIGHT below COMPS.}
\end{figure}
%
%
%
...
\gll Dw i ddim wedi gweld neb.\\
     am I \NEG{} \textsc{perf} see nobody\\ \hfill  (\ili{Welsh}, \citealt[\page 108]{Borsley:05})
\itd{Stefan: perfect vs. perfective. If this is perfect, we change it to prf and remove perf from
  the appendix.}
...
languages. Such an analysis, however,
  ...
  negative.\itd{JP: adverb negative to adverbial negative}
...
former, whereas the clitic \textit{loro} `to them' \added{belongs} to the
...
\itdobl{Stefan: I removed RESTR and PRED.}
%% \begin{avm}
%% \[form\ \q<\normalfont non\q>\\
%%   \SYN\ \; \[head\ \@1\\
%%          \COMPS\ \; \<V\[head\ \ \; \@1\\
...
\rep{a}{the} verb complex structure given in Figure~\ref{negation-read-it-st}.
...
\citet{Borsley:06}\addpages, accepting the analysis of \citet{Kim:00} where \textit{non} is taken to be a
...
\citealt{Borsley:06}\addpages):\footnote{Different from \citet[\iaddpages]{Borsley:06}, \citet{BJ:00} offer  a selectional analysis of \ili{Welsh} negation.
...
constituents\itd{JP: constraints?}
...
\itd{Stefan: You talk about the morpheme \emph{nie} and the prefix \emph{nie}. It is written
  ...
  With \changed{the lexical specification in (\mex{1})}, his analysis introduces
 the following principle (adopted from \citealt[\page 143]{Prz:00}):

\ea
\label{negation-polish-gen-case}
\added{Part of the} Case Principle for \ili{Polish}:\\
\avm{
[ head & [\type*{verb}
          neg & $+$ ]\\
...
\z
The principle allows a \textsc{neg}\added{+} verbal expression to assign
 the \textsc{case} value \textit{gen} to its\itdopt{Stefan: Is there just one? What if there are
   multiple arguments with structural case?} non-initial argument.
 % ensures that
...
allows the VP complement of a raising verb like \type{seem} to optionally undergo lexical
...
\itdobl{Stefan: Having the list + NP after \ibox{1} is redundant. It is confusing as well since it
  differs in having [\type{str}]. And how can the argument be a VP and have something on COMPS?}
\z
%
...
construction-based case assignment (or satisfaction).\itdopt{Stefan: Not clear what you mean by
  construction-based here. If it is a phrasal construction in the sense of CxG, then the claim is
  not correct, since case assignment happens in the lexicon in Adam's system.}
...
\gll Personne (n')                 aime personne.\\
     no.one   \hphantom{(}\NEG{} likes no.one\\\hfill\added{(\ili{French})}
...
\citet{Swart:02}.\itdopt{Stefan: Check italics of \textit{NO}.}}
...
for why \emph{pas} does not induce a resumptive interpretation with a quantifier (from \citealt{Swart:02}\addpages):


\ea
\gll Il ne     va   pas    nulle part, il va   à  son travail.\\
     he \NEG{} goes \NEG{} no    where he goes at his work\\\hfill\added{(\ili{French})}
...
and offer HPSG analyses. Consider a \ili{Welsh} example from \citet{BJ:00}\addpages:

\ea
\gll Nid         oes neb yn yr ystafell\\
\textsc{not}    is no.one in the room\\\hfill\added{(\ili{Welsh})}
...
and specifies the subject \emph{neb\del{ yn}} to carry the positive \textsc{nc} (negative
...
and syntactic categories.  By admitting morphological and syntactic categories, \rep{we have been
  able}{it was possible} to capture their idiosyncratic properties in a simple and natural manner. Furthermore, this theory has
...
\end{tabularx}
\itdobl{Stefan: Check above and if appropriate remove \textsc{perf}.}

\section*{\acknowledgmentsUS}

I thank \added{the} reviewers of this chapter for detailed comments and suggestions, which
helped improve the quality of this chapter a lot. I also thank
Anne Abeill\'{e}, Bob Borsley, \added{Jean-Pierre Koenig,} and Stefan Müller for constructive comments on
