...
English. Section~\ref{sec-warlpiri} discusses examples from \ili{Warlpiri}, a language having even freer constituent order.
...
order domains allow for discontinuous constituents and have been used to account for languages like \ili{Warlpiri}\il{Warlpiri}
...
English and \ili{Romance} languages do not.
...
So in summary, \ili{German} differs from English in the way the arguments are distributed on the valence lists, 
on the one hand in order to capture the similarity in English between combinations of subjects with VPs and 
determiners with nouns, and on the other hand to allow \ili{German} the flexible constituent order it needs. However,
...
\ea
\begin{tabular}[t]{@{}ll@{}}
\emph{gives} (English, SVO language): & \emph{gibt} (\ili{German}, SOV language):\\
\avmtmp{
[ spr    & < \1 >\\
  comps  & \2\\
  arg-st & < \1 NP > \+ \2 < NP, NP >  ]
...
The \ili{Germanic} languages signal the clause type by verb position. All \ili{German}ic languages with the
...
objects, or adverbials. (\mex{1}) shows an example from the V2 language \ili{German} and its English translation.
\ea 
...
Bouma and van Noord's analysis of \ili{Dutch} clauses show \citep[\page 62, 71]{BvN98a}. The alternative to
...
\ili{Warlpiri}\il{\ili{Warlpiri}|(} are even more so, since they have much freer constituent order. In
...
\iboxb{2}, \ibox{1} is realized initially and \ibox{3} follows the auxiliary.\il{\ili{Warlpiri}|)}
...
mother (the analysis of \ili{Warlpiri}), and in the next
...
those treated by \citet{DS99a} can be handled without discontinuous constituents.\il{\ili{Warlpiri}|(}
...
languages can be handled within HPSG without assuming discontinuous constituents.\il{\ili{Warlpiri}|)}
...
\ili{German} and how even languages with extremely free constituent order can be analyzed without assuming
...
Borsley (1989) was concerned with English, not \ili{Welsh}. It made the point that you could have an analogue of verb-fronting for English auxiliary-initial sentences. Borsley (2006) discusses whether \ili{Welsh} finite clauses involve some form of VP and argues that they do not and hence that they involve a flat structure.
