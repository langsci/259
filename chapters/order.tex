...
HPSG was developed out of Generalized Phrase Structure Grammar\indexgpsg (GPSG) and \isi{Categorial
...
out.\itd{fix, extraposition} This is done by linearization rules, which can refer to features or to the function of a
...
\ili{German}, independent of the finiteness of the verb:\itd{Das stand schon oben.}
...
and \textsc{non-head-dtrs} (see \citealt[\page
  ...
  In Sign-Based Construction Grammar (SBCG; \citealt{Sag2012a}) the objects in valence lists are of the same type as the
...
\ili{English} the function \rel{order-constituents} would just return a concatenation of the \phonvs of the
...
\gll Kommt Peter. (Was ist morgen?)\\
     comes  \hphantom{(}what is tomorrow\\
...
\ea
\gll Kim [kennt die Schallplatte] und [liest das Buch].\\
     Kim \spacebr{}knows the record and \spacebr{}reads the book\\
...
inserted at a place where it scopes over the coordination relation. The only alternative would be to
add the semantics contributed by clause types via a unary rule as in \citew[\page 266--267]{GSag2000a-u}.
}
...
\z
The symbol `$\bigcirc$'\is{$\bigcirc$}\is{relation!$\bigcirc$}\isrel{shuffle}\label{rel-shuffle}\label{order:rel-shuffle}
