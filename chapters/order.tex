...
to analyze \ili{German} would be the one that was suggested by \citet{Uszkoreit87a} within the framework
...
generalizations can be captured. See \citet{Uszkoreit87a} for weighted constraints for the ordering
...
\end{schema}
There is a head-daughter and a list of non-head daughters. The respective daughters are specified a
...
nh-dtrs & \liste{ \onems{ phon \ibox{2} }} \\
}&%
\ms{
phon & \phonliste{ squirrel, from, America }\\[2mm]
head-dtr & \onems{ phon \phonliste{ squirrel } }\\
nh-dtrs & \phonliste{ \onems{ phon \phonliste{ from, America } }} \\
}\\
\end{tabular}
}
...
\ms{
phon & \phonliste{ grey, squirrel }\\[2mm]
head-dtr & \onems{ phon \phonliste{ squirrel } }\\
nh-dtrs & \liste{ \onems{ phon \phonliste{ grey } }} \\
}\\
\end{tabular}
\zl
...
and if the order has to be the other way round as in the case of adjectives modifying nouns, the
...
head-features (see \crossrefchaptert{wherever} on head features), they are also accessible at the
...
\label{cop}
~\\[-3mm]
\type{phrase} \impl \ms{ phon & \upshape order-constituents(\ibox{1})\\
                  dtrs & \ibox{1}\\
                }
\end{principle-break}
...
English the function order-constituents would just return a concatenation of the \phonvs of the
daughters, but for other languages it would be way more complicated. In fact this function and its
...
apprentice's pocket.'
\z
A straightforward analysis of adjunct placement in \ili{German} and Dutch is to assume that adjuncts can
...
two objects while it's \ili{German} counterpart \emph{gab} has three elements in the \compsl. The
...
known from other \ili{German}ic languages.
\ea
...
\end{forest}
\caption{\label{fig-did-kim-get-the-job}Analysis of English auxiliary constructions according to \citep{Sag2020a}}
\end{figure}
...
\type{aux-initial-cxt}. For example, \citet{Sag2020a} posit a subtype \type{polar-int-cl} for polar
...
modeling of discontinuous constituents.\footnote{
  See also \citew{Wells47a}, \citew{Dowty90a-Eng}, and \citew{Blevins94a-u} for proposals assuming discontinuous constituents in other frameworks.%
...
discussed below in Section~\ref{sec-problems-dom}, there were reasons for abandoning linearization"=based analyses of \ili{German} assuming discontinuous constituents \citep{Mueller2005d,MuellerGS} but constituent order
domains still play a major role in analyzing ellipsis\is{ellipsis} \crossrefchapterp{ellipsis} and
coordination\is{coordination} \crossrefchapterp{coordination}.  \citet*{BGM99a} show that complex
...
[{V[\dom \phonliste{ dem Kind, ein Mann, das Buch, gibt }]}
  ...
  [{V[\dom \phonliste{ dem Kind, das Buch, gibt }]}
   [{NP[\type{dat}, \dom \phonliste{ der, Frau  }]} [dem Kind;the child,roof] ]
...
\caption{\label{fig-the-child-reads-the-book-reape-binary}Analysis of \emph{dass dem Kind ein Mann das Buch
    gibt} `that a man gives the child the book' with binary branching structures and discontinuous constituents}
\end{figure}%%
...
While \ili{German} is more exciting than English in terms of constituent order, languages like
Warlpiri\il{Warlpiri|(} are even more exciting since they have much freer constituent order. In
...
analysis of \ili{German}), an analysis that inserts all domain objects of objects into the domain of the
...
constraint is violated and on the other hand, there are approaches assuming binary branching
