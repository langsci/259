...
\z
(\mex{-1}) shows that in addition to the unmarked order in (\mex{-1}a) (see \citet{Hoehle82a} on the
...
because of \isi{scope} issues. This cannot be dealt with in detail here, but see \citet[Section~3.6]{LH2006a} and
\citet{Chaves2009a} for discussion.
...
known from other Germanic languages with the finite verb in second position.\footnote{%
  ...
  and V2 \citep{Haider2020a}. See \citet{MuellerGermanic} for discussion and the analysis of this variation in HPSG.
}
\ea
...
discussions.\footnote{%
  For a discussion including \ili{French} verb placement see \citet{AG97a-u} and \citet{KS2002a}.
}
...
\citet[\page 937--939]{MuellerUnifying}), so this rule for combining linguistic objects is very simple, but this
...
used in HPSG (but see \citet{Kathol2001a} and \citet{Wetta2011a}, and see Section~\ref{sec-surface-order} on why such approaches
...
and a fully worked out analysis see \citet{MuellerGS}.%
} 
...
%%
%% Note that in Constructional HPSG as layed out by \citet{Sag97a} implicational constraints can refer
...
modeling of discontinuous constituents.\footnote{
  See also \citet[\page 105--106]{Wells47a}, \citet{Dowty90a-Eng}, and \citet{Blevins94a-u} for proposals assuming discontinuous constituents in other frameworks.%
...
Warlpiri the auxiliary has to be in first or in second position \parencites[\page
...
the mother.\footnote{
  ...
  extraposition to the right, a different feature is used (\feat{extra}). See \citet{Keller95b} and
  \citet[Section~13.2]{Mueller99a} for discussion. More recent approaches assume the
...
to space limitations, this constraint will not be discussed here, but see \citet[\page
...
extraposition like the one in (\mex{1}) discussed in \citet[\page82]{Reis80a}.
\ea
...
\emph{damit} `there.with'. See \citet{Mueller2005c,Mueller2005d,MuellerGS} for details.
...
combined with the head; that is, arguments are not canceled off from valence lists. See also \citet{Meurers99b,Prze99} and
\citet{Mueller2008a} for earlier non"=cancellation approaches.\footnote{
\citet[\page 560]{Higginbotham85a} and \citet[\page 239]{Winkler97a} make similar suggestions with regard to the
