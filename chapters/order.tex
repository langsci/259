...
\abstract{This chapter discusses local ordering variants and how they can be analyzed in
  ...
  assuming flat rules or by binary branching rules with arbitrary order of saturation. The difference between SVO and
...
of the finite verb in initial or final position in languages like German can be accounted for by
...
freedom than languages that just allow to scramble arguments. It is shown how one such langauge, namely Warlpiri, can be analyzed with so-called
...
verbs. As with the examples just given, I will use German if a phenomenon does not exist in
...
to 12: verb initial placement and 6 permutations of the NPs and verb final placement with 6
permutations of the arguments.

The LP rule in (\mex{0}c) is more general than (\mex{0}b) in that it does not
...
rules. Note that we do not need a linearization rule for every ID rule. For example, in a grammar
with rules for intransitive, transitive, and ditransitive verbs, head ordering is taken care of by
general LP rules of the type in (\ref{lp-head}) applying to the respective ID schemata.
...
as pre-modifiers have to preceede their head while those that are marked as post-modifiers follow
it. For languages with free variation in head-adjunct order, it would suffice to not state any LP
...
head-dtr       \ms{ synsem|loc|cat|comps \ibox{1}\\
                   }\\
non-head-dtrs \upshape synsem2signs(\ibox{1})\\
}
\end{schema}
...
Researchers working on English usually assume a flat structure \parencites[\page 39--40, 362]{ps2}[\page
  479]{Sag97a}[\page 34]{GSag2000a-u} but assuming binary
...
\type{head-complement-phrase} \impl\\*
\ms{
synsem|loc|cat|comps & \ibox{1} $\oplus$ \ibox{2}\\
head-dtr      & \ms{ synsem|loc|cat|comps \ibox{1} $\oplus$ \sliste{ \ibox{3} } $\oplus$ \ibox{2}\\
                   }\\
...
\end{figure}
The second tree in the figure is the German counterpart of \emph{gave Sandy a book}: the finite verb
in final position with its two objects in normal order. Section~\ref{sec-svo-sov} explains why SOV
languages like German and Japanese contain their subject in the \compsl while SVO languages like
...
two objects while it's German counterpart \emph{gab} has three elements in the \compsl. The
...
NP[\type{nom}] precede the finite verb while other arguments follow it. This would get the facts
...
\z 
The subject in English main clauses is similar to the determiner in nominal structures and hence one
way of expressing this similarity is using the same valence features and the same schema for
subject-VP combinations as for determiner-noun combinations.\footnote{
  ...
  for determiners. I follow the German HPSG tradition and use \subj for unexpressed subjects. See
...
non-head-dtrs \liste{ \onems{ synsem \ibox{2} } }  \\
}
%\end{tabular}
\end{schema}
...
\end{figure}
For German, it is standardly assumed that the subjects of finite verbs are treated like complements (\citealp[\page 295--296]{Pollard90a-Eng}, \citealp[\page
...
of a verb it is explained why it can be placed in any position before, between, and after them. So,
German differs from English in the way the arguments are distributed on the valence lists. However,
HPSG ha a more basic representation in which the languages behave the same: the argument structure
...
\crossrefchapterp{binding}. Ditransitive verbs in German and English have three NP arguments on
...
spr    & \sliste{ \ibox{1} }\\
comps  & \ibox{2}\\
arg-st & \sliste{ \ibox{1} NP} $\oplus$ \ibox{2} \sliste{ NP, NP }\\
}&
\ms{
spr    & \eliste\\
comps  & \ibox{1}\\
arg-st & \ibox{1} \sliste{ NP, NP, NP }\\
}
\end{tabular}
\z
...
known from other Germanic languages.
\ea
...
The analysis of (\mex{1}a) is shown in Figure~\ref{fig-did-kim-get-the-job-hm}.
\eal
...
English auxiliary constructions here, since they have figured prominently in linguistic discussions.
...
} As is common in HPSG since the mid-1990s
...
assumed in HPSG (but see \citew{Kathol2001a} and \citew{Wetta2011a}. See Section~\ref{sec-surface-order} on why such approaches
...
\citeyearpar{Reape91,Reape92a,Reape94a} working on German suggested formal tools that allow for the
modeling of discontinuous constituents.\footnote{
  See also \citew{Wells47a,Dowty90a-Eng,Blevins94a} for proposals assuming discontinuous constituents in other frameworks.%
} His original motivation was to account for scrambling of arguments in
...
discussed below in Section~\ref{sec-problems-dom}, there were reasons for abandoning linearization"=based analyses of German assuming discontinuous constituents \citep{Mueller2005d,MuellerGS} but constituent order
...
While German is more exciting than English in terms of constituent order it is still
boring\todostefan{Anne: No, no} in
...
% S91:257 
     child-\DU-\ERG{} \PRS-3\DU.\textsc{subj} dog.\ABS{} chase-\textsc{npast} small-\DU-\ERG\\%\jambox{(Warlpiri)}
...
problems. Figure~\ref{fig-warlpiri} shows the \citegen{DS99a} analysis of a version of
...
[IP\\\ms{ phon \phonliste{ kurdu-jarra-rlu, ka-pala, wita-jarra-rlu, \ldots{} }\\
        dom \liste{ \ibox{1} \ms{ phon \phonliste{ kurdu-jarra-rlu } },
                    ...
                    \ibox{3} \ms{ phon \phonliste{ wita-jarra-rlu} }, \ldots{} }\\
      }
...
analysis of German), an analysis that inserts all domain objects of objects into the domain of the
...
Kathol \& Pollard's analysis of (\mex{1}) is given in
...
\inlinetodostefan{Anne: You should mention that linearization was also used for various ordering
  ...
  (Bonami \& Godard 2003), this could go in part 6.
...
I thank Anne Abeillé, Bob Borsley, and Doug Ball for very detailed and very useful comments.
