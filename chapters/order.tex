...
Chapter~\ref{chap-nld}). Hence, these sentences will not be handled here.
...
HPSG was developed out of \isi{GPSG} and \isi{Categorial Grammar}. The ideas concerning linearization of
...
about ordering. Instead there are separate linearization rules. A schema like (\mex{0}) licenses 24 
...
like German it is usually assumed that structures are binary branching (but see \citew[\page 156]{Reape94a} and
...
subject \iboxb{1} and its VP complement \iboxb{2} in one go. As is common in HPSG since 1995
...
\citep{HN89a,HN94a} since pureley linearization-based approaches are unable to account for \isi{agreement}
...
domains still play a major role in analyzing ellipsis\is{ellipsis} (Chapter~\ref{chap-ellipsis}) and
coordination\is{coordination} (Chapter~\ref{chap-coordination}).
...
the mother. Kathol \& Pollard's analysis of (\mex{1}) is given in Figure~\ref{fig-extraposition-via-domain-union}.
\ea
...
is, maximally specific (\citealp{King99a-u}, see also Chapter~\ref{chap-formal-background}). Leaving the
...
those treated by \citet{DS99a} can be handled without discontinuous constituents.\il{Warlpiri|(} The trick is that
all arguments of a head are projected to higher nodes even when they are combined with the head,
that is, arguments are not canceled off from valence lists. See also \citew{Meurers99b,Prze99} and
\citew{Mueller2008a} for earlier non-cancellation approaches. The example (\ref{ex-warlpiri}) from
