...
freedom than just scrambling arguments. It is shown how one such language, namely Warlpiri, can be analyzed with so-called
...
that pronouns precede non-pronouns. The LP rules apply a large set of ID rules, for example for
...
Godard propose a three valued \feat{weight} feature to account for the ordering of light,
middle-weight and heavy constituents \citep{AG2000a,AG2004a-u}. See also
\crossrefchaptert[Section~\ref{sec-romance-complex-predicates}]{complex-predicates} on \isi{complex
...
\ili{German}, independent of the finiteness of the verb:
...
constraints regarding linearization hold across rules. By factoring these constraints out,
...
\ili{English} allow adjuncts to precede or follow the head they
modify. The schema in (\ref{schema-head-adjunct}) corresponds to a phrase structure rule in
GPSG. The values of features like \textsc{head-dtr} and \textsc{non-head-dtrs} are feature
descriptions that correspond to daughters in local trees or to symbols on right-hand sides of phrase
structure rules (see \crossrefchapteralt[\page \pageref{prop:sec-elements}]{properties} for the
representation of dominance structure in HPSG). The schema in (\mex{1}) does not say anything about the order of the daughters:
\ea
Head-Adjunct Schema:\\
...
\itd{JP: Since DTRS and NON-HEAD-DTRS are both lists, I find this footnote confusing. I am not sure
...
\z
\rel{synsem2signs} is a \isi{relational constraint} mapping a list of \type{synsem} objects as they are contained in
the \compsl onto a list of objects of type \type{sign} as they are contained in daughters\itd{JP:
  ``as they are continaed in daughters'' This information is awkward/ambiguous. Clarify what is
  contained in what? With ``daughters'' do you mean the features or the actual daughters? Stefan:
  JP, I do not understand your question. head-dtr and non-head-dtrs-elements are signs. This was
  shown in the previous examples.}  (see \citealt[\page
  ...
  theory. Theories working with a binary branching Head-Complement Schema as (\ref{hcs-binary}) on
...
morphemes). Every linguistic object has its own phonology representation\itd{JP: Delete representation}. So in principle one could
...
\ili{English} the function \texttt{order-constituents} would just return a concatenation of the \phonvs of the
...
because of \isi{scope} issues.\itd{JP: I disagree: Either you deal with it through linear order or
  it is inverse scope, but then binary branching has the same problem. Stefan: No.} This cannot be dealt with in detail here, but see \citew[Section~3.6]{LH2006a} and
...
and \crossrefchaptert{binding} on Binding Theory).\itd{JP: Are they requiring an order of \comps?
  Could be relational constraints between \argst and \comps.} In the approach proposed above and in Müller
...
rationale behind this difference is explained in this section.
...
features. For example, the subject of the VP \emph{to read  more books} is not realized but is
...
arguments of finite verbs are complements is also made by researchers working in different research traditions
...
\crossrefchapterp{binding}. Ditransitive verbs in \ili{German} and \ili{English} have three NP arguments on
...
\ea
\begin{tabular}[t]{@{}ll@{}}
\emph{gives} (\ili{English}, SVO language): & \emph{gibt} (\ili{German}, SOV language):\\
\avm{
[ spr    & < \1 >\\
  comps  & \2\\
  arg-st & < \1 NP > \+ \2 < NP, NP >  ]
...
The \ili{Germanic} languages signal clause type by verb position. All \ili{Germanic} languages with the
exception of \ili{English} are V2 languages: the finite verb is in second position in declarative main
...
\z
\ili{English} is a so-called \emph{residual V2 language}, that is, there are
...
known from other \ili{Germanic} languages with the finite verb in second position.\footnote{%
  ...
  dimensions are independent. For example, \ili{Danish} is an SVO language that is V2, while \ili{German} is SOV
  and V2 \citep{Haider2020a}. See \citew{MuellerGermanic} for discussion and the analysis of this variation in HPSG.
}
\ea
...
\end{figure}
\itd{JP: You don't mention the unary rule in the text. Stefan: I do. But it was after the figure.}
...
Principle). The empty element for head movement is rather simple:\footnote{%
...
\itd{JP: This is to charitable. One would have to assume aux/verb-inital meanings are the same for
  all ???? or ???? ``features'' paring these empty heads (with particular meanings) with movement
  where it occurs.}
...
yes/no
\itd{JP: Dissel 1997 on V1 constructions in German. \citet{Diessel97a}. Stefan: I had a look, but
  Diessel only talks about semantics. He does not say anything about syntax.}
...
\gll Kommt. (Was ist mit Peter?)\\
     comes  \hphantom{(}what is with Peter\\
...
shows the construction-based and the lexical-rule"=based analysis\itd{JP: analyses Stefan: Hm.} in the abstract for comparison.
\begin{figure}
\hfill
...
\end{figure}
\itd{JP: Has general questions not for the handbook.}
...
\itd{JP: This presupposes that intonation is relevant for the difference between the sentences but
...
allow for the modeling of discontinuous constituents.\footnote{ See also \citew[\page
...
\parencites{Mueller2005d}[Chapter~6]{MuellerGS} but constituent order domains still play a major
role in analyzing ellipsis\is{ellipsis} \crossrefchapterp{ellipsis} and
...
The description in (\mex{0}) may seem strange at first glance, since it is cyclic\is{cycle!in
  feature description}, but it can be understood as a statement saying that \emph{gibt} contributes
...
together \parencites{Fanselow93a}[\page 1634]{Hoberg97a} was confirmed
...
categories in the examples collected by \citet{Mueller2003b} and \citet{Bildhauer2011a}. It would
