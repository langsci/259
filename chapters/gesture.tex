...
\color{red}
% I understand why you want to have a section \enquote{Why gestures?} before you really say what gestures are (i.e., the taxonomy of gestures that is introduced in Section 2).  But for any reader who's a linguist that has never considered gestures at all, this section is really hard to follow.

% The section should introduce specific and natural examples of multimodal moves that can anchor the reader's intuitions and so contribute to the main points you're making here:

1) There is a range of non-verbal signals that are a part of the contribution a speaker makes to the discourse
2) That contribution may be paralinguistic, but it can only be a contribution to the propositional content.
3) gesture contributes certain parts of the message that the speaker is trying to convey that go above and beyond that given in speech;
4) the relative timing of gesture to speech is a part of the signal that helps the interlocutor retrieve that meaning (and a bad relative timing can  make a multimodal move anomalous)

For each of these 4 main points, you should introduce seminal and prototypical examples, rather than simply introducing a lot of technical terms.
\color{black}
...
\textit{both hands rise up with open palm handshape, palms facing}; brackets indicate segments concurrent with the gesture stroke. \textcolor{red}{check/provide ref to stroke phase}
\z 
%
