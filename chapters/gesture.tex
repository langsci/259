...
Contradictions or speech-gesture mismatches can occur when saying \enquote{right} but pointing left (as is the case in SaGA dialogue V24, at 4:50).
%
A more complex case is given in (\ref{ex:contradiction}) and Figure~\ref{fig:contradiction}, where the speaker talks about a \enquote{rectangular arch} (which is of course a a contradition in itself), but produces a roundish movement with the extended index finger of her right hand (the objects she talks about is an archway).
%
Note that the gesture just overlaps with \enquote{rectangular}: its temporal extension in (\ref{ex:contradiction}) is again indicated by means of square within the original German utterances. 
%
...
\begin{figure}
  \centering
  \includegraphics[width=5cm]{figures/rechteckiger-Bogen}
...
An obvious interpretation of this mismatch is that \enquote{rectangular} is a slip of tongue; interestingly, we found no \enquote{slip of hand} in our data so far.
...
%
Furthermore, the lion's share of everyday gestures seem to consist of rather sloppy movements that do not contribute to the content of the utterance, though they might signal other information like speaker states.
%
In this sense they are \emph{contingent} \is{contingent gesture} as opposed to be an obligatory \is{obligatory gesture} semantic component \citep{Luecking:2013:a}.
%
Gestures can become obligatory when they are produced within the scope of a demonstrative expression (cf. (\ref{ex:so})/Figure~\ref{fig:so}).
%
...
%
This distinction reflects two traditions within gestures studies: according to one tradition most prominently bound up with the work of \citet{McNeill:1992}, gesture is a \emph{byproduct} \is{gesture-as-byproduct} of speaking and therefore opens a \enquote{window into the speaker's mind}.
%
The other tradition, early on represented by \citet{Goodwin:2003} and \citet{Clark:1996}, conceive of gestures as a \emph{product} \is{gesture-as-product} of speaking, that is, as interaction means designed with a communicative intention.
%
Since a gesture cannot be both, a byproduct and a product, at the same time, as noted by \citet{Cooperrider:2017}, a bifurcation has to acknowledged that is rooted in the cause and the production process of the gesture. 
%
...
%
Accordingly, gestures can be cross-classified into \emph{concrete} and \emph{abstract} or \emph{metaphorical} \is{metaphorical gesture} \is{abstract gesture} ones (see the volume of \citet{Cienki:Mueller:2008} on gesture and metaphor).
...
Perhaps it should be noted that the spontaneous, usually co-verbal hand and arm movements considered in this chapter are different from the signed signs of sign languages \is{sign language} (see \crossrefchaptert{sign-lg}) and pantomime \is{pantomime} (not spontaneous and co-verbal).\footnote{In languages like German the difference between free gesticulation and sign language signs is also reflected terminologically: the former are called \textit{Gesten}, the latter \textit{Gebärden}.}
...
%
Coincidentially, these early works used typed feature structure description akin to the descriptive format used in HPSG grammars.
%
...
\begin{figure}[tb]
  ...
  \caption[ASL fingerspelling]{American Sign Language fingerspelling alphabet (image released to Public Domain by user Ds13 in the English Wikipedia at \printdate{2004-12-18}, \url{https://commons.wikimedia.org/wiki/File:Asl_alphabet_gallaudet.png}).}
  \label{fig:asl}
\end{figure}
...
%
He then points with the right hand at his left hand, and thereby indirectly to the pond.
%
Such instances of indirect or proxy pointing \is{indirect reference} has been interpreted as \emph{dual points} \is{dual point} by \citet{Goodwin:2003}; in standard semantics they are anaysed in terms of \emph{deferred reference}, \is{deferred reference}\is{deferred ostension} where one thing is indicated, but another but related thing is referred to \citep{Quine:1950,Nunberg:1993}), 
%
The complete gesture sequence depicts \textit{driving towards} or \textit{heading straight}.
%
The movement aspect is subsequently enacted by moving the right hand/driver towards the left hand/pond. \todo{check example}
%
\ea \label{ex:drivetowards}
\glt wenn du dort eingefahren bist, fährst Du geradeaus auf einen Teich zu .. 'n Teich .. und .. [\textit{gesture from Figure~\ref{fig:drivetowards} occurs here}] .. und 
...
\begin{figure}[tb]
  ...
  \caption[Drive towards]{Driving towards/heading straight}
  \label{fig:drivetowards}
\end{figure}

\color{red}
This example highlights two phenomena: firstly, the region pointed at is not the region referred to. 
%
...
%
Within HPSG, such constraints can be formulated within an interface to metrical trees from the phonological model of \citet{Klein:2000} or phonetic information packing from \citet{Engdahl:Vallduvi:1996} -- see also \crossrefchaptert{phonology} and \crossrefchaptert{information-structure}.
%
The well-developed basic integration scheme of \citet[\page 445]{Alahverdzhieva:Lascarides:Flickinger:2017} rests on a strict speech and gesture overlap and is called the \emph{Situated Prosodic Word Constraint} \is{Situated Prosodic Word Constraint}, which allows to combine a speech daughter (\textsc{s-dtr}) \isfeat{s-dtr} and a gesture daughter (\textsc{g-dtr}) \isfeat{g-dtr} -- see Figure~\ref{fig:pwc}.
...
\]
\end{avm}
\caption{\emph{Situated Prosodic Word Constraint} \protect\citep[\page 445]{Alahverdzhieva:Lascarides:Flickinger:2017}.}
\label{fig:pwc}
\end{figure} 
...
%
On this account, the affiliation problem (the second desideratum identified in Section~\ref{sec:empir-desid-gramm}) has a well-motivated solution on both the word and the phrasal level, at least for temporally overlapping speech-gesture occurrences.
%
...
A dialogue-oriented focus on pointing is taken in \citet{Luecking:2018:a}: here pointing gesture play a role in formulating processing instructions that guide the addressee in where to look for the referent of a demonstrative noun phrases. % (see also \crossrefchaptert{pragmatics}).
\is{pointing gesture|)} \is{deictic gesture|)}
...
There is nearly no semantic work on the grounds of which the meanings assigned to iconic gestures should be assigned to them (that is the first basic question from Section~\ref{sec:empir-desid-gramm}).
%
Semantic modelling usually focuses on the interplay of (presumed) gesture content with speech content, that is, with the third of the basic questions from Section~\ref{sec:empir-desid-gramm}.
%
...
%
Thus, he tries to address the  the first basic question from Section~\ref{sec:empir-desid-gramm} in terms of an empirically extracted \is{gesture typology}.
%
...
%
In order to make this approach work, common perceptual features are extracted and represented as part of a lexical extension of lexemes, serving as an interface between hand and arm movements and word meanings. 
%
This conception in turn is motivated by psychophysic theories of the perception of biological events \citep{Johansson:1973}, draws on philosophical \isi{similarity} conception beyond isomorphic mappings \citep{Peacocke:1987},\footnote{\label{fn:resemblance}That mere resemblance, usually associated with iconic signs, is too empty a notion to provide the basis for a signifying relation has been emphasised on various occasions \citep{Burks:1949,Bierman:1962,Eco:1976,Goodman:1976,Sonesson:1998}.} and, using a somewhat related approach, has been proven to work in robotics drawing on \emph{imagistic description trees} \citep{Sowa:2006:a}.
%
...
Comparable to a modifier, a gesture attaches to an affiliate via feature \textsc{aff}, \isfeat{aff} which is required to carry intonational accent, expressed in terms of information packaging developd by \cite{Engdahl:Vallduvi:1996} (cf. \crossrefchaptert{information-structure}).
%
The semantic contribution of a gesture is contributed via the new mode \isfeat{mode} \textit{exemplification}, that is, a gesture displays a predication from the \textsc{restr} list of its affiliate.
%
...
The lexical entry for \textit{semicircle} gets a \emph{conceptual vector meaning} \is{conceptual vector meaning} attribute \textsc{cvm}. \isfeat{cvm}
%
Within \textsc{cvm} it is specified (or underspecified) what kind of vector (\textsc{vec}) \isfeat{vec} is at stake (axis vector, shape vector, place vector), and how it looks like, that is, which \textsc{path} \isfeat{path} it describes.
%
A semicircle is an axis vector whose path is a 180\textdegree\ trajectory.
%
180\textdegree\ is the root of a type hierarchy which hosts all vector sequences within gestures space that describe a half circle. 
%
This information is added in terms of a form predicate to the restriction list of \textit{semicircle}, as shown in the speech daughter's (\textsc{s-dtr}) \isfeat{s-dtr} content (\textsc{cont}) in (\ref{ex:half-circle}).
%
...
Extending lexical entries with frame information from \isi{frame semantics} \citep{Fillmore82a-u}, also the exemplification of predicates not overtly expressed becomes feasible \citep[Sec.~9.2.1]{Luecking:2013:a}; a datum in case is illustrated by the \emph{spiral staircases} in \ref{ex:staircases}/Figure~\ref{fig:staircases}.
%
A highly improved version of the \enquote{vectorisation} of gestures with a translation protocol has been spelled out in \citet{Luecking:2016}, but within the semantic framework of a \emph{Type Theory with Records} (\citet{Cooper:ms,Cooper:Ginzburg:2015}; cf. also \crossrefchaptert{pragmatics}).
...
Each feature value pair from the gesture's representation in (\ref{ex:mud-gesture}) is mapped onto an RMRS-based underpecified representation \citep[\page 442]{Alahverdzhieva:Lascarides:Flickinger:2017}:
%
  ...
  \begin{avm}
    ...
    cont & \[top & $h_1$ \\ 
             ...
             rl & \<
                  ...
                  m-arg & $x_2$ 
                  \], 
                  \avmbox{N\textsubscript{sem}}, 
                  \avmbox{G\textsubscript{sem}}, 
                  \> \\
             hc & \avmbox{G\textsubscript{$=q$}}
           \]
    \]
  \end{avm}
}
...
[{
  \begin{avm}
    \[time & \textup{\@2} \\
    cont & \[top & $h_1$ \\
            hk & \[ltop & $l_0$ \\ idx & $i_{1--5}$\] \\
            rl & \avmbox{G\textsubscript{sem}}\<
                                              \[\asort{$[\mathcal{G}]$}
                                              lbl & $l_0$ \\
                                              arg0 & $h_0$
                                              \], 
                                              ...
                                              \]
                                              \>\\
            hc & \avmbox{G\textsubscript{$=q$}}\{$h_0 =_q l_1, \ldots, h_0 =_q l_5$\}
           \]
    \]
  \end{avm}
} [{
...
\end{forest}
\caption{Derivation tree for depicting gesture and its affiliate noun \enquote{mud} \protect\citep[\page 447]{Alahverdzhieva:Lascarides:Flickinger:2017}.}
\label{fig:mud}
\end{figure}
...
%
When integrated with speech, the substance resolves to the mud and the rotating event the mixing.
%
On the other hand, the gesture can depict from the perspective of \isi{character viewpoint} \citep{McNeill:1992}, which corresponds to the predication from the right branch of (\ref{ex:interpret-circular}).
%
...
In addition to addressing (solving) the three basic questions identified in Section~\ref{sec:empir-desid-gramm} -- roughly, founding gesture meanings, governing affiliation, and characterise semantic integration -- another issue has received some attention recently, namely the \isi{projection} of behaviour of gestures when interacting with logical operators \citep{Ebert:2014:a,Schlenker:2018}.
%
For instance, the unembedded gesture in (\ref{ex:slap-unembedded}) \citep[\page 303]{Schlenker:2018}:
%
\ea \label{ex:slap-unembedded}
John [\textit{slapping gesture}] punished his son. \\
$\Rightarrow$ John punished his sone by slapping him.
\z
...
%
Unlike presupposition, however, \citet[\page 303]{Schlenker:2018} claims that the inference is conditionalized on the at-issue contribution of (\ref{ex:slap-none}). 
%
...
%
An HPSG account of cosupposition projection has to be provided yet, however.
...
%
Two examples in case are given in Figures~\ref{fig:mypoint} and \ref{fig:yourturn} and has been discussed by \citet{Bavelas:Chovil:Coates:Roe:1995}.


The \enquote{delivery gesture} \is{delivery gesture} in Figure~\ref{fig:mypoint} is be used to underline an argument or to refer to the fact that the current issue is known to the interlocutors. 
%
...
The open hand pointing gesture in Figure~\ref{fig:yourturn} acts as a turn-taking device: it can function as a \isi{turn-assigning gesture} (underlined by the caption of Figure~\ref{fig:yourturn}).
%
When used to point at the current speaker, a gesture of this king can also indicate that the gesturer wants to take the turn and address the turn holder.
...
%
Given their entrechment in dialogue processes, their natural home is in a dialogue theory anyway (see \crossrefchaptert{pragmatics}).
%
Accordingly, the presumably only formal approach to some of these gestures has been spelled out within dialogical framework PTT \citep{Rieser:Poesio:2009}.
\is{interactive gestures|)}
