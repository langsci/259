...
The main focus of the discussion will, however, be on prototypical idioms.
...
definition of \emph{multiword expression}\is{multiword expression} from  \citet[\page \added{269}]{Baldwin:Kim:10}.
...
\footnote{In the phraseological tradition, the aspect of \emph{lexicalization} is added
  ...
  criterion might have the same coverage as \emph{conventionalization} used in
  \citet[\iaddpages]{NSW94a}. These criteria are addressing the mental representation of idioms as a
...
For \citet{Baldwin:Kim:10}, \emph{lexical idiosyncrasy} concerns expressions with words that only occur in an idiom, so-called \emph{phraseologically bound words}\is{bound word}, or \emph{cranberry words}\is{cranberry word|see{bound word}} \citep{Aronoff76a-u}\addpages. Examples include \bspT{make headway}{make progress}, 
\bspT{take umbrage}{take \rep{offence}{offense}},
...
One challenge of the \rep{modelling}{modeling} of idioms is capturing the relation between the literal and the idiomatic meaning of an expression.
...
To give just one classical example, \citet{Chomsky81a}\addpages uses the passivizability of some idioms as an argument for the existence of Deep Structure, i.e., a structure on which the idiom is inserted holistically. 
%
...
\crossrefchaptert[Section~\ref{minimalism:sec-intro}]{minimalism}.\itdopt{Stefan: Do you refer to
  Section~\ref{minimalism:sec-intro}? If not, which one?} Nonetheless, idioms have been used to motivate assumptions about the architecture of linguistic signs in HPSG as well.
...
While there are many aspects of \citet{NSW94a} that have not been integrated into the formal \rep{modelling}{modeling} of idioms, 
...
\citet{WSN84a-u} and \citet{NSW94a} propose a simplified first approach to a theory that would be in line with this quote. They argue that, for \ili{English}, there is a correlation between syntactic flexibility and semantic decomposability in that non"=decomposable idioms are only semi-fixed, whereas decomposable idioms are flexible, to use our terminology from Section~\ref{Sec-EmpiricalDomain}. 
...
Since non"=decomposable idioms are inserted as units, their parts cannot be accessed for syntactic operations such as passivization or movement. Consequently, the generalization about semantic non"=decomposability and syntactic fixedness of \ili{English} idioms from \citet{WSN84a-u} is implemented directly.
...
\citew[756]{Fellbaum:19}\label{ex-kick-fellbaum}}
\z 
...
\vbox{
\eanoraggedright
Strong Non"=locality Hypothesis\is{locality!Strong Non-locality Hypothese} (SNH)\is{SNH|see{locality!Strong Non-locality Hypothesis}}\label{snh}\\
...
They look at a wide variety of different types of \ili{German} idioms, including support verb constructions. 
...
\rep{F}{f}irst, they use a lexeme-identification feature, \feat{lexeme}, which is located inside the \feat{index} value and whose value is the semantic constant associated with a lexeme. 
Second, they include a feature \feat{theta-role}, whose value indicates which thematic role a sign is assigned in a structure. In addition to standard thematic roles, they include a dummy value \type{nil}.
...
\footnote{We do not need to specify the \feat{rel} value for the noun \emph{beans}, as the \feat{lexeme} and the \feat{rel} value are usually identical.}
\itdobl{Stefan: Convert to \subj and \comps or add footnote explaining what \subcat is.}
...
\avm{
[\phon <spill> \\
synsem & [cat &	[subcat & <NP, NP[lexeme & beans\_i]>] \\
          cont & [rel & spill\_i] ] ]
}
\ex 
\avm{
[\phon <beans> \\
synsem & [content & [index & [lexeme & beans\_i] ] ] ]
}
\zl 

The \feat{lexeme} values of these words can be used to distinguish them from their ordinary, non"=idiomatic homonyms. 
...
\itdobl{Stefan: I do not understand this. Are you referring to (\mex{0}a)? But then why plural ``entries''? If
  you refer to (\mex{0}), then how can \emph{beans} undergo passive?} 

The idiomatic verb \emph{spill} selects an NP complement with the \feat{lexeme} value \type{beans\_i}. 
The lexicon is built in such a way that no other word selects for this \feat{lexeme} value. 
This models the lexical fixedness of the idiom.

The choice of putting the lexical identifier into the \feat{index} guarantees that it is shared between a lexical head and its phrase, which allows for syntactic flexibility inside the NP. 
Similarly, the information shared between a trace and its antecedent contains the \feat{index} value. Consequently, participation in unbounded dependency constructions is equally accounted for.
Finally, since a pronoun has the same \feat{index} value as its antecedent, pronominalization is
also possible.
\itdopt{Stefan: Does the reader know why coindexation and pronominalization would be nice to have? I
  think you need (to refer to) an example here.}
%
...
The special \feat{theta-role} value \type{nil} will be used to 
...
\avm{
[\phon <kick> \\
synsem & [cat & [subcat & <NP, NP [\punk{phon}{\phonliste{ the, bucket }} \\
                                   theta-role & nil] > ] \\
         cont & [rel & die] ] ]
}
\z 
...
There are also some undesired properties of the \textsc{lexeme} value selection. The index identity between a pronoun and its antecedent would require that the subject of the relative clause\is{relative clause} in \refer{strings-relcl} has the same \feat{index} value as the head noun \emph{strings}. However, the account of the lexical fixedness of idioms is built on the assumption that no verb except for the idiomatic \emph{pull} selects for an argument with \feat{lexeme} value \type{strings\_i}.%
\footnote{\citet{Pulman:93} discusses an analogous problem for the denotational theory of \citet{GKPS85a}.}
...
the analytic ingredients of \citet{KE94a} constitute the basis of later HPSG analyses. In particular, a mechanism for lexeme-specific selection has been widely assumed in most approaches. The attribute \feat{theta-role} can be seen as a simple form of an \emph{inside-out} mechanism\is{inside-out constraint}, i.e., as a mechanism of encoding information about the larger structure within which a sign appears. 
...
%
The overall framework of \citet{Riehemann2001a} is Constructional HPSG with \emph{Minimal Recursion Semantics}\is{Minimal Recursion Semantics}  \citep{CFMRS95a-u,CFPS2005a}; see also \crossrefchaptert[Section~\ref{semantics:sec-mrs}]{semantics}.
...
The proposal in \citet{Riehemann2001a} simultaneously is phrasal and obeys the SLH. To achieve this, \citet{Riehemann2001a} assumes an attribute \feat{words}, whose value contains all words dominated by a phrase. This makes it possible to say that a phrase of type \type{spill-beans-idiomatic-phrase} dominates the words \emph{spill} and \emph{beans}. This is shown in the relevant type constraint for the idiom \emph{spill the beans} in \refer{sr-spillbeans}.%
\footnote{The percolation mechanism for the feature \feat{words} is rather complex. 
%In fact, in \citew[Section 5.2.1]{Riehemann2001a} the idiom-specific words appear within a \feat{c-words} value and the other words are dominated by the idiomatic phrase in the value of an attribute \feat{other-words}, both of which together form the value of \feat{words}. While all the values of these features are subject to local percolation principles, 
...
\noindent
The \feat{words} value of the idiomatic phrase contains at least two elements, the idiomatic words of type \type{i\_spill} and \type{i\_beans}. 
The special symbol {\scriptsize \srdefault} used in this constraint expresses a default\is{default}. It says that the idiomatic version of the word \emph{spill} is just like its non"=idiomatic homonym, except for the parts specified in the left"=hand side of the default. 
...
Since the \feat{words} list contains entire words, it is also possible to require that the idiomatic word \emph{kick} be in active voice and/""or that it take a complement compatible with the description of the idiomatic word \emph{bucket}.
This analysis captures the syntactically regular internal structure of this type of idioms and is
compatible with the occurrence of modifiers such as \emph{proverbial}. At the same time, it prevents
passivization and excludes extraction of the complement.
\itdopt{Stefan: Did you explain how extraction is excluded?}
...
is part of a system for parsing and machine translation\is{machine translation}. Idioms in the source language are identified by bits of semantic representation -- analogous to the elements in the \feat{words} set. This approach, however, does not constitute a theoretical \rep{modelling}{modeling} of idioms; it does not exclude ill-formed uses of idioms but identifies potential occurrences of an idiom in the output of a parser.}
...
%
\citet{Bender2000a} discusses the same idea under the notions of \emph{minimal} versus \emph{maximal} grammars, i.e., grammars that are as free of redundancy as possible to capture the grammatical sentences of a language with their correct meaning versus grammars that might be open to a connection with usage-based approaches\is{usage-based grammar} to language \rep{modelling}{modeling}.
...
\footnote{An as-yet unexplored solution to the problem of free occurrence of idiomatic words within
  an experience-based version of HPSG could be to assign the type \type{idiomatic-word} an extremely
  low probability of occurring. This might have the effect that such a word can only be used if it
  is explicitly required in a construction. However, note that neither defaults\is{default} nor
  probabilities are well-defined part of the formal foundations of theoretical work on HPSG; see
  \crossrefchaptert{formal-background}.}
\itdopt{This was also assumed by \citet{MK2000a} in the \verbmobil project. See also \citew{OFTM2004a-u} for an
  HPSG parser with probabilities or weighted constraints. This is mentioned in
  \crossrefchapterw{properties} in footnote 3.}
\is{Constructional HPSG|)}
...
feature \feat{coll} defined on all signs. The value of this feature specifies the type of internal irregularity. 
...
Fully regular phrases such as \emph{read a book} have a trivial value of \feat{coll}. 
...
%
The phrasal lexical entry for \emph{trip the light fantastic} is sketched in  \refer{rs-trip}, adjusted to the feature geometry of \citet{Sag97a}.
...
\citet{Soehn2006a} applies this theory to \ili{German}\il{German}. He solves the problem of the relatively large degree of flexibility of non"=decomposable idioms in \ili{German}
by using underspecified descriptions of the constituent structure dominated by the idiomatic phrase.

For decomposable idioms, the two-dimensional theory assumes a collocational component. This component is integrated into the value of an attribute \feat{req}, which is only defined on \type{coll} objects of one of the irregularity types. 
...
To give an example, the idiom \emph{spill the beans} would be analyzed as consisting of two  idiomatic words \emph{spill} and \emph{beans} with special \feat{listeme} values \type{spill-i} and \mbox{\type{beans-i}}. The idiomatic verb \emph{spill} imposes a lexeme selection on its complement. The idiomatic noun \emph{beans} has a non"=empty \feat{req} value, which specifies that it must be selected by a word with \feat{listeme} value \type{spill-i} within the smallest complete clause dominating it.
...
\footnote{This idea has been previously expressed within a  Minimalist perspective \rep{in}{by}
  \citew{Everaert:10} \added{and G.\ \citew[\page 21]{GMueller2011a}}. 
\itdopt{Stefan: quote from Gereon's paper: Nicht immer muss dabei einem Teil eines Phrasems selbst ein nicht- trivialer semantischer Beitrag attribuiert werden. Für kick the bucket kann man z.B. annehmen, dass kick ‚sterben‘ heißt im Kontext von bucket, dass bucket aber ein Expletivum ist im Kontext eines solchen Verbs kick (oder hier die Identitätsfunktion denotiert); the wäre dann ein Expletivum im Kon- text eines Expletivums.3 Dies vermehrt dann selbstverständlich (aber auf harmlose Art und Weise) die Zahl der Lexikoneinträge.}
}
...
selection, using a feature \feat{lid} (for \emph{lexical identifier}).  The authors assume that
there is a difference between idiomatic and non"=idiomatic \feat{lid} values.  Only heads that are
...
the SLH has been a major conceptual motivation for Sign-based Construction Grammar\is{Sign-based
  Construction Grammar}, \citeauthor{KSF2015a}'s paper is an important contribution showing the
...
\citet{Richards:01} to motivate a lexical decomposition of \rep{these}{the involved} verbs.  A similar argument has been
...
be \rep{modelled}{modeled} in HPSG.  One option would be to capture the \emph{give}--\emph{get}--\emph{have}
...
\textsc{lexical-id} values.  However, the extent of this phenomenon across the set of idioms is not
known empirically.
...
\footnote{See \citew{Koenig1999b} for an analysis of along these lines.\itdopt{Add reference to
    Briscoe and Copestake and \citew{MuellerLFGphrasal} for benefactive lexical rule? As far as I can see this rule is not
    explicit in Koenig's paper.}}

Related to the question of lexical variation are \emph{phraseological
  patterns}\is{phraseological patterns}, i.e., very schematic idioms in which the lexical material is largely free. Some
...
construction, or \emph{\isi{Comparative Correlative Construction}} -- see
\crossrefchaptert[Section~\ref{coord:sec-comparative-correlatives}]{coordination} \added{and
...
positions (focus position in \ili{Hungarian}\is{Hungarian}).
...
take umbrage & take \rep{offence}{offense} & bound word: \emph{umbrage}\\
...
\itdopt{Stefan: Is this appendix needed?}
...
%
I would not have been able to format this chapter without the support of Sebastian
Nordhoff.\itdopt{Mention Language Science Press?}
%
