...
\abstract{ This chapter first sketches basic empirical properties of idioms. The state of the art
  before the emergence of HPSG is presented, followed by a discussion of four types of HPSG
  approaches to idioms. A section on future research closes the discussion.}
...
\emph{phraseme}\is{phraseme|see{idiom}}, \emph{phraseologism}\is{phraseologism|see{idiom}},
\emph{phraseological unit}\is{phraseological unit|see{idiom}}, or \emph{multiword expression}\is{MWE|see{idiom}}. 
...
feature, \feat{lexeme}, which is located inside the \feat{index} value and whose value is the
...
\noindent
The \feat{lexeme} values of these words can be used to distinguish them from their ordinary,
...
The idiomatic verb \emph{spill} selects an NP complement with the \feat{lexeme} value
\type{beans\_i}.  The lexicon is built in such a way that no other word selects for this
\feat{lexeme} value.  This models the lexical fixedness of the idiom.

The choice of putting the lexical identifier into the \feat{index} guarantees that it is shared
between a lexical head and its phrase, which allows for syntactic flexibility inside the NP.
Similarly, the information shared between a trace and its antecedent contains the \feat{index}
value. Consequently, participation in unbounded dependency constructions is equally accounted for.
Finally, since a pronoun has the same \feat{index} value as its antecedent, pronominalization as in
...
clause\is{relative clause} in \refer{strings-relcl} has the same \feat{index} value as the head noun
\emph{strings}. However, the account of the lexical fixedness of idioms is built on the assumption
that no verb except for the idiomatic \emph{pull} selects for an argument with \feat{lexeme} value
...
idiomaticity is a central empirical domain for Construction Grammar\is{Construction Grammar}; see
...
\noindent
The \feat{words} value of the idiomatic phrase contains at least two elements, the idiomatic words
...
phrase.  Since the \feat{words} list contains entire words, it is also possible to require that the
...
restricted.\footnote{Since the problem of free occurrences of idiomatic words is not an issue for
  ...
  semantic representation -- analogous to the elements in the \feat{words} set. This approach,
...
idiomaticity\is{statistical idiomaticity}, based on the variationist study in \citet{Bender2000a}.
...
This approach is another attempt to reify the GPSG\is{Generalized Phrase Structure Grammar} analysis
...
feature \feat{coll} defined on all signs. The value of this feature specifies the type of internal
...
predictable.  Fully regular phrases such as \emph{read a book} have a trivial value of \feat{coll}.
...
component is integrated into the value of an attribute \feat{req}, which is only defined on
...
idiomatic words \emph{spill} and \emph{beans} with special \feat{listeme} values \type{spill-i} and
\mbox{\type{beans-i}}. The idiomatic verb \emph{spill} imposes a lexeme selection on its
complement. The idiomatic noun \emph{beans} has a non"=empty \feat{req} value, which specifies that
it must be selected by a word with \feat{listeme} value \type{spill-i} within the smallest complete
...
the SLH has been a major conceptual motivation for Sign-Based Construction Grammar\is{Construction Grammar!Sign-Based}, \citeauthor{KSF2015a}'s paper is an important contribution showing the
