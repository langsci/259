...
Semi-fixed idioms allow for morphosyntactic variation such as inflection. These include some prototypical idioms (\bsp{trip the light fantastic}, \bsp{kick the bucket}) and complex proper names. In English, semi-flexible idioms show inflection, but cannot easily be passivized, nor do they allow for parts of the idiom being topicalized, see \refer{kick-ex}.
...
For such expressions, the idiomatic words that constitute them are assumed to have an empty semantics and the semantics of the idiom is contributed as a constructional semantic contribution only by the idiomatic phrase. 
...
In the case mentioned in footnote \ref{fn-semmeln}, the alternation consists of substituting a word with a (near) synonym and keeping the meaning of the idiom intact. Again, HPSG seems to have all the required tools to model this phemonenon -- for example, by means of hierarchies of \textit{lexical-id} values. 
