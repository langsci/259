...
\abstract{This chapter aims to offer an up-to-date comparison of HPSG and
 ...
 human sentence processing (Section~\ref{implementation}). \rep{Thoughtout}{Throughout}
...
\citealt[Section~2.2.5, 46--49]{jaeger05})\itdobl{Are the section and the pages referring to the
...
technically precise way, how (\ref{redundant-DV}) and (\ref{NCC-arg-deriv})\itdopt{Change order in
...
movement-free analyses of local dependencies\itdopt{non-local dependencies?} along the lines noted
...
valence features in HPSG,\itdopt{Why is it different?} given that CCG and TLCG share many key
...
\crossrefchapteralt{minimalism}\itdobl{Which section?} for discussion on a related point).
