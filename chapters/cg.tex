...
varieties of CG, \emph{Combinatory Categorial Grammar}\is{Combinatory Categorial Grammar (CCG)} (CCG;
\citealt{Steedman2000a-u,steedman2012}) and \emph{Type-Logical Categorial Grammar}\is{Type-Logical
...
form in Figure~\ref{fig-john-loves-mary}, except that the symbol VP is replaced by \syncat{NP\ensuremath{\backslash}{}S}:
...
\end{forest}
\caption{PSG analysis of (\ref{cg-tree1})}\label{fig-john-loves-mary}
\end{figure}
\noindent
...
\begin{exe}
 \ex\label{ACC-CCG-DV}
\attop{\begin{prooftree}[separation=.25em,label separation=.25em]
\hypo{\MultiLine{\mathsf{ \ptfont mary  }; \\ \sem{ \trns{m}  }; \\\syncat{NP}}}
...
\citet{Dowty82a-u} and \citet{Dowty90a-Eng} in a variant of CG which is essentially an
...
Finally, \isi{pied-piping} poses a somewhat tricky issue for the
analysis of relativization in CG (see, for example,
\citealt{Pollard88a,Morrill94a-u};
\citealt[Section~8.6]{MuellerGT-Eng1}; see also
\crossrefchaptert[footnote 3]{relative-clauses}). To see this point,
...
a more sophisticated treatment in TLG (see \citealt[Chapter 9, section 9.7]{Carpenter98a-u}
...
by the vertical slash in this analysis.\footnote{It is hard to tell
  whether this is a vice or a virtue. I only note here that
  \crossrefchaptert[section 2.1.1]{relative-clauses} emphasize the difference between these two types
...
of the \citet{ps2} analysis. One possible difference between an
analysis involving a general constructional schema in the current HPSG
literature (of the sort found in \citetalt{MuellerGT-Eng1}) and the
lexically-triggered analysis in CG sketched above is that the latter
...
\crossrefchaptert[section 2.1.1]{relative-clauses} for some
discussion.} there may be an advantage for an analysis in HPSG that
posits a general constructional schema for licensing pied-piping
relative clauses.
