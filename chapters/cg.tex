...
\abstract{This chapter aims to offer an up-to-date comparison of HPSG and
 ...
 of these two variants of CG. This is
 followed by a comparison of HPSG and CG at a broad level, in terms of
 the general architecture of the theory, 
 and then, by a  more focused comparison of specific linguistic analyses
 of some selected phenomena. The chapter ends by briefly
 touching on issues related to computational implementation and
 human sentence processing. \rep{Thoughtout}{Throughout}
...
\itdobl{References to Sections should not be in the abstract. The abstract is something that has to
  be a standalone text for databases and so on. References and crossrefs should be given in the
  introduction. Material from the abstract may be repeated there.}


\colorcodingexplanation


\inlinetodostefan{add page numbers to references}
\section{Introduction}

The goal of this chapter is to provide a comparison between HPSG and
\isi{Categorial Grammar} (CG). The two theories share certain important
...
\itdobl{Add overview here: ``The rest of the chapter is structured as follows:''}
...
\citealt{KB82a-u,BATW2015a}\added{; \crossrefchapteralt{lfg}}) is. For this reason, I will
start my discussion by sketching some important features of two major
varieties of CG, Combinatory Categorial Grammar\is{Combinatory Categorial Grammar (CCG)} (CCG;
...
of the slash.\itdopt{slash is not introduced. Maybe OK.} I stick to the TLCG notation throughout this chapter for
...
\end{prooftree}}
\itdobl{mary and loves seem to be in the wrong order.\\
Yusuke: This is intentional. But I now see that I forgot to add
explanation. I've added the relevant clarification in the text.}
\end{exe}
...
Thus,  just like the analysis trees in Linearization-based HPSG, the
...
\end{forest}
\caption{PSG analysis of \changed{\emph{John loves Mary.}}}\label{fig-john-loves-mary}
\end{figure}
\noindent
Things will start looking more interesting as one makes the fragment
more complex (and also by adding the semantics\itdopt{remove ``the''?? Not sure. My English is bad.}), but before doing so,
I first introduce
...
From this logical\itdopt{remove quotes?} point of view, the two slashes should really be
...
\end{prooftree}}
\itdobl{mary and loves in wrong order?}
\end{exe}
...
Some\is{Combinatory Categorial Grammar (CCG)|(} more machinery is needed to do some interesting linguistic analysis.
...
As noted by \citet{Steedman85a-u}\addpages, with Type Raising and Function
...
\citet{partee-rooth1983a}\addpages and the polymorphic syntactic category
...
\begin{exe}
 \ex\label{ACC-CCG-DV}
\attop{\oneline{\begin{prooftree}[separation=.25em,label separation=.25em]
\hypo{\MultiLine{\mathsf{ \ptfont mary  }; \\ \sem{ \trns{m}  }; \\\syncat{NP}}}
...
\end{exe}
\vspace{-\baselineskip}
\itdobl{mary and read in wrong order?}
\end{figure}
% 
...
(constituting \add{the} extraction pathway) such
...
development in CCG.\is{Combinatory Categorial Grammar (CCG)|)}
...
\begin{exe}
 \ex\label{SE} %\\[-15pt]
     \begin{multicols}{2} 
\begin{xlist}
 \ex\label{rsethree} \mbox{\isi{Forward Slash Elimination}}:\\[.5\baselineskip]
\begin{prooftree}
\hypo{\LexEnt{\pt{\ptv{a}}}{\sem{ \sF }}{\syncat{\textit{A}\ensuremath{/}\textit{B}}}}
\hypo{\LexEnt{\pt{\ptv{b}}}{\sem{ \sG }}{\syncat{\textit{B}}}}
\infer2[\ensuremath{/}E]{\LexEnt{\pt{\ptv{a} \ensuremath{\circ}\xspace \ptv{b}}}{\sem{ \sF(\sG)}}{\syncat{\textit{A}}}}
\end{prooftree}
 \ex\label{lsethree} \mbox{\isi{Backward Slash Elimination}}:\\[.5\baselineskip]
\begin{prooftree}
\hypo{\LexEnt{\pt{\ptv{b}}}{\sem{ \sG }}{\syncat{\textit{B}}}}
\hypo{\LexEnt{\pt{\ptv{a}}}{\sem{ \sF }}{\syncat{\textit{B}\ensuremath{\backslash}{}\textit{A}}}}
\infer2[\ensuremath{\backslash}E]{\LexEnt{\pt{\ptv{b} \ensuremath{\circ}\xspace \ptv{a}}}{\sem{ \sF(\sG) }}{\syncat{\textit{A}}}}
...
\begin{multicols}{2}
 \begin{xlist}
 \ex\label{rsi} \mbox{\isi{Forward Slash Introduction}}:\\[.5\baselineskip]
\begin{prooftree}
\hypo{$\vdots$}
...
\end{prooftree}
 \ex\label{lsi} \mbox{\isi{Backward  Slash Introduction}}:\\[.5\baselineskip]
\begin{prooftree}
\hypo{$\vdots$}
\infer[no rule]1{$\vdots$}
...
\citealt[46--49]{jaeger05})\itdobl{Are the section and the pages referring to the
  same region? If so, remove one of them.} are now derivable as theorems. As an
...
\end{prooftree}}
\itdobl{Topmost two items reversed?}
\end{exe}
% % \end{multicols}
...
\attop{\oneline{\begin{prooftree}[separation=.5em, label separation=.25em]
...
technically precise way, how (\ref{NCC-arg-deriv}) and (\ref{redundant-DV})  \itdopt{Change order in
  which examples are referenced?} are
...
\begin{exe}
 \ex\label{scoping} \begin{xlist}
      \ex\label{upI} \mbox{\isi{Vertical  Slash Introduction}}:\\[.33\baselineskip]
\begin{prooftree}
\hypo{$\vdots$}
\infer[no rule]1{$\vdots$}
\hypo{[\LexEnt{\pt{\p}}{\sem{ x}}{\syncat{\textit{A}}}]\ensuremath{^n}}
...
\end{prooftree}
\ex\label{upE} \mbox{\isi{Vertical  Slash Elimination}}:\\[.5\baselineskip]
\begin{prooftree}
...
\yusukest\hskip.4em\shortarrow\hskip.1em\yusukest\xspace)  is obtained (\maru{1}),
...
\attop{\oneline{\begin{prooftree}[label separation=.5em,separation=.25em]
...
\end{prooftree}}\\
\oneline{\begin{prooftree}[separation=.5em,label separation=.25em]
\hypo{$\vdots$}
...
\infer2[\ensuremath{\backslash}E]{\MultiLine{\mathsf{ \ptfont to
    ...
    \\ \sem{ [ \lambda x \lambda P. \trns{onTh}(P(x)(\trns{r}))] \sqcup\xspace [ \lambda
      x \lambda P. \trns{onFr}(P(x)(\trns{l}))] }; \syncat{NP\ensuremath{\backslash}{}(VP\ensuremath{/}PP\ensuremath{/}NP)\ensuremath{\backslash}{}VP}}}
\end{prooftree}}\\
\oneline{\begin{prooftree}[separation=.5em,label separation=.25em]
...
\infer2[\ensuremath{\backslash}E]{\MultiLine{\mathsf{  \ptfont \ensuremath{\greekp_3} \ensuremath{\circ}\xspace to \ensuremath{\circ}\xspace robin \ensuremath{\circ}\xspace on \ensuremath{\circ}\xspace thursday \ensuremath{\circ}\xspace     {} }\\\mathsf{  \ptfont   \hspace{2ex} or \ensuremath{\circ}\xspace to \ensuremath{\circ}\xspace leslie \ensuremath{\circ}\xspace on \ensuremath{\circ}\xspace friday  }; \\   \sem{ [ \lambda P. \trns{onTh}(P(x)(\trns{r}))] \sqcup\xspace [ 
      \lambda P. \trns{onFr}(P(x)(\trns{l}))] }; \\\syncat{(VP\ensuremath{/}PP\ensuremath{/}NP)\ensuremath{\backslash}{}VP}}}
...
\z
\vspace{-\baselineskip}
\itdopt{This looks strange since the font size changes in the middle of the proof. Maybe there is no
  other way.\\
Yusuke: I agree this does look strange. If it's okay to add newline in
the middle of a semantic translation formula (such as between the two
lambda terms on the top line conjoined by $\sqcup$), that will reduce
horizonal length so that the final chunk of proof doesn't need to be
shrunk as a whole as much as it is now. Another option would be to
reduce font size for the first two chunks so that the font size
...
lexicalist\itdopt{Remove quotes? In general: there are a lot of quotes.} theories of syntax in the broader sense.\footnote{I say
...
local dependencies 
is perhaps the most important property that is shared \del{in common} in
...
movement-free analyses of local dependencies\itdopt{non-local
...
valence features in HPSG (for example, note the way in which 
nonconstituents in RNR and ACC are analyzed),\itdopt{Why is it
  different? \\ Yusuke: I've added clarification in parentheses.}
given that CCG and TLCG share many key 
...
HPSG and CG is that CG currently lacks a detailed theory of \add{(phrasal)}
``constructions'',
\itdopt{Lexical items are constructions as well, as are lexical rules. The thing that is missing in
  CG is phrasal constructions.} 
...
cf.\ \crossrefchapteralt[Section~\ref{sec-domains}]{order}),\itdopt{added \citew{Mueller95c}} has its origin 
...
the Curry/Dowty idea of \add{the} pheno/tecto distinction has also been the core motivation for the
...
overcome in the more recent, multi"=modal\itdopt{remove quotes} variant of CCG
\citep{Baldridge2002a-u}, which controls the application of such
non-order-preserving rules via a fine-grained system of
lexicalized modality\itdopt{remove quotes}. The modality specifications in this new version of CCG
...
Beavers \& Sag address this issue by introducing a \rep{condition}{constraint} called
...
\crossrefchapteralt{minimalism}\itdobl{Which section?\\Yusuke: When I
  wrote this passage initially, I skimmed the minimalism chapter and
  somehow seemed to have formed the idea that your discussion of
  HPSG vs. minimalism was getting at a similar point I'm trying to
  make here. But when I
  looked at your chapter again, I noticed that it was much more 
  ruthless on minimalism than I thought it would be. So, it may be somewhat
  misleading to quote it in this context. What do you think? I want to
cite everything that's relevant, but I certainly don't cite people in
inappropriate ways.}
...
\citep{PerlmutterRoss70,Link84a-u,Kiss2005a,yatabe-tam2017}.\itdopt{Added Link and Kiss}
...
verb:\footnote{There is some disagreement as to whether Gapping\itdopt{gapping? uniformly everywhere} is
...
which the modal \textit{can't} scopes over \add{the} conjunction (`it's not
...
on the (initially plausible) assumption that Gapping sentences \rep{would
be}{are} interpreted by simply supplying the meaning of the missing material
...
\itdopt{Mention (\ref{ex-leslie-bought-a-cd})? Otherwise the reader needs some time to relate the
...
overcomes the limitations of previous (H)PSG analyses \del{of Gapping}
...
literature.\itdopt{Really?\\Yusuke: As I note in the text, so far as I
  am aware, there's none. I know Philip Miller was working on it a
  couple of years ago, but it never seems to have been published.}
...
\itdopt{But this is a semantic binding theory and it fails because of binding and passive, doesn't
  it? I discuss this in the Binding chapter.\\Yusuke: Can you be a bit
more specific? Here, I'm just reproducing Steedman's 1996 analysis
without necessarily intending to endorse it, but 
it's not purely semantic, so, I don't think it fails in the case
of passive. But to see whether it fails or not, we need to be specific
about what's the possible issue.}
\end{figure}
...
theories of syntax.\itdopt{I also thought so, but with Branco's theory, binding can be done
  lexically. You just have to adopt his theory. It is described in the BT chapter.} While Principles A and
...
in which the referring term\itdobl{It is not just proper nouns, is
  it?} appears (but see \citet{Branco2002a} for an alternative view;
see also \crossrefchaptert{binding}). In fact,
\textcites{PS92a}[Chapter~6]{ps2} opt for this
