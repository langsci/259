...
(\crossrefchapteralt{cxg} identifies the \textit{N after N}
...
Finally, pied-piping poses a somewhat  tricky issue for the analysis
of relativization in CG 
(see, for example,
\citealt{Pollard88a,Morrill94a-u,MuellerGT-Eng2}). To see this point,
note that the analysis of (simple cases of) relative clauses in CCG
outlined in section 2.4.2 above does not  straightforwardly extend to
...
the fronted relative phrase, so, a simple \syncat{(N\ensuremath{\backslash}{}N)/(S/NP)}
    ...
    a translation of the HPSG analysis \citep{ps2} involving two types
    of long-distance dependency (handled by the REL and SLASH features
    in HPSG).
...
\begin{exe}
 \ex\label{piedPlex}
  \LexEnt{\pt{\ensuremath{\lambda} \ensuremath{\greeks_1} \ensuremath{\lambda} \ensuremath{\greeks_2}. \ensuremath{\greeks_1}(whom) \ensuremath{\circ}\xspace \ensuremath{\greeks_2}(\E)}}{\sem{ \lambda F \lambda P \lambda Q \lambda x. P(F(x)) \ensuremath{ \wedge\xspace } Q(x)}}{\syncat{(N\ensuremath{\backslash}{}N)\vs (S\vs PP)\vs (PP\vs NP)}}
\end{exe}
The entry in (\ref{piedPlex}) says that the relative pronoun takes two
arguments, PP missing an NP inside itself and an S missing a PP, and
