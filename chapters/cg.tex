...
syntax in the 1980s (see \citet{BB2011a} and \citet{MuellerGT-Eng3}
...
feature unification in HPSG, though details are typically not spelled
out in detail. In TLCG, there are occasional suggestions in the
literature (see, e.g., \citet{Morrill94a-u,pogodallapompigne2011})
...
values)), the function \SemTyp\ (which returns, for each
...
a VP (\textit{gave Bill a book}) would be obtained. 
...
to \citet{Geach70a},  but \citeposs{Humberstone2005} careful
...
\begin{exe}
 \ex\label{ldd-EX}
  This is the book that John thought that Mary read {\trace}\xspace .  
\end{exe}
\begin{samepage2}
\begin{exe}
 \ex\label{ldd-CCG-DV}
...
example, \citet{kuhlmann-etal-2015-lexicalization}):

\begin{exe}
 \ex\label{overg}
  *a$_{\mathrm{NP/N}}$ [$_{\mathrm{N/N}}$ powerful$_{\mathrm{N/N}}$ by
  Rivaldo$_{\mathrm{N\backslash N}}$] shot$_{\mathrm{N}}$ 
\end{exe}
...
of TLCG, see \citet{Carpenter98a-u} and \citet{KubotaLevineBook}).
...
Function Composition (as well as other theorems; see, e.g.,
\citet[Section~2.2.5, pp.~46--]{jaeger05}) are now derivable as theorems. As an
...
an expression of
type \syncat{S\vs NP} (phonologically \st\xspace \shortarrow
\st\xspace)  is obtained (\maru{1}),
...
\crossrefchaptert{lexicon}), which says that syntax and 
...
\citet{Bruening2018a,Bruening2018c-u,MuellerLexicalism,MWArgSt}
...
(see \crossrefchaptert{evolution} and \crossrefchaptert{lexicon}).\footnote{This
...
\citealt{Sag97a,Fillmore99a,GSag2000a-u}),  recent Sign-Based Construction
...
(see, e.g., \citet{MWArgSt} and \crossrefchaptert{cxg}; see also
\citet{steedman2011ccg}, which discusses ways in which some of the
empirical generalizations that \citet{Goldberg95a} adduces to the
...
(\crossrefchaptert{cxg} identifies the \textit{N after N}
...
mechanisms for dealing ``peripheral'' patterns, but whether such
...
Minimal Recursion Semantics \citep{CFPS2005a} and Lexical Resources
Semantics \citep{RS2004a-u}). CG, on the other hand, tends to adhere
...
Berthold Crysmann (see \crossrefchaptert{morphology}), which builds on
and extends Greg Stump's \term{Paradigm Function Morphology} (PFM;
\citet{Stump2001a-u-kopiert}), and early CG work on morphology
...
cf.\ \crossrefchaptert{order}), has its origin 
...
the prosodic component is modellled as a lambda calculus
...
\citet{moortgat1997} and \citet{morrill-ea11}). 
...
(cf.~\crossrefchaptert{case}, Section~2). The work by
...
Logics (cf.~\citet{BernardiPhD,Moortgat2011a-u};
...
tradition includes \citet{deaneBook}, \citet{kluender98},
\citet{HS2010a-u} and \citet{CP2020a-u}; see
\crossrefchaptert{islands}, \citet{levine2017} and
\citet{newmeyer2016} for an overview of this line of work and pointers
...
\crossrefchaptert{coordination} and \crossrefchaptert{ellipsis}). Here, I take up
\citet{BS2004a} and \citet{Yatabe2001a} (updated in
\citet{yatabe-tam2017}) as two representative proposals in this line
...
\crossrefchaptert{minimalism} for discussion on a related point).
...
\crossrefchaptert{processing} and \crossrefchaptert{minimalism}).
...
mechanism (cf.\ \crossrefchaptert{ellipsis},
...
nontransformational literature; cf.~\crossrefchaptert{binding})
...
\AxiomC{\MultiLine{\mathsf{ \ptfont praises }; \\ \sem{ \trns{praise} }; \syncat{VP\ensuremath{/}NP[\ensuremath{-}p]}}}
\dashedLine
...
\AxiomC{\LexEnt{\pt{john}}{\sem{ \trns{j} }}{\syncat{NP[\ensuremath{-}p]}}}
...
controversial to begin with (see \citet[122--124]{buringbinding} for
...
example, \citet{SW2011a}, \citet{steedman2012},
\crossrefchaptert{processing}), other things being equal, it is more
...
how exactly are we to understand the qualification ``other things
...
on the relationship between processing and TLCG (see \cite[Chapters 9
and 10]{morrill2011}, and references therein). In any event, a serious
...
with HPSG (see \crossrefchaptert{processing}).

As for connections to computational linguistics (CL)/natural language
processing (NLP) research, like
HPSG (cf.\ \crossrefchaptert{cl}), large"=scale computational
implementation has been an important 
research agenda for CCG (see, e.g., \citealt{WhiteBaldridge2003,CC2007a-u}). I refer
...
noncanonical types of coordination (nonconstituent coordination,
