...
\begin{forest}
sm edges
[{V[\subcat \liste{ NP }, \slasch \liste{ NP, NP }]}
  [{V[\subcat \liste{ PP, NP, NP }]} [play]]
  [{NP[\slasch \liste{ NP }]} [t]]
  [{PP[\slasch \liste{ NP }]}
    [{P[\subcat \liste{ NP }]} [on]]
    [{NP[\slasch \liste{ NP }]} [t]]]]
\end{forest}
}
...
\end{quote}
SBCG is committed to the position alluded to in the parenthetical sentence in this quote, namely, that general principles of information transmission within sentences makes it possible to insist on Constructional Localism.   See \crossrefchaptert{cxg} for a much more detailed discussion, and \citet{VanEynde2015a} for a review of the 2012 SBCG book.  
