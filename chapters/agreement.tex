...
\itdopt{The abstract seems a bit technical for a standalone summary. Maybe rephrase?}
...
HPSG offers an integrated account of these phenomena.\itdopt{which ones? Coordination agreement was
  the most recent you mentioned.}  In most cases the analysis of agreement phenomena does not involve any  special formal devices dedicated for agreement, comparable to the \textit{probe} and \textit{goal}, or the \textsc{agree} relation, found in \isi{Minimalist} accounts \citep{Chomsky2000b-u}.  Instead, the observed agreement phenomena arise as a side effect of other grammatical mechanisms responsible for valence saturation, the semantics of modification, and coreference.  
...
Since unification
is commutative,\itdopt{
Add a footnote and a pointer to \crossrefchaptert{formal-background}? Since unification is not assumed any longer.} the representation of an agreement construction is the same regardless of whether a feature originates from the trigger or the target.  This immediately accounts for common agreement behavior observed when triggers are underspecified  \citep{Barlow:1988}.  
...
this-\textsc{nom.f.sg}	old-\textsc{nom.f.sg}	book-\textsc{nom.sg} always fall-\textsc{3sg} \\\hfill\added{(Serbo-Croatian)}
\glt `This old book keeps falling.' 
\itdopt{LGR has a way to gloss inherent gender: (\textsc{f}) Shall I do this?}
\end{exe} 
...
\gll Taj	stari	sudija	je	dobro	sudio.	 \\
     that.\textsc{m}	old.\textsc{m}	judge	\textsc{aux}	well	judged.\textsc{m} \\\hfill\added{(Serbo-Croatian)}
...
\itdopt{They would have a disjunction and two derivations. It is uggly and probaly psycholinguistically implausible, but hey \ldots. Just saying.}   
...
\itdopt{Cite somebody on this? For example \citew[\page 465]{MG2010a} suggested such an analysis for Persian.}
...
In HPSG, predicate-argument agreement arises directly from \del{the} valence saturation, as illustrated already in (\ref{is}) above.  Thus the locality conditions on the trigger-target relation follow from the conditions on the subject-head or complement"=head relation.   Similarly, attributive adjectives agree with nouns directly through the composition of the modifier with the head that it selects via the \textsc{mod} feature.  For example, the \ili{Serbo-Croatian} feminine adjective form \textit{stara} `old.\textsc{f}' in (\ref{ex-ta-stara-studija})
\itdopt{JP: Reference to (\mex{1}) instead of (\ref{ex-ta-stara-studija})?} specifies feminine singular features for the common noun phrase (N$'$) that it modifies.  

\begin{exe} 
\ex	\label{stara}  Simplified lexical sign for  \textit{stara} \added{`old.\textsc{f}'}:\\*
\avm{
[\phon < stara > \\
mod & [head &	[\type*{noun}
                 num & sg \\
...
\noindent
In head-adjunct phrases, the \textsc{mod} value of the adjunct daughter is token-identical with the \synsem value of the head daughter.  So \textit{stara}'s feminine singular features  cannot conflict with the features of the noun it modifies \added{(see also \crossrefchapteralt[Section~\ref{np:sec-specifier-analysis}]{np})}.  

The predicted locality conditions are also affected by the percolation of features from words to phrasal nodes, and this depends on the location of the features within the feature description.   Agreement features of the \textit{trigger} appear either within the \textsc{head} value or the semantic \textsc{content} value (these give rise to \textsc{concord} and \textsc{index} agreement, respectively; see Section~\ref{ind-con}).  In either case these features percolate from the trigger's head word to its maximal phrasal projection, due to the Head Feature Principle \added{\crossrefchapterp[\pageref{page-hfp}]{properties}} in the former case and the Semantics Principle \added{\crossrefchapterp{semantics}} in the latter.  For example the noun phrase \textit{the books} inherits its [\textsc{num}~\textit{pl}] feature\itdopt{I find this strange. The feature is \textsc{num}. What you specified is a feature value combination. Also using ``inherits'' is dangerous since CxG people have strange ideas what inheritance could be. Replace by ``the noun phrase shares its \textsc{num} value with the value of its head and hence it is \type{pl}.''?} from the head word \textit{books}.  This determines plural agreement on a verb:  \textit{These books are/*is interesting.}  Apparent exceptions, where a target seems to fail to agree with the head of the trigger, are discussed below.\itdopt{Add ref to section.}   

However, agreement features of the \textit{target} appear in neither the \textsc{head} nor the \textsc{content} value of the target form, but rather appear embedded in an  \textsc{arg-st} list item or \textsc{mod} features.  So agreement features of the target do not project to the  target's phrasal projection such as VP, S, or AP.  This is a welcome consequence.  If the subject agreement features of the verb projected to the VP, for example, we would expect to find VP-modifying adverbs that consistently agree with them, but we do not.\footnote{VP-modifying secondary predicates sometimes agree with their own subjects.  What we do not find are adjuncts that consistently agree with the subject agreement features of the VP \del{even} when the adjunct is not predicated of that subject.}  


\section{Varieties of agreement target} 

\itdopt{Add a brief description of what the subsections will be about.}

\subsection{Anaphoric agreement}
\label{ana-agr}
In anaphoric agreement, an anaphoric pronoun agrees in person, number, and gender with its antecedent.  Since \citet{Pollard+Sag:1992,Pollard+Sag:1994}, anaphoric agreement has been analyzed in HPSG by assuming that person, number, and gender are formal features of the referential index associated with an NP.  Anaphoric binding in HPSG is modeled as coindexation, i.e.\ sharing of the \textsc{index} value, between the binder and bindee.  Thus any specifications for agreement features of the \textsc{index} contributed by the binder and bindee must be mutually consistent.  In (\ref{admire}), Principle A of the \isi{Binding Theory} \added{\crossrefchapterp{binding}} requires the reflexive pronoun to be coindexed with an o-commanding item, here the subject pronoun: 
...
entity.  \textsc{index} agreement is enforced only within the  syntactic domain defined by \changed{Binding Theory},
...
\itdopt{Swap order of c and d?}
\end{xlist}
\end{exe}

\noindent
The bound reflexive must agree formally with its antecedent, while other
coreferential pronouns need not agree, as they are not coarguments of the antecedent and not subject to the structural \changed{Binding Theory}.  
...
\added{See also \citew[Section~20.4.4.]{Mueller99a} for a discussion of similar cases in German and of problems for HPSG's Binding Theory.}
}
...
\itdopt{Has devojče inherent gender? It would be (N). Added without a dot before it.}
%\end{exe}
\begin{xlist}
\ex  
...
\gll 	Devojče            je               volelo              samo                  /  ?* samu              sebe. \\
        girl.\textsc{nom.n.sg} \textsc{aux.3.sg} liked.\textsc{n.sg} own.\textsc{acc.n.sg} {} {} \textsc{acc.f.sg} \changed{\textsc{self}}.\textsc{acc} \\
...
As noted above, in  HPSG agreement effectively piggy-backs on other independently justified grammatical processes.  Anaphoric agreement is a side-effect of binding (Section~\ref{ana-agr}) while  grammatical agreement is a side-effect of valence saturation and modification (Section~\ref{arg-mod-agr}).  The formal HPSG analysis of a particular agreement process mainly consists of positing agreement features somewhere in the feature \rep{structure}{description}; the observed properties follow from the location of those agreement features.  With regard to the location of the features, grammatical agreement bifurcates into two types, \textsc{index} and \textsc{concord}.\footnote{The \textsc{index}/""\textsc{concord} theory is sketched in  \citet[Chapter~2]{Pollard+Sag:1994} and \citet{Kathol99b}, and developed in detail in  \citet{Wechsler+Zlatic:2000,Wechsler+Zlatic:2003}, all in the HPSG framework.  It has since been adopted into LFG (\citealt{king+dalrymple:2004}, inter alia) and GB/Minimalism \citep{Danon:2009}.}  (The attribute name  \textsc{concord} was introduced by \citealt[799]{Wechsler+Zlatic:2000}, \citealt[14]{Wechsler+Zlatic:2003}; precursors to the idea were treated as \textsc{head} features in \citealt[\added{Section~2.5.1}]{Pollard+Sag:1994}, and called \textsc{agr} by \citealt{Kathol99b}.)   The best way to understand this bifurcation of agreement, and indeed the operation of grammatical agreement systems generally, is by considering their diachronic\is{diachrony} origin.   Although our primary goal is the description of synchronic grammar, a look at diachrony can help explain the forms that the grammar takes, and can also provide clues as to the best formalization of it.  
...
\avm{
[\phon < is > \\
subj & < NP [content|index &	[\changed{per} & 3rd \\
								num & sg ] ] > \\ 
comps & < XP > ]
}
\end{exe}
...
\avm{
[\phon < I > \\
\punk{content|index}{ \1	[\changed{per} & 1st \\
					num & sg ]} \\
context &  speaker(\1) ]
}
\itdopt{In \citew[94]{ps2} speaker is a feature not a relation. \textsc{c-indices|speaker}.}
\end{exe}
...
\gll 	Gagu	   wamanggal	  kerre    ngeben-da. \\
	     animal	wallaby	      big	      1\textsc{sg.sb.aux}-shoot \\\hfill\added{(Ngan’gityemerri)}
...
\end{xlist}
\itdopt{Stefan: Minimal pairs would be better. c is different.}
\itdobl{JP: I assume this was meant to be SUBJ as per the LGR. If not, the meaning of this glossing abbreviation should be put in an Appendix at the end of the paper.}
\itdopt{JP: Should I know the meaning of ``initiand''?}
\end{exe}
...
\glt `My old book fell.' \hfill\added{(Serbo-Croatian)}
\itdopt{Gloss inherent gender?}
\end{exe}
...
\avm{
 [ phon   & \phonliste{ knjiga }  \\
   synsem & [ category & [ head & [\type*{noun}
              ...
              content & [ index & \tag{i} [\changed{per} & 3rd\\ 
                                           num  & \1\\ 
                                           gen  & \2 ]\\ 
                          restr & \{ !$book(i)$! \} ]]] 
}
...
%% content & \[ 
%% index & \@i\[\changed{per} & \it{3rd}\\ num & \ibox{1}\\ gen & \ibox{2}\]\\ 
%% restr & \{ $book(i)$ \} 
%% \]\] 
%% \end{avm}
\zs
...
\itdopt{(\ref{stara}) does not have a \textsc{concord} feature. But maybe this is OK.}
...
\ex 
\gll Hus-et 	är	gott. \\
     house-\textsc{def.n.sg}	is	good.\textsc{n.sg} \\\hfill\added{(Swedish)}
...
\ex	
\label{metonymy}
\textit{metonymy-\changed{phrase}} \added{\impl} \\
\avm{
[mtr &	[syn & NP \\
		sem &	[index & s$_{unm}$ \\
...
\gll   Vous		\^{e}tes		loyal. \\
		you.\textsc{pl}		be.\textsc{2pl}	loyal.\textsc{m.sg} \\\hfill\added{(French)}
...
\ili{Passamaquoddy} long-distance agreement is illustrated by this sentence \citep[example\,(5)]{LeSourd:2018}, with the relevant elements indicated in \bolddescriptionintext:
\inlinetodostefan{I replaced bold by italics.}
...
\gll N-kosicíy-a-\textbfemph{k}  [eli- Píyel -litah\'{a}si-t           {[eli-kis-ankum-í-hti-t}   \textbfemph{n\`{i}kt}  \textbfemph{ehpíc-ik}	{poson\'{u}ti-yil]]} \\
     1-know-\textsc{dir-prox.pl} \spacebr{}thus- Píyel -think-\textsc{3an} { \spacebr{}thus-\textsc{pst}-sell-\textsc{3/1-prox.pl-3an}}         those.\textsc{prox}  woman-\textsc{prox.pl}     basket-\textsc{in.pl}\\\hfill\added{(Passamaquoddy)}
...
\gll N-kosicíy-a-\textbfemph{k} \textbfemph{n\`{i}kt}  \textbfemph{ehpíc-ik}$_i$ [eli- Píyel -litah\'{a}si-t  [eli-kis-ankum-í-hti-t  \textbfemph{e$_i$} 	{poson\'{u}ti-yil]]} \\
     1-know-\textsc{dir-prox.pl} those.\textsc{prox}  woman-\textsc{prox.pl} \spacebr{}thus- Píyel -think-\textsc{3an} \spacebr{}thus-\textsc{pst}-sell-\textsc{3/1-prox.pl-3an}         {}    basket-\textsc{in.pl}\\\hfill\added{(Passamaquoddy)}
...
\avm{
[\phon < kosicíy > \\
arg-st & < NP$_i$ , NP$_j$, S:[restr < \ldots\added{,} [prd|arg & j ]\added{,} \ldots> ]> ]
}  
\end{exe}
\itdobl{I checked LeSourd. He does not have structures involving a path \textsc{prd|arg}. He uses PRD$_j$ to refer to a predicate with the index $j$. Also it does not work with \textsc{arg} as one of the arguments, since the argument roles are \textsc{arg1}, \ldots \textsc{argn} or more specificlike \textsc{actor}. I would suggest using PRD$_j$ as in the original. Of course this begs the question how the index is accessed. I guess one would need a function returning all indices from a certain predication. Also adapt text below.} 
...
see.\textsc{past.3pl} they dragon \\\hfill\added{(Welsh)}
...
\noindent
The trigger is the subject in (\ref{welsh}a), object in (\ref{welsh}b), and the first conjunct of a coordinate subject in (\ref{welsh}c).  But in every case, ``An agreeing element agrees with an immediately following noun phrase if and only if the latter is a pronoun'' \citep[\changed{237}]{Borsley:2009}. \citet[\changed{257}]{Borsley:2009} expresses this as an HPSG implicational constraint using the \textsc{dom}ain feature from linearization theory (\citealt{reape:1994,Mueller95c,Mueller99a,Kathol:2000}; see also \crossrefchapteralt[Section~\ref{sec-domains}]{order}):
