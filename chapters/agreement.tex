...
trigger's head word to its maximal phrasal projection, due to the Head Feature Principle
\crossrefchapterp[\pageref{page-hfp}]{properties} in the former case and the Semantics Principle
...
(\ref{admire}), Principle A of the \isi{Binding Theory} \crossrefchapterp{binding} requires the
...
\punk{content|index}{ \1 [per & 1st \\
			  num & sg ]} \\
\punk{context|c-indices|speaker}{\1} ]
}
\end{exe}
...
%% content & \[ 
%% index & \@i\[per & \it{3rd}\\ num & \ibox{1}\\ gen & \ibox{2}\]\\ 
%% restr & \{ $book(i)$ \} 
%% \]\] 
%% \end{avm}
\zs
...
\ex 
\gll Hus-et 	är	gott. \\
     house-\textsc{def.n.sg}	is	good.\textsc{n.sg} \\\hfill(Swedish)
...
\ea	
\label{metonymy}
Metonomy schema adapted from \citet[82]{Wechsler:2013}:\\
...
alphabetical order): Anne Abeillé, Robert Borsley, George Aaron Broadwell, Jean-Pierre Koenig, and
Stefan Müller.  All of the reviews were valuable and together they greatly improved the paper.  
