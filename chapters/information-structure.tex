...
\abstract{Information structure as the hinge between sentence and
  ...
  information about discourse properties. In this chapter, I discuss
...
approach into \citegen{vallduvi:92} information packaging account,
  ...
  \emph{phrase} \impl &
    ...
    \avmtmp{
    	[synsem|loc [cat|head & verb\\
        	         cont|lf & \3 ]\\
       info-str|focus <\3>\\
       non-head-dtrs  <\ldots,[synsem & [fpp & plus\\
                                      \punk{loc|cont|lf}{\4} ] \\
...
all her daughters as ensured by the principle in~\pref{fig:focus-projection}.%
...
\emph{any-dtr}\,!(\1)! := [non-head-dtrs & \normalfont{element} !(\1)!]. \smallskip\\
\emph{collect-focus} !(! < > !)! := < >.\\ 
...
\emph{phrase} \impl &                 
	...
	\avmtmp{
      [ss|loc|cont|lf  \3\\
       struc-meaning|focus  <\3>\\
       \rel{dtrs-list(\rel{\normalfont{\textit{given-sign-list}}} \shuffle
                      <[ss|l|cont|lf & \4\\
                      struc-meaning|focus & <\4>]> 
                      )}]
	} $\lor$ \ldots
...
\ea
\avmtmp{
\emph{dtrs-list} !(!<\1|\2>!)! := [head-dtr & \1 \\ non-hd-dtrs & \2] \\

\emph{given-sign-list} := < >. \\

\emph{given-sign-list} := <[ss|l|cont|lf & \1\\
                          struc-meaning & [given & <\1>]] | \type{given-sign-list} >.
}
\zlast} 
...
sign is shuffled ($\bigcirc$).\footnote{The relation ``shuffle'' $\bigcirc$ is used as originally introduced in  \cite{Reape94}: the result is a list that contains all element from the two input lists and the order of elements from the original lists is preserved, see also the discussion in \crossrefchaptert[Section~\ref{order:sec-domains}]{order}.}  \footnote{If only binary structures are
...
\ex  \textit{Head-Dislocation Schema for \ili{Catalan}:}\\
  ...
  description\\
  \avmtmp{ [head \type{verb} [vform \type{finite}], subcat < >]
  }, and whose
...
\avmtmp{
	...
	\avmtmp{
		...
		[\avmtmp{[p|ps & <\type{hat}>\\is|focus & < >]},edge label={node[very near end,left]{H}},l+=4cm]
		[\avmtmp{[p|ps & <\type{hier}>\\is|focus & < >]},edge label={node[very near end,left]{C}},l+=4cm]
		[\avmtmp{[p|ps & <\type{noch nie}>\\is|focus & < >]},edge label={node[very near end,right]{C}},l+=4cm]
		[%
		\avmtmp{
			[p|ps & < >\\
			s & [l & \2\\
...
\label{sec:prosody}
A lot of languages mark information structure \is{prosody} prosodically, like for
example English and \ili{German}, where pitch accents of various shapes are
...
%  \centering
  ...
  \avmtmp{
    \normalfont{\textit{decl-tune(\1)}}} & $\leftrightarrow$ & \avmtmp{ \1 = \2 \+ <[pa & low-high-star\\
                                                                  bd & low]> }
$\wedge$\\
& &
\avmtmp{
...
[\normalfont{\textit{sign}}\\
  embed & $-$]} & \impl & \avmtmp{ [phon \1]\ $\wedge$ \normalfont{\textit{decl-tune}}(\1)
  }  \\
  \end{tabular}
...
English, several components of \citegen{Bildhauer2008a} phonological
