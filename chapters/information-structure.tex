...
has been proposed by \cite{Manandhar94a-u}. He assumes that all signs
...
\end{forest}
\caption{Type hierarchy under \textit{info-type} of \cite{Manandhar94a-u}}\label{fig:manand-info-struc}
\end{figure}
...
\cite{Paggio2009a-u} uses Minimal Recursion Semantics (MRS,
...
\begin{tabular}{rl}
  \emph{phrase} \impl &
    \avm{
      [info-str|focus & \1\ \+ \type{collect-focus} (\2) \\
...
\avm{
\emph{any-dtr}\,!(\1)! := [head-dtr & \1].\\ 
\emph{any-dtr}\,!(\1)! := [non-head-dtrs & \type{element} !(\1)!]. \smallskip\\
\emph{collect-focus} !(! < > !)! := < >.\\ 
\emph{collect-focus} ( <[info-struc|focus & <\1>] | \2> ) :=
<\1 | \type{collect-focus} !(\2)! >.
...
projection potential (\textsc{fpp} \textit{+}) is itself entirely focused.
...
\cite{deKuthy2002a}, discussed in Section~\ref{sec:struc-meaning}. The
...
projection in the presence of givenness \citep{DeKuthy.Meurers-11}. Figure~\ref{fig:verbal-focus-projection} shows the resulting principle.\footnote{ \citet[293]{DeKuthy.Meurers-11} introduce the feature \textsc{structured-meaning} as appropriate for all signs, \textsc{info-struc} is changed to only be appropriate for unembedded-signs. An additional constraint ensures that the value of \textsc{info-struc} for unembedded signs is that composed in \textsc{structured-meaning}.}
...
\emph{phrase} \impl &     
    ...
    \avm{
      [synsem|loc [cat|head & verb\\
       ...
       struc-meaning|focus <\3>\\
       non-head-dtrs  < \ldots, [synsem [fpp \type*{plus}\\
                                          loc|cont|lf  \4]\\
...
\zs
\itdobl{FFP should be uniform: either \type{plus} or +.}
...
sign is shuffled ($\bigcirc$).\footnote{The relation ``shuffle'' $\bigcirc$ is used as originally introduced in  \cite{Reape94}: the result is a list that contains all elements from the two input lists and the order of elements from the original lists is preserved, see the discussion in \crossrefchaptert[Section~\ref{order:sec-domains}]{order}.}\footnote{If only binary structures are
...
\ea
LP constraint on information structure in \ili{Catalan}  \citep{EV96a}:\\
  \centering
...
\cite{EV96a} ensure that left-dislocated constituents are always
...
dislocation, as illustrated in \pref{ex:clld-greek}, an
...
\cite{abeille:godard:sabio:08}, where two types of sentences with
...
over the entire NP \textit{ein Außenseiter}, i.e., the NP's \textsc{focus}
element is identical to this NP's \textsc{lf} value \idx{3}. \footnote{Note that the focus value of the entire NP is different from the focus value of just the noun \textit{Außenseiter}. If the focus value of the noun was structure shared with the focus value of the entire NP, this would mean that there is only a narrow focus on the noun itself, excluding the determiner and possible modifiers.} Since \textit{ein
...
\textit{+}, the principle governing focus projection in the verbal
...
\textit{local}.\footnote{In \citegen{Mueller2005d} formalization, filler daughters in multiple fronting configurations (and only in these) have a  \textsc{head|dsl} value of type \textit{local}, i.e., they contain information about an empty verbal head. The \textsc{dsl} (‘double slash’) feature is needed to model the HPSG equivalent of verb movement from the sentence-final position to initial position. } \cite{BC2010a} assume that an
...
Relating multiple fronting to focus \citep[75]{BC2010a}:\\
    \avm{
      [\type*{head-filler-phrase}\\
      non-head-dtrs & <[loc|cat|head|dsl & local]>]
    }
\impl\ 
  \avm{
...
\inlinetodostefan{Second constraint seems redundant since hd-fill-ph could be a supertype of pred-prepos-ph}
\ea
...
prosodic word. The constraint\footnote{\cite{Bildhauer2008a} uses the symbol $\leftrightarrow$ for the definition of relational type constraints and the symbol $\Rightarrow$ for other type constraints.} is shown in~\pref{fig:spanish-intonation}.
\inlinetodostefan{What about the $\leftrightarrow$?}
\ea
Intonational contour of \ili{Spanish} declarative utterances \citep[142]{Bildhauer2008a}:\\
%  \centering
  \begin{tabular}[t]{@{}l@{  }r@{}l@{}}
  \avm{
    \normalfont{\textit{decl-tune(\1)}}} & $\leftrightarrow$ & \avm{ \1 = \2 \+ <[pa & low-high-star\\
                                                                  bd & low]> }
$\wedge$\\
& &
\avm{
