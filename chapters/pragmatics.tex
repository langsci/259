...
%
This position is strengthened by the primacy of \isi{spoken language} in both ontogenetic and language \isi{acquisition} areas (on acquisition see \crossrefchapteralt{acquisition}).
\is{language system|)}
...
\end{tikzpicture}
\\
Here it is the truck, not the speaker, or rather the authorm of the note, that is for rent. Such examples of the German cognate of \enquote{I}, namely \textit{Ich}, are collected and discussed in \citet{Kratzer:1978}.}
%
...
information about phonetic, syntactic and semantic aspects is present in every sign, from words to phrases and clauses \citep[\page 5]{Pollard:1997} -- see also \crossrefchaptert{cg}, \crossrefchaptert{minimalism}, \crossrefchaptert{cxg}, \crossrefchaptert{lfg} and \crossrefchaptert{dg} for a comparison of HPSG to other grammar theories; a benchmark source is \citet{Mueller:2016}.
...
%
Records consist of fields of pairs of labels and objects, record types of fields of pairs of labels and types, which both can be nested \citep{Cooper:ms}.
%
...
%
For instance, (\ref{ex:hpsgttr-sign}) shows the record type (the judgement colon indicates that we now talk about TTR objects) for our attribute value matricesa genot abneral \isi{sign} according to \citet{Pollard:Sag:1994} (where \ttrtype{PhonType}, \ttrtype{CategoryType} and \ttrtype{SemType} denote obvious types -- see the Appendix for a minimal HPSG fragment defined in terms of TTR).
%
