...
%
In the HPSG format of \citet[\page 20]{Pollard:Sag:1994}, this constraint is expressed as in (\ref{ex:she})\deleted{, where \enquote{\textsc{head} \textit{noun}[\textsc{case} \textit{nom}]} abbreviates a head structure of type \textit{noun} which bears a case attribute with value \textit{nom} (nominative)}:
...
The \textsc{content} \isfeat{content} value is of type \textit{ppro} (\textit{personal-pronoun}), which is related to the NP type ($+$p, $-$a) from \emph{Government and Binding} theory \citep{Chomsky:1981}\addpages and interacts with HPSG's Binding Theory (see \crossrefchapteralt{binding}; see also \crossrefchapteralt[\added{Section~\ref{agreement:sec-anaphotic-agreement}}]{agreement}).
%
...
Here it is the truck, not the speaker, or rather the author of the note, that is for rent. Such examples of the \ili{German} cognate of \enquote{I}, namely \textit{ich}, are collected and discussed in \citet[\iaddpages]{Kratzer:1978}.}
%
...
\itdopt{Is there something else we could cite rahter than \citew{Pollard:1997}?}
...
%
Information structure realised by prosodic stress is also part of the speech-gesture interfaces within multimodal extensions of HPSG (cf.\ \crossrefchapteralt{gesture}\addpages.
\is{information structure|(}\is{focus|)}\is{information packaging|)}
...
context &	[\type*{context}
			c-inds &	[\type*{psoa}
						spkr & \2 \\
						addr & \3 ] \\
      		background &	\{[\type*{mutually-believe}
							experiencer & \2 \\
        					standard & \3 \\
...
\item \speaking{} \rep{w}{W}hat did you read?
...
%
Here the tuple represents a temporarily ordered sequence of objects of a given type (i.e.\ \textsc{dgb} and \textsc{ms} in case of dialogue agents' information state models)\is{information state}\is{information state model} which is witnessed by a \emph{string}\is{string} of respective events which \add{is} at least of length 1, as required by the \enquote{Kleene +}\is{+}  (see \citealt[Section~2.7]{Cooper:Ginzburg:2015} on a type-theoretical variant of the \isi{string theory of events} of \citealt{Fernando:2011}).
\is{dialogue gameboard|)}
...
%
In what follows, two sources of motivation are presented, \del{the} one drawing on semantic data gained from the clarification potential of reprise fragments (Section~\ref{sec:sub-sentential-meanings}), the other resulting from HPSG's struggle with connecting to semantic theories (Section~\ref{sec:semantic-objects}).
...
%
Due to structure sharing, certain slots of a head are \emph{identified} with semantic contributions of modifier or argument constructions (see  \crossrefchapteralt{arg-st} \added{on linking and \crossrefchapteralt[Section~\ref{properties:sec-adjuncts}]{properties} on head-adjunct phrases}).
%
...
%
Rather, grammars following this paradigm are model-theoretic, constraint-based grammars, resting on \emph{Relational Speciate Re-entrant Language} (RSRL) as formal foundation (\citealt{Richter:2000} via precursors like \citealt{Penn:1999}).\itdopt{Really Penn? I thought King was the main figure in this part \ldots.}
%
The formalism RSRL in its most recent implementation \citep{Richter:2004} has the advantage that the models it describes can be interpreted in different ways.\footnote{Richter (2019 p.c.); see also \crossrefchaptert[\iaddpages]{formal-background}.}
%
...
%
TTR, though looking similar to feature \rep{structures}{descriptions}, directly provides semantic entities, namely \isi{types} \citep[Sec.~5.2.2]{Ginzburg:2012}.
%
...
%
Accordingly, in a series of works, a theory of \isi{quantified noun phrases} (QNPs) has been developed that refrains from type raising and that analyses QNPs in terms of the intuitively expected and clarificationally required denotations of types \emph{individual} and \emph{sets of individuals}, respectively  \citep{Purver:Ginzburg:2004,Ginzburg:Purver:2012,Ginzburg:2012,Cooper:2013,Luecking:Ginzburg:2018,Cooper:ms}. 
%
...
% %
% This function is characterised by the set of ordered pairs $\{\langle v, \emph{cat}(v) \rangle \mid v : \emph{Ind} \}$ and thereby is linked to a set theoretical notion of extension.
...
\itdopt{For Stefan: Do list properly.}
\z
...
%
It is difficult to imagine how to model this aspect\del{s} of \isi{linguistic knowledge} if not by means of \emph{grammar in dialogue}.
\is{interjections|)}
...
[pre & :	[spkr & : Ind \\
			addr & : Ind \\
			p & : Prop \\
			LatestMove=Assert!(spkr,addr,p)! & : IllocProp \\ qud=<p?,subqud> & : poset!(Question)! ] \\
...
\avm{
[pre & :	[p & : Prop \\
			LatestMove=Accept!(spkr,addr,p)! & : IllocProp \\ qud=<p?,subqud> & : poset!(Question)! ] \\
effects & :	[facts=pre.facts $\cup$ \{p\} & : Set!(Prop)! \\
...
\avm{
[phon & : < \phonfont{yes} > \\
cat & :	[head=\type{partcl} & : syncat] \\
dgb-params & :	[qud=[max & : PolQuestion \\
				rest & : set!(Question)! ] & : poset!(Question)! ] \\
\punk{cont=dgb-params.qud.max( [ ] ): \emph{Prop}}{} ]%todo punk doesnt work with !!
}
\z 
%
...
%
Using the merge operation, the word extension on signs can \add{be} represented compactly as in (\ref{ex:word}a), which expands to the structure given in (\ref{ex:word}b):
%
...
Words -- that is, cxtype\itdopt{small caps?} \emph{word} -- are usually the result of lexical rules, whose input are lexemes.
%
...
A phrasal sign can be seen as a word with daughters:\itdopt{I find this formulation problematic: 1) There are internally complex words that have daughters. 2) It reminds me of classical AI examples with default inheritance: A horse is a table in that it has four legs except that it can walk.}
%
\ea 
...
%
This aspect of headedness is captured in terms of the \changed{\emph{Head-Feature Principle}} (HFP), which can be implemented by means of tag types as follows:
%
\ea \label{ex:HFP}
HFP $\coloneqq$
\avm{
[cxtype & : phrase \\
cat & : [head$_{\2}$ & : PoS] \\
dtrs & : [hd-dtr & : [cat & : [head=\2 & : PoS]] ] ]
}
\z
\itdopt{Isn't PoS redundant? It should follow from the rest of the grammar/the signature. The specification of \textsc{cxtype} in the HFP seems to be redundant since this is in \type{hd-phrase} already.}

The fact that the daughters' locutions combine to the mother's utterance is captured in terms of a \enquote{\changed{Phon Principle}}  (we use a slash notation in order to indicate paths starting at the outermost level of a feature structure):
%
\ea 
PHON $\coloneqq$ \label{ex:phon-principle}
\avm{
[cxtype & : phrase \\
phon & : List!(!/dtrs.hd.dtr/phon, /dtrs.nhd.dtrs/pos1.phon,  \\
	& \ldots, /dtrs.nhd.dtrs/pos$n$.phon!)!
% dtrs & : [hd-dtr & : Sign \\
%           nhd-dtrs & : List!(Sign)!
%         ]
]
...
\z 
\itdopt{(\mex{0}) seems to differ from (\mex{1}). In (\mex{1}) the path is in small caps and it contains \textsc{hd-dtr}, while it is hd.dtr in (\mex{0}). In (\mex{0}), there is a dash before pos1, which seems to be wrong.}
...
%
This is achieved in terms of \rep{a}{the} \changed{\emph{DGB-Params Principle}} (\emph{DGBPP}) in (\ref{ex:dgbpp}) which unifies the daughters' \textsc{dgb-params} into the mother's \textsc{dgb-params}  (see \citealt[126 \textit{et seq.}]{Ginzburg:2012}  for a similar principle): 
%
\ea \label{ex:dgbpp}
...
\avm{
[cxtype & : phrase \\
dgb-params & :	[/dtrs.hd-dtr.dgb-params\ \ttrmerge\
				/dtrs.nhd-dtrs.pos1.dgb-params\ \ttrmerge&\\
				\ldots \ttrmerge\ /dtrs.nhd-dtrs.pos$n$.dgb-params ] \\
dtrs & : [hd-dtr & : [q-params & : RecType] \\
...
A headed phrase is well-formed when it is a headed phrase and it obeys the head feature principle, the \changed{Phon Principle} and the \changed{DGB-Params Principle}, which is expressed by extending \emph{hd-phrase} by the following constraint:
%
