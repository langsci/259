...
%--------------------------------------------------------------
%%% comments for keeping track:
\tabulinesep=2mm
\begin{longtabu} to \textwidth {| X[l,6] | X[l] |}
\hline
\textbf{Comment} & \textbf{Remark} \\
\hline
\endfirsthead
...
\multicolumn{2}{|>{\columncolor{gray!30}}l|}{Commentator 1} \\
\hline

As I understand it, the goal of the chapter is to show that a version of
HPSG can form part of an approach which captures the fact that language
is first and foremost a face-to-face communication system. This strikes
me as important, and it would be good if it could be as accessible as
possible to a broad range of readers, including those with limited
knowledge of HPSG and formal semantics. I know plenty about HPSG and a
bit about formal semantics, but I found it pretty heavy going. For
example, I found the following at the bottom p.443 pretty opaque


Propositions in TTR can be developed in an explicit Austinian (1950)
way, where a proposition is individuated in terms of a situation and
situation type (Ginzburg 2011: 845) -- this is the truth-making (and
Austin's original) interpretation of \enquote{It takes two to make a truth}.

& elaborated briefly on propositions \\
\hline

% No doubt making the discussion more accessible is not a simple matter,
% but I think some more discussion of how the framework accommodates some
% specific phenomena might help.

% & \\
% \hline

Some more specific points:
p.432. The concept of mutual beliefs probably needs explaining.

& added brief explanation \\
\hline


p.435. You say of (8) that \enquote{neither of Bs turns 4 and 7 is redundant
although neither of them contribute new information}. It seems to me
that both communicate that what has just been said has been understood,
and that is new information. Relevance theorists would see these
examples as evidence that what is communicated may be quite distantly
related to the words used. I suppose you have a concept of new
information which is fairly closely related to the speaker's words. Some
clarification seems necessary here.

& made the reference to new info from info struc more explicit\\ 
\hline


pp.435-6. It would be good if the discussion of facts, questions under
discussion, latest moves, grounding, and public and private information
could refer back more to aspects of (8) to make it clear what these
ideas involve in practice. In general, I think this discussion needs
some fleshing out.

& fleshed out following the example of Jonathan's book  \\
\hline

p.441, l.8. It seems odd to say \enquote{to conclude this chapter} roughly half
way through the chapter.

& deleted \\
\hline

pp.441-2. The brief primer to TTR seems a bit too brief to me.

& still bried but revised \\
\hline

p.443. Are what you call descriptive uses as in The thief (however he
is) essentially what Donnellan called attributive uses? If so, this
might be worth a mention.

& yes, added ref \\ 
\hline

p.445. It might be better to say \enquote{affixes} here rather than \enquote{morphemes} given that most HPSG work rejects the traditional view of morphemes in
which they are essentially a kind of sign.

& why not, followed the replacement advice \\
\hline

A reference that you should probably mention somewhere is Lee-Goldman
(2011).
REFERENCE
Lee-Goldman, Russell (2011), Context in Constructions, PhD dissertation,
University of California, Berkeley,
https://escholarship.org/uc/item/02m6b3hx

& added wrt. to the term ``dialogue context'' \\
\hline

\multicolumn{2}{|>{\columncolor{gray!30}}l|}{Commentator 2} \\
\hline

% very good chapter
% with concrete examples, a large vision, and some touch of humor included
% If we aim at a handbook audience, clearly
% needs some more explanations (or footnotes) at some points 

% & \\
% \hline

- P. 427 \enquote{bridging anaphora} explain

& added brief explanation \\
\hline

- P429 \enquote{for a collection of indirect uses of
I, or rather its German cognate Ich, where identification with the
circumstantial speaker role would lead to wrong results, see Kratzer (1978)).}
difficult to understand; maybe drop or add examples in a footnote?

& put in footnote, added example \\
\hline

- chap4 on the the  lexicon does not discuss indexicals nor provide an
entry for I

& deleted chaptercrossref\\
\hline

 P 431 \enquote{the fractal architecture of HPSG}: explain?

& added brief explanation \\
\hline

P432 \enquote{The focus-linktail
approach entails, however, that the packages of information structure
coincide
with syntactic constituents.} are you sure ? in the AVM these features
have set(content) as values, and a content object does not necessary
correspond to a syntactic constituent, does it ?

& fixed \\
\hline

P435 would it be possible to draw some tentative simplified AVM for part
of example (8)
Just examplifying DGB with Facts, QUD, Lastest-move etc?

& see first commentator above \\
\hline

% p440 a theory ..have=> has

% & done \\
% \hline

% P443 the thief however he is => whoever ?

% & done \\
% \hline

section 4 could be developed with some more (dialogic ) examples

& \textcolor{red}{any suggestions? or leave it like it is?}\\
\hline

Please provide lexical entry for Yes, for example, or Hello ! (as first
move or answer) including HPSG attributes CAT, CONT etc

&  done \\
\hline

\multicolumn{2}{|>{\columncolor{gray!30}}l|}{Commentator 3} \\
\hline

In general, I found this enjoyable very interesting and informative to read, and I don't have much to offer in the way of possible improvements, except for a small number of detailed, very minor, comments, and one larger observation.

The larger observation relates to the Introduction and sections 4 and 5. It seems to me that neither of these sections is really a \enquote{Conclusion}, though either might become one. I think this lack of a conclusion is a flaw that could easily be remedied. It seems to me that the Introduction sets up a problem (dealing with dialogue) and the rest of the paper show how this problem can be addressed. A lot of it is (quite rightly) involved in laying foundations. What I miss is discussion that specifically relates the problem and the solution. There
is a little of this at the end of Section 4, where we see how to deal with feed-back expressions, but there are other things mentioned in the introduction that are not explicitly brought out in section 4. I think this would be quite easy to do (I mean it might take less than a page of
text).

& should be addressed by substantially expanded KoS discussion \\
\hline 

% - 425 Ray discovered -> Ray discovers 

% & done \\
% \hline
  
- 428 `a unification-based framework' -- this is potentially confusing, because a) modern HPSG is not really unification based, and b) there is much more nuanced discussion in 3.1; I'm not sure how to fix this (maybe something vague like 'has implications for the way semantics should be done in a constraint based framework' or something)

& extended discussion to constraint-based grammars \\
\hline

- 429 (2) contains a typo around noun[case nom] -- I assume it the type label noun is misplaced (should be inside the bracket); 

& no, this is correct, added a remark on notational convention\\
\hline 

- 430 (3) the PHON value is a bit inconsistent across the paper. In (2) it seems to be orthography inside slashes, but then here it should be /I/ (or a better phonological representation, I don't care, but it should be consistent).; I think there is another typo in the Index value (REF should be PER, I assume)
 
& used angle brackets, as suggested by Stefan \\
\hline 

- 431 Not surprisingly, some of the reference/citations in para 2 are missing from the biblio, so this will need checking in the final version.

& ?? (probably the chaptercrossrefs) \\
\hline

- 441 ``Chagall is an individual (mc: Ind)''  -- so the notation we have for a judgement is a:T, for an object a and type T. Good. It would be helpful to have an example of a judgement corresponding to `cat(x)' as in appears in (12). Also, a footnote pointing out that while (12) looks like an AVM, it isn't -- the things that look like attributes are actually objects.

& sh'd be done with revision of TTR sec. \\
\hline

- 442 I got lost at the end here. I assume `c1' should be small caps, as should the `path' X. It maybe just needs a little more explanation. (For me, it raised more questions than answers -- e.g. I began to wonder how the different instances of the individual variable 'x' in (14) [I mean: cat(x) and on(x,y)] were tied up, I presume it is via the path <X>, but ... I would not be surprised if the answer is to be found in the appendix, but I did not find it. Perhaps this is closely related to my previous point. Or perhaps the point here is just that there is a big jump in the level of discussion here, from the highly intuitive and
informal to a much more formal level, and I was/am not sure why this jump is necessary here.
...
%
This position is strengthened by the primacy of \isi{spoken language} in both ontogenetic and language \isi{acquisition} areas (on acquisition see \crossrefchaptert{acquisition}).
\is{language system|)}
...
\begin{tikzpicture}[
   StickyNote/.style={
      ...
      % \node [scale=5, gray!60, inner sep=0pt] (truck) {\faTruck};
...
\end{tikzpicture}
\\
Here it is the truck, not the speaker respective author of the note, that is for rent. Such examples of the German cognate of \enquote{I}, namely \textit{Ich}, are collected and discussed in \citet{Kratzer:1978}).}
%
...
Inasmuch as the  \isi{contextual anchors} (see \citet[\page 72--73]{Barwise:Perry:1983} or \citet[\page 52--63]{Devlin:1991} on anchors in \isi{Situation Semantics}) indicated by a boxed notation from (\ref{ex:I}) provide a semantic value for the speaker in a directly referential \is{direct reference} manner (see \citet{Marcus:1961} and \citet{Kripke:1980} on the notion of direct reference with regard to \is{proper name} proper names), they also provide semantic values for the \isi{addressee} (figuring in the content of \textit{you}), the time (\textit{now}) and the place (\textit{here}) of speaking.\footnote{Of these, in fact, only the speaker is straightforwardly given by the context; all others can potentially involve complex inference.}
%
...
Focus, expressed by sentence accent in English, can be used for information packaging that may lead to truth-conditional differences \is{truth-conditional difference} even in case the surface structures (i.e. strings; see Section~\ref{sec:introduction} on a brief juxtaposition of spoken and written language) are the same \citep{Halliday:1967}.
%
An example is given in (\ref{ex:show-pictures}), taken from \citet[\page 246]{Krifka:2008}, where capitalisation indicates main accent and subscript \enquote{F} labels the focused constituent (see also \crossrefchaptert{processing} on incremental processing also with respect to aspects of information structure):
%
...
%
The \emph{ground} is further bifurcated into \textsc{link} \isfeat{link} \is{link} and \textsc{tail}, \isfeat{tail} \is{tail} which connect to the preceding discourse in different ways (basically, the link corresponds to a discourse referent or file, the tail corresponds to a predication which is already subsumed by the interlocutors' information states).
%
...
%
The \textsc{focus-link-tail} approach works via structure sharing: the values of \textsc{focus}, \textsc{link} and \textsc{tail}, gets instantiated by whatever means (syntactic, phonologic, \ldots) the language under consideration uses in order to tie up information packages.
\is{given information|)} \is{new information|)}
%
In case \isi{prosodic information} is utilised for signalling information structure, a grammar has to account for the fact that prosodic constituency is not isomorphic to syntactic constituency, that is, prosodic structures cannot be build up in parallel to syntactic trees. 
%
...
%
Starting from Klein's work, an architecture is developed that generalises over prosody-syntax mismatches: on this account, syntax, phonology and information structure are parallel features of a common list of \isi{domain objects} (usually the inflected word forms; see also \crossrefchaptert{phonology}).
%
...
%
Here the tuple represents a temporarily ordered sequence of objects of a given type (i.e., \textsc{dgb} and \textsc{ms} in case of dialogue agents' information state models) \is{information state}\is{information state model} which is witnessed by a \emph{string} \is{string} of respective events which at least of length 1, as required by the \enquote{Kleene +} \is{+}  (see \citet[Sec.~2.7]{Cooper:Ginzburg:2015} on a type-theoretical variant of the \isi{string theory of events} of \citet{Fernando:2011}).
\is{dialogue gameboard|)}
...
%
We want to point this out since the representations used subsequently look deceptively similar to attribute-value matrices (the risk of confusion is know from the essentially identical representations employed within unification- and constraint-based HPSG variants).
%
...
%
In particular with regard to the values of the \textsc{content} and \textsc{context} attribute, the crucial question is \enquote{[\ldots] how types in the [feature] logic should correspond to the semantic types being represented.}  \citep[\page 70]{Penn:2000}.
%
...
%
There is a justification for this restatement from the actual semantic practice in HPSG (cf. \crossrefchaptert{semantics}).
...
%
His above-given quotation continues: \enquote{[\ldots] but at that point feature structures are not being used as a formal device to represent knowledge but as a formal device to represent data structures that encode formal devices to represent knowledge.} (\citet[\page 63]{Penn:2000}; see also the discussion given by \citet[Sec.~5.2.2]{Ginzburg:2012}.)
...
%
Accordingly, in a series of works a theory of \isi{quantified noun phrases} (QNPs) has been developed that refrains from type raising and analyses QNPs in terms of the intuitively expected and clarificationally required denotations of types \ttrtype{individual} and \ttrtype{sets of individuals}, respectively  \citep{Purver:Ginzburg:2004,Ginzburg:Purver:2012,Ginzburg:2012,Cooper:2013,Luecking:Ginzburg:2018,Cooper:ms}. 
%
...
%
Types are \isi{intensional entities} so they directly provide \isi{belief objects} as touched upon in Section~\ref{sec:mutual-beliefs} and needed for intensional readings as figuring in \isi{attitude reports} such as in the assertion that \textit{Flat Earthers believe that the earth is flat} (see also \citet{Cooper:2005:b} and \citet{Cooper:ms} on attitude reports in TTR).
...
\subsection{A brief primer to TTR}
\label{sec:brief-primer-ttr}

TTR (a Type Theory with Records which has its philosophical roots in the intuitionistic Type Theory of \citet{Martin-Loef:1984} and its mathematical foundation in proof-theoretical logic and constructive type theory \citep[see][]{Ranta:2015}) provides semantic objects at both the \isi{token} and the \isi{type} level and structures to organise these objects, namely \isi{records} and \isi{record types} % , and (Montagovian) $\lambda$-abstraction and functional application 
...
%
A \isi{judgement} of the form $a : T$ means that object $a$ is of type $T$, or, put it differently, that $a$ is a witness for $T$.
%
...
%
Obviously, the \isi{situation} depicted in Figure~\ref{fig:cat}  (adapted from \citet[\page 270]{Luecking:2018:a}) is a witness for the type in (\ref{ex:fritz-rt}).
%
...
%
The scene form the figure then corresponds to the following record, which is of the type expressed by the record type from (\ref{ex:fritz-rt}):
%
...
\ea If $T_1$ and $T_1$ are types, then $(T_1 \rightarrow T_2)$ is a type, namely the type of functions that map $T_1$ to $T_2$.
\ex If a function $f = (T_1).T_2$ is of type $(T_1 \rightarrow T_2)$ then $f$'s domain is $\{a \mid a : T_1\}$ and its range is included in $\{a \mid a : T_2\}$.
\z
\z 
%
...
\ex $b : T_a$ (i.e., object $b$ is of type $T_a$) just in case $b = a$.
\z
\z
%
...
%
For instance, (\ref{ex:hpsgttr-sign}) shows the record type (the judgement colon indicates that we now talk about TTR objects, ) for out attribute value matricesa genot abneral \isi{sign} according to \citet{Pollard:Sag:1994} (where \ttrtype{PhonType}, \ttrtype{CategoryType} and \ttrtype{SemType} denote obvious types -- see the Appendix for a minimal HPSG fragment defined in terms of TTR).
%
...
%
This involves that propositions are modelled in terms of situations, not in terms of sets of possible worlds.
%
...
%
A conversational rule is a mapping between an input and an output information state, where the input DGB is constrained by a type labeled \emph{preconditions} (pre) \is{preconditions, conversational rule} and the output DGB is subject to \emph{effects}.\is{effects, conversational rule}
%
...
\begin{avm}
\[
pre &:& \[spkr &:& Ind \\ addr &:& Ind \\ p &:& Prop \\ LatestMove=Assert(spkr,addr,p) &:& IllocProp \\ qud=\<p?,pre.qud\> &:& poset(Question)\] \\
...
\begin{avm}
\[
pre &:& \[q &:& Question \\ p &:& Prop \\ LatestMove=Accept(spkr,addr,p) &:& IllocProp \\ qud=\<q,pre.qud\> &:& poset(Question) \\ qbg &:& Qspecific(p,q) \] \\
...
\begin{avm}
\[
pre &:& \[p &:& Prop \\ LatestMove=Accept(spkr,addr,p) &:& IllocProp \\ qud=\<p?,pre.qud\> &:& poset(Question) \] \\
...
The discourse particle \enquote{yes} can be used to answer a polar yes/no-question. \isi{polar question}
%
In this use, \enquote{yes} has a propositional content $p$ that asserts the propositional content of the polar question $p$?, which has to be the maximal element in \textsc{qud} \citep[Chap.~2, \page 231\,f.]{Ginzburg:2012}.
%
That is, \enquote{yes} affirmatively resolves a given polar question.
%
Polar questions, in turn, are 0-ary propositional abstracts \citep[\page 231]{Ginzburg:2012}, that is, the polar question $p$? corresponding to a proposition $p$ is a function mapping an empty record to $p$: $([]).p$.
%
Thus, applying $p$? to an empty record $[]$ returns $p$, which is exactly what \enquote{yes} does.
%
The affirmative particle (used to answer a yea/no question) is a propositional lexeme which applies a polar question which is maximal in \textsc{qud} to an empty record \citep[cf.][\page 232]{Ginzburg:2012}:
%
\ea
\begin{avm}
\[
...
%
Since \isi{non-verbal communication} means are informative, not only a (dialogue) semantic representation has to be developed, but also the rules of their interaction with speech have to be formulated. 
...
%
Rather, processing begins with the initial sound and proceeds word for word or even on smaller units like affixes and phonemes -- that is, processing is incremental \is{incremental} \is{incremental processing} (e.g. \citet{Sedivy:Tanenhaus:Chambers:Carlson:1999}; see also \crossrefchaptert{processing}). This is a key ingredient in the efficient (relatively gap-free and interruption-less) managing of turn taking.
%
