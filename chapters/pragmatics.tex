...
%
For instance, it is part of the background information of the \isi{pronoun} \textit{she} of the \enquote{natural gender language} English that its referent is female (this does not hold for \enquote{grammatical gender languages} like \ili{French} or \ili{German}).
%
...
\end{tikzpicture}
\end{center}
Here it is the truck, not the speaker, or rather the author of the note, that is for rent. Such examples of the \ili{German} cognate of \enquote{I}, namely \textit{Ich}, are collected and discussed in \citet{Kratzer:1978}.}
%
...
\avmtmp{[cxtype & : word]} : \emph{RecType}
\ex 
\avmtmp{
[cxtype & : word \\
phon & : Phoneme \\
...
Words -- that is, cxtype \emph{word} -- are usually the result of lexical rules, whose input are lexemes.
%
Lexemes differ from words in their constituent type:
%
\ea
\emph{lexeme} $\coloneqq$ \emph{sign} \ttrmerge\
\avmtmp{[cxtype & : lexeme]}
: \emph{RecType}
\z
