...
\abstract{The distinction between raising and control predicates has been a hallmark of syntactic
  ...
  \citep{PollardandSag1994}. The distinction between ``raising'' and ``control'' lexeme types is also
  relevant for non"=verbal predicates such as adjectives (\word{likely} vs.\ \word{eager}). We will
  provide creole data (from Mauritian) which support a phrasal analysis of the complement of both
  control and raising verbs, and argue against a clausal (or small clause) analysis
  \citep{HenriandLaurens2011}.  We will provide some data from ergative languages to discuss the
  distinction between subject and first syntactic argument, and also show how the HPSG analysis can
...
\itdobl{Stefan: Abstracts should be standalone pieces of text that can go to databases without the
  references. So references should not be included here.\\
Stefan: Please change all occurances of academic ``we'' into ``I'' or passive. I would remove the
quotes around raising and control. I know that you hate Minimalist terminology, but in HPSG we also
raise things. We just raise on ARG-ST or COMPS. I think it is ok to use raise and control without quotes.}
...
reference at all,\itdobl{But if the controlled element does not have a reference, it is an expletive
  and then we would have raising, wouldn't we? I think, what you have here is Bresnan's definition,
  but HPSG is not using this.}
...
expletive \emph{it} and \emph{there} and idiom-chunks\itd{Stefan: Isn't the idiom-chunk the whole
  idiom? Then I would write here ``parts of idiom chunks'' Anne: for me idiom chunk means part of an
  idiomatic expression.\\
Stefan: How about: ``non-referential parts of idioms''.}.

% \itd{Anne:should be indented}
...
non"=finite verb.
\itdobl{Stefan: HPSG assumes that the subject and other raised arguments are also selected by the
  raising verb. We need this because of passive. Maybe better: ``The kind and number of the relevant
  arguments selected by the raising verb depends on the embedded verb.''}
...
\itdobl{Stefan: The difference between she/he is confusing. The examples are not minimal pairs. I
  think having ``he'' in (\mex{0}b) would make the point as well.}
...
the embedded verb, but not the raising verbs.\footnote{%
  ...
  a non"=verbal complement (\emph{They wanted a raise.}), while raising verbs may not (\emph{*She
    ...
    started.})\itdobl{Stefan: Again two versions of phase verbs. Maybe use ``It just started (to rain).''} as
...
(\citealt[Chapter~3]{PollardandSag1992}\itdobl{Stefan: Chapter 3 is inappropriate for articles. Do
  you mean Section~3? And do you mean the control paper? \citew{SP91a-u}?} and also \citealt{JackendoffandCulicover2003}).  Verbs of
...
\footnote{Some verbs may be ambiguous and allow for subject control (\emph{John proposed to come
  ...
  \citew[Section~3]{CP2020a-u}\itdobl{Stefan: This is a book. Section~3 is inappropriate.} and
...
\itd{Stefan: Are there sources for these analyses? Anne: would be better with John barred instead of
  ei in S.\\
Stefan: Like this? Sources are still missing. References. The e$_i$ notation is used later in the
chapter. Maybe keep things uniform.}
\eal
\ex subject-raising:\\
...
Anne: d in virdast should be fixed\\
Stefan: I do not know what you mean. I still need sources for these examples.}
...
\zl
\itd{Stefan: Rizzi 1990 is missing. Could you please state what exactly you are referring to?}
...
complement. We\itdobl{no we} continue to use the term \emph{raising}, but it is just a cover term, since no raising
is taking place in HPSG analyses.\itdobl{Stefan: Well, there is raising: we raise arguments from an
  embedded \comps/\argst to a higher one. Some authors even have a RAISED$+$/$-$ feature.} 
...
We\itdobl{No academic ``we''.} will first present in more details the HPSG analysis of raising and
...
and \type{subject-raising"=verb"=lexeme}, respectively.\itdobl{Stefan: Add \type{sr-a-lx}.}
\added{The figure also shows three instances inheriting from \type{sr-a-lx}, \type{sr-v-lx}, and \type{or-v-lx},respectively.}  The constraints on the types \type{subj-rsg-lx} and
...
\crossrefchapterw{order} on constituent order \added{and \crossrefchapterw{complex-predicates} on complex predicates}). Furthermore, \changed{subtypes of these lexical types}
% \itd{Stefan: the
...
\noindent
The subject of the non"=finite complement shares its \type{synsem}\itdobl{Stefan: There is nothing
  but this synsem. I would prefer ``The \subj element of the \ldots is shared with the \ldots''}
...
features\itdopt{Stefan: ``that their syntactic and semantic features are shared''}: they have the
same semantic index (if any)\itdobl{Stefan: Remove ``if any'', expletives have a semantic index as
  well.}, but also the same part of speech, the same case etc. Thus a subject appropriate for the
...
Anne: the pb is that I want to show valence features and they are not available for lexemes, only
for words.\\
Stefan: Ah, I see. Then maybe do not give a type at all and say: ``Lexical item for the word \emph{seem}''}
\eas
Lexical item for the word \emph{seem}:\\
\avm{
[%\type*{sr-v-word}
subj  & <\1 > \\
...
Raising Principle should be reformulated in terms of argument-structure and not valence
features.\itdobl{Stefan: I guess something is missing here. The reader does not know why \argst
  helps here.}
...
but not all (see \crossrefchapteralt{arg-st} on argument structure). \added{(\mex{1}) shows the
  respective \argst values:}
...
given in Figure~\ref{verb-hier3}, adapted from \citet{SagandPollard1991}\addpages.\footnote{For further semantic classification of main predicates in order to account for optional control in languages such as Modern \ili{Greek} and Modern Standard \ili{Arabic}, see \citew{GreshlerMelnikWintner2017a-u}.}
...
\itd{Stefan: Stating \type{relation} in the lexical items seems redundant since this is the most
  general type for something having \textsc{arg} as a feature and being the value of \textsc{soa}.\\
...
\itdobl{RELS should be a list since it is a list everywhere else in the volume. The types of the
  lexical items should be removed.}
\eas
\type{want}:\\
\avm{
[\type*{sc-v-word}
...
30\,\% verbs showing a syncretic form (\added{as for example} \emph{bwar} `drink'). The following list of examples provides pairs of short and
...
short form is required before a phrasal complement\itdopt{Stefan: What does ``phrasal complement''
  mean? sega and pom are words.} and the long form appears otherwise.\footnote{\textit{yer} `yesterday' is an adjunct. See \citew{Hassamal2017} for an analysis of \ili{Mauritian} adverbs which treats as complements those triggering the verb short form.}
...
\type{sr-v-lx} and \type{sc-v-lx}\itdopt{Stefan: The type sc-v-lx has never been mentioned
  before. One can get it by analogy but may be it would be better to explain this explicitely.} respectively.\footnote{\citeauthor{HenriandLaurens2011} use Sign-based Construction Grammar (SBCG) (see
...
following lexical items are licensed.''}
\eal
\ex 
\type{av-verb} \impl\\
\avm{
...
This is adopted by \citet{HenriandLaurens2011}\addpages for \ili{Mauritian}.  After some
...
\gll Zan$_{i}$ pans              pou           (li$_{i}$)            vini.\footnotemark\\
     Zan       think.\textsc{sf} \textsc{fut} \hphantom{(}3\SG{} come.\textsc{lf}  \\\hfill(\ili{Mauritian})
\footnotetext{
...
subject;\itdobl{Stefan: And be a pronoun? ``Peter looks like Peter is tired.''?} it is a raising predicate, as shown by the possibility of the expletive \emph{there}:
...
Assuming a tripartiion of index with \type{referential}, \type{there} and \type{it}, \citep[\page
...
[ arg-st <NP$_i$, S[marking & like\\
                    xarg & pro$_i$] > ]}
\itdopt{Stefan: Does \type{pro} follow from Binding Theory? Do we have to state this explicitely here?}
\z
This\itdopt{Stefan: What?} is also the case in \ili{English} tag questions, since the  subject of the tag question must be
...
\zl
\itdobl{Stefan: But this does not work for all languages. It sounds as if this was asssumed to be
...
To account for \rep{these}{the auxiliary} properties, Transformational Grammar since \citet{Stowell1983}\addpages and
...
\citet[Chapter~3]{PollardandSag1994} present several arguments against a (small) clause
analysis. The putative sentential source is sometimes attested (\ref{cons1}) but more often
ungrammatical:
\itdopt{Stefan: Example from ps94? Page numbers?}
...
as follows, with L meaning any list\itdopt{Stefan: Please use \type{list} to be consistent with the
  rest of the volume.} of arguments before the predicative complement, and the two subtypes of verbs taking a predicative complement inherit from subject-raising verbs and object-raising-verbs respectively:
...
\ex \type{pred-lx} \impl  \avm{ [arg-st & L \+ \upshape \prd $+$ ]}
...
predicative \word{consider} inherit from the \type{s-pred-v} type and \type{o-pred-v}\itdobl{Stefan:
  -lx missing in type names?}
type respectively, and are as follows: \inlinetodostefan{Stefan: fix these lexical items. Raise everything? Use append for consider. Should this be \argst? it is easier to show a word that is
  later used in the figure ; Anne: I use append in the types, but for \ili{English} verbs, we can assume that there are no  subjectless predicates.\\
  Stefan: But this assumption is not in the theory. Hence you have to use append to reflect what you  stated in the types, don't you?\\
  Anne: arg-st added: the types are more general and the words are more specified.\\
  Stefan: Yes, but then you have to explain it. Say something about additional constraints and where they come from.}
...
\itdobl{Stefan: I would add the following line here: ``With the assumption that the \subj list of
  ...
  and \emph{consider} are licensed:'' And the types should be removed.}
\eas
\type{seem}:\\
\avm{
[\type*{s-pred-v-word}
...
complement; the same holds for the first complement of \word{consider}:\itd{Stefan: Say something
  about the fact that it has to be exactly one element. Where does this information come from?}
...
As shown by \citet{HenriandLaurens2011}, and as noted in Section~\ref{sec-maurit}, \ili{Mauritian} data
argue\itd{Stefan: Data argue? AA: yes\\
Stefan: Data cannot argue. We can use data to argue. I asked Elizabeth and she said it is indeed strange.} in favor
of a non"=clausal analysis.\itd{Stefan: Where exactly did you show this? What you suggested in
  (\ref{ex-pans-Maritian}) was an embedded clause.Anne: no, see also ex 45.\\
Stefan: (59) allows for both complete sentences and VPs. (45) says that there has to be an argument.} A copular verb takes a short form before a
...
be realized as a strong pronoun\itdobl{Stefan: But d has ki and strong pronoun.} (\word{mwa} `me', \word{twa} `you'). See Section~\ref{sec-maurit}
...
(\type{Aux} or \type{Infl}){Having Infl as a syntactic category and sentences defined as IP does not account for languages without inflection, nor for verbless sentences, see for example Laurens 2008.} but verbs with the head property in
(\ref{ex-head-value-of-aux-elements}):

\begin{exe}
\ex \label{ex-head-value-of-aux-elements}
  \type{aux-verb} \impl \type{sr-v-lx} \& 
\avm{[head &	[aux & $+$] ]}
 \end{exe}
\itdobl{Stefan: I said this already in some discussion, but the subtype relation is part of the type
  hierarchy (the signature, see formal background). So \type{sr-v-lx} should not be stated here. You
  can mention the inheritance relation in prose.}
...
needed for verb initial languages like \ili{Welsh} \parencites[\iaddpages]{Borsley99c-u}[\iaddpages]{SWB2003a}.\footnote{As
...
\itd{Anne:I'm simplifying with NEG +, if you could replace by POL 1 in the modified clause and POL
  -1 in the tag-aux, it would be better.\\
Stefan: Oh, have there been solutions to this? I made one up. You have to explain that ``not'' is a
relational constraint returning the oposite of + or $-$.}
\ea
