...
expletive \emph{it} and \emph{there} and idiom-chunks\itd{Stefan: Isn't the idiom-chunk the whole
  idiom? Then I would write here ``parts of idiom chunks''}.
    ...
    \itd{Anne:should be indented}Let us first consider non"=referential subjects: meteorological \word{it} is selected by predicates
...
\zl
 ...
 \itd{Anne:should not be indented}However, the putative correspondence between source and target for raising structures is not
...
\zl
 ...
 \itd{Anne:should not be indented}Similarly, while some object-raising verbs (\word{expect, see}) may take a sentential complement (\ref{expect3}), others do not (\word{let}, \word{make}, \word{prevent}).
...
information (\type{synsem}) \crossrefchapterp{properties}\itd{Stefan: What for are you citing this
  chapter? Give the reader a clue. And me for linking to the page. Anne: section 4.1 where "raising-v" is introduced} with the unexpressed subject, while
...
\itd{Anne: should not be indented}As in \crossrefchapterw[Section~\ref{properties:lexemes-and-words}]{properties}, upper case letters
...
\crossrefchapterw{order} on constituent order). Furthermore, the same lexical types\itd{Stefan: the
  ``same''? You define new types in (\ref{ex-subj-pred-v} Anne: you are right copular verbs are subtypes of raising verbs). Maybe better write ``similar
...
\itd{Anne: should not be indented}The subject of the non"=finite complement shares its \type{synsem} with the subject of
...
(\ref{german1}), this information is shared too (\ibox{1} can be the empty list). The L in \label{rsg} stands for a possibly empty list, 
and accounts for a possible PP complement as in \textit{Kim seems to Sandy to be smart.}, which we ignore in what follows.
...
shown in Figures~\ref{cons-1} and~\ref{cons2}. Notice that the syntactic structures are the same as for control verbs (Figure 5 and 6). 
...
\ili{Mauritian}, which is a \ili{French}-based creole, provide some evidence for a phrasal (and not sentential) analysis of the verbal complement of raising and control verbs.
...
languages.  Since \citet*{BMS2001a}, it is widely assumed
%\itd{Stefan: Not everybody assumes this. I do not.  See also \citew{LH2006a}.Anne: Bob suggest widely assumed} 
...
promiser should therefore be the actor of the lower verb. This semantic constraint interacts
...
\itd{Anne:no indent}Using \xarg, \citet[\page 214]{HenriandLaurens2011} propose a description for \word{pans} `think' that is
...
\itd{Anne: no new paragraph}See also \citew[\page 408--409]{Sag2007a} and \citew{KaySag2009} for the obligatory control of possessive determiners in \ili{English} expressions such as \emph{keep one's cool}, \emph{lose one's temper}, with an \xarg feature on nouns and NPs:
\begin{exe}
\ex \begin{xlist}
...
[ \argst < \1, S[xarg & \1] > ]}
\z
\itd{Anne: should like be a value of Marking?}
...
\ex \type{subj-cont-lx} \impl \avm{ [\argst < NP$i$, \ldots, [x-arg & < [ind & $i$] > ] > ] }
\zl
...
%We ignore here a possible PP complement (John seems smart to me).
\eas
\type{seem}:\\
\avm{
[\type*{s-pred-v-word}
...
\itd{Stefan: Ellipsis chapter (p.\,\pageref{sec-analyses-of-noncon}) points here and does not have
  any analysis on pseudogapping. Anne: should be section 2.2 predicate ellipsis and argument ellipsis}
