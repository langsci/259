...
%Anne: I'm not saying phase verbs come in two versions, I just analyse them here as raising verbs; i changed the ex to it...rain}
...
Let us now look at non-nominal subjects: \emph{be obvious} allows for a sentential subject (\ref{a}) and
...
%} \label{under}
%\zl
%\itdopt{Stefan: Maybe better to separate examples: (7) = That ... is obvious/seems obvious. (8) =  Under the bed.}
...
\noindent
The \subj element of the non"=finite verb is shared
%shares its \type{synsem}
%\itdobl{Stefan: There is nothing but this synsem. I would prefer `` \ldots  is shared with the \ldots'' Anne: I prefer it my way, the subject is a sign with other features.\\Stefan: There is no sign for the subject of the non-finite complement.}
...
%Stefan: I would remove the \type{v-} of \type{v-word} since it is redundant (head value \type{verb}follows from the presence of \vform and it may be wrong since there is \type{v-lx} but the words are derived by lexical rules. Anne: redundancy does not hurt, might help the naive reader}
...
it has been proposed, e.g. in \citep[\page 65]{ManningandSag1998} that null subject verbs have
a first argument with non-canonical\type{synsem} features typed \type{pro}, representing the unexpressed subject in the \argst list, but nothing in
their \subj list.
...
\ili{Balinese}\il{Balinese|(}, an ergativity language, provides another example of non-canonical subjects.  \citep{WechslerandArka1998} argue that the subject is not necessarily the first syntactic argument in this language. A transitive verb has two verb forms, called ``voice'', and there is  rigid SVO order,
...
\eal
\ex 
Lexical description for \emph{ng-adol} `sell.AV':\\
\avm{
	[subj & <NP$_i$> \\
	comps & <NP$_j$>\\
	arg-st & <NP$_i$, NP$_j$> ] 
...
\ex 
Lexical description for \emph{adol} `sell.OV':\\
\avm{
	[subj & <NP$_j$> \\
	comps & <NP$_i$>\\
	arg-st & <NP$_i$, NP$_j$> ]
...
\gll Zan$_{i}$ pans          (*ki)           (li$_{i}$)            vini.\footnotemark\\
     Zan       think.\textsc{sf} (*that) \hphantom{(}3\SG{} come.\textsc{lf}  \\\hfill(\ili{Mauritian})
\footnotetext{
...
Lexical description for \type{pans} `think':\\
\avm{
	[subj & <NP$_i$ > \\
	comps &	<[head [\type*{verb}\\
			xarg &	[ind & $i$] ]	]>
...
Assuming a tripartition of \type{index} with \type{referential}, \type{there} and \type{it} \citep[\page
...
\ea
Lexical description for \emph{look (like)}:\\
\avm{
[ arg-st <NP$_i$, S[marking & like\\
                    xarg & [cont & [\type{pron}\\
 ind & $_i$]] ] > ]}
\itdopt{Stefan: Does \type{pro} follow from Binding Theory? Do we have to state this explicitely here? Anne: here it is a pronominal content, like in tag questions; not a pro synsem as with prodrop or VP ellipsis in the last section}
\z
...
\noindent
To account for the above cases, the types for subject-raising and subject-control verb lexemes in (\ref{rais-1}) and (\ref{subj-cont-lx}) can thus be revised  as follows, the only difference being that the \textsc{index} of the subject of control verbs must be a referential NP:\footnote{Some authors have argued that some verbs have either a control-like or a raising-like behaviour depending ont the agentivity of their subject, see Perlmutter for English aspectual verbs (begin, stop) and Ruwet 1991 (page 56) for French.}
\eal
\ex \type{sr-v-lx}  \impl \avm{ [\argst < XP$_i$, \ldots, [x-arg & [ind & $i$] ] > ] } 
\ex \type{sc-v-lx} \impl \avm{ [\argst < NP$_i$, \ldots, [x-arg & [ind & $i$\[referential\]]  ] > ] }
\zl
\itdobl{Stefan please add :David Perlmutter 1970  The two verbs 'begin.' In Jacobs & Rosenbaum, 1970:107–119; Nicolas Ruwet Syntax and human experience, 1991, University of Chicago Press}
%\itodo{Stefan: But this does not work for all languages. It sounds as if this was asssumed to be the final version now. Anne: are you thinking of German subjectless verbs again? it says 'to account for the cases above'}
...
\citep[page134]{HenriandAbeille2007}, in \ili{Slavic} languages \citep{Zec87a-u}\addpages, and in \ili{Semitic} languages.
...
Following \citet[pages420-423]{Bresnan1982},
...
\eas
Lexical description for \emph{seem} (\type{s-pred-v-lx}):\\
\avm{
[subj  & < \1 > \\
comps & < \2 [head & [prd & $+$] \\
	      subj & < \1 > \\
...
\eas
Lexical description for \emph{consider} (\type{o-pred-v-lx}):\\
\avm{
[subj & < \1 NP$_i$ > \\
comps & < \2, \3 [head &	[prd & $+$] \\
	          subj & < \2 > \\
...
\eas
\label{happy2}%
Lexical description for \type{happy}:\\
\avm{
[\phon <happy> \\
head &	[\type*{adj}
	 	prd & $+$] \\
...
(\word{mwa} `me', \word{twa} `you'). So \word{malad} ('sick') is an adjectival complement in \ref{1}, \ref{2} and \ref{4} and not a small clause, \emph{trouv}('find') takes two complements in \ref{2} and \ref{4}, and \emph{trouve}('find') one clausal complement in \ref{3}. See Section~\ref{sec-maurit}
...
modals (e.g., \word{can}, \word{should}) in HPSG are not considered to have a special part of speech
(\type{Aux} or \type{Infl})\footnote{Having Infl as a syntactic category and sentences defined as IP does not account for languages without inflection, nor for verbless sentences, see for example \citew{Laurens2008}.} but are verbs with the head feature [aux $+$].
...
\z
\rel{not} is a function that returns `$+$' for the input `$-$' and `$-$' for the input `$+$'. I use \type{tense} to ensure time concordance between the main verb and the tag auxiliary. \type{pron} denotes a subject with a pronominal content.
...
A revised version of \emph{will} with complement inheritance could be the following, using the non-canonical \type{synsem} type \type{pro} for the unexpressed VP:
\ea
