...
For example, \emph{want}, \emph{promise} and \emph{persuade} have a semantic content such as the
...
% be a referential NP:
\changed{the only difference between subject raising and subject control being that the \textsc{index} of the subject of control verbs must be a
referential NP, which follows from the fact that control verbs assign a semantic role to their
subject and the subject is coindexed with the subject of the controlled verb:}\footnote{Some authors have argued that some verbs have either a control-like or a raising-like behaviour depending ont the agentivity of their subject, see \citew{Perlmutter70} for \ili{English} aspectual verbs (\emph{begin}, \emph{stop}) and \citew[56]{Ruwet1991a-u} for \ili{French}.}
\eal
...
\zl
\added{Note that this approach works only for those languages requiring a subject to be raised. So,
  while (\ref{rais-1}) allows for the embedding of subjectless predicates like (\mex{1}) quoted from
  \citew[\page 48]{Mueller2002b}, (\mex{0}a) requires an XP$_i$.
\ea
\gll Dort schien noch gearbeitet zu werden.\\
