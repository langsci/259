...
\abstract{The distinction between raising and control predicates has been a hallmark of syntactic
  theory since the 60's.
  %\citew{Rosenbaum67a-u} and \citew{Postal1974}. 
  Unlike transformational analyses, HPSG
  ...
  %\citep{PollardandSag1994}. 
  The distinction between raising and control lexeme types is also
  relevant for non"=verbal predicates such as adjectives (\word{likely} vs.\ \word{eager}). Creole data (from Mauritian) will be provided which support a phrasal analysis of the complement of both
  control and raising verbs, and argue against a clausal (or small clause) analysis.
  %\citep{HenriandLaurens2011}. 
  The
  distinction between subject and first syntactic argument will be discussed together with data from ergative languages, and the HPSG analysis will
  be extended to cover cases of obligatory control of the expressed subject of some finite clausal
  complements in certain languages. The raising analysis naturally extends to copular constructions
  (\word{become}, \word{consider}) and most auxiliary verbs.} %\citep{PollardandSag1994,Sagetal2020}
...
reference at all,
\itdobl{But if the controlled element does not have a reference, it is an expletive and then we
  would have raising, wouldn't we? I think, what you have here is Bresnan's definition, but HPSG is
  not using this.\\
Anne: I added this on Bob's request, that is control in a broad sense, before going into the distinction between 'control' and 'raising'}
...
\parencites[\page 75--76]{Chomsky1981}[\page 379]{Bresnan1982}\footnote{\citet{Bresnan1982} proposes
  ...
  (obligatory) ``anaphoric control''. See also \crossrefchapterw[Section~\ref{lfg:sec-raising-control}]{lfg}.}. 
...
Anne: I'm not saying phase verbs come in two versions, I just analyse them here as raising verbs}
...
\zl
\itdopt{Stefan: Maybe better to separate examples: (7) = That ... is obvious/seems obvious. (8) =
  Under the bed.}
...
\eal
\ex[]{
Bill promised Sandy [to come] / [that he will come]. \label{promise1}
}
\ex[]{
Bill expected Sandy [to come] / *[that she will come].
}
\zl
...
the embedded verb, but not the raising verbs.\footnote{%
 ...
 \citew[\page 444]{Jacobson1990} is that control verbs may allow for a null complement (\emph{She tried.}), or
  a non"=verbal complement (\emph{They wanted a raise.}), while raising verbs may not (\emph{*She
    ...
    started (to rain).})
\itdobl{Stefan: Again two versions of phase verbs. Maybe use ``It just started (to rain).''\\Anne: I'm not saying verbs of phase have two entries}
    as
...
(\citealt[Chapter~3]{PollardandSag1994}
%\itdobl{Stefan: Chapter 3 is inappropriate for articles. Do
 % you mean Section~3? And do you mean the control paper? \citew{SP91a-u}?}
 and also \citealt{JackendoffandCulicover2003}).  Verbs of
...
\footnote{Some verbs may be ambiguous and allow for subject control (\emph{John proposed to come
  ...
  \citew[Chapter~3]{CP2020a-u}
  %\itdobl{Stefan: This is a book. Section~3 is inappropriate.} 
  and
...
(with a verbal complement). However, there are counterexamples like (\ref{promise-pass2}) from \citet{JB1976a-u}\addpages 
\\Anne: Stefan you added this ref, I dont have the paper (is it a good paper ? see answer by jackendoff 1987)\\
and the generalization does not seem to
...
%Stefan: Like this? Sources are still missing. References. The e$_i$ notation is used later in the chapter. Maybe keep things uniform. Anne: this is fine}
\eal
\ex subject-raising:\\
{}[\sub{NP} $e$ ] seems [\sub{S} John to leave] 
$\leadsto$  
\changed{{}[\sub{NP} John] seems [\sub{S} \st{John} to leave]}	
%\ex subject-control:  
...
Anne: d in virdist should be fixed\\
Stefan: I do not know what you mean. I still need sources for these examples. Anne :they are in  Sag, Wasow, Bender 2003: p 387}

\eal
\ex 
\gll Hana vantar peninga.\\
     She.\textsc{acc} need money.\textsc{acc} \\\hfill(\ili{Icelandic})
     \glt `She needs money.'
\ex 
\gll Hana virdist vanta peninga. \label{need} \\
     She.\textsc{acc} seem need money.\textsc{acc} \\
\glt `She seems to need money.'
\ex 
...
\zl
\itd{Stefan: Rizzi 1990 is missing. Could you please state what exactly you are referring to? Anne : Relativized minimality, MIT Press}
...
complement, and not with a sentential complement \parencites[\page 423]{Bresnan1982}[\page 113]{PollardandSag1994}: this shows that \word{expect} has two complements in (\ref{ex-we-expect-all-students-to-unterstand}) and only one in (\ref{ex-we-expect-that-all-understand}).
...
The figure also shows three examples (likely, seem and expect) inheriting from \type{sr-a-lx}, \type{sr-v-lx}, and \type{or-v-lx},respectively.  The constraints on the types \type{subj-rsg-lx} and
...
\crossrefchapterw{order} on constituent order and \crossrefchapterw{complex-predicates} on complex predicates). Furthermore, other subtypes of these lexical types
% \itd{Stefan: the
%   ``same''? You define new types in (\ref{ex-subj-pred-v} Anne: you are right : copular verbs are
%   subtypes of raising verbs).  I fixed that in the copular verb section.\\
% }
  %Maybe better write ``similar  types''. But after all, the copula types should be subtypes. Here is an open issue.} 
  will also be
...
\noindent
The subject of the non"=finite complement shares its \type{synsem}
%\itdobl{Stefan: There is nothing but this synsem. I would prefer ``The \subj element of the \ldots is shared with the \ldots'' Anne: I prefer it my way, the subject is a sign with other features}
with the subject of the subject-raising verb in (\ref{rais-1}), and with the object of the
object-raising verb in (\ref{rais2}). This means that  their syntactic and semantic features are shared : they have the
same semantic index 
%\itdobl{Stefan: Remove ``if any'', expletives have a semantic index as
 % well. Anne: Bob wanted this to be added but I guess you are right}
 , but also the same part of speech, the same case etc. Thus a subject appropriate for the
...
%\itd{Stefan: Schouldn't these be \type{subj-rsg-lx} and \type{obj-rsg-lx}?}
 respectively, which are subtypes of \type{subj-rsg-lx} and \type{obj-rsg-lx} (see Figure~\ref{raising:fig-verb-hier2}); their lexical descriptions look as follows,
assuming an MRS semantics (\citealp{CFPS2005a} and \crossrefchapteralp[Section~\ref{semantics:sec-mrs}]{semantics}):
...
%Stefan: Ah, I see. Then maybe do not give a type at all and say: ``Lexical item for the word \emph{seem}'' Anne: no, it is a lexical description and it is important to show how it derives from the lexeme type}
\eas
Lexical description for \emph{seem} (sr-v-lx):\\
\avm{
[%\type*{sr-v-word}
subj  & <\1 > \\
...
\zs
\itdobl{Stefan: For all the semantic contributions to work, the index should fill a role in the
  predicate introduced in \rels. This is systematically missing from all semantic representations
  here.Anne: I dont udnerstand what you want, maybe Jean-Pierre can help
}
...
\eas
Lexical description for \emph{expect} (or-v-lx):\\
\avm{
[%\type*{or-v-word}
subj &	<\1 NP$_i$> \\
...
verbs \citep[page444]{Jacobson1990}. Without a non"=finite complement, the subject of \word{seem} is not
...
subject) (\ref{seem4}), another lexical description is needed (see 34a), and the same holds for
object"=raising verbs which allow a sentential complement (with no object) (\ref{expect3}, see 34b). These
can be seen as valence alternations, which are available for some items (or some classes of items)
but not all (see \crossrefchapteralt{arg-st} on argument structure). %\added{(\mex{1}) shows the respective \argst values:}
...
given in Figure~\ref{verb-hier3}, adapted from \citet[page78]{SagandPollard1991}.\footnote{For further semantic classification of main predicates in order to account for optional control in languages such as Modern \ili{Greek} and Modern Standard \ili{Arabic}, see \citew{GreshlerMelnikWintner2017a-u}.}
...
For example, \emph{want}, \emph{promise} and \emph{persuade} have a semantic content such as the following, with \textsc{soa} meaning state-of-affairs and denoting the content of the non"=finite complement:
...
% Anne: I guess it is useful for the reader, it is not the same rel as that of the matrix verb.\\
 %  Stefan: I still find it confusing. Anne: you're not the target reader}
\eal
%\word{want}:\\
\ex 
\avm{
...
\avm{ [ \argst  < NP$_i$, \ldots, [subj & < [ind & $i$ ] > ] > ]}
...
\eas
Lexical description for \type{want} (sc-v-lx):\\
\avm{
[subj  & < \1 NP$_i$ > \\
	comps & < \2 VP[head &	[vform & inf] \\
					subj &	<[ind & i]> \\
...
\zs
\eas
Lexical description for \type{persuade} (oc-v-lx):\\*
\avm{
	[subj & <\1 NP$_i$ > \\
	comps & <\2 NP$_j$, \3 VP[head & [vform & inf] \\
	subj &	<[ind & j]> \\
	cont &  \4 ]>\\
	arg-st & <\1, \2, \3> \\
	cont &	[ind & s \\
...
%\itdobl{Stefan: persuade seems wrong here. Use promise instead?}
\zl
...
Verb morphology in \ili{Mauritian} provides an argument for the phrasal (and not clausal) status of the complement of both control and raising verbs. Unlike in \ili{French}, its superstrate, in \ili{Mauritian},  verbs neither inflect for tense, mood and aspect nor for person, number, and
...
As described in \citet[chapter4]{Henri2010}, the verb form is determined by the construction: the
...
%\itdopt{Stefan: What does ``phrasal complement'' mean? sega and pom are words. Anne: they are also noun phrases}
  and the long form appears otherwise.\footnote{\textit{yer} `yesterday' is an adjunct. See \citew{Hassamal2017} for an analysis of \ili{Mauritian} adverbs which treats as complements those triggering the verb short form.}
...
\itdobl{Stefan: You misquoted Henri since she has a different formulation. Your version does not
  work. The reason is that your constraint applies to all AVMs with VFORM sf. This includes verbs
  that have been combined with complements. The constraint causes a contradiction, which makes it
  impossible to analyze sentences with VFORM sf verbs. What Henri has is a constraint applying to
  lexical verbs only. This is what you need.Anne: I say it is a lexical constraint so it applies to verbs, not to phrases}
\z
Interestingly, clausal complements do not trigger the verb short form (\citet[page131]{Henri2010} analyses them as extraposed). The complementizer (\emph{ki}) is optional.
...
an element typed \type*{pro} representing the unexpressed subject in the \argst list of the main verb but nothing in
the \subj list.
...
\ex 
\type{voglio} `want' (sc-v-lx):\\
\avm{
	[subj & <> \\
	comps & < \2 > \\
	arg-st & < NP$_i$![\type{pro}]!, \2[subj & < [ind & $i$] >] > ]
}
%\itdobl{Stefan: Remove types.Anne: I need them to show where theses entries come from}
\zl


\ili{Balinese}\il{Balinese|(}, an ergativity language, provides another example of non-canonical subjects.  \citep{WechslerandArka1998} argue the the subject is not necessarily the first syntactic argument in this language. A transitive verb has two verb forms, called ``voice'', and there is  rigid SVO order,
...
\itd{Stefan: Both \ibox{1} and \ibox{2} may be the empty list or more generally lists of arbitrary
  length.\\
Anne: not relevant here; I'm summarizing W\&A's paper.\\
Stefan: Chapter~9 does not have these constraints. W\&A have a general constraint but mention
subjects explicitely. This is not done in your implicational constraints below. But you can say:
``Together with a constraint requiring that the SUBJ list has to have exactly one element, the
following lexical items are licensed.'' Anne I prefer not to go into that, It is the general case, I guess you're thinking about German subjectless verbs, they are not relevant here}
\eal
\ex 
\type{av-verb} \impl\\
\avm{
...
semantic control over the action promised \citep{Farkas1988,Kroeger1993,SagandPollard1991}\addpages:Anne: p78 for sagandpollard. The
...
subject argument available at the clausal level.  \citet[page409]{Sag2007a} proposed to introduce a Head
feature \xarg that takes as its value the first syntactic argument of the head verb, and is
accessible at the clause level.

This is adopted by \citet[section6]{HenriandLaurens2011} for \ili{Mauritian}.  After some
subject-control verbs like \word{pans} `think', the VP complement may have an optional clitic
...
\ea
\gll Zan$_{i}$ pans                     (li$_{i}$)            vini.\footnotemark\\
     Zan       think.\textsc{sf}  \hphantom{(}3\SG{} come.\textsc{lf}  \\\hfill(\ili{Mauritian})
\footnotetext{
...
subject;\itdobl{Stefan: And be a pronoun? ``Peter looks like Peter is tired.''? Anne: maybe ok :Peter looks like his wallet has been stolen?} it is a raising predicate, as shown by the possibility of the expletive \emph{there}:
...
Assuming a tripartiion of \type{index} with \type{referential}, \type{there} and \type{it} \citep[\page
138]{ps2}, the verb \emph{look like} can thus have the subject of its sentential complement
...
\emph{look (like)}:\\
\avm{
[ arg-st <NP$_i$, S[marking & like\\
                    xarg & pro$_i$] > ]}
\itdopt{Stefan: Does \type{pro} follow from Binding Theory? Do we have to state this explicitely here? Anne: here it is a pronominal content, not a pro synsem as with VP ellipsis in the last section}
\z
This is similar to \ili{English} tag questions, since the  subject of the tag question must be
...
\noindent
To account for the above cases, the types for subject-raising and subject-control verb lexemes in (\ref{rais-1}) and (\ref{subj-cont-lx}) can thus be revised  as follows, the only difference being that the Index of the subject of control verbs must be a referential NP:
\eal
\ex \type{sr-v-lx}  \impl \avm{ [\argst < XP$_i$, \ldots, [x-arg & [ind & $i$] ] > ] } 
\ex \type{sc-v-lx} \impl \avm{ [\argst < NP$_i$, \ldots, [x-arg & [ind & $i$]  ] > ] }
\zl
%\itdobl{Stefan: But this does not work for all languages. It sounds as if this was asssumed to be the final version now. Anne: are you thinking of German subjectless verbs again?}
...
subject in \ili{English} with a proper predicate like `a good place to hide' (\ref{under2}), \emph{werden} takes no subject when combined with a
...
%\itdopt{Stefan: Adapt earlier example.}
\ex
\label{ecm}
...
case in African American Vernacular English (AAVE)\il{English!African American Vernacular} \citep{Bender2001a}\addpages, in \ili{French} \citep{Laurens2008}\addpages and creole languages
\citep{HenriandAbeille2007}\addpages, in \ili{Slavic} languages \citep{Zec87a-u}\addpages, and in \ili{Semitic} languages (see
...
%\citet[Chapter~3]{PollardandSag1994} present 
...
%Following \citet{Bresnan1982}\addpages,
...
as follows, with \type{list} meaning any list
%\itdopt{Stefan: Please use  to be consistent with the
  rest of the volume.} 
  of arguments before the predicative complement, and the two subtypes of verbs taking a predicative complement : \type{s-pred-v-lx}  for verbs like \word{be}, which also inherit from subject-raising verbs, and \type{o-pred-v-lx} for verbs like \word{consider} which also inherit from object-raising verbs:
...
\ex \type{pred-lx} \impl  \avm{ [arg-st & L \+ \upshape \changed{\sliste{ [\prd $+$] }} ]}
\ex \type{s-pred-v-lx} \impl \type{sr-v-lx} \& \type{pred-lx}
\ex \type{o-pred-v-lx} \impl  \type{or-v-lx} \& \type{pred-lx}
\itdobl{Stefan: b and c are part of the signature and do not have to be given as separate
  implicational constraints. Anne: but I dont have a signature here, so I need to say this somehow so that the reader can understand}
\zl
...
more details, see \citew[chapter3]{PollardandSag1994},
...
As in Section~\ref{control:sec-HPSG-anaylsis-of-raising}, we ignore here a possible PP complement (\textit{John seems smart to me}) (see
\crossrefchapterw{binding}\itdobl{for Stefan: check and add section} ).
\itdobl{Stefan: I would add the following line here: ``With the assumption that the \subj list of
  English verbs contains exactly one element, the following lexical items for the words \emph{seem}
  and \emph{consider} are licensed:'' And the types should be removed. Anne: I need some types to distinguish them from other instances}
\eas
\type{seem (s-pred-v-lx)}:\\
\avm{
[subj  & < \1 > \\
comps & < \2 [head & [prd & $+$] \\
	      subj & < \1 > \\
...
\citet{PollardandSag1994}\addpages Anne:I cant find it; mention a few verbs taking a predicative complement which can be
...
%\itd{Stefan: Data argue? AA: yes\\Stefan: Data cannot argue. We can use data to argue. I asked Elizabeth and she said it is indeed strange.} 
in favor
of a non"=clausal analysis.
%\itd{Stefan: Where exactly did you show this? What you suggested in (\ref{ex-pans-Maritian}) was an embedded clause.Anne: no, see also ex 45.\\
%Stefan: (59) allows for both complete sentences and VPs. (45) says that there has to be an argument.} 
A copular verb takes a short form before a
predicative complement, and
a long form before a clausal one.
%\itd{Stefan: reference examples in (\mex{1}).} 
Despite the lack of inflection on the embedded verb, and the
...
%\itdobl{Stefan: But d has ki and strong pronoun.}
(\word{mwa} `me', \word{twa} `you'). See Section~\ref{sec-maurit}
...
%\itdobl{Stefan: Something is wrong in the following paragraph.}
...
(\type{Aux} or \type{Infl})\footnote{Having Infl as a syntactic category and sentences defined as IP does not account for languages without inflection, nor for verbless sentences, see for example Laurens 2008.} but verbs with the head property in
...
%\itdobl{Stefan: I said this already in some discussion, but the subtype relation is part of the type hierarchy (the signature, see formal background). So \type{sr-v-lx} should not be stated here. You can mention the inheritance relation in prose.}
 ...
 \ili{English} auxiliaries take VP (or XP) complements and do not impose categorial restrictions on their subject, nor assign it a semantic role,
...
XP) complement and share their subject with the unexpressed subject of the non"=finite verb (see section 2.1).\footnote{ \emph{Be} is an auxiliary and a subj-raising verb with a \prd$+$ complement, see
...
needed for verb initial languages like \ili{Welsh} \parencites[\page285]{Borsley99c-u}[\page410]{SWB2003a}.\footnote{As
...
[subj     & < > \\
 comps    & < > \\
 head-dtr & \1[head & [aux & $+$ \\ inv & $+$]\\
	       subj & \2\\
               comps & \3...\n ] \\
 dtrs & < \1 > \+ <SS \2> \+ <SS \3>... \+ <SS \n> ]
}
%\itdobl{Stefan: Replace cx by phrase. The structure sharing does not work since \comps has synsem objects.}
\z
I use SS for \type{synsem} features.\\
...
As for tag questions (Paul left, didn't he? \ref{extag}), they can be defined as special adjuncts, coindexing their subject with that of the sentence they adjoin to, using the \xarg feature (see above Section~\ref{section-xarg}).
...
\z
\rel{not} is a function that returns `$+$' for the input `$-$' and `$-$' for the input `$+$'. I use \type{tense} to ensure time concordance between the main verb and the tag auxiliary. \type*{pro} denotes a realized subject with a pronominal content.
...
\crossrefchaptert[Section~\ref{ellipsis-sec-Predicate-ellipsis-and-argument-ellipsis}]{ellipsis} and
...
A revised version of \emph{will} with complement inheritance could be the following, using \type*{pro} for the unexpressed VP:
...
complements in HPSG \citep[chapter3]{PollardandSag1994}, with sharing or coindexing the (unexpressed) subject
...
object-control. The ``subject-raising''\itdopt{Stefan: remove quotes?} analysis has been successfully extended to copular and
