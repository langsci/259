...
\abstract{The distinction between raising and control predicates has been a hallmark of syntactic
  ...
  \citep{PollardandSag1994}. The distinction between 'raising' and 'control' lexeme types is also relevant for non"=verbal predicates such as
  adjectives (\word{likely} vs.\ \word{eager}). We will provide creole data (from Mauritian) which support a phrasal
  analysis of the complement of both control and raising verbs, and argue against a clausal (or small clause) analysis
  \citep{HenriandLaurens2011}.  We will provide some data from ergative languages to discuss the distinction between subject and first syntactic argument, and also show how the HPSG analysis can be extended to cover cases of obligatory control of the expressed subject of some finite clausal complements in certain languages. The raising analysis naturally extends to copular
...
reference at all,%\itd{expletives?}
...
\parencites[\page 75--76]{Chomsky1981}[\page 379]{Bresnan1982}\footnote{\citet{Bresnan1982} proposes
  ...
  (obligatory) ``anaphoric control''. See also the LFG and HPSG chapter}. 
...
expletive \emph{it} and \emph{there} and idiom-chunks\itd{Stefan: Isn't the idiom-chunk the whole
  idiom? Then I would write here ``parts of idiom chunks'' Anne: for me idiom chunk means part of an idiomatic expression}.
...
\footnote{Some verbs may be ambiguous and allow for subject control (John proposed to come later.), object control (John proposed to Mary to wash herself.) as well as joint control (John proposed to Mary to go to the movies together). For the later, a cumulative (i+j) index is needed, as as also the case with long distance depenencies, see Chaves and Putnam 2020, Chaves Handbook section 3, Borsley and Crysmann Handbook section 3, ex: Setting aside illegal poaching for a moment, how many sharksi+j do you estimate [[$_i$ died naturally] and [$_j$ were killed recreationally]]?}
\eal
...
select a subject and a sentential complement \citep{Postal1974, Chomsky81a}.\footnote{We disregard here the Movement Theory of Control \citep{Hornstein99a-u}, see \citew{Landau2000a-u} for a criticism.}
...
block subject movement according to Chain formation or Relativized Minimality \citep{Rizzi1986,Rizzi1990a-something}.
\eal
\ex Carol seems to Kim to be gifted
\ex Carol$_i$ seems to herself$_i$ [e$_i$ to have been quite fortunate]\hfill\citep[\page 50]{McGinnis2004a-u}
\zl
\itd{Stefan: Rizzi 1990 is missing.}
...
We will first present in more details the HPSG analysis of raising and control verbs, then provide creole data (from Mauritian) which support a phrasal analysis of 
of their complement, then the implication of control/raising for prodrop and ergative languages, to end up with a revised HPSG analysis, based on sharing XARG instead of SUBJ.
...
\crossrefchapterw{order} on constituent order). Furthermore, the same lexical types\itd{Stefan: the
  ``same''? You define new types in (\ref{ex-subj-pred-v} Anne: you are right : copular verbs are subtypes of raising verbs).  I fixed that in the copular verb section}
...
the subject-raising verb in (\ref{rais-1}), and with the object of the object-raising verb in
...
(\ref{german1}), this information is shared too (\ibox{1} can be the empty list). The dots in \label{rsg} accounts for a possible PP complement as in \textit{Kim seems to Sandy to be smart.}, which we ignore in what follows.
...
%\itd{Stefan: Schouldn't these be \type{subj-rsg-lx} and \type{obj-rsg-lx}?}
 respectively, which are subtypes of \type{subj-rsg-lx} and \type{obj-rsg-lx} (see Figure 1); their lexical descriptions look as follows,
...
lexeme. This would make it possible to use the lexeme type. Anne: the pb is that I want to show valence features and they are not available for lexemes, only for words}
\eas
\type{seem}:\\
\avm{
[\type*{sr-v-word}
...
given in Figure~\ref{verb-hier3}, adapted from \citet{SagandPollard1991}.\footnote{For further semantic classification of main predicates, on order to account for optional control in languages such as Modern Greek and Modern standard Arabic, see Ara Greshler et al 2017.}
...
\itd{Stefan: Stating \type{relation} in the lexical items seems redundant since this is the most
  general type for something having \textsc{arg} as a feature and being the value of \textsc{soa}.Anne: I guess it is usefull for the reader, it is not the same rel as that of the matrix verb}
\eal
%\word{want}:\\
\ex 
\avm{
...
\avm{ [ \argst  < NP$i$, \ldots, [subj & < [ind & $i$ ] > ] > ]}
...
controller here is simply coindexed with the subject of the non"=finite complement. Since the controller is referential and since it is coindexed with the controlee, the controlee has to be referential as well.
%\itd{Stefan: ``This means it must have a semantic role''. This is not correct. Expletives also have an index. A better formulation would be: ``Since the controller is referential and since it is
  %coindexed with the controlee, the controlee has to be referential as well.''. I would also talk
 % about ``non-referential idiom parts'' since there are decomposable %idioms with referential parts. Anne: ok} 
This means it
must have a semantic role (since it has a referential index), thus expletives and (non referential)idiom parts are not
...
\avm{
	[\type*{oc-v-word} \\
	subj & <\1 NP$i$ > \\
	comps & <\2 NP$j$, \3 VP	[head &	[vform & inf] \\
	subj &	<[ind & j]> \\
	cont &  \4 ]>\\
	arg-st & <\1, \2, \3> \\
	cont &	[ind & s \\
...
In some \ili{Slavic} languages (\ili{Russian}, \ili{Czech}, \ili{Polish}), the predicative adjective must share case with the subject of the copular verb (ex a): some subject-control
verbs may allow case sharing like subject-raising verbs (ex b), unlike object-control verbs (ex c).
...
allow for both (shared) nominative and (default) instrumental case for the unexpressed subject and the predicative adjective, and a specific constraint may be added to enforce only (nominative) case sharing with the relevant set of verbs
...
\gll Janek zaczal / chce by'c miły.\\
     Janek.\textsc{nom} started/ wants be.\textsc{inf} nice.\textsc{nom} \\ \hfill(\ili{Polish})
\glt `Janek started / wants to be nice.'
\ex 
\gll Janek kazal Tomkowi by'c miłym/ * milemu.\\
     Janek.\textsc{nom} ordered Tomek.\textsc{dat} be.\textsc{inf} nice.\textsc{inst}/*dat \\ \hfill(\ili{Polish})
...
\type{sr-v-lx} and \type{sc-v-lv} respectively 
...
that only canonical ones are present in the valence lists (\subj, \spr and \comps). See The Argument Realization Principle (ARP) in Chapter 1 (section 4.1).
\itd{Stefan: I think the UDC chapter does not contain such a discussion. Anne: there is the traceless analysis with a non canonical member of arg-st, refer to Elts and property chapter, section 4.1, ARP is in (20)}
...
Assuming the lexical types for sr-v-lx and sc-v-lx in (\ref{rsg}) and
...
\avm{
	...
	subj & <> \\
	comps & < \2> \\
	arg-st & < NP$i$[\type{pro}], \2[subj & < [ind & $i$] >] > ]
}
\zl
...
\itd{Stefan: Both \ibox{1} and \ibox{2} may be the empty list or more generally lists of arbitrary length. Anne: not relevant here; I'm summarizing W\&A's paper}
\eal
\ex 
\type{av-verb} \impl\\
\avm{
...
\itd{Stefan: \xarg is never really introduced. You have to say that it is a \type{synsem} object
  structure shared with the subject (or whatever). Anne: I say  it is a Head
feature  that takes as its value the first syntactic argument of the head verb,}
...
like \word{pans} `think', the VP complement may have an optional clitic subject which must be coindexed with the matrix subject. 
It is not a clausal complement since the matrix verb is in the short form (\textsc{sf}) and not in the long form (see ex 45 above).

\ea
\gll Zan$_{i}$ pans              pou           (li$_{i}$)            vini.\\
     Zan       think.\textsc{sf} \textsc{fut} \hphantom{(}3\SG{} come.\textsc{lf}  \\\hfill(p.\,202)
...
This coindexing can also be extrended to some subject-raising verbs such as \emph{look like}, which has been called ``copy raising'' (\citealp{Rogers74a-u,Hornstein99a-u} a.o.): it takes a finite complement with an overt subject, but this subject must be coindexed with the matrix subject; it is a raising predicate, as shown by the possibility of the expletive \emph{there}:
...
Assuming a tripartiion of Index with referential, \textit{there} and \textit{it}, (Pollard \& Sag 1994, this volume?), the verb \emph{look like} can thus have the subject of its sentential complement  coindexed with its own subject:
...
\emph{look (like)}:\\
\avm{
[ \argst <NP$_i$, S[marking & like\\
xarg & pro$_i$] > ]}
\z
\itd{Anne: pro should be in small letters, it is a type of content, 'like' is a value of Marking?}
...
\eal
\ex \type{sr-v-lx}  \impl \avm{ [\argst < XP$i$, \ldots, [x-arg & [ind & $i$] ] > ] } 
\ex \type{sc-v-lx} \impl \avm{ [\argst < NP$i$, \ldots, [x-arg & [ind & $i$]  ] > ] }
\zl
...
\ex \type{pred-lx} \impl  \avm{ [arg-st & L \+ \prd $+$ ]}
...
predicative \word{consider} inherit from the \type{s-pred-v} type and \type{o-pred-v}
...
As in section 2.1, we ignore here a possible PP complement (\textit{John seems smart to me}) (see Binding chapter section?).
\eas
\type{seem}:\\
\avm{
[\type*{s-pred-v-word}
...
As shown by \citet{HenriandLaurens2011}, and as noted in section 2.3, \ili{Mauritian} data
argue\itd{Stefan: Data argue? AA: yes} in favor
of a non"=clausal analysis.\itd{Stefan: Where exactly did you show this? What you suggested in
  (\ref{ex-pans-Maritian}) was an embedded clause.Anne: no, see also ex 45} A copular verb takes a short form before a
...
\itd{Stefan: Why is this important? AA: because Bob asked for it}
\eal
\ex[]{
...
\crossrefchapterw[Section~\ref{sec-analyses-of-pred-ellipsis}]{ellipsis} on post-auxiliary ellipsis.\footnote{Copular \word{be} has
  ...
  \emph{John promised to work and he started to}. See \citew*{GPS82a-u}.\itd{add pages, please cite also :Levine, R. D. (2012), Auxiliaries: to's company. Journal of Linguistics 48.187--203}} 
...
As for tag questions, they can be defined as special adjuncts, coindexing their subject with that of the sentence they adjoin to, using the XARG feature (see above section 2.5).
\itd{Anne:I'm simplifying with NEG +, if you could replace by POL 1 in the modifed clause and POL -1 in the tag-aux, it would be better; pro should be small letters since it is a type of content}
\ea
...
\itd{Stefan: Ellipsis chapter (p.\,\pageref{sec-analyses-of-noncon}) points here and does not have
  any analysis on pseudogapping. Anne: it has data, please point to section 2.2 predicate ellipsis and argument ellipsis}
restricted to auxiliaries, for example pseudogapping (see also \crossrefchaptert{ellipsis} and \citealt{Miller2014a-u}).

\eal
\ex[]{
John can eat more pizza than Mary can tacos. (Sag et al 2020, ex.52)
}
...
\ex[*]{
Kim tried to impress Lee, which Sandy didn't try. (Sag et al. 2020, ex 54a) \label{nonaux}
}
\zl
