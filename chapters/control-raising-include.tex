...
(see \citew[\page 129]{Mueller2002b} for counterexamples in \ili{German}).
...
\itdobl{Stefan: There seems to be a problem with (\mex{0}a), since \ibox{1} is a list and the commas
  are not correct. Commas are for elements. I guess it should be the following.}
\avm{ [ \argst  \1 \+ \ldots \+ < [subj & \1 ] >  ]} \label{rais-1}
...
\itdobl{Stefan: There is nothing but this synsem. I would prefer ``The \subj element of the \ldots
  is shared with the \ldots'' Anne: I prefer it my way, the subject is a sign with other
  features.\\Stefan: There is no sign for the subject of the non-finite complement.}
...
% type.}
\eas
Lexical description for \emph{seem} (\type{sr-v-lx}):\\
\avm{
[%\type*{sr-v-word}
subj  & <\1 > \\
...
\itdobl{Stefan: For all the semantic contributions to work, the index should fill a role in the
  ...
  here.\\
Anne: I dont udnerstand what you want, maybe Jean-Pierre can help.\\
Stefan: s has to be an argumet of \type{seem-rel}. Like this:
\avm{
[ind & s \\
 rels &	<[\type*{seem-rel}
          event & s\\
...
\eas
Lexical description for \emph{expect} (\type{or-v-lx}):\\
\avm{
[%\type*{or-v-word}
subj &	<\1 NP$_i$> \\
...
Anne: you're not the target reader.}
\eal
%\word{want}:\\
\ex 
\avm{
...
\eas
Lexical description for \emph{want} (\type{sc-v-lx}):\\
\avm{
[subj  & < \1 NP$_i$ > \\
	comps & < \2 VP[head &	[vform & inf] \\
					subj &	<[ind & i]> \\
...
\zs
\eas
Lexical description for \emph{persuade} (\type{oc-v-lx}):\\*
\avm{
	[subj & <\1 NP$_i$ > \\
...
Stefan: I would remove the \type{v-} of \type{v-word} since it is redundant (head value \type{verb}
follows from the presence of \vform and it may be wrong since there is \type{v-lx} but the words are
derived by lexical rules.}
\z
Interestingly, clausal complements do not trigger the verb short form (\citet[131]{Henri2010} analyses them as extraposed). The complementizer (\emph{ki}) is optional.
...
\noindent
To account for the above cases, the types for subject-raising and subject-control verb lexemes in (\ref{rais-1}) and (\ref{subj-cont-lx}) can thus be revised  as follows, the only difference being that the \textsc{index} of the subject of control verbs must be a referential NP:
\eal
...
as follows, with \type{list} meaning any list  of arguments before the predicative complement, and the two subtypes of verbs taking a predicative complement: \type{s-pred-v-lx}  for verbs like \word{be}, which also inherit from subject-raising verbs, and \type{o-pred-v-lx} for verbs like \word{consider} which also inherit from object-raising verbs:
...
\ex \type{pred-lx} \impl  \avm{ [arg-st & \type{list} \+ \upshape \sliste{ [\prd $+$] } ]}
...
Anne: but I dont have a signature here, so I need to say this somehow so that the reader can
understand.\\
Stefan: Yes, but you should not say this in technical terms since otherwise the reader may think
...
\inlinetodostefan{Stefan: fix these lexical items. Raise everything? Use append for consider. Should
  this be \argst? it is easier to show a word that is later used in the figure ; Anne: I use append
  in the types, but for \ili{English} verbs, we can assume that there are no  subjectless
  predicates.\\  Stefan: But this assumption is not in the theory. Hence you have to use append to
  reflect what you  stated in the types, don't you?\\
...
\citet{PollardandSag1994}\addpages\itdopt{Anne:I cant find it.} mention a few verbs taking a predicative complement which can be
...
\itd{Stefan: Where exactly did you show this? What you suggested in (\ref{ex-pans-Maritian}) was an
  embedded clause.\\
Anne: no, see also ex (\ref{sf-constraint}).\\
Stefan: (\ref{ex-pans-Maritian}) allows for both complete sentences and VPs. (\ref{sf-constraint})
says that there has to be an argument.
} 
...
(\type{Aux} or \type{Infl})\footnote{Having Infl as a syntactic category and sentences defined as IP does not account for languages without inflection, nor for verbless sentences, see for example \citew{Laurens2008}.} but verbs with the head property in
