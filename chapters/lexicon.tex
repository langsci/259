...
\word{destruct} and \word{destruction}) are to be modeled not via a generative device but through a \itddone{Jackendoff does not talk about inflection so, no stems to words in his work}
...
JPK Date is wrong-- our bad-- it should be 2016, but lecture notes are OK with us.}
...
\itdopt{\citet{Goldberg2013a} hates them and we do not use them. We use what Goldberg calls
  ...
  JPK we agree but that's kinda irrelevant since we are talking about the origin of lexical rules. What Adele thinks in this context is not the point :)} 
...
The second challenge to lexicalism presented in \citet{Bruening2018} bears more directly on HPSG's assumption that words are syntactic atoms. Word-internal conjunction/ellipsis examples, illustrated in (\ref{ch08}) (adapted from Bruening's (31a), p.\,14), seem to violate the assumption that syntactic constraints cannot ``see'' the \isi{internal structure of words}\is{morphology}\is{lexical integrity}, as ellipsis in these kinds of examples seems to have access to the internal part of the word \word{over-application}. In fact, though, such examples do not violate lexical integrity\is{lexical integrity} if one enriches the representation of composite words (to borrow a term from \citealt[Chapter 11]{Anderson1992}\itddone{Done}) to include a representation of their internal phonological parts as proposed in \citet{Chaves2008}  and \citet{Chaves2014}.\is{affixation}
...
\itddone{Add page numbers for all examples.}
\zl
...
\ili{Udi} subject agreement clitics such as \word{q'un} in (\ref{udi-endo}) typically attach to a focused constituent, which can be a noun, a questioned constituent, or a negation particle as well as a verb \citep{Harris2000}\itddone{This is an entire section, not pages. Adding pages would be misleading when this is the entire empirical point of the paper}.
...
Because lexical entries (derived or not\footnote{\added{%
...
One misunderstanding about lexical entries conflates descriptions and the entities they describe, or, in other words, fails to distinguish between constructions in the abstract and a particular word or phrase (i.e., a lexical entry vs.\ a fully instantiated lexeme or word).\itdopt{But lexemes are a subset of the lexical items.\\
JPK We agree that the terminology is confusing as different people mean different things so we clarify this section, we hope!} 
...
To see the importance of the distinction between descriptions (stored entries)\itdopt{not all lexical items are stored. \\
JPK As said above, we clarified the beginning of this section, but since we talk about descrptions as stored entries, this particular sentence is properly restricted :)} and the fully instantiated objects that are being described, consider HPSG's model of subcategorization with reference to the relevant portion of the tree for sentence (\ref{expect-b}). 
...
\gll Muriel va à Lourdes. \\
     Muriel go.\ig{prs.3sg} at Lourdes \\\hfill\added{(French)}
...
The \feat{arg-st}\isfeat{arg-st} list, which we included in the categorial information of signs in (\ref{va-lx}) and (\ref{y-va-lx}), might be regarded as a feature confined to lexical signs (see, among others, \citealt[361]{GinzburgandSag2001}\itddone{Done, we also dropped PHON as the way we said it was clearly confusing}), on the premise that lexical items alone specify combinatorial requirements (but see \citealt{Przepiorkowski2001} for a contrary view, and see \crossrefchapteralt[Section~\ref{sec-free-without-domains}]{order} for other views questioning this assumption).\itdopt{You referenced the CxG chapter, but I think you meant to refer to the discussion of Bender's projection of \argst, didn't you? If so, I can change this.\\
...
\zl
\itddone{Example is not referenced in the text.}
\ea
...
\itddone{If this is a feature it should be in small caps \textsc{ana}+. Better use types \type{ana}, \type{ppro}, \ldots as in the Binding chapter. Type hierarchy is given there.}
\zl
...
JPK We clarified what we meant}
...
\itddone{You already mentioned Dowty above. We again did not mention pages for Carpenter's chapter as the entire chapter is devoted to the topic and any page number would be arbitrary and again lead to an inappropriate implicature}
...
JPK OK. But this is a level of technical detail that is not really helpful to readers, not to mention that not all HPSG-ers view things the way Frank does}
...
\itdopt{Could be a total specification. For instance for adverbs, it is just the part of speech and that's it, isn't it?\\
JPK we added ``typically'' to help clarify what we meant} Typing of \feat{head} information allows the ascription of appropriate properties to different classes of lexical entries. For example, case information is only relevant to nouns in English,\itddone{not in English. remove ``adjectives'' to be consistent with claims below.} and whether a verb is an auxiliary or not is only relevant to verbs. 
...
\itddone{Inconsistent with mentioning ``adjective'' above.}
\ex\label{pos-def-b} 
...
JPK In the context of what we are saying we care of the inference in this direction. Not all true inferences are drawn and we only care about this one here}
\zl
...
Thus, the pronoun \word{him} need only include the fact that it bears accusative case; the fact that it is a noun can be inferred.\itddone{
...
The mapping from semantic roles to subjects and objects in these sentences can be described by the following informally stated constraints:\is{linking}\is{argument realization|see{linking}}
...
\itddone{If \argst is under \cat, it should be above \cont.}
\zl
...
The \word{zu}+infinitive passive, for instance, requires not only that \word{sein} is the auxiliary and that the main verb is infinitive, but that the semantics involves some additional modal meaning.\itddone{It can be any modal meaning.}
...
In YADU, each type is represented with a nondefeasible typed feature structure\itddone{JPK I checked. It is ``feature structure'' in the original, so it would be wrong for us to change the formal architecture they assume-- I know it's a pain}, plus a set of defeasible feature structures, each with an associated type.
...
JPK Indeed Thanks for noticing Tony reworded nicely, I think}
...
JPK We wondered about that, but we cannot find the citation either. But we added something to allude to that fact. Thanks for noticing}

%JPK 3-3-21 edited the following after Tony's wording as well as caption of figure

In Figure~\ref{def-verb-hier}, the most general type \type{verb} stipulates the identity of the past participle and passive participle forms as nondefault information. The value of \feat{past}, the simple past tense form, is unspecified, because some English verbs have irregular past tense forms (the symbol $\top$ denotes the most general type, and here indicates merely that nothing more specific can be stated about the \feat{past} form of every English verb). On the right-hand side of the / is default information; this states that normally, the value of \feat{past} is shared with both the values of both participle forms, whether the verb is regular (e.g., \word{walked}) or irregular (e.g., \word{understood}), although there are also verbs for which this is not true, such as \word{give} (\word{gave}, \word{given}), which override this default.  For regular verbs (type \type{regverb}), the value of \feat{past} will be, by default, the result of a function that suffixes \word{-ed} to the verb stem (Lascarides \& Copestake gloss over the details of morphology and phonology here), but this is defeasible. In the more specific type \type{pst-t-verb}, for instance, the default \word{-ed} is overridden by (again default) information that the suffix is \word{-t}.
...
\itddone{JPK We reworded things quite a bit to make things clearer and address your bubble regarding no SUBJ list. We got confused between the PhD thesis and the book-- at least that's one interpretation. The other is we are confused! Good catch. Thanks. We also talk of ``direct complements'' to avoid confusion}
However, in the subtype of \type{v} called \type{vger}, the default value \type{verb} is overridden.  In \type{vger}, the \feat{head} value is of the type \type{gerund}, which is a subtype of both \type{noun} and \type{relational}, but not of \type{verb}.
...
But, as its \feat{head} is also a subtype of \type{noun}, its \feat{subj} list is empty and the first element on its \feat{arg-st} list is its \feat{spr} value. In addition, gerunds allow direct complements (unlike ordinary nouns), but not subjects (unlike ordinary verbs).
...
%JPK 3-3-21 edited a little the following para. following Tony's wording

While it would be possible to construct type hierarchies of lexical types, \feat{head} types, and so on that would allow for ``anti-gerunds'' -- those that would act externally as nouns, allow subjects, but not permit direct complements -- this would require reorganizing these type hierarchies to a considerable extent.
Given that many nouns besides gerunds -- nominalizations, for example -- are relational \itdobl{How can that be? The governing verbs require an index.\\
JPK I don't think this would be a problem. But we removed this talk of psoa as it could be confuding or people could disagree and it's not relevant to the point we are making} it could be difficult to model a hypothetical language that permits only the anti-gerunds rather than the normal ones.
...
In a slight variant of the representation of lexical rules proposed by Copestake \& Briscoe and \citeauthor{Meurers2001}, the \feat{out} attribute can be dispensed with; the information in the lexical rule type that is not within the \feat{in} value then constitutes the output of the rule.\itdopt{I try to reconstruct who first suggested this encoding with out OUT. I think it was JP, wasn't it? With mü-daughters? And Riehemann. You may cite them for this here. Interestingly OUT reappeared as MOTHER over a decade later.\\
JPK I  wish I could take credit. But it was not me. I honestly don't know who did it this first. It could be Ann Copestake in one of her early 90s papers on polysemy?} The difference between the two representations with only the attributes \feat{synsem} and \feat{phon} included for expository purposes is shown in (\ref{two-lex}).
...
In the variant in (\ref{two-lex-b}), lexical rules are treated as subtypes of a \type{derived-lexical-sign}\is{derived word} type,\itdopt{This cannot be right. There has to be a type complex-sign and then there can be lexical rules for words and lexical rules for stems. They are not all of type word.\\ JPK we reworded; you are right} which can combine with other types in the lexical hierarchy, merely adding the derivational source via the \feat{in} value. 
...
JPK Not sure what you meant or what you want us to do}
...
Is there inherent directionality, for example, between the causative and inchoative alternants of verbs such as \word{melt} or \word{slide} or between the ditransitive and prepositional object frames of verbs such as \word{give}, as \citet[731]{Goldberg1991} or \citet[\add{18}]{Goldberg1995} ask?
...
JPK I am OK with page 18-- I checked. And I added another of Adele's paper which is much more explicit and even the relevant page number! I will send you the relevant BiBteX entry}
\itddone{In contrast to what? Reads strangely.} The alternatives to lexical rules described in the following section lack this notion of directionality.
...
\itdobl{I think there is an issue with terminology here. Frank defines ``atomic'' as something that does not have features. Something like \type{nominative}. I guess what you have in mind as non-atomic is types that are \type{type1}+\type{type2}.\\
JPK Really good catch. I meant ``atomic'' the way it's understand in the sum of individuals literature. We reworded to avoid confusion along your suggestions. THANKS!}  either sets of species or non-atomic sums of species,  just in cases where orthogonal dimensions of classification have been used since \citet{Flickinger1987}. For reasons of space, we do not pursue this line of inquiry in this chapter.}
...
\gll Marc est             allé        à  Paris. \\
     Marc be.\ig{prs.3sg} go.\ig{ptcp} to Paris \\\hfill\added{(French)}
...
JPK I cannot do it until I have the final verison of the chapter, right? WHAT SHOULD I DO TO AVOID FORGETTING?}). 
Non-canonical realization of syntactic arguments as affixes or fillers in unbounded dependencies is  modeled by many (but see \citealt{HukariandLevine2006}, among others, for an opposing view)\itddone{Not by everyone. \citet{LH2006a} argue against this and I use traces as well. I guess the whole German crowd does not assume this approach. I suggest adding a footnote.}  by distinguishing kinds of members of the \feat{arg-st} list and constraints on words that relate valence, argument structure, and dependents lists.\footnote{\added{But see \citet{LH2006a}, \citet{MuellerCoreGram}, \citet[Section~4.9]{MuellerCurrentApproaches} and \citew{MuellerGS} for trace-based approaches.}}
...
JPK Neither Tony and I can find other reviews but yours. And Tony is much more organized than me!}
