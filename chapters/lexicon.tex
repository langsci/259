...
The nature, structure, and role of the lexicon in the grammar of natural languages has been a subject of debate for at least the last 50 years. For some, the lexicon is a prison that ``contains only the lawless'',  to borrow a memorable phrase from \citew[3]{DiSciulloandWilliams1987}, and not much of interest resides there. In some recent views, the lexicon records merely phonological information and some world knowledge about each lexical entry (see \citealt{Marantz1997}). All of the action is in the syntax, save the expression of complex syntactic objects as inflected words.
...
Lexicalism began as a reaction to \isi{Generative Semantics}, which treated any regularity in the structure of words (derivational patterns, broadly speaking) as only epiphenomenally a matter of word structure and underlyingly as a matter of syntactic structure (see \citealt{Lakoff1970}, among others). In the Generative Semantics view, all grammatical regularities are a matter of syntax (much of it, in fact, logical syntax). \citet{Chomsky1970} presented many arguments that lexical knowledge differs qualitatively from syntactic knowledge and should be modeled differently. \citet{Jackendoff1975} is an explicit model of lexical knowledge that follows Chomsky's insights, although it focuses exclusively on derivational morphological processes. The main insight that Jackendoff formalizes is that relations between stems and relations between words (say, between \word{destruct} and \word{destruction}) are to be modeled not via a generative device but through a redundancy mechanism that measures the relative complexity of a lexicon where these relations are present or not present (the idea is that a lexicon where \word{construct} and \word{construction} are related is simpler than one where they are not). \citet{Bochner1993} is the most formalized and detailed version of this approach to lexical relations. Lexicalist approaches, including LFG\is{LFG|see{Lexical Functional Grammar}} and HPSG, took their lead from Jackendoff's work.  LFG has relied heavily on treating relations between stems and between words as lexical rules, rather than the kind of generative devices that one finds in syntax. But, as accounts of linguistic phenomena in LFG focused increasingly on the lexicon, the question of whether lexical rules retain the character of redundancy rules or turn into yet another kind of generative device arose.  Consequently, the necessity of lexical rules has been questioned as well (see \citealt{KoenigandJurafsky1994} and \citealt[29--49]{Koenig1999c} for potential issues that arise once lexical rules are assumed to be involved in the creation of new lexical items). 
...
The second challenge to lexicalism presented in \citet{Bruening2018} bears more directly on HPSG's assumption that words are syntactic atoms. Word-internal conjunction/ellipsis examples, illustrated in (\ref{ch08}) (adapted from Bruening's (31a), p.\,14), seem to violate the assumption that syntactic constraints cannot ``see'' the \isi{internal structure of words}\is{morphology}\is{lexical integrity}, as ellipsis in these kinds of examples seems to have access to the internal part of the word \word{over-application}. In fact, though, such examples do not violate lexical integrity \is{lexical integrity} if one enriches the representation of composite words (to borrow a term from \citealt{Anderson1992}) to include a representation of their internal phonological parts as proposed in \citet{Chaves2008,Chaves2014}.\is{affixation}

\ea
\label{ch08}Over- and under-application of stress rules plagues Jim's analysis.
\z
\is{ellipsis!word internal|see{affixation, suspended}}%
Chaves' analysis assumes that the phonology of compound words and words that contain affixoids (to borrow a term from \citealt{Booij2005}) is structured. The MorphoPhonology or \feat{mp} attribute of words (and phrases) is a list of phonological forms and morphs information. The \feat{mp} of compound words and words that contain affixoids includes a separate member for each member of the compound, or for the affixoid and stem.  Thus in (\ref{ch08}), the \feat{mp}s of \word{overapplication} and \word{underapplication} each contain two elements: one for \word{over}/\word{under}, and one for \word{application}. Given this enriched representation of the morphophonology of words like \word{under/overapplication}, a single ellipsis rule can apply both to phrases and to composite words, eliding the second member of the word \word{overapplication}'s \feat{mp}. As Chaves (p.\,304) makes clear, such an analysis is fully compatible with lexical integrity, as there is no access to the \is{morphology!internal structure of words} internal structure of composite words, only to the (enriched) morphophonology of the entire word.
...
But what makes these particularly odd is that these clitics are situated word-internally, even morpheme-internally. \is{clitics!in Udi}
...
prosodic, or something else), thereby seemingly violating \is{lexical integrity} lexical
...
\citep{Kathol2000a}\addpages (see \crossrefchaptert{order}). The ``morphosyntactic paradox'' in \ili{Udi}, to borrow
a phrase from \citet[\page 373]{Crysmann2003d}, is effectively ``resolved on the basis of discontinuous lexical items''; this account then ``parallels HPSG's representation of syntactic discontinuity'' (\citealt{Crysmann2000b}; \crossrefchapterp{order}).
...
We illustrate the second leading idea behind HPSG or LFG's lexicalism \is{LFG|see{Lexical Functional Grammar}} -- that there are different variants of lexical heads for different contexts in which heads occur -- with the \ili{French} examples in (\ref{va}). \is{clitics!in \ili{Romance} languages} The verb \word{aller} `go' in (\ref{va-a}) combines with a PP headed by \word{à} that expresses its goal argument and a subject that expresses its theme argument. The same verb in (\ref{va-b}) combines with the so-called non-subject clitic \word{y} that expresses its goal argument. We follow \citet{MillerandSag1997} and assume here that \ili{French} non-subject clitics are prefixes. Since the context of occurrence of the head of the sentence, \word{aller}, differs across these two sentences (\feat{NP\_\_\_\_PP}[\word{à}] and \feat{NP} \word{y}\_\_\_\_ , respectively and informally), there will be two distinct entries for \word{aller} for both sentences, shown in (\ref{va-lx}) and (\ref{y-va-lx}) (we simplify the entries' \isi{feature geometry} for expository purposes). Information in the entry in (\ref{y-va-lx}) that differs from the information in the entry in (\ref{va-lx}) appears in red.
...
Are there attributes or values that occur solely within lexical signs, and not in phrasal ones? \is{lexical sign}
...
Likewise, the \feat{arg-st}\isfeat{arg-st} list, which we included in the categorial information of signs in (\ref{va-lx}) and (\ref{y-va-lx}), might be regarded as a feature confined to lexical signs (see, among others, \citealt{GinzburgandSag2001}), on the premise that lexical items alone specify combinatorial requirements (but see \citealt{Przepiorkowski2001} for a contrary view, and see \crossrefchapteralt{cxg} for other views questioning this assumption).
...
Not only are syntactic arguments recorded; so is their relative obliqueness \is{obliqueness hierarchy}(in terms of grammatical function), as per the partial hierarchy in (\ref{obl-hier}) from \citet{PollardandSag1992}. 
...
We illustrate this explanatory role by noting the role of the lexicon in HPSG's approach to binding, as described in \citew{PollardandSag1992} (see \crossrefchaptert{binding} for details). As lexical entries of heads record both syntactic and semantic properties of their dependents, constraints between properties of heads and properties of dependents, e.g., subject-verb agreement, or between dependents, e.g., binding constraints, illustrated in (\ref{princ-a}), can be stated, at least partially, as constraints on classes of lexical entries. The principle in (\ref{princ-a}) is such a constraint.
...
\ex[]{\label{a-st-bind-a} 
	\avmtmp{[arg-st & <NP$_{i, + ana}$, \ldots >]
}}
\ex[]{\label{a-st-bind-b}
	\avmtmp{[arg-st & <NP$_{i, - ana}$, \ldots, NP$_{i, + ana}$, \ldots >]
	}
}
\ex[*]{\label{a-st-bind-c}
	\avmtmp{[arg-st & <XP$_{j}$, \ldots, NP$_{i, +ana}$, \ldots >]
	}	
}
\zl
...
HPSG's approach to lexical knowledge is quite similar to that of \isi{Categorial Grammar} (to some degree this is due to HPSG's borrowing from Categorial Grammar important aspects of its treatment of subcategorization). As in HPSG, the combinatorial potential of words is recorded in lexical entries so that two distinct contexts of occurrence correspond to two distinct entries. The difference from HPSG lies in how lexical entries relate to each other. In many forms of Categorial Grammar (be it Combinatorial or Lambek-calculus style), relations between entries are the result of a few general rules (e.g., type raising, function composition, hypothetical reasoning, etc.) (see \citealt{Dowty1978} for an approach that countenances lexical rules, though).
...
The assumption is that those rules are universally available; however, those rules may be organized in a type hierarchy and an individual language might avail itself of only a portion of this hierarchy, as in \citet{Baldridge2002}. Relations between entries in HPSG can be much more idiosyncratic and language-specific. We note, however, that nothing prevents lexical rules constituting a part of a Categorial Grammar (see \citealt{Carpenter1992b}), so that this difference is not necessarily qualitative, but concerns how much of researchers' efforts are typically spent on extracting lexical regularities; HPSG has focused much more, it seems, on such efforts.
...
Appeals to similar mechanisms feature prominently in Generative Lexicon accounts of lexical semantics, for example.
Both the lexical typing structure and qualia structures within GL, in particular the formal quale, have values situated in type hierarchies \citet{PustejovskyandJezek1996} and
...
[head 
  ...
  [\ldots] ]
\end{forest}
\caption{\label{pos-hier}A hierarchy of types of \type{head}}
\end{figure}

Each of these types is a partial specification of a lexical entry's head properties. Typing of \feat{head} information allows the ascription of appropriate properties to different classes of lexical entries. For example, case information is only relevant to nouns and adjectives, and whether a verb is an auxiliary or not is only relevant to verbs. 
The subtypes of \type{head} in (\ref{pos-hier}) allow us to define additional specifications of the properties appropriate for different parts of speech.
For example, lexical entries with a \feat{head} value of \type{noun} contain an attribute for \feat{case}, while those for \type{verb} contain the attributes \feat{aux}, \feat{tense}, and \feat{aspect}, as shown in (\ref{pos-def}) (we use implicational statements in (\ref{pos-def}) to indicate feature appropriateness conditions for the types \type{noun} and \type{verb} for perspicuity only; such conditions would be part of the grammar's signature, see \crossrefchaptert{formal-background}). \is{signature}
...
\begin{exe}
	...
	\begin{xlist}
		\ex\label{pos-def-a}
		If the attribute \feat{case} is an attribute within a lexical entry's \feat{head} value, then the value of \feat{head} is of type \type{noun}.
		\ex\label{pos-def-b}	
		If the attributes \feat{aux}, \feat{tense}, or \feat{aspect} are attributes within a lexical entry's \feat{head} value, then the value of \feat{head} is of type \type{verb}.
	\end{xlist}
\end{exe}
...
Thus, the pronoun \word{him} need only include the fact that its \feat{head} is of type \word{noun}; the fact that it might bear case can be inferred. 
Similarly, the entry for the verb \word{can} need only include the head information \avmtmp{[aux & $+$]} for us to be able to infer that it is a \type{verb} (assuming \feat{aux} is not an appropriate attribute for another type).
...
In other words, we take advantage of HPSG's \isi{feature geometry} and of the hierarchies of types appropriate for a particular substructure within signs to express generalizations as ``locally'' as possible (see \crossrefchaptert{formal-background}).

Implicational statements \is{implicational constraints} can serve well for expressing generalizations as ``locally'' as possible; they constrain the range of possible values of attributes and can stipulate structure sharing among them.
...
\ex 
\begin{xlist}
\ex If a \type{category} object has a \feat{head} of type \type{prep}, then the value of its \feat{val|comps} is a list that contains at most one element.
\ex \avmtmp{
[\type*{cat}
head & prep ]~\impl~
[val|comps & elist $\vee$ < \type{synsem} >]
}
\end{xlist}
\end{exe}
...
\ex If a \type{synsem} object's \feat{cat|head} value is of type \type{verb}, and its \feat{cont} value is of type \type{cause-rel}, then its value of \feat{cont|act} is token identical to the index of the first element of its \feat{arg-st} list.
	...
	\avmtmp{[cat|head & verb \\
	cont & cause-rel ]}
	\impl \avmtmp{[cont & [act & \1] \\
		arg-st & <NP$_{\1}$, \ldots>]
	}
	\end{xlist}
\end{exe}
...
Both of these implicational statements \is{implicational constraints} apply to a verb with a \feat{cont} value of type \type{caused-motion-rel}.
...
Drawn from \citew{AckermanandWebelhuth1998}, this sample analysis concerns \ili{German} passives, \is{passives} which come in several varieties, each with its own constraints.
...
In summary, the hierarchical lexicon is deployed here to factor out the differing properties of the various \ili{German} passive constructions, each of which includes its particular combination of properties via multiple inheritance.
...
They explore the properties of their system, called \is{YADU} YADU, in considerable detail.
...
In (\ref{def-verb-hier}), the most general type \type{verb} stipulates the identity of the past participle and passive participle forms as nondefault information.
...
As an example of the use of default, nonmonotonic inheritance outside of morphology, consider the account of the syntax of gerunds in various languages developed by \citet{Malouf2000a}. \is{gerunds}
% TD, 27 July: this citation comes out wrong in the PDF; not sure why, and don't know about citet
...
Instead of such unmotivated syntactic rules, Malouf posits a lexical rule, which converts the lexical category of a verb to \type{noun}, but otherwise preserves its verbal properties, such as subcategorization.
...
Malouf then specifies default \feat{head} values for the lexical classes \type{n} and \type{v} (see (\ref{v-def}) for the latter's definition). As gerunds have both properties of nominal and relational heads, they are subtypes of both, as shown in the multiple inheritance hierarchy in Figure~\ref{ger-hier}. The \type{v} type, which concerns us here, has a default \feat{head} value \type{verb}, as shown in (\ref{v-def}) in addition to the non-default, more general type \type{relational} it also includes (default information follows $/$).
...
But, as its \feat{head} is also a subtype of \type{noun}, it lacks a \feat{subj} attribute and instead has a \feat{spr} attribute.
...
Given that many nouns besides gerunds -- nominalizations, for example -- are relational (that is, have a \feat{content} value of type \type{psoa}), it could be difficult to model a hypothetical language that permits only the anti-gerunds rather than the normal ones.

Malouf further notes a key difference between gerunds and exceptions like *\word{childs}/\word{children}: \ili{English} gerunds are productive (and completely regular morphologically).
...
In a slight variant of the representation of lexical rules proposed by Copestake \& Briscoe and Meurers, the \feat{out} attribute can be dispensed with; the information in the lexical rule type that is not within the \feat{in} value then constitutes the output of the rule. The difference between the two representations with only the attributes \feat{synsem} and \feat{phon} included for expository purposes is shown in (\ref{two-lex}).
...
In the variant in (\ref{two-lex-b}), lexical rules are treated as subtypes of a \type{derived-word} \is{derived word} type, which can combine with other types in the lexical hierarchy, merely adding the derivational source via the \feat{in} value. 
Formulated in either fashion, lexical rules are essentially equivalent to unary syntactic rules, with the \feat{in} attribute corresponding to the daughter and the \feat{out} attribute to the mother (or the rest of the information in the rule, if the \feat{out} attribute is done away with). This is the way lexical rules are implemented in the \ili{English} Resource Grammar (see \url{http://www.delph-in.net/erg/} for demos and details about this large-scale implemented grammar of \ili{English}) as well in the CoreGram Project and the Grammix grammar development environment  (see \citealt{MuellerGrammix} and \url{https://hpsg.hu-berlin.de/Software/Grammix/} for details on the Grammix software) \crossrefchapterp{cl}.
...
Is there inherent directionality, for example, between the causative and inchoative alternants of verbs such as \word{melt} or \word{slide} or between the ditransitive and prepositional object frames of verbs such as \word{give}, as \citet{Goldberg1995} asks?
...
[synsem
  ...
  [non-canon-ss
    ...
    [aff-ss]]]	
\end{forest}
\caption{\label{synsem} Types of \type{synsem}}
\end{figure}
...
\begin{exe}
	...
	\begin{xlist}
		...
		\glt `I must go there.'
	\end{xlist}	
\end{exe}\todosatz{redraw figure~\ref{all-hier} so that lines go downwards, lines should not
  enter/overlap with boxes, use uniform style for this stuff so that previous figures are in the
  same layout}
...
One is by creating a hierarchy of subtypes of \type{word}, each with associated constraints.
