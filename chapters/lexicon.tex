...
Chaves' analysis assumes that the phonology of compound words and words that contain affixoids (to borrom a term from \citealt{Booij2005}) is structured. The MorphoPhonology or \attrib{mp} attribute of words (and phrases) is a list of phonological forms and morphs information. The \attrib{mp} of compound words and words that contain affixoids includes a separate member for each member of the compound, or for the affixoid and stem.  Thus in (\ref{ch08}), the \attrib{mp}s of \word{overapplication} and \word{underapplication} each contain two elements: one for \word{over}/\word{under}, and one for \word{application}. Given this enriched representation of the morphophonology of words like \word{under/overapplication}, a single ellipsis rule can apply both to phrases and to composite words, eliding the second member of the word \word{overapplication}'s \attrib{mp}. As Chaves (p.\,304) makes clear, such an analysis is fully compatible with lexical integrity, as there is no access to the \is{internal structure of words} internal structure of composite words, only to the (enriched) morphophonology of the entire word.
...
Likewise, the \attrib{arg-st}  \is{ARG-ST} list, which we included in the categorial information of signs in (\ref{va-lx}) and (\ref{y-va-lx}), might be regarded as a feature confined to lexical signs (see, among others, \citealt{GinzburgandSag2001}), on the premise that lexical items alone specify combinatorial requirements (but see \citet{Przepiorkowski2001} for a contrary view, and see \crossrefchaptert{chap-cxg} for other views questioning this assumption).
...
Different classes of words are thus reified as subtypes of \type{word} in the hierarchy, and all lexical items that belong to that subtype inherit its constraints.
...
The information within \attrib{head}, for example, is grouped there because it is all subject to the Head Feature Principle (cross ref here).
...
Informally, the linking constraints we wish to capture are:\is{linking}\is{argument realization}
...
First, we require that the causer be linked to the subject (the first element of \attrib{arg-st}:
...
Then, we link the moving entity in a caused motion verb to any NP on \attrib{arg-st}:
...
\begin{exe}
\ex \begin{xlist}
If a \type{synsem} object's \attrib{cat $|$ head} value is of type \type{verb}, and its \attrib{cont} value is of type \type{self-move-rel}, then its values of \attrib{cont | act} and \attrib{cont | und} are token identical.
	\ex
	\begin{avm}\[cat$|$head & verb \\
...
\tikzset{level 1/.style={level distance=3.5cm}}
\tikzset{level 2/.style={level distance=1.5cm}}
\Tree
...
Example (\ref{bar}) (Riehemann's (1)) illustrates \word{-bar} suffixation \is{-bar suffixation} \is{derivational morphology} in German, a process by which an adjective that includes a modal component can be derived from verb stems (similar to English \word{-able} suffixation). 
...
A similar use of lexical rules to model alternative realizations of arguments can be found in \citew{Monachesi1993}, \is{Romance clitics} who analyzes alternations between complements and pronominal object affixes (traditionally called object clitics) in Italian in a way that parallels the French examples in (\ref{va}).$\bigcirc$ in the rule, shown in ( \ref{cl-lr}), a.k.a. the ``shuffle'' \is{shuffle operation} operation, stands for the unordered concatenation of two lists, since any member of the input's \attrib{comps} list can be realized as a clitic and therefore not included in the output's \attrib{comps} list (see \crossrefchaptert{chap-order} for a more formal explanation of $\bigcirc$). In the output of her lexical rule, in (\ref{cl-lr}), a subset of the list of complements in the input (\avmbox{2}) corresponds to a list of clitic \attrib{synsem}s, realized as prefixes through inflectional rules not shown here. 
...
\citet{MillerandSag1997} \is{Romance clitics} make a similar use of non-canonical relations between the \attrib{arg-st} list and the valence lists, eschewing lexical rules to model French pronominal object affixes (traditionally called clitics) and proposing instead the constraint in (\ref{cl-wd}), where a subset of \attrib{arg-st} members, those that are realized as affixes (of type \type{aff}) are not also subcategorized for as complements. 
