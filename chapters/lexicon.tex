...
\itdopt{\citet{Goldberg2013a} hates them and we do not use them. We use what Goldberg calls
  ...
  JPK we agree but that's kinda irrelevant since we are talking about the origin of lexical
  rules. What Adele thinks in this context is not the point :)\\
Stefan: But for me it reads as if it is about HPSG and that we apeal to Jackendoff-style LRs and we do
not. We use what Adele calls templates. These are OK for her (and me) and the Jackendoff redundancy
rules are different.\\
JPK: Ah got it, added some text to make things clearer} 
...
JPK As said above, we clarified the beginning of this section, but since we talk about descrptions
as stored entries, this particular sentence is properly restricted :)\\
Stefan: Hm, I think we are talking past each other. I think there is an entry, stored for the stem
\stem{love} and there are derived forms \emph{loeves} and \emph{loves}. I would not call the latter
ones stored lexical items. Do you? This would explain your appeal to Jackendoff's notion of lexical rules.\\
JPK: I get that but it's irrelevant to what we are talking about I think. 
} and the fully instantiated objects that are being described, consider HPSG's model of subcategorization with reference to the relevant portion of the tree for sentence (\ref{expect-b}). 
...
Stefan: But the sentence immediately following (\mex{1}) is about the other direction: ``what it means to be a noun.'' And being a noun means that there has to be a CASE feature.\\
JPK: I see. Hmm, not sure. We go from properties to creating a sort, you go from a set up sort to the properties. Both are valid. We are using the properties to sorts direction because for readers it is better: properties is what you observe, sorts is what you ``induce''. }
\zl
...
\ealnoraggedright
\ex If a \type{category} object has a \feat{head} value of type \type{prep}, then the value of its \feat{comps} feature is a list that contains at most one element.
\ex \avm{
[%\type*{cat}
head & prep ] \impl
...
\itdobl{Stefan: There was another bummer in the figure. The second sharing is past and passp not pastp again. I fixed this.\\
I still do not understand why $\top$ is there. It does not add anything since all features can have $\top$ as the value since it is the most general type. But maybe it is OK the way it is. Readers get some idea what is going on and have to consult the paper to get the rest.\\
JPK Thanks for noticing the error. Yes, I agree it's a little cryptic but we can't really get into the rather elaborate details of the chapter. It's kinda like a preview.}
...
JPK I  wish I could take credit. But it was not me. I honestly don't know who did it this first. It could be Ann Copestake in one of her early 90s papers on polysemy?\\
Stefan: Then it was Riehemann. I checked the Copestake papers and she had IN and OUT or avoided it altogether.\\
JPK: We added a reference to A\&W just that people can see an example and one that pertains to an issue we discussed earlier} \citet[87]{AckermanandWebelhuth1998} employ this style of representation; their type \type{derived-lci} adds a \feat{lexdtr} attribute (equivalent to \feat{in}) that contains the input lexical entry's information. The difference between the two representations with only the attributes \feat{synsem} and \feat{phon} included for expository purposes is shown in (\ref{two-lex}).
...
In the variant in (\ref{two-lex-b}), lexical rules are treated as subtypes of a \type{derived-lexical-sign}\is{derived word} type, which can combine with other types in the lexical hierarchy, merely adding the derivational source via the \feat{in} value. 
Formulated in either fashion, lexical rules are essentially equivalent to unary syntactic rules, with the \feat{in} attribute corresponding to the daughter and the \feat{out} attribute to the mother (or the rest of the information in the rule, if the \feat{out} attribute is done away with). This is the way lexical rules are implemented in the \ili{English} Resource Grammar (see \url{http://www.delph-in.net/erg/} for demos and details about this large-scale implemented grammar of \ili{English}) as well in the CoreGram Project and the Grammix grammar development environment  (see \citealt{MuellerGrammix} and \url{https://hpsg.hu-berlin.de/Software/Grammix/} for details on the Grammix software). See also \crossrefchapterw[Section~\ref{cl:resources}]{cl} for remarks on implementations.
...
Stefan: The local$_1$ and local$_2$ worries me. It is like SpecCP which sometimes appears in trees of GB people. But it is just a way to talk about tree positions not a category.\\
JPK: I see. We could edit and have just variables like x and y. B ut then it's adapted from R. Can't win!}
...
\itddone{
JPK I am OK with page 18-- I checked. \\
Stefan: I made it 18--23, OK?}
...
\ea
\label{cl-wd}
\added{Constraints on words with clitics adapted from \citew[587]{MS97a-u}:}\\
\avm{
...
Stefan: Should be stable enough. But you can open a github issue.\\
JPK: Good idea. Now, I just need to remember how to do it, but that's good practice}). 
...
We thank Anne Abeill\'e for very helpful comments and Stefan Müller for so carefully reading and commenting on several versions of the manuscript  and helping
