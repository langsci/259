...
\itd{Stefan: Why is this VP rather than S?}
...
obtained is again the same as of an X constituent, what \citet[\page 752]{pullumzwicky} refer \added{to} as \isi{Wasow's Generalization}.
...
much less frequently  \citep{sagwasowbender,Sag:03}.\footnote{See \citet{Borsley2005a} for criticism
  ...
  coordinands. \added{ConjP is also discussed in \crossrefchapterw[Section~\ref{sec-coordination-minimalism}]{minimalism}.}}
...
%usually the prior one.\itd{Stefan: What does ``the prior one'' mean if there are three or more?}
\changed{usually the first one in a coordination of two items}.
...
\type{coord-phr\added{ase}}, assuming a lexical feature \textsc{coord} to distinguish between   coordination
...
\ex
\avm{
\emph{simple-coord-phr\added{ase}} \impl \\
	[dtrs & ne-list(<[coord & none]>) \+ ne-list([coord & crd ]) ]
}

\ex
\avm{
\emph{omnisyndetic-coord\added{-phrase}}  \impl \\
	[dtrs & ne-list([coord & crd ]) ]
}\label{omni}
\ex
\avm{
\emph{asyndetic-coord\added{-phrase}}  \impl \\
...
coordinates S nodes that are \textsc{mrkg} \type{that} (i.e.\ CPs \added{in the analysis of
...
\added{Head-Functor Schema:}\\
\avm{
\emph{head-func\changed{tor-phrase}} \impl
...
%\itd{Stefan: But is this an affix? Then it does not work as an argument for syntactic structures}
\glt `Lee and Kim'
\zl
...
\changed{arguments and hence listed in valence lists like \comps and \textsc{arg-st}}, then they are
...
\itdobl{Stefan: but (\ref{omni}) does not say that the \coord feature has to be the same for all
  coordinads. It says that the value has to be \type{crd}, so it can be any subtype. To make it the
  same, structure sharing and a relational constraint walking through a list would be required.}
...
must start with \emph{the} and a comparative phrase, as the oddness
of (\ref{ex-the-more-b}) shows,\itdobl{Stefan: I
  do not understand this. (\mex{1}b) is ungrammatical. It does not start with \emph{the} and a
  comparative phrase. Maybe you should say \emph{unlike (\mex{1}b)}.} and may involve a long
distance dependency (\ref{ex-the-more-c}). Each clause must be finite and allow for copula omission, as shown in (\ref{ex-the-more-d}).
...
[\type{construction}
  [\type{causality}
    ...
    [\type{declar-clause}
      [{\ldots{}}] 
      [\type{correl-\changed{phrase}}, name=correl
        [{\ldots{}}]
        [\type{symmetric-correl-\changed{phrase}}, name = sym ] ] ] ]
  [\type{headedness}, 
    [\type{non-headed-phr\added{ase}} 
        [{\ldots{}}]
        [\type{coord-phr\added{ase}}, name=coord]  ]
    [\type{headed-phr\added{ase}}
        [{\ldots{}}]
        [\type{head-adj-phr\added{ase}}, name=adj
          [{\ldots{}}]
          [\type{asymmetric-correl-\changed{phrase}}, name = asym ] ] ]    
        ] ] 
...
\ea
\label{ecc}
\type{asymmetric-c\changed{orrel}-\changed{phrase}} \impl\\ %\type{assymetric-correl-cx} \& 
\oneline{%
\avm{
[hd-dtr \1\\
...
the absence of  correlative marking.\itd{Stefan: I do not understand the
  explanation. \textsc{correl} is of type \type{correl} and \coord is of type \type{coord}, right?
  But then you say that \type{et} is a subtype of \type{compar} and \type{none} of
  \type{correl}. This seems to be mixed up. \type{none} and \type{et} should be subtypes of \type{coord}.}

\ea
\label{ccf}
\type{symmetric-c\changed{orrel}-\changed{phrase}} \impl\\ %\type{symmetric-correl-cx} \& 
\oneline{%
\avm{
...
Mispredictions also exist. We already discussed \changed{the example in (\ref{ex-did-you-make-and-your-asessable-income-is})}, concerning the feature \textsc{inv}, but there are others. For example, requiring that the \slashv of the coordinands be the same readily predicts \isi{Coordinate Structure Constraint} effects like 
...
\itd{Stefan: But the lower box on \emph{amount} stands for an index, doesn't it? The element in \slasch is not an index. And \emph{amount} is not a filler in this construction. I would add a footnote and crossref the relative clause chapter.}
\zl
...
\citep{anderson83}. This is illustrated  in (\ref{syn}) using examples by \citet[205]{levineetal} and \citet[75]{goodall87}, respectively.
...
`good'.  There are  HPSG
analyses that argue that what appears to be closest coordinand agreement is in fact agreement with
...
(CCA)\is{agreement!closest conjunct}.\itdopt{Stefan: Isn't the standard term \emph{Closest Conjunct
    Agreement}? Maybe introduce it here as such and then go on talking about coordinands.} 
...
be just that. The alternative developed by \citet{Villavicencio:Sadler:ea:05} assumes that
...
\ea
\avm{
\type{nom-coord-ph\added{rase}} \impl\\
...
example, verbs like \emph{remain} are compatible with both A\del{dj}P and NP complements, whereas
\emph{grew} is only compatible with A\skp\del{dj}Ps.  This is shown in (\ref{republican}).  Crucially,
...
Consider, for example, the non-constituent constructions\itdopt{Stefan: But why is the cover term for all three identical with the name for (\mex{1}a)?} in (\ref{lpecx}). 
...
\label{lines}
\label{schema-ac-cx}
\added{Argument Cluster Schema:}\\
\type{\changed{argument-cluster-phrase}} \impl\\ 
\avm{ [head|cluster < \1, \ldots{}, \tag{n} >\\
       dtrs < [ synsem \1 ], \ldots{}, [synsem  \tag{n} ] > ]
}
\z
\itdopt{Note: I expanded the type names since otherwise one could think ac-phrase is an abbreviation
  of asymmetric-correl used above.}

\noindent
Mouret defines argument clusters as instances of the underspecified non"=headed construction
\type{\changed{argument-cluster-phrase}} with one daughter or more. The construction is valence
...
This approach is motivated by non-clausal coordinators (\textit{as well as} \added{and its \ili{French} equivalent} \textit{ainsi que}), which are possible in Argument Cluster Coordination, but cannot conjoin tensed VPs:
...
In the examples that follow, small capital letters indicate prosodic focus\itd{Stefan: I am confused about the meaning of the term coordinand. Isn't much more coordinated then the stuff in small caps?} and material shared between both coordinands is delineated by square brackets.
...
\itdopt{Stefan: This is a very strange example. Why would one want to add these numbers? Did I get this right that Mia may have lost \$6000 and Fred spent \$4.000?}}
...
\emph{Robin likes Sam and Tim - Sue}.\itd{Stefan: Sometimes you use `--' and sometimes a lower dash
  as in (\mex{1}). This should be uniform.}
...
to account for Closest Coordinand Agreement. Finally, in some accounts, non-constituent coordination
...
We are thankful to Bob Borsley, \added{Jean-Pierre Koenig,} Stefan Müller, and other reviewers for comments and suggestions on earlier drafts. 
