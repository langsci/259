...
\inlinetodoobl{JP: This example is not referenced in the text not clear what it does.}
\eal
...
Although prosodic phrasing is not generally believed to always align with syntactic phrasing, the fact that the coordinator prosodifies with the  coordinand suggests that the former forms a unit with the latter.
 ...
 \avmtmp{[synsem|loc|cat & \1]}.

\ea \textsc{Coordination construction} (preliminary)
\avmtmp{
\type{coord-phr} \impl
	[\punk{synsem|cat}{\1} \\
...
\ex
\avmtmp{
\emph{simple-coord-phr} \impl \\
	[dtrs & ne-list (<[coord & none]>) \+ ne-list ([coord & crd ] ) ]
}

\ex
\avmtmp{
\emph{omnisyndetic-coord}  \impl 
	[dtrs & ne-list ([coord & crd ] ) ]
}\label{omni}
\ex
\avmtmp{
\emph{asyndetic-coord}  \impl 
...
\inlinetodoobl{JP: conjuncts or coordinants?}
...
\inlinetodoobl{JP: You need to introduce the term \emph{correlative} early on}
...
%\pagebreak
\inlinetodoobl{JP: suggested adding \emph{finite} here, but Stefan thinks that verbs subcategorize
  for NPs with structural case. The point is that no case is specified in the valence frames. So
  what seems correct here is: \emph{Finite verbs assign structural nominative to their subjects and
    structural accusatives to their objects.}}
...
In case of conjuncts with conflicting agreement values, various resolution strategies are observed 
...
masculine, at least for animates \citep[186]{Corbet91}. This is illustrated in (\ref{pt}) for Portuguese taken from or based on examples by \citet*{Villavicencio:Sadler:ea:05}.
\eal
\label{pt}
\ex 
...
In the following examples, again from \ili{Portuguese} and taken from \citew{Villavicencio:Sadler:ea:05}, the determiner and prenominal adjective agree with the first noun (\ref{fo}a) and the postnominal adjective with the last noun (\ref{fo}b).
...
\footnotetext{
\citep[\page 17]{Abeille:An:Shiraishi:18}
}
...
among  others; see \citet{Chaves2013b-u}\addpages.
...
\eal
\label{unlk}
\ex Tom gave a book to Mary, and \sout{gave} a magazine to Sue.\footnote{\citep[\page 171]{Chaves2013b-u}}

\ex He drinks coffee with milk at breakfast and \sout{drinks coffee with} cream in the evening.\footnote{\citep[\page 214]{hudson84}}
...
\eal
\label{baaad}
\ex Simultaneously shocked and in awe, Fred couldn't believe his eyes.\footnote{\citep[\page 172]{Chaves2013b-u}}
\ex  Both tired and in a foul mood, Bob packed his gear and headed North.\footnote{\citep[\page 112]{chaves06}}
\ex Both poor and a Republican, \emph{no one} can possibly be. \footnote{\citep[\page 172]{Chaves2013b-u}}
\ex  Dead drunk and yet in complete control of the situation, \emph{no one} can be.\footnote{\citep[\page 142]{levine11}}
\zl
...
\avmtmp{
[phon & \1 \+ \2 \+ \0 \+ \3 \\
 dtrs &	< [phon & \1 \+ \2\subscr{ne-list} \\
	   \punk{synsem|loc|cat|coord}{none}],
	  [phon & \0 <(\type{coord})> \+ \1 \+ \3\subscr{ne-list} \\
...
\oneline{%    
\begin{forest}
[\avmtmp{VP [phon & \1 \+ \2 \+ \0 \+ \3]} 
	[\avmtmp{VP [phon & < \type{give} > \+ \2 < \type{a, book, to, Mary} > ]}]
	[\avmtmp{VP [phon & \0 < \type{and} > \+ \1 < \type{give} > \+ \3 < \type{a, magazine, to, Sue} > ]}
	[Coord]
	[VP] ] ]
\end{forest}
}
...
for more data and discussion.\footnote{%
  ...
  is semantically vacuous) with \emph{which}. 
\inlinetodoobl{JP: \emph{that} is not part of the example.}
\citet[17]{steedmanbook} argues that Right-Node
...
\ex  It is neither \textsc{un}- nor \textsc{overly} [patriotic] to tread that path.\footnote{\citep[\page 267]{chaveslp}} 
\ex The \textsc{ex-} or \textsc{current} [smokers] had a higher blood pressure.\footnote{\citep[\page 267]{chaveslp}} 
...
% }
\ex[*] {John \textsc{will} and Sandy \textsc{built the} [drive].\footnote{\citep[\page 936]{Milward94}}
}
...
%contain examples}
\ex[*] {Robin \textsc{swung}  and Leslie \textsc{tamed} [an unusual bat].\footnote{\citep[156]{levhubook}}}%OK
\ex[*] {We need new \textsc{black}- and \textsc{floor}[boards].\footnote{adapted from \citealt[\page 371]{artstein5})}}
\ex[*] {I caught \textsc{butter}- and \textsc{fire}[flies].\footnote{\citep[\page 274]{chaveslp}}}
\ex[*] {There stood a \textsc{one-} and \textsc{well}-[armed man].\footnote{\citep[\page 869]{chavesrnr}}}
\zl
...
\ex  It's interesting to compare the people who \textsc{like} with the people
       who \textsc{dislike} [the power of the big unions].\footnote{\citep[550]{hudson}}%OK

 \ex Anyone  who \textsc{meets} really comes to \textsc{like} [our sales people].\footnote{adapted from \citealt[\page 267]{williams}}\label{will}

\ex   Spies who learn \textsc{when} can be more valuable than those
able to learn \textsc{where} [major troop movements are going to occur].\footnote{\citep[\page 101]{postal94}}

\ex Politicians who have fought \textsc{for} may well snub those
 who have fought \textsc{against} [chimpanzee rights]. \footnote{\citep[\page 104]{postal94}}
...
\ex If there are people who \textsc{oppose} then maybe there are also some
  ...
  workers].\footnote{\citep[\page 840]{chavesrnr}}
...
\avmtmp{
	...
	\avmtmp{
		S [qud & \1 \\
		content & \{ \0 $\exists e'$!(\type{like}(\type{robin,sam}))! \}]
	}]
	[%
	\avmtmp{
		S [qud & \1 \\
		content & \{ \0 $\land \exists e$!(\type{like}(\type{tim,sue}))! \}]
	}
		[Conj [and] ]
		[%
		\avmtmp{
			...
			content & \{$\exists e$!(\type{like}(\type{tim,sue}))! \} ]
  			...
  			[\avmtmp{NP [content & \{$tim$ \}]}]
			[\avmtmp{NP [content & \{$sue$ \}]}] ] ] ]
\end{forest} 
