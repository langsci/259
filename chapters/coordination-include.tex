...
See \citet{borsley94,Borsley2005a}\todostefan{check all these} 
% ok: Borsley2005a 
and 
...
feature in \citet{Bonami:2004} for \ili{French} \isi{liaison}). Assuming a degree word \textit{the}, which can only appear as a specifier of
...
\citet[843--844]{chavesrnr} and \crossrefchaptert[Section~\ref{sec-ellipsis-rnr}]{ellipsis}). In French, the first conjunct cannot end with a clitic article or with a weak preposition as in (\ref{fr1}b,c), quoted from \citep[\page 14]{Abeille:06}.
...
[VP
  ...
  [{XP\ibox{1}[\coord+, \textsc{cluster} \ibox{2} \sliste{ NP, PP }]}
...
\end{figure}
The respective NPs and PPs form a cluster that is licensed by (\ref{schema-ac-cx}). The phrases \emph{a book to Mary} and \emph{a magazine to Sue} are coordinated and the respective \textsc{cluster} values identified \iboxb{2}. The lexical item for \emph{give} is licensed by the lexical rule in (\ref{lr-cluster-selection}). This version of \emph{give} selects the cluster coordination rather than selecting the NP and PP directly.
