...
verbs require their arguments to be \textsc{crd}$-$, the grammar does not allow *\emph{Sam likes and Sandy}, so the \emph{and Sandy} must be combined with X$_{crd-}$ by the second rule to allow \emph{Kim and Sandy}. Since the mother of (\mex{0}b) is not marked for its
\textsc{crd} value, it can enter both into (\mex{0}a) and (\mex{0}b). If it enters into (\mex{0}b),
\emph{Robin, Kim, and Sandy} is licensed, which is underspecified for \textsc{crd} and therefore can
function as an argument. If it enters into (\mex{0}a) instead, \emph{and Kim and Sandy} is licensed, which is again \textsc{crd}$+$, and requires another application of (\mex{0}b) in order to license \emph{Robin and Kim and Sandy}.
...
\ili{Spanish}, they must refer to the same event (see also \citealt{Bosque1987a-u}).
%\todostefan{Stefan: Could not find paper. Please add page numbers. answer:I dont have it}
% Was the wrong paper, is in the big grammar.
