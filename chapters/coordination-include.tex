...
It  preserves the \textsc{marking} feature when conjuncts are themselves marked. The conjunction takes the adjacent conjunct as a complement. This captures its being first in head initial languages like \ili{English}, and its final position in head final languages like \ili{Japanese}.
\end{sloppypar}

\inlinetodostefan{Stefan: There is a bracket too much in (\mex{1}a).}
\eal
...
facts as the unacceptability of  (\ref{ex-case-match-coordination}).\footnote{There are nonetheless collocational cases where the distribution of pronouns defies this pattern, due to presumably prescriptive forces; see \citet{grano} and \citet{binomial}.}
\inlinetodostefan{Stefan: \citet{binomial} is a book. I tried to find the place you refer to. I could not find it. Please provide a page number.}
...
due to \citet[205]{levineetal} and \citet{Goodall87a-u}\addpages, respectively. 
...
%\todostefan{Chaves:Diss:33 attributes this to Goodall87a}
\zl
...
\crossrefchapterw[Section~\ref{agreement-sec-index-concord}]{agreement}). \citet{Moosally}\todostefan{Stefan:
  Do not have this paper, please add page numbers; XY: dont have it either, sorry} proposes an account
...
two lexical verbs, sharing a preverbal clitic in French, must share the same verbal root, and in
Spanish, they must refer to the same event \citep{Bosque:86}.\todostefan{Stefan: Could not find paper. Please add page numbers. answer:I dont have it}
...
\citet[843--844]{chavesrnr} and \crossrefchaptert{ellipsis}\todostefan{To Stefan: add a label to the page/section in ellipsis.tex}). In French, the first conjunct cannot end with a clitic article or with a weak preposition as in (\ref{fr1}b,c), quoted from \citep[\page 14]{Abeille:06}.
...
\inlinetodostefan{Stefan: I found the Africa example in \citet[\page 1327]{rodney}. The
  \emph{republican} is in Bayer. Where are the other examples from? ``among others'' is sloppy.}
\begin{exe}
\ex \begin{xlista}
...
\ex
\begin{xlista}
\ex Tom gave a book to Mary, and \sout{gave} a magazine to Sue.\\\citep{bookivan}\addpages 
...
\flushright\avmtmp{
[comps  < [coord & $+$ \\
 cluster & <[loc|cat \1], \ldots{}, [loc|cat \tag{n}]> ] >]
}
\z
\begin{figure}
\begin{forest}
  ...
  [XP
    ...
    [XP
      ...
      [XP
        ...
        [PP [to Sue, roof]]]]]]
\end{forest}
\caption{\red{Add a caption}}\label{fig-give-a-book-to-mary-and-a-mag-to-sue}
\end{figure}
...
Like RNR, Gapping is not restricted to coordinate structures as \citegen[\page 30--31]{sangheepark} attested 
examples in (\ref{gnc}) illustrate, contrary to widespread assumption.   Thus, the Gapping rule
proposed by \citet[\page 125]{sangheepark}\todostefan{Stefan: Rui please check page nubmer}  that allows a gapped clause to follow a non-gapped clause, is not
