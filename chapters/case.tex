...
On the basis of \ili{German} examples such as~\rref{ex:unterstutzen}–\rref{ex:helfen}, \citet{HM94a} argue that out of four morphological cases in German – nominative, accusative, genitive and dative — the first three (i.e., with the exception of the dative) may be assigned structurally, by general case assignment principles.  Similarly, they argue that the last three (i.e., apart from the nominative) may also be assigned lexically, in which case they are stable across various syntactic environments.  These empirical observations are translated into the following \ftype{case} hierarchy:
\begin{examples}
\item\label{ex:case:germ}
...
\item \label{ex:hmsubcats}
  \begin{examples}
  \item \textsc{unterst\"utzen}: [\path{subcat}~\la{}NP[\emph{str}],
    NP[\emph{str}]\ra{}] 
    \item \textsc{helfen}: [\path{subcat}~\la{}NP[\emph{str}], NP[\emph{ldat}]\ra{}]
  \end{examples}
\end{examples}
...
\item \label{ex:islsubcats}
  \begin{examples}
  \item \textsc{elska}: [\path{subcat}~\la{}NP[\emph{str}], …\ra{}] 
    \item \textsc{vanta}: [\path{subcat}~\la{}NP[\emph{lacc}], …\ra{}]
  \end{examples}
\end{examples}
...
\item \label{hm:ea}
  \notion{Syntactically External Argument (`Subject')}: \\ [1.5ex]
...
\item \label{hm:ia}
  \notion{Syntactically Internal Argument (`Direct Object')}: \\ [1.5ex]
    ...
    \fs{
      \ft{head}{
        \fsl{verb}{
          \ft{vform}{fin}}} \\
      \ft{subcat}{\la\ra}}}
  \newsavebox{\fsboxfindtrs}
  \savebox{\fsboxfindtrs}{
    \fsl{h-c-str}{
      \ft{head-dtr$|$\ldots$|$subcat}{\la{}\textnormal{NP[\ftype{str}]}, \ldots \ra}}}
  \newsavebox{\fsboxverbcat}
  \savebox{\fsboxverbcat}{
    \fs{
      \ft{head}{verb} \\
      \ft{subcat}{\la\ra{} $\vee$ \la{}synsem\ra}}}
  \newsavebox{\fsboxdtrs}
  \savebox{\fsboxdtrs}{
    \fsl{h-c-str}{\ft{head-dtr$|$\ldots$|$subcat}{\la{}synsem, \textnormal{NP[\ftype{str}]}, \ldots \ra}}}
  \newsavebox{\fsboxnouncat}
  \savebox{\fsboxnouncat}{
    \fs{
      \ft{head}{noun} \\
      \ft{subcat}{\la\ra{} $\vee$ \la{}synsem\ra{}}}}
\item \label{ex:cons:snom}
  \fsl{phrase}{
    \ft{synsem$|$loc$|$cat}{\usebox{\fsboxfincat}} \\
    \ft{dtrs}{\usebox{\fsboxfindtrs}}} $\implies$ \\ [2ex]
  \hspace*{\fill}
  \fs{
    \ft{dtrs$|$head-dtr$|$\ldots$|$subcat}{\la{}\textnormal{NP[\ftype{snom}]}, \ldots \ra}}
\item \label{ex:cons:sacc}
  \fsl{phrase}{
    \ft{synsem$|$loc$|$cat}{\usebox{\fsboxverbcat}} \\
    \ft{dtrs}{\usebox{\fsboxdtrs}}} $\implies$  \\ [2ex]
  \hspace*{\fill}
  \fs{
    \ft{dtrs$|$head-dtr$|$\ldots$|$subcat}{\la{}synsem, \textnormal{NP[\ftype{sacc}]}, \ldots \ra}}
\item
  \fsl{phrase}{
    \ft{synsem$|$loc$|$cat}{\usebox{\fsboxnouncat}} \\
    \ft{dtrs}{\usebox{\fsboxdtrs}}}  $\implies$ \\ [2ex]
  \hspace*{\fill}
  \fs{
    \ft{dtrs$|$head-dtr$|$\ldots$|$subcat}{\la{}synsem, \textnormal{NP[\ftype{sgen}]}, \ldots \ra}}
\end{examples}
...
\begin{examples}
\item\label{ex:syn:pc} \ili{Polish} coordination \citep[701–702]{Dyla84}:
  \begin{examples}
  \item 
    \begin{gloss}
  ...
  \end{examples}
\item\label{ex:syn:ec} \ili{English} coordination (\citealt[70]{Goodall87a-u}, \citealt[206]{LHC2001a-u}): \\ [1ex]
  This is the man who$_i$.\textsc{nom/acc} Robin saw $e_i$.\textsc{acc} and thinks $e_i$.\textsc{nom} is handsome.
\item\label{ex:syn:gc} \ili{German} coordination \citep[764–765]{PZ86}:
  \begin{examples}
  \item
    \begin{gloss}
  ...
  \item
    ...
    ‘She finds and helps men.’\hfill(intended)
  \end{examples}
\item\label{ex:syn:gfr} \ili{German} free relatives \citep[212]{GR81}: \\ [1ex]
  \begin{gloss}
...
\item\label{ex:syn:epg} \ili{English} parasitic gaps (\citealt[482]{HL96a}, \citealt[205]{LHC2001a-u}): \\ [1ex]
...
\begin{examples}
\item\label{ex:dis:pl} \ili{Polish} \citep[175]{Prze99b}: \\ [1ex]
  \begin{gloss}
    Dajcie wina i całą świnię! \\
    give wine\Gen{} and whole\Acc{} pig\Acc{}
  \end{gloss}\\[\glosslen]
  ‘Serve (some) wine and a whole pig!’
\item\label{ex:dis:ru} \ili{Russian} \citep[11]{levy:01}: \\ [1ex]
  \begin{gloss}
...
\item\label{ex:dis:pl:again}
  ...
  ‘Serve (some) wine and a whole pig!’
\end{examples}
As these examples involve accusative and genitive, we will assume that the complete case hierarchy contains a~subhierarchy such as~\rref{daniels20} above, but with all occurrences of \ftype{nom} replaced by \ftype{gen}:
\begin{examples}
\item
\begin{tikzpicture}[text height=1.5ex,text depth=.25ex,
  % inner sep=2pt,
