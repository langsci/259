...
in failure.\footnote{See \citealt[196]{Ingria90} for an earlier implementation of roughly the same
  idea in the context of unification grammars.}  Specifically, assuming that nominative and
accusative are structural cases in \ili{English}, they propose the following part of the structural
case hierarchy.\footnote{Type names follow the convention in \citealt{dani:01}, for increased
  uniformity with the remainder of this section.} The enriched case hierarchy is shown in Figure~\ref{fig:daniels18}.
%\pagebreak
\begin{figure}
...
\caption{Add caption}\label{fig:daniels18} 
\end{figure}
% \begin{verbatim}
%              scase
%             /     \
...
in~\rref{daniels18}, represents forms which are simultaneously accusative and nominative, the common
...
types.  This is proposed and illustrated in detail in \citealt{sag:02}.  A~more conservative
solution, proposed in \citealt{dani:01}, is to add dedicated maximal types to all such non-maximal
types; for example, the hierarchy in Figure~~\ref{fig:daniels18} becomes Figure~\ref{fig:daniels20}.
\begin{figure}
\begin{tikzpicture}[text height=1.5ex,text depth=.25ex,
  % inner sep=2pt,
  node distance=4em,
...
contains a~subhierarchy such as Figure~\ref{fig:daniels20} above, but with all occurrences of
...
\end{tikzpicture}
\caption{Add caption}\label{fig:case4}
\end{figure}
...
(\citealt[272]{sag:02}, \citealt[63–66]{dal:kin:sad:09}).  Let us, following \citealt{dani:01},
simplify the presentation of type hierarchies such as Figure~\ref{fig:daniels20} in the way shown in
Figure~\ref{fig:case5}, by removing all those “pure” types which are only needed to represent some non-maximal types as maximal:
\begin{figure}
\label{daniels21} 
\begin{tikzpicture}[text height=1.5ex,text depth=.25ex,
  % inner sep=2pt,
...
\end{tikzpicture}
\caption{Add caption}\label{fig:case5}
\end{figure}
Hence, the above representation corresponds to seven types shown explicitly in~\rref{daniels20}
(each non-maximal type in~\rref{daniels21} has an additional \ftype{p-} type, while the maximal
\ftype{nom-acc} in Figure~\ref{fig:daniels21} is the same as \ftype{p-nom-acc} in~\rref{daniels20}).
What would a~similar hierarchy for three morphological cases look like?  \citealt[143]{dani:01}
provides the following visualisation, involving 18 nodes corresponding to 35 types in the full type
hierarchy in Figure~\ref{fig:daniels22}.
%\pagebreak
\begin{figure}
