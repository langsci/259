...
\itdopt{Shouldn't this be agentive? Like ``helping'' rather than ``help''?}
\zl
...
Such examples may still be handled without any general principles of case assignment.  For example, lexical rules \citep[209–218]{ps} responsible for forming passive participles (as in the b.~examples above) and \rep{gerunds}{nominalisations}\itdopt{German does not have gerunds. Just checked Wikipedia again to be on the safe side \ldots} (as in the c.~examples) might be responsible for manipulating case values of arguments, e.g., for translating nominative and accusative – but not dative – to genitive in the case of \rep{gerunds}{nominalisations}.  However, the interaction of the structural/inherent case dichotomy with raising (and – in some languages – with control) motivates a~more comprehensive approach to case assignment.
...
\gll \emph{Hann} virðist elska hana. \\
     he.\NOM{} seems love.\INF{} her.\ACC{}\\\hfill\added{(Icelandic)}
...
\gll  \emph{Barninu} virðist hafa batnað veikin. \\
      the.child.\DAT{} seems have.\INF{} recovered.from the.disease\\\hfill\added{(Icelandic)}
...
\gll  \emph{Verkjanna} virðist ekki g\ae{}ta. \\
      the.pains.\GEN{} seems not be.noticeable.\INF{}\\\hfill\added{(Icelandic)}
...
\end{tikzpicture}
\caption{\added{\citegen[\page 207]{HM94a}} case hierarchy for German encoding the structural/lexical distinction}
\label{fig:case:germ}
\end{figure}
\itdopt{Note: The figure differs from what is in the paper. Stefan will redo it with type hierarchy style for forest.}
...
\itdopt{I deleted \type{phrase} from all antecedents. First because it is redundant and second since it was not in the original.}
\z
...
\gll Liisa muisti matkan vuoden. \\
     Liisa.\NOM{} remembered trip.\ACC{} year.\ACC{}\\\hfill\added{(Finnish)}
...
While these generalisations are developed in the context of \isi{Lexical Functional Grammar}, it is
...
an unordered set.\footnote{But see \citet{Prze2016a} for an attempt to introduce a~single ordered
  list of dependents and formalise the functional hierarchy in LFG\@.}\is{Lexical Functional
  Grammar|)} On the other hand, given the “adjuncts as complements” approach of \citet{BMS2001a} and
others, upon which at least some adjuncts are added to \path{arg-st} (perhaps renamed to
\path{deps}), and assuming – as is standard in HPSG – that \path{arg-st} elements satisfy the
obliqueness hierarchy, formalisation of the “case tier” approach is easy and consists of two
implicational \rep{principles}{constraints} similar to \rref{nonc:cp:germ1}–\rref{nonc:cp:germ2}.
The first \skp[3]\rep{principle}{constraint} resolves the first structurally-cased element of \added{the}
extended \path{arg-st} to nominative, whether this element is the first element of \path{arg-st} or
not (it is not in the case of \rref{obj}–\rref{adv2}), and whether it corresponds to the subject,
the direct object or an adjunct.  The second \rep{principle}{constraint} resolves the structural
case of all subsequent elements, if any, to accusative.  \itdopt{I replaced principle by constraint
  since the whole set of implicational constraints is the principle, isn't it? We do not have three
  case principles, but one consisting of three implicational constraints.}
\itdopt{This approach allowing for the first element with structural case being not the first element in the list is what I suggested in \citew{Mueller2002b}. I argue that in the remote passive the \argst list may start with a dative so that the raised object is the first element with structural case although being in second position (p.\,157). The same works for Icelandic in \citew{MuellerGermanic}. It would be great if you could add this somewhere here.}
...
\itdopt{This seems to be the place for crossreferencing \crossrefchaptert{udc} and \crossrefchaptert{relative-clauses}, but I cannot find a plausible way to do it. If you find one, please do.}
...
\itdopt{I will redraw this with forest. Shouldn't nominative be to the left. For me, nominative is first. Applies to other figures as well.}
\end{figure}
% \begin{verbatim}
%              scase
%             /     \
...
\zl
\itdopt{Mark the valence of give and expected as in the examples before?}
...
\changed{\rref{ex-case-underspecification}} may be seen as special cases.\footnote{Another HPSG
...
\added{See also the chapters by \citet{chapters/ellipsis} and \citet{chapters/coordination} for discussions of HPSG analyses of ellipsis and coordination, respectively.}}
...
governing verbs.  In particular, dependents of the subject are raised to \changed{the valence list
  for subjects} \path{\del{val$|$}subj}, resulting in multiple valency subjects.
\itdopt{I removed VAL for consistency in the volume.}
\itdopt{I find the term ``multiple valency subjects'' strange. Is this supposed ``multiple subjects''? Represented in valence lists? Maybe ``multiple subjects valency''?}  Configurational
...
to deny the existence of \rep{this}{the affix/postposition} distinction.  \citet{thui:11} shows that, in this case, the
...
\ili{Hungarian} case affixes and postpositions is couched within \isi{Sign-Based Construction
  Grammar} \citep{BS2012a-ed}.
