...
\fbox{MORPHOTACTICS} dimension on the left defines the conditions for
the corresponding placement constraints, whereas the \fbox{EXPONENCE}
...
position affixes. Sorani Kurdish  endoclitic agreement markers
surface after the initial morph, be it the stem, or some prefixal
...
\textit{da-} in position~3, which is the lowest \textsc{pc} index in
of the word. Accordingly \textit{=jân} is placed relative to the
prefix \textit{da-}, in position~4.
