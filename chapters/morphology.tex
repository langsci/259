...
\is{completeness} and \is{coherence} \citep{bresnan_j82}, as well as to the
...
Finally, a word's sequence of morphs, and hence its \isi{phonology}, will be obtained by shuffling
(\isi{$\bigcirc$}) the rules' \textsc{mph} lists in ascending order of position class (\textsc{pc})
indices (see Chapter~\crossrefchaptert[\page \pageref{rel-shuffle}]{order} for a definition of the
shuffle relation, also known as sequence union). This is ensured by the Morph Ordering
Principle\is{principle!Morph Ordering}
given in (\ref{ex-MOP}), adapted from \citet{Crysmann:Bonami:2016}.
...
Composing complex pairings of morphological form and function by means
of cross-classification of partial rule descriptions is not only
...
possible, generally yielding much more parsimonious descriptions.
...
integrate lexicalist and constructional views. \is{modularity|)}
...
their helpful comments and suggestions. Finally, I also would like to
express my gratitude to the type-setters who helped to make the
rendering of complex type hierarchies much more consistent and
visually appealing.
