...
properties, but not others.% \todostefan{redraw this. Dots touch lines, words touch lines, should be
  % STRUCTURE not STUCTURE}
...
\begin{figure}[htb]
  ...
  \begin{tabular}{ccccccc}
    ...
    \rnode{kill-act}{\colorbox[gray]{.7}{\textit{kill-lxm} $\wedge$ \textit{trans-lxm}}} &
    \rnode{kill-pass}{\colorbox[gray]{.7}{\textit{kill-lxm} $\wedge$ \textit{pass-lxm}}} &
    \rnode{resurrect-act}{\colorbox[gray]{.7}{\textit{resurrect-lxm} $\wedge$ \textit{trans-lxm}}} &
    \rnode{resurrect-pass}{\colorbox[gray]{.7}{\textit{resurrect-lxm} $\wedge$ \textit{pass-lxm}}}
...
morphology.\footnote{\citet{Blevins2003a} discusses the interaction
  ...
  that can scale up to more complex interactions.  } % \footnote{%
  % It should be noted that the OTC approach did not go unchallenged. A
  ...
  % causativization in Turkish, which can be applied multiple times
...
serious formal model of morphology, namely Paradigm Function
...
small part of the cells feature overt exponents. E.g. in the paradigm
of \textit{Oma} `granny' (Table~\ref{tab:ParaSplit}a), singular number
is solely expressed by the signifant absence of any
...
economic in that it saves restating the identity function for every
rule block. On the downside, a meta-level default, such as the IFD
...
nor polarity had a non-default value.  E.g. the rule for zero
exponence suggested in \citet{Crysmann:Bonami:2016} realises a
