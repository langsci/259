...
capturing the relatedness to subregular formations. 
...
pre-typing: in English for instance, some transitive verbs, like
possessive \textit{have} fail to undergo passivisation. Rather than
marking these verbs diacritically by exception features, pre-typing to the
...
pre-typing makes this type already a leaf type in both dimensions.
...
\inlinetodostefan{Blevins quote is wrong and has to be fixed.}
...
the bare form \textit{Brezen} `pretzel', for instance, has a wider
denotation than \textit{Rechner} just because the latter stands in
opposition to more forms.
...
pattern (here: zero exponence). Default forms, however, need not involve zero
...
`human(s)'), with the exception of the nominative singular (\textit{Mensch}
...
default and what is the override (cf., however, \citealt{Crysmann:Kihm:2018} for an
analysis of the Old French declension system).
...
Over the years, several different proposals have been made regardingthe
...
follows this line: e.in platforms like the LKB \citep{Copestake02},
...
\midrule 
\textsc{1sg} & ni-{ta}-tak-{a} & \textit{{si}}-{ta}-tak-{a}        & \textsc{1pl} & tu-{ta}-tak-{a}     & {ha}-tu-{ta}-tak-{a}\\
...
subregular types, e.g.\ \textit{sing/sang/sung} or
...
admits multiple exponents, realisation rules establish in fact $m:n$
relations between function and form: i.e. the many-to-many nature of
...
terms of exhaustion of the morphosyntactic property set.  
...
% not an appropriate characteristic of morphosyntactic property sets,
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex \textit{ge}-setz-\textit{t} `set/put'
...
limited to adjacency. For example,  relative negative \textit{si} is realised
...
small part of the cells feature overt exponents. For example in the paradigm
...
every inflectional dimension that witnesses zero exponence: for
instance the rule type in (\ref{ex:ifd}) could be specialised by
