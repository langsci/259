...
component --- e.g. the grammar fragment of \ili{English} presented in
...
\inlinetodoobl{JP: This does not show that we have recursion.}
they cite repetitive prefixation in \ili{German} as the decisive example
...
\begin{figure}[htb]
  ...
  \caption{\citegen[\page 64]{Riehemann98} type hierarchy of German \textit{-bar}
    derivation\inlinetodoobl{Riehemann has much more disjuncts and `\ldots'}}
  \label{fig:Riehemann}
\end{figure}
...
\inlinetodoobl{Stefan: I adapted feature geometry and added footnote due to problem pointed out by JP.}
\ea
...
book. \citet[\page 68]{Riehemann98} shares the whole semantic content between accusative object and
...
\inlinetodoopt{JP: Would be nice to refer to (\mex{1}) and (\mex{2}) in the text.}
\ea
	\avmtmp{
		[\type*{suffixed}
		phon & \1 \+ list \\
...
\begin{sloppypar}
  ...
  \ili{German} is that they denote ``possibility''. Exceptions include
...
\inlinetodoobl{Stefan: Replace \textsc{ph} by \textsc{phon} in the whole paper? \textsc{phon} in
  brackets? Replace \textsc{hd} by \head throughout the paper.}
...
numbers, yielding eight cells. However, in some paradigms\inlinetodoobl{JP: remove \emph{only}}
 only very
...
locate 
\inlinetodoobl{JP: replace \emph{inflectional meaning} by \emph{inflection}}
...
\inlinetodoobl{JP: remove \emph{serious}, I already added \emph{into HPSG} JP suggested \emph{in HPSG}}
...
\inlinetodoopt{JP: referent of \emph{this property} is unclear}
Because of this property, \citet[Section~5.5.2]{Koenig99} suggests
that the inflectional morphology of \ili{Swahili} can be directly described
...
markers in \ili{Swahili} --- so-called parallel position classes
...
\inlinetodoobl{JP: offset rather than set off}
...
%\oneline{%
\avmtmp[extraskip=\bigskipamount]{
	[mph & <\rnode{xx}{\tag{g}	[ph & !<ge>! \\
								pc & -1]},
			\rnode{aa}{\tag{s}	[ph & !<setz>! \\
								pc & 0]},
			\rnode{ab}{\tag{t}	[ph & !<t>! \\
								pc & 1]}> \\ \\
	rr &	\{[mph & <\rnode{ba}{\tag{s}	[ph & !<setz>! \\
											pc & 0]}> \\
			mud & \{\rnode{ca}{\tag{l}	[lid & setzen]}\} ],
			[mph & <\rnode{yy}{\tag{g}	[ph & !<ge>! \\
										pc & -1]},
					\rnode{bb}{\tag{t}	[ph & !<t>! \\
										pc & 1]}> \\
...
\begin{exe}
  \ex \label{ex:MudMS}
\avmtmp{
\emph{realisation-rule} $\rightarrow$
	[mud & \1 set!(msp)! \\
	ms & \1 $\cup$ set!(msp)! \\
	mph & list!(morph)!]
}
...
\avmtmp{
\emph{word} $\rightarrow$
	[mph & \tag{e$_1$} \shuffle \ldots \shuffle \tag{e$_n$} \\
	rr &	\{[mph & \tag{e$_1$} \\
			mud & \tag{m$_1$} \\
			ms & \0 ]
			, \ldots\ ,
			...
			mud & \tag{m$_n$} \\
			ms & \0] \} \\
	ms & \tag{m$_1$} $\uplus\dots\uplus$ \tag{m$_n$} ]
}
\z
...
\inlinetodoobl{JP: The ``, \ldots'' after \type{neg} should be deleted. Same in the negation in b. b. should
  look like a. $\cup$ set $\land$ }
\begin{exe}
  \ex \label{ex:PanExp}
...
\inlinetodoobl{JP: Should be made consistent with (\mex{0}).}
\begin{exe}
...
\begin{figure} 
  ...
  \end{forest}
\caption{\inlinetodoobl{provide caption}}
\end{figure}
...
\end{forest}
  ...
  \caption{Nyanja pre-prefixation \citep{Crysmann:14:OUP}\inlinetodoobl{add pages}}\label{fig:Nyanja}
\end{sidewaysfigure}
...
(\ref{ex:SwahiliRel:suff})--(\ref{ex:SwahiliRel:pref}).\footnote{Conditioned
  ...
  side. See the discussion of mesoclisis in European \ili{Portuguese} in
...
\inlinetodoopt{JP: But cf. Koenig \& Tickels (2020) for a case where structure is/seems to be needed.}
...
affixes. Sorani \ili{Kurdish} endoclitic agreement markers surface after the
...
\end{tabular}
\hfill
  \caption{Partial paradigms exemplifying three Estonian noun declensions (core cases; \citealp{Blevins05})\inlinetodoobl{add pages}}
\label{tab:Estonian}
\end{table}
...
\begin{table}[hbt]
  ...
  \begin{tabular}{llllllllll}
    ...
    \lspbottomrule
\end{tabular}
  \caption{Swahili person markers \citep{Stump93}\inlinetodoobl{add pages}} \label{ex:SwaPer} 
...
\end{forest}
  ...
  \caption{Rule type hierarchy for Swahili parallel position classes \citep{Crysmann:Bonami:2016}}\inlinetodoobl{add pages}
  \label{fig:Swahili}
\end{figure*}
...
\oneline{%
  ...
  \caption{Position classes of Oneida inflectional prefixes
    \citep{diaz:koenig:michelson:19}\inlinetodoobl{add pages}}
  \label{tab:Oneida}
\end{table}
...
\inlinetodoopt{JP: Wants to replace the : by a .}
