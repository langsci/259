...
and structured \phon values and \crossrefchaptert{information-structure}, \crossrefchaptert[\page \pageref{coord:page-rnr-I-phi-start}--\pageref{coord:page-rnr-I-phi-end}]{coordination} for reference to structured \phon values.
...
\footnote{AVMs associated with types used to be combined by unification \citep[Chapter~2]{ps}. See
  \crossrefchaptert[\page \pageref{formal:page-unification-start}--{formal:page-unification-end}]{formal-background} for
...
conventional, Montague-style possible-worlds semantics \citep{Montague74a-ed} in his analysis of English filler-gap constructions and SBCG (Section~\ref{sec:prop7.2}) has generally employed a version of Frame Semantics. See \crossrefchaptert{semantics} for a discussion of the issues.
...
constraints\is{constraint!implicational}.\inlinetodoobl{There are various kinds of constraints, \eg
  relational constraints. I added ``implicational'' here.\\
Bob\&Anne: We suggest: \emph{We turn now to constraints. These are mainly implicational statements, saying that if a linguistic object has some property or properties then it must have some other property or properties.}
} These are implicational statements, saying that if a linguistic object has some property or properties then it must have some other property or properties. They take the following form:%
%
...
\noindent
In much the same way, the type \type{adjective} might have a feature distinguishing between positive, comparative, and superlative forms, in English and many other languages.
...
proposed for other languages (see \citealt{MS99a}, \crossrefchapterw{arg-st}\addpages for
...
%
\footnote{The sign $\oplus$ means concatenation of lists. \citet[\page 170]{GSag2000a-u} state the
...
\feat{soa} is for \term{state-of-affairs} (see \crossrefchapteralp{semantics}). \emph{Seem} takes an infinitival VP complement.%
%
...
\noindent
Once these more specific features are combined with features from type \type{s-rsg-lxm}, we get a
...
\inlinetodoopt{The reader will search for \synsem since you primed this in the text. But there is no
  \synsem in the mother. I would really prefer to be fully explicit in schemata. I think
  abbreviations are fine in trees and so on, but I would go for the real thing in all the things
  that are related to the theory. I would do the changes if you agree. Anne\&Bob: Abbreviations are
  common. Yes, I know. I just reported my reading experience. In the case at hand there is a
  \synsem, but you mention a \synsem that is implicit. Your choice.}
...
%\inlinetodoobl{Suggestion: ``is a set of descriptions of type \type{local}''} 
and it encodes information about unbounded dependency gaps (see \crossrefchapteralp{udc}). Here is the relevant constraint:%
%
...
\textsc{slash} set of the head-filler phrase plus one other \type{local} feature structure,
%\inlinetodoobl{``feature description''}, 
and a non-head daughter, whose \textsc{local} value is the additional \type{local} feature structure
%\inlinetodoobl{``feature description''} 
...
\zllast
~\newline\inlinetodoobl{This should be \textsc{comps-dtr} since linearization rules affect pairs of
...
Some recent work, \eg \citew[\page 84]{Sag2012a} has adopted a rather different view of at least
...
\textsc{select} feature (\citealp{VanEynde98a}; \crossrefchapteralp{np}) but determine the
...
A marker analysis was originally proposed for complementizers. However, they have also been analysed as heads within HPSG, e.g. in \citet[456--458]{Sag97a} and \citet[Section~2.8]{GSag2000a-u}. Thus, there is no consensus here.
\inlinetodoobl{You use ``Thus'', but that there is no consensus does not follow from what you
  cited. You cite \citew{Sag97a} and \citew{GSag2000a-u}. Ivan should have had reasons for changing
  his minds. In order to show that there is no consensus, you would have to cite researchers arguing
  against the other position or at least cite publications suggesting incompatible approaches. A
  paper arguing for a non-marker approach is \citep{MyPM2020a}, revise and resubmit for Glossa. This
  is in opposition to Van Eynde, which I would think of the original proposer of the marker
  analysis.\\
Anne\&Bob: In our view ``thus'' is reasonable. It indicates that the associated proposition follows
from what has been said together with appropriate background assumptions. In this case the
appropriate background assumption is the assumption that neither of the two positions mentioned has
become generally accepted, which we assume is right. We want to cite something that is revise and
resubmit.\\
Stefan: Ah, do you make that claim on the whole section? Then this is a completely different story,
but then the sentence starting with ``Thus'' should be a separate paragraph.}
