...
%
\footnote{Idioms have also been an important focus of research in HPSG. See e.g. \citealt{RS2009a,KM2019a}, and \crossrefchaptert{idioms}.}
%
...
objects\footnote{%
See \citet{BK94a-u}, \citet{Hoehle99a-u} and \crossrefchaptert{information-structure}, \crossrefchaptert{coordination}.},
...
Most HPSG work on morphology has assumed a realizational approach, in which there are no morphemes (see \crossrefchapteralp{chap-morphology}). Hence, words do not have internal structures in the way that phrases do. However, it is widely assumed that lexemes and words that are derived through a lexical rule have the lexeme from which they are derived as a daughter (see \citew{Koenig99a} and below section~4.3).\todosatz{add sec ref} Hence, the \textsc{dtrs} feature is relevant to words as well as phrases.
...
%
\footnote{AVMs associated with types used to be combined by type unification \citep{ps}. See \crossrefchaptert{formal-background}.}\addpages
%
...
Commonly X is a type and Y a feature description, and this is the case in all the constraints that we discuss below. However, X may also be a feature description with or without an associated type. This is necessary, for example, in the constraints that constitute binding theory. See \crossrefchapterw{chap-binding}. Here is a very simple constraint:
...
%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%
\section{The lexicon}\todosatz{The other section titles are capitalised tho}
...
%
\footnote{\textsc{arg-st} is also crucial for binding theory, which takes the form of a number of constraints on \textsc{arg-st} lists. See \crossrefchapterw{chap-binding}.}
%
...
%
\footnote{\citet[177--183]{GSag2000a-u} explicitly allow gaps in \textsc{subj} lists, but this is controversial, as discussed in \crossrefchapterw{chap-udc}.}
%
...
For example, for a subject-raising verb like \emph{seem}, its \textsc{cat} and \textsc{content} features are the following, using Minimal Recursion semantics: \textsc{rels} is the attribute for the list of elementary predications associated with a word, a lexeme or a phrase, \textsc{soa} is for state-of-affair (see \crossrefchapteralp{chap-semantics}). \emph{Seem} takes an infinitival VP complement.%
%
...
%
Notice that no semantic role is assigned to the first argument (see \crossrefchapteralp{chap-control-raising}).
...
Notice that the \textsc{subj} feature is underspecified. Thus, \emph{seem} combines with an infinitival complement and with any subject (nominal or verbal, expletive or referential), provided this subject is appropriate for its complement (see \crossrefchapteralp{chap-control-raising}):
...
%
\footnote{Lexical rules can be seen as a generative device, or alternatively, as a set of well-formedness conditions on the lexicon: if the lexicon contains items with description x, it must also contain items with description y \citep{Meurers2001a}. See \crossrefchapterw{chap-lexicon}.}
%
They are used in morphology to relate lexemes to words (inflection) and lexemes to lexemes (derivation) (see \crossrefchapteralp{chap-morphology}). For syntax, they are relevant especially to valence alternations such as that illustrated in the following (see \crossrefchapteralp{chap-arg-st}):
...
%
\footnote{Implemented HPSG grammars for various languages may assume binary branching structures \citep{Flickinger2000a,Copestake2002a,MuellerCoreGram}, see \crossrefchaptert{cl}.}
%
...
Finally, we consider the type \emph{hd-fill-ph}. This involves the feature \textsc{slash}, one of the features contained in the value of the feature \textsc{nonlocal} introduced earlier in (\ref{ex:prop9}). Its value is a set of local feature structures and it encodes information about unbounded dependency gaps (see \crossrefchapteralp{chap-udc}). Here is the relevant constraint:%
%
...
%
\footnote{As discussed in section~7.1,\inlinetodostefan{add secref} in some HPSG work, linear order is a property of so-called order domains, which essentially mediate constituent structure and phonology (see \crossrefchapteralp{chap-order}). }
%
...
But it should be remembered that they are well-formedness constraints on structures built with certain concatenations of \textsc{phon} values as in (38)\todosatz{This is supposed to be 39, right?}.%
%
...
Various arguments have been advanced for this position, but it is controversial and it is rejected by Levine (2002) and Levine and Hukari (2006: chapter 3) and Chaves (2009).\todosatz{All 3 not given in references} There is an unresolved issue here.%
%
\footnote{It has been argued that some adverbs and PPs are adjuncts and others are complements, depending on word order, case etc. (see for example \citealp{Prze99} Abeillé \& Hassamal 2014\inlinetodostefan{not in references} and \crossrefchapteralp{negation}).}
%

%%%%%%%%%%%%%%%%%%%%
\subsection{Specifiers and markers}

As noted earlier, some HPSG work assumes a feature \textsc{spr (specifier)} which is realized by various categories. In some work subjects are analysed as specifiers (Sag et al 2003: 100-103),\todosatz{Not given in references} but in other work they are realizations of a \textsc{subj(ect)} feature, as discussed in the last section. For some HPSG work, e.g. \citet[section~9.4]{ps2}, Sag et al. (2003: 4.3), determiners within NPs are an important example of specifiers. On this view, \emph{the pub} has the schematic structure in Figure~\ref{fig:prop11}.
...
%
\footnote{Work which assumes the \textsc{select} feature also uses it instead of \textsc{mod} for adjuncts, and consider both markers and adjuncts to be ‘functors’ (Van Eynde 1998; \crossrefchapteralp{np})\inlinetodostefan{not in references}.}
%
...
Here the clause has two daughters but three domain elements. The simpler example in (44a)\todosatz{This is supposed to be 45, right?} will have two daughters and two domain elements.

It is worth noting that this approach allows a different analysis for interrogatives like (41b).\todosatz{This is supposed to be 42, right?} It would be possible to propose that an analysis in which they have two daughters and three domain elements as follows:
...
%
\footnote{\citet{Kathol2000a} assumes that order domains are divided into topological fields and shows how this idea allows an interesting approach to various aspects of clausal word order. See \citet{Borsley:06}\inlinetodostefan{ref taken from hpsg-handbook-bib} for an application of this idea to negation.}
%

Order domains seem most plausible as an approach to the sorts of discontinuity that are found in so-called nonconfigurational languages such as Warlpiri \citep{DS99a}. However, they may well have a role to play in more familiar languages \citep{BonamiEtc1999} (Chaves 2014).\todosatz{not given in references} But exactly how much of a role they should play in syntax is an unresolved matter.
...
%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%
\section{Concluding remarks}\todosatz{Other section titles were capitalised tho}
