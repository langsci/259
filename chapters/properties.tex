...
often called \type{valence} features. Thus, the following AVM provides a fuller representation of signs:
...
bits of phonology realising certain morphosyntactic features \citep{Stump2001a,Anderson92a-u}. One
...
Generalised Head Feature Principle\is{Head Feature Principle!Generalized}, which takes the following form:
...
%
\footnote{We use \cupAVM for set union. Notice that the mother category does not have to have an
...
\noindent
In much HPSG work, adjuncts select the heads they combine with through a feature \textsc{mod(ifies)} whose value is a \type{synsem} object, while other signs are \mbox{[\textsc{mod} \type{none}]}. Thus, (\ref{ex:prop43a}) involves the schematic structure in Figure~\ref{fig:prop10}.
