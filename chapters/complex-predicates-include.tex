...
\parencites{manning1999lexical}.\footnote{
  \added{\citet{Gunji99a-u} proposes a dual representation of Japanese causatives, with a VP embedding structure
...
\eal
	...
	\gll Lola a   achet\'e des   pommes, et  Alice (a)             cueilli des   p\^eches.\\
	     Lola has bought   some  apples  and Alice \spacebr{}has   picked  some  peaches\\}{\ili{French}}
...
(for the feature [\light{}$\pm$], see Section~\ref{GSsection3}).\footnote{
  \added{$\oplus$ stands for the relation \emph{append} and simply concatenates two lists. For
    example, \nliste{ a, b } $\oplus$ \nliste{ c, d } = \nliste{ a, b, c, d }.}
}
...
\changed{ and  the complements}, but also a list of non-canonical elements (possibly empty; see below).\footnote{\added{\citet[\page 170]{GSag2000a-u} state the
...
\begin{exe}
	...
	Hablar-le 	 a  Mar\'ia  seguramente quiere \hphantom{(}pero          no  a  su  madre \\
	     talk-to.her to Mar\'ia  certainly   wants  \spacebr{}but  not to her mother\\}{\ili{Spanish}}
...
\judgewidth{\%}
	...
	\gll Lo comincia \textsc{Maria} a  capire,     il problema, oppure no?\\ 
	     it begins   Maria          to understand  the problem  or     no\\}{\ili{Italian}}
...
\gll Giovanni lo vuole comprare subito      e   dare a  Maria.\\ 
     Giovanni it wants buy      immediately and give to Maria\\}{\ili{Italian}}
...
\eal
	...
	\gll Juan da    [el           libro de Proust] [a           Mar\'ia] y   [el          (libro)            de Camus] [a           Pablo].\\
		 Juan gives \spacebr{}the book  of Proust  \spacebr{}to Mar\'ia  and \spacebr{}the \spacebr{}book    of Camus  \spacebr{}to Pablo\\}{\ili{Spanish}}
...
proposal using \texttt{synsem2sign}). \type{ne-list} stands for non-empty list and this
...
%\rep{\ibox{3}}
\ibox{1} in(\ref{GSexemple25}) is usually empty, but does not have to be (see the case of the copula in Section~\ref{GSsection3.4}). An example of the flat structure with a restructuring verb is given in Figure~\ref{GSfigure4}.
...
\citealt[39]{muller2018clause}).\footnote{This rule is also used in \ili{Romanian}. As in
  ...
  daughter \added{(see (\ref{ex-head-cluster-phrase-German}))}. This is not necessary since the auxiliaries select for the non-head daughter and hence
...
for them.\footnote{
  \added{For more on the definition of such constraints, see \crossrefchapterw[Section~\ref{sec-id-lp}]{order}.}
} 
...
[Context] Is John faithful to his friends?}\label{GSexemple31} 
	\begin{xlist}
		...
		\gll Fid\`ele \`a ses amis,   il l'est plus qu'\`a    ses convictions politiques.\\
		     faithful to  his friends he {it is} more {than to} his convictions political\\}{\ili{French}}
...
\eal 
	...
	\gll Fid\`ele, il l'est plus \`a ses amis    qu'\`a  ses convictions politiques.\\
	     faithful  he {it is} more to  his friends than.to his convictions political\\}{\ili{French}}
	...
	\gll Fiel,    lo es m\'as a  sus amigos  que  a  sus convicciones pol\'iticas.\\
	     faithful it is more  to his friends than to his convictions  political\\}{\ili{Spanish}}
...
[Context] Is he really faithful to his friends?}\label{GSexemple33} 
	...
	\gll Aussi fid\`ele \`a ses amis    qu'il   soit, il ne          perd  pas de vue   ses int\'er\^ets.\\
		 as    faithful to  his friends {as he} is    he \textsc{ne} lose  not of sight his interests\\}{\ili{French}}
...
that it is non-light in all cases. \added{If the predicative complement is a lexical item, a unary
branching phrase makes it [\light{}$–$] (see Figure~\ref{GSfigure8}).}

\eas
Description of the copula \changed{in Romance languages:} \\
\avm{
[arg-st & \1 \+ < [head & [prd +] \\
                 subj  & \1 \\
                 comps & \2 \\
...
for its category.\footnote{ \added{The predicative complement in French can be a PP (\emph{Il est contre
...
Figures \ref{newGSfigure7}, \ref{GSfigure8}).\footnote{Note that the complements included in a
  predicative PP are not attracted by the copula.\itdopt{How is this assured? Why couldn't there be
    a P selecting for an affix and this affix is raised.}}
...
\added{Regarding the point made in Section~\ref{sec-copula-romance}, that argument attraction is compatible with different structures (a flat structure or a verbal complex), what the Romance copula shows is that still another structure is possible: the copula can inherit arguments from a phrasal complement.}
...
copula with \changed{predicative adjectives}. \changed{In coherent constructions}, the two
predicates cannot be separated and form a \changed{predicate} complex. 
...
predicative verbs, \added{verbs entering resultative constructions,} or particle verbs (see
...
head verb and the infinitive, extraposition of the infinitive with its complements, pied-piping in
...
and the infinitive \changed{may be} pied-piped with its relative pronoun complement as in (\ref{GSexemple36d})
...
subject and the complements \added{of finite verbs} \citep{Pollard90a}: the subject \added{of finite
...
position, allows for a free order. The phrase combining \changed{a head} with a complement is [\light{}$-$].\footnote{The feature \light is the equivalent of \lex used in \ili{German}
...
(\ref{GSexemple26})).\footnote{Following \citet[\page 23]{HN94a} and \citet[\page 177]{dKM2001a}, but contrary to
  Müller (\added{\citealt[\page 23]{Mueller2005c}}; \citealt{muller2018clause}), we mention the lightness of the mother.}\footnote{The description of the \emph{head-cluster-phrase} in (\ref{GSexemple42}) is the same as that in (\ref{GSexemple26}), only more general, (\ref{GSexemple26}) being specified as having a verbal head.}  
...
[synsem & [\punk{loc|cat|comps}{ \1} \\
 ...
 head-dtr|synsem & [\punk{loc|cat|comps}{ \1 \+ < \2 >} \\
                    light $+$ ] \\
 non-head-dtrs & <[synsem & \2 \added{[light & $+$ ]} ]> ]
}
\zs
...
\z
\changed{As mentioned above, subjects of non-finite verbs are represented under \subj. Since the verbs above
attract all arguments, the \subjv and the \compsv are concatenated and represented on the \argstl of
the governing verb.}
Hence, these lexical items are parallel to the ones given for the Romance languages \added{(see
(\ref{GSexemple8b}) and (\ref{GSexemple19}))} with the
  ...
  \added{always} attracts the arguments from the \compsl rather than from \argst. The reason for attracting
...
part, is as follows \citep[\page 226]{MuellerPredication}:\footnote{
  \added{As \citet[\page 227]{MuellerPredication} notes, this copula also works for \ili{English} and
...
complement. So, while German allows for the formation of a predicate complex \added{(a head–cluster phrase)} with predicative
  adjectives and normal head-complement structures with predicative NPs and PPs, the Romance copula \changed{only
...
unioned (see \citealt{Reape94a} \added{and also \crossrefchapteralt[Section~\ref{sec-domains}]{order}}); or there is a complex predicate: the complement of the embedded verb (\emph{ku chaykul} `the book' + accusative) is attracted by the auxiliary verb. 
...
predicative complement gives rise to a participle \changed{from which a noun or adjective can be derived} (depending on the
...
It is worth noticing that, as indicated in (\ref{GSexemple74}), \changed{the compound noun takes the NP expected by the verb} as its complement (indicated by the brackets in (\ref{GSexemple74})).   
...
Within a hierarchical organization of the lexicon \crossrefchapterp{lexicon}, the simplex
...
Daniele: modify: 
This makes sense within a hierarchical organization of the lexicon (...): the content of the simplex
verb and that of predicative noun  belong to the same hierarchy, with that of the verb being higher
and less specialized than that of the noun. Thus, the content of the complex predicate can simply be
that of the predicative noun.\\
Stefan: No, this does not work.\\
Daniele: Not exactly:  the meaning of the complex predicate is a specialization of the meaning of
the verb. The meaning of the verb is not empty, it meshes with that of the noun. It is true that
this is not said in the description, but it is an explanation (it's like unification, but I am told
that we don't use that concept anymore): it is not just any verb which works with any noun.\\
Stefan: But in (\mex{1}) you have a representation where you take the semantics of the noun over and
make it the semantics of zadan1-lexeme. So this is not done via inheritance in the type hierarchy
but by embedding. In principle you could add information from the verb and truly embed the
semantics of the noun. Like saying something about tense or modality or so. You cannot do this with inheritance.}
...
Another case where the combination gives more information than the simplex verb is when this verb takes as its predicative complement a noun which can also occur as a referential noun denoting an instrument crucially involved in the situation \citep{bonami2010persian}. Examples are, in different domains, \emph{\v c\=aqu zadan} (lit.\ knife hit) `stab', \emph{telefon zadan} (lit.\ phone hit) `phone', \emph{pi\=ano zadan} (lit.\ piano hit) `play the piano'. We illustrate here with \emph{\v s\=ane zadan} (lit.\ comb hit) `comb'.    
...
\itdopt{Daniele: If it makes things clearer, it is possible to add:
``It relies on the same semantic process as a denominal verb, derived from an instrumental noun (to
ski, to iron).''\\
Stefan: The marking was unclear. I do not know where this relates to and where it should be added.}
...
associated with lexemes in the lexicon, contains semantic information and allows the
  ...
  idiom selects the second one. \changed{The noun \emph{dast} in the idiomatic complex predicate \emph{dast zadan}
