...
\parencites{manning1999lexical}[\iaddpages]{gunji2012topics}. Thus, we do not consider
...
situation. \added{This may be appropriate for some complex predicates, such as light verb constructions (\emph{to have a rest}, \emph{to make a proposal}) \citep[Section~5]{Butt2010a}. However,} such a semantic definition does not coincide with the syntactic one. It is true that the head verb of a complex predicate tends to add tense, aspectual or modal information, while the other element describes a situation type. Thus, in (\ref{GSexemple1}), the two verbs jointly describe one situation, the auxiliary adding tense and aspect information. But the semantics of a complex predicate is not always different from that of ordinary verbal complements. Thus, there is no evident semantic distinction depending on whether the \ili{Italian} restructuring verb \emph{volere} `to want' is the head of a complex predicate (\ref{GSexemple4a}) or not (\ref{GSexemple4b}), and the two verbs do not seem to describe just one situation \citep[314]{Monachesi98a}.  
...
\begin{itemize}
	...
	\item \ili{Korean} auxiliaries, control verbs, \emph{ha} causative \changed{verb} and light verb constructions \citep{ Sells1991, Ryu:93, Chung98a-u, lee2001argument, choi2001mixed, Yoo2003, Kim2016a-u};
...
raising verb. The notation \changed{\ibox{1} instead of \sliste{ \ibox{1} }} indicates that this element may be
...
\emph{head-cluster-phrase}, given in (\ref{GSexemple26}) (see \citealt[\changed{87}]{Mueller2002b};
\citealt[39]{muller2018clause}).\footnote{This rule is also used in \ili{Romanian}. As in
  ...
  \ili{Spanish}, for instance). \added{Note that Müller does not specify the \light value of the non-head
  daughter. This is not necessary since the auxiliaries select for the non-head daughter and hence
...
complement, which can be saturated or not. \changed{This analysis is 	implemented by saying that the predicative complement is underspecified with respect 	to complement saturation or attraction, and that it is non-light in all cases.}
...
which is not the case for tense auxiliaries,\itdopt{Shouldn't (\ref{GSexemple8b}) be [prd –] then?}
and it is non-light; in addition, it is not specified for its category. Being non-light, it may have
combined with its complements or some of them, while the complement of the auxiliary is light, hence
all its complements are attracted (see Figures \ref{newGSfigure7}, \ref{GSfigure8}).\footnote{Note
  that the complements included in a predicative PP are not attracted by the copula.\itdopt{How is
    this assured? Why couldn't there be a P selecting for an affix and this affix is raised.}}  
...
to the copula.
\itdopt{So does this mean that lexical items are underspecified for LIGHT? Otherwise you would need a
  unary projection turning \emph{fid\`ele} into LIGHT– in Figure~\ref{GSfigure8}.}
...
\added{A \isi{coherence field} consists of all verbs entering a coherent construction and all arguments and
...
to the same \changed{coherence field. This means that \emph{zu reparieren} `to repair',
...
position, allows for a free order. The phrase combining the verb with a complement is [\light $-$].\footnote{The feature \light is the equivalent of \lex used in \ili{German}
...
(\ref{GSexemple26})).\footnote{Following \citet{HN94a} and \citet{dKM2001a}, but contrary to
  \citet{muller2018clause}, we mention the lightness of the mother.}\footnote{The description of the \emph{head-cluster-phrase} in (\ref{GSexemple42}) is the same as that in (\ref{GSexemple26}), only more general, (\ref{GSexemple26}) being specified as having a verbal head.}  

\eas
\label{GSexemple42}
\type{head-cluster-phrase} (\ili{German}) \impl \\
\avm{
...
\citealt[102]{Mueller2002b}). The subject and other arguemtns are raised from the embedded verb. The
...
\z
\added{So these lexical items are parallel to the ones given for the Romance languages with the
  exception that the selected verb is the last argument in German (SVO vs.\ SOV) and that German
  attracts the arguments from the \compsl rather than from \argst. The reason for attracting
  arguments from \comps is so called partial verb phrase fronting \citep{Mueller96a}: verbs may be combined with a
...
The German literature follows \citew{Pollard90a} in assuming that subjects of finite verbs are
listed on the \compsl, while subjects of non-finite verbs are represented under \subj.  
...
\emph{immer} `always' can modify the modal or the adjective. \changed{This follows if there is just one
coherence field and both the modal and the copula being the head of a complex predicate (see
...
part, is as follows \citep[\page 226]{MuellerPredication}:
\ea
\label{GSexemple49}
\emph{sein} (copula): \\
\changed{\avm{
...
complement. \added{So, while German allows for the formation of a predicate complex with predicative
  adjectives and normal head-complement structures with predicative NPs and PPs, the Romance copula
  allows XPs as arguments only.}
...
\z
\itdobl{Daniele: Add here: the text should be modified for clarification:
	``Forms with the suffix \suffix{ande} are morphologically participles. They can give rise to
        a derived noun or adjective. \emph{Baz-konande} is a noun, based on a compound made of
        two lexemes, an adjective and a verb (\mex{1}). The verb is very similar to its description
        in (\mex{0}).''\\
Stefan: I do not know what to replace with what. Should all the text below be replaced?}     
...
\avm{
	  	[\type*{lexeme}
		\phon \1 \+ \2 \+ < ande > \\ 
		synsem &	[cat &	[head & \changed{noun} \\
				         comps & < \3 NP > ] \\
      				cont &	[ind & k] ] \\
    	m-dtrs &	<[phon \1 \\ 
			  ss \4 [loc [cat & [ head & \changed{adj} \\
                                              arg-st & < \3 NP$_j$ > ]\\
				      cont& \5 adj-rel!$(j)$! ] ] ],
			  [phon \2 \\ 
...
Within a hierarchical organization of the lexicon \crossrefchapterp{lexicon}, \changed{the simplex
  verb is higher and less specialized than the predicative noun. Thus, the content of the complex
  predicate can be simply that of the noun, if they are of the same semantic type. In the example,
  they both denote events (the nucleus is an event-relation).}
\itdopt{
I do not understand why the type hierarchy is relevant here. The semantics of the noun is taken over
from the argument. It is not inherited. So the hierarchy seems to be entirely irrelevant. The only
important thing is that the verb does not have another specification of semantics.
...
Another case where the combination gives more information than the simplex verb is when this verb takes as its predicative complement \changed{a noun which can also occur as a referential noun denoting an instrument} crucially involved in the situation \citep{bonami2010persian}. Examples are, in different domains, \emph{\v c\=aqu zadan} (lit.\ knife hit) `stab', \emph{telefon zadan} (lit.\ phone hit) `phone', \emph{pi\=ano zadan} (lit.\ piano hit) `play the piano'. We illustrate here with \emph{\v s\=ane zadan} (lit.\ comb hit) `comb'.    
...
the situation: a state of fatigue, but not a heart attack. We group them as \added{objects of type} \emph{internal-problematic-state}. Here the combination of the verb and the noun is standard, in that the noun is a semantic argument of the verb, but the meaning of the verb is unpredictable.
...
\changed{is associated with lexemes in the lexicon, contains semantic information and allows the
  ...
  idiom selects the second one. The noun dast in the idiomatic complex predicate dast zadan
  corresponds to the i-dast-relation and is selected by the idiomatic zadan. The preposition}
...
[\type*{zadan4-lexeme}
		cat & [ head & verb \\
		        arg-st & < NP$_k$, PP![\type{be}]!:\1, N![\textsc{prd}$+$, \textsc{lid} \type{\changed{i-dast-relation}}]! > ]\\
		cont &	[\type*{soa}
				nucleus &	[\type*{\changed{i-start-relation}}
							actor & k \\
