...
\judgewidth{\%}
	...
	\glt Intended: `He started again to ask him for an autograph and to make propositions to him/her.'}\label{GSexemple23b}	
\zl
...
[synsem|loc|cat [head  & \1 \\
 ...
 head-dtr|synsem|loc|cat [head  & \1 \\
		          comps & \2 \shuffle\ \3\\
		          light & $+$\\]\\
		  non-head-dtrs \ibox{2} \type{non-empty list}\\
		]}
\z
...
\avmtmp{
[light \1\\
 non-head-dtrs \normalfont{list}( [light \1] ) \\ ] % todo normalfont?
...
These data point to a verbal complex (see Section \ref{GSsection3.2}). However, before coming to this conclusion, we must show that the two verbs do not form a compound word. \cite{no1991case} (summarized in \citealt{Chung98a-u}, \citealt{Kim2016a-u}) presents arguments to the effect that they combine in the syntax. The main one relies on the use of delimiters. A delimiter (such as \emph{-man} `only' or \emph{-to} `also') can combine with the embedded verb (e.g.\ \emph{mekkoman issta} `to be only eating'). Delimiters are a syntactic phenomenon, not limited to verbal morphology. 
...
[synsem|loc|cat & [ subj  & < >\\
 ...
 \punk{head-dtr|synsem|loc|cat}{  [ head  & [\type*{verb}\\ 
                             ...
                             comps & \2 \\
                             light & $+$]}\\
 non-head-dtrs & (\1 \shuffle\ \2) nonempty list ]
    }
\z
...
[synsem|loc|cat & [%head & [\type*{verb}\\   superfluous St.Mü. 03.08.2020
      ...
      \punk{head-dtr|synsem|loc|cat}{  [head  & [\type*{verb}\\
                                 ...
                                 comps & < \2 > \+ \3\\
                                 light & $+$]}\\
      non-head-dtrs & < \2 %[head  & verb\\ superfluous St.Mü. 03.08.2020
                           % subj  & \1\\
                           % comps & \3\\
                           % light & $+$] 
> ]}
