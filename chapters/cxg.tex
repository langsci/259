...
Grammar (HPSG). The short version of the\todostefan{JP: I think you take a bird eye’s view which is correct, but could be seen as somewhat distorting the historical record. So, while true that HPSG always had the possibility of Constructional HPSG, it explicitly did not want to do so until the mid 90’s and that corresponded to a view of the nature of phrase-structure “constructions” at the time.}
...
\citet*{FKoC88a} discuss sentences like (\mex{1}) and notice that they pose puzzles for standard
...
\zl
The \emph{the}-\emph{the}\todostefan{the -er the -er?} Construction is remarkable since it combines aspects of normal syntax
...
%% \zl
%% }
So, \cg, \isi{GB}-style theories paired with semantics \citep{HK98a-u}, \gpsg, \treeag,
...
shows an example lexical item for the word \emph{ate}:\todostefan{JP: has this convention been
  introduced before or is it standard?}
...
This\todostefan{JP: referent of `this' is unclear} supports the argument I made on \mother in the previous subsection.
...
are more specific then what is usually used in other variants of HPSG.\footnote{
  ...
  HPSG \parencites[Section~8.5.3]{ps2}[\page 24, Fn.~1]{Mueller99a}.
}
...
\citew[Section~6.3.2]{Uszkoreit87a} for such a proposal in a \gpsg approach to \ili{German}) or that the phrasal
...
Note also that the resultative construction interacts with \bard. (\mex{1}) shows an example of the resultative
...
The\is{morphology|(}\is{derivation|(} first publications in \isi{Construction Morphology} were the masters thesis of \citet{Riehemann93a},
...
}\todostafan{JP: `This is also the case
for mass nouns with a certain reading.' too vague for me to figure out what kind of nouns you are
referring to.}
\ea
