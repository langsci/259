...
\noindent
Because   the {\sc slash} value \ibox{1} is structure-shared between the mother and the daughters
...
including \ili{Japanese} and  \ili{Korean} \citep{kunojap,Nishigauchi99},  Ahan \citep{saah},  \ili{Danish}  \citep[Chapter 2]{shir}, \ili{Swedish}  \citep{allwood,engdahl82} \ili{Norwegian}  \citep{taraldsen82} and \ili{Romance} languages \citep{Cinque10}.  In some languages that have support verb constructions, the CNPC is apparently 
...
Rightward movement is traditionally regarded as being clause bounded. Such
 \emph{\isi{Right Roof Constraint}}  \citep[Section~5.1.2]{Ross67} effects are illustrated in (\ref{rrc}), 
...
\citep{Mueller2004d,Mueller2007c} and experimental findings  \citep{SS2013b-u,strunk}, which confirm
 ...
 \citet[\page 106, 109, 111]{SS2013b-u} and \citet{strunk}, and those in (\ref{nosubza}d--f) are from \citet[863]{chavesrnr}.
...
\ili{English} \citep{Ross67,chomsky73}, an effect referred to as a \emph{Subject Island} (SI).
This constraint is  much less severe  in languages like \ili{Japanese} , \ili{German}, and \ili{Spanish},
...
\noindent
However, \ili{English} exceptions were noticed early on, and have since accumulated in the literature.
In fact,  for \citet{Ross67},  \ili{English} extractions like (\ref{sie}a) are not illicit,
...
and that no such island effect exists in \ili{French} either.\footnote{For completeness, other authors argue that PP extractions from
...
\noindent
\ili{English} exceptions to the SI constraint are not restricted to PP extractions, however. Although
...
Incidentally, subject phrases are not  extraposition islands either, as shown in (\ref{subxxx}).
...
Acceptable tensed AIC violations are more frequent in languages like \ili{Japanese}, \ili{Korean},  and \ili{Malayalam}.
...
\ili{English} interrogative intonation, \citet[251]{ginzsag} propose that in a multiple \emph{wh}-interrogative construction, all \emph{wh}-phrases must be in focus except  the first. Crucially, focus is typically -- but not always --  associated with clearly discernible pitch accent. Thus, (\ref{badsc1})  and (\ref{badsc2}) are odd because  the second \emph{wh}-word is unnaccented. In this account, a word like \emph{who} has two possible lexical descriptions, shown in (\ref{whoavm}).
...
This is illustrated below in (\ref{langslbc}), with \ili{French} data. 
...
the extracted phrase is has a similar independent status to the \ili{French} {\it de-N}$'$ phrase in (\ref{langslbc}a).
