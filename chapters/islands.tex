...
gramaticized constraints, induced by parsing and performance considerations  
...
Let us take stock. The CSC does not receive a unitary account in modern HPSG, given that the Conjunct Constraint and the Element Constraint  are of a very different nature. Whereas the former does not admit ATB extraction, and is predicted by a traceless analysis, the latter allows ATB extraction as seen by the contrast between  (\ref{cc}c) and  (\ref{cc4}a,b). 
...
Given that  finite tensed verbs can be regarded as definite, and infinitival verbs as indefinite \citet{partee84}, and given that finiteness can create processing difficulty 
...
Let us consider now SI effects involving more complex subjects.
...
The conclusion that Subject Island effects are contingent on the particular proposition expressed by the utterance 
and its pragmatics thus seems unavoidable \citep{chavesresp}. In order to test this hypothesis, \citet{chavesresp} examine the 
the acceptability of sentences like (\ref{pairs}), which crucially  express nearly-identical truth conditions and have equally acceptable declarative counterparts. This way, any source of acceptability contrast must come from the extraction itself, not from the felicity of the proposition. 
...
After 8 exposures, the acceptability of near-truth-conditionally equivalent sentences like (\ref{pairs}) becomes non-statistically different.
...
but have a particular PP construction appears to allow LBC violations.
...
\noindent
But the LBC violation is only apparent. The {\it de livres} is in fact a post-verbal {\it de}-N' nominal, and thus no LBC violation incurs in (\ref{langslbc}a). See \citet{Abeille:Bonami:ea:04} for details.
...
Many thanks to Stefan Müller, Bob Borsley, and Berthold
Crysmann for invaluable comments and discussion.
