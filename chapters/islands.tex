...
For example, I take it as relatively clear that {\it factive islands}  are purely pragmatic in nature \citep{Oshima:2006:FIP:1761528.1761544}, as are {\it negative islands} \citep{kroch89,sza,abrusan,foxneg,abrusanspec},
although one can quibble about the particular technical details of how such accounts are best articulated. 
Similarly,  the {\it NP Constraint} in the sense of \citet{horn72} is likely to be semantic-pragmatic in nature \citep{kuno87,godard92,dubinsky2009}. Conversely, I take it as relatively uncontroversial that the {\it Clause Non-Final Incomplete Constituent  Constraint}  is due to processing difficulty \citep{lev91,fodor92}. In the present chapter I focus on islands that have garnered more attention from members of the HPSG community, and that have caused more 
...
{\sc slash}. Because the theory consists of a feature-based declarative system of constraints, virtually all that goes on in the grammar involves constraints stating which value a given feature takes. By allowing {\sc slash} sets to be unioned, 
...
\ex Which celebrity  did you [hire \spcs [without auditioning \spc first]]?
\ex Which celebrity  did you [meet \spcs at a party] and [dated \spc for a few months]]?
\zl 
...
are lexically required to have on complement in \textsc{slash} rather
...
gramaticized constraints, induced by parsing and performance considerations; see 
\citet{prichett}, \citet{fodor78}, and \citet{fodor83-removed} for example.}
 The problem is that island effects are riddled with exceptions witch
defy purely syntactic accounts of the phenomena.  
...
\ex[]{Which celebrity you say Robin arrived earlier than \spc?}
\zl

\noindent
HPSG accounts of extraction that assume the existence of traces \citep{pollardsag,levhubook} the CC must be stipulated at the level of the coordination construction, by stating that conjuncts cannot be empty elements.\footnote{See however \citet[317--318]{levine17} for the claim that  each conjunct must contain at least one stressed syllable. Given that traces are phonologically silent, noting is there to bear stress and the CC is obtained. This raises the question of why no such stress constraint exists in P-stranding, for example, or indeed in any kind of extraction.}  On the other hand, the CC  follows immediately in a traceless account of  filler-gap dependencies  \citep{fodorsagt,bouma,ginzsag,fgsag08} since there is simply nothing to conjoin in (\ref{cc}), and thus nothing else needs to be said about conjunct extraction; see \citet{sagonline} for more criticism of traces.

HPSG's traceless account of the CC is semantic in nature, in a sense.  Coordinators like \emph{and}, \emph{or}, \emph{but} and so on are not regarded as heads that select arguments, and therefore have empty {\sc arg-st} and valence specifications. And given that HPSG assumes that the signs that can appear  in a given lexical head {\sc slash} values are valents, then it follows that the signs that coordinators combine with cannot instead be registered
in the coordinator's {\sc slash} feature. Hence, words like \emph{and} have no valents, no arguments
and therefore no conjunct extraction. Incidentally,  adnominal adjectives cannot be extracted either, for exactly the same reason, as they are not selected  by any head, and therefore  are not listed in any {\sc arg-st} list. However, in order to allow certain adverbials to be extractable,  \citet{ginzsag} assume that they are  members of {\sc arg-st}.
See \citet{levhubook} for more on adverbial extraction, and see  \crossrefchaptert{udc} for further discussion.\footnote{The empirical facts are less clear when it comes to adnominal PPs, however. Even PPs that are usually regarded as modifiers can sometimes extract, as in {\it From which shelf am I not supposed to read any books?} In many such extractions the PP can alternatively be parsed as VP modifier, which complicates judgements. }
...
\eal
\label{cc4}
\ex Which celebrity did you read  [[the memoir of \spc] and [the novel about \spc]]?
\ex Which celebrity did you [[research  the life of \spc] and [write a book about \spc]]?
...
as a major empirical advantage of GPSG/HPSG. A similar constraint is assumed in \citet[202]{pollardsag}
\citet[60]{Beavers}, among many others, as illustrated in (\ref{cschpsgr}). See  \crossrefchaptert{coordination} for more discussion about coordination. 
...
in asymmetric coordination the order of the conjuncts has no interpretational  differences,
...
\ex What did your get [the impression [that the problem really was \spc]]?\\
\citep{kluender}
\zl
...
The amelioration caused by definite \emph{wh}-phrases as in  (\ref{k}d), (\ref{dd4}b) and (\ref{fininf}c) 
...
Thus, RRC island effects in can be easily modeled by stipulating that the  {\sc extra} value of an S node must be empty. One way to do so is to state that any S dependent (valent or adjunct) must be [{\sc extra} $\lbrace \rbrace$].
...
\ex[*]{Who$_j$ did you [give \spc$_i$ [to John] [a picture of \spc$_j$]$_i$]?} 
...
\eal  \label{rosssi}
\ex The hoods of these cars were damaged by the explosion. 
...
Whereas SI violations involving subject NPs are not attested,  those involving
...
which  are significantly  more acceptable than  *\emph{$[$Pictures \spc$]$ frighten people $[$of John$]$}  \citep{drummond}.
...
\noindent
Infinitival subjects contrast dramatically with finite clausal subjects in that the 
...
There are  few good functional reasons for why clausal SI violations are so strong. First, subject clauses are notorious for being particularly difficult to process, independent of extraction. For example, it is known that tense can induce greater processing costs
 \citep{kluender92,gibson0000}. Second, clausal  subjects  are often  stylistically marked and difficult  to process,   as  (\ref{that}a) illustrates. Thus, it is extremely hard to embed a  clausal subject within another clausal subject, even though such constructions ought to be perfectly grammatical,  like (\ref{that}b,c). 
...
\ea If -- if every time it snowed there was no school -- then there wouldn't be classes all winter.
\z \label{pinker}
...
The evidence discussed so far suggests that  sentences involving extraction and clausal subjects  are odd 
...
Although some experimental research has confirmed that sentences with SI violations  are less acceptable than grammatical controls \citep{sprsat2,goodall11,crawfordwccfl,clausencuny,greco}, and that their acceptability
...
Although the constraint on {\sc slash} discussed above in (\ref{relcxt}) would effectively ban 
...
Several different kinds of exceptions to this {\it Superiority Constraint} (SC) have been 
...
\eal \label{whex}
\ex[*]{Which movie did John wonder wether Bill liked \spc?}
...
These facts are accounted for in versions of HPSG like REF where determiner phrases are not valents of the nominal head. If the DP is not listed in the argument structure of the nominal head, then there is no way for the DP to appear in {\sc slash}.
According to \citet[133]{cxsag07}, genitive DPs combine with nominal heads and bind their {\sc x-arg} index
via a dedicated  construction, not as valents. Thus, in nominalizations like {\it Kim's description of the problem}, for example, the  DP {\it Kim's} is not a valent of {\it description} but is instead constructionally co-indexed with the agent role of the noun {\it description}  via {\sc x-arg}. Thus, the DP cannot be extracted. Moreover, the clitic {\it s} in {\it Kim's} must lean phonologically on the NP it selects, and therefore cannot be stranded for independently motivated phonological reasons, predicting  the oddness of *{\it It was Kim who I read 's description of the problem}.


There are various  languages  which do not usually permit extraction of left branches from noun phrases, 
but which have a particular PP construction which  allows NP extraction with PP stranding:

\eal   \label{langslbc}
\ex \gll dare-kara-no$_i$  Taroo-ga  {[\spc$_i$  tegami]-o}  sute-ta-no?\\
...
%\ex \gll Vad$_i$  l?er  du  [\spc$_i$  for  en  bok]?\\
% what read  you  for  a  book\\
...
\zl
 ...
 \noindent
Such extractions also appear to arise in English, as (\ref{kjh}) illustrates. 

\eal
 \ex \,[How many books]$_i$ do you have [ \spc$_i$ about mathematics]?
...
\noindent
It is possible that none of (\ref{langslbc}) and (\ref{kjh}) are in fact LBC violations, if
 these are simply leftward extractions followed by PP extrapositions.  Ross noted other languages that appear to not obey the LBC at all, such as those sampled in (\ref{lbcx}).  These examples are completely
  ungrammatical in languages like English.
 
\eal   \label{lbcx}
 \ex \gll Jak\k{a}$_i$  kupi\l{}e\'{s} { [ \spc$_i$ } ksi\k{a}\.{z}k\k{e}]\\
...
\noindent
 However, the languages in question lack determiners, and therefore it is possible that what is 
 extracted is an argument of the nominal head that corresponds to a PP in languages like those in (\ref{langslbc}). 
If so, a plausible account would be a constructional one, along roughly the same lines as  \citet[133]{cxsag07}.
...
reduced in the presence of a adverbial intervening between the complementizer and the gap:
