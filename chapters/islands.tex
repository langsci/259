...
See \citet{levhubook} for more on adverbial extraction, and see  \crossrefchaptert{udc} for further discussion.\footnote{The empirical facts are less clear when it comes to adnominal PPs, however. Even PPs that are usually regarded as modifiers can sometimes extract, as in \emph{From which shelf am I not supposed to read any books?} In many such extractions the PP can alternatively be parsed as VP modifier, which complicates judgements. }
...
via a dedicated  construction, not as valents. Thus, in nominalizations like \emph{Kim's description of the problem}, for example, the  DP \emph{Kim's} is not a valent of \emph{description} but is instead constructionally co-indexed with the agent role of the noun \emph{description}  via {\sc x-arg}. Thus, the DP cannot be extracted.\footnote{Moreover, the clitic \emph{s} in \emph{Kim's} must lean phonologically on the NP it selects, and therefore cannot be stranded for independently motivated phonological reasons, predicting  the oddness of *\emph{It was Kim who I read 's description of the problem}.}
