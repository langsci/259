...
Extraction constraints on long-distance dependencies -- so-called \emph{islands} -- have
  ...
  others in this volume in that its focus is not on HPSG analyses of some phenomena, but
...
\citep{kuno87,godard92f,dubinsky2009,SS2013b-u,chavesking2020}. 
Conversely, I take it as relatively uncontroversial that the
...
\ea
\label{cschpsgr}
Coordination Schema (abbreviated):\\
...
% that extraposition does not always obey island constraints.\itdopt{Strunk's and my stuff is about
%   subjacency, it is not about the right roof constraint! The RRC is not violated in the examples.\\
% Rui: Calm down, no need for `!!!'. Subjacency *is* an island constraint which people have argued is also active in extraposition,
% but as Strunk's data shows, such examples are attested. Maybe add ``such as subjacency'' after
% ``obey island constraints.''\\
% Stefan: Hey, the ! is meant to assert importance. I am totally relaxed. I always am. I never get
% excited. Never¡¡¡¡ I even got the inverted exclamation mark for you to make this point! =:-)\\
% But appart from all this. You write that you have further evidence against syntactic accounts of
% RRC. And the problem is that our data does not violate the right roof constraint but as you say: it
% violates subjacency and the respective island constraints. So it should be in this chapter, but it
% does not show that the RRC is wrong. I see your point now since you wrote that the data shows that
% ``extraposition does not always obey island constraints''. But for me this is too implicit. You
% first talked about RRC and you do not say why subjacency should be relevant here at all. I think the
% case is really clear in German: You can cross as many NP boundaries as you want. There is no
% island. Extraposition is nonlocal. So why should this have anything to do with RRC (in German)?
...
As noted by \citet[8]{kroch89}, the reduced acceptability of an example like (\ref{kro}a) is better
