...
\abstract{ 
Extraction constraints on long-distance dependencies -- so-called \emph{islands} -- have
  been the subject of intense linguistic and psycholinguistic research for the last half century.
  Despite of their importance in syntactic theory, the heterogeneity of island constraints has posed
  many difficult challenges to linguistic theory, across all frameworks. The HPSG perspective of
  island phenomena is that they are unlikely to be due to a unitary syntactic constraint given the
  fact that virtually all such island constraints have known exceptions.  Rather, it is more
  plausible that island constraints result from a combination of independently motivated syntactic,
  semantic, pragmatic and processing phenomena.  The present chapter is somewhat different from
  others in this volume in that its focus \rep{in}{is} not on HPSG analyses of some phenomena, but
  rather on the nature of the phenomena itself.  This is because there is evidence that most of the
  phenomena are not purely grammatical, and to that extent independent from HPSG or indeed any
  theory of grammar. One may call this view of island phenomena ``minimalist'' in the sense that
  much of it does not involve formal grammar.}
...
\citep{kroch89,sza,abrusan,foxneg,abrusanspec}, although one can quibble about the particular
...
\citep{kuno87,godard92f,dubinsky2009}. Conversely, I take it as relatively uncontroversial that the
\emph{Clause Non-Final Incomplete Constituent Constraint} is due to processing difficulty
\citep{lev91,fodor92}. See also \citet{kothari} for evidence that ``bridge'' verb effects in
filler-gap dependencies are partly due to lack of contextualization.
...
given feature takes. By allowing \textsc{slash} sets to be \rep{unified}{identified} (or unioned), it
...
it suffices to state that prepositions are necessarily specified as [\textsc{slash} \eliste]. Thus,
...
\ex[]{Who$_i$ can you assure me \trace$_i$ to be the most competent?\footnote{
 \citew[\iaddpages]{kayne80}
}
}
\itdobl{These examples are not in \citew{kayne80}!}
\zl
...
assuming that islands are syntactic, in contrast to \rep{mainstream linguistic theory}{Mainstream
  Generative Grammar}.
...
\crossrefchaptert{udc} for further discussion.\footnote{The empirical facts are less clear when it
  ...
  judgements.\itdopt{Add ref to De Kuthy, also De Kuthy chapter. Kordula suggests reanalysis.} }
...
\ex Which celebrity  did you [[meet \spcs at a party] and [date \spcs for a few months]]?
\itdopt{(\mex{0}b) is identical to (\ref{cc5}f). Intended?}
\zl
...
\ea
\label{cschpsgr}
Coordination \rep{Construction}{Schema} (abbreviated):\\

\avm{
\emph{coordinate-phr\added{ase}} \impl\\
...
the ATB exceptions straightforwardly. The failure of \rep{mainstream Chomskyan grammar}{Mainstream
  Generative Grammar}\itdopt{we use this term in the book. Less ad hominem.} to predict these
...
\citet{levinepostal} and \citet{kehler}\addpages point out, there is no empirical reason to assume that the
...
symmetrically or not.  See \crossrefchaptert{coordination}\addpages for more discussion.
...
\ea \label{relcxt}
Head-Modifier \rep{Construction}{Schema} (abbreviated):\\

\avm{
\emph{head-mod-phr} \impl\\
[%synsem \| nonloc \| slash  $\lbrace \rbrace$ \\
head-dtr \1 \\
dtrs  < [synsem \1 ], [synsem [loc|mod & \1 \\ 
                                nonloc|slash & \{ \} ] ] > ]
}
\z 
...
\citep{Ross67,pollardsag,kluender,postal98,saghof}\addpages.  The sample in (\ref{csubok}) involves
acceptable extractions from NP-embedded complement CPs (some of which are definite), and
...
\eal
 \label{csubok}
\ex The money which I am making [the claim [that the company squandered \spc]] amounts to \$400,000.\footnote{
 \citew[206--207]{pollardsag}
}
\itdobl{This example is not from \citew[206--207]{pollardsag}!}
...
\ex What did you get [the impression [that the problem really was \spc]]?\footnote{
\citew[\iaddpages]{kluender}
}
\itdopt{Add $_i$ to the previous examples?} 
...
knew \trace$_i$]].\footnote{
\citew[\iaddpages]{chungmc}
}
\itdobl{This example is not in \citew{chungmc}!}
...
\ex John was able to find someone  who is willing to learn every \added{Germanic} language
...
\citet[242, 268--271]{levhubook}. Second, the counterexamples above\itdopt{Which ones? Link to latex
  label} are given out of the blue, and
...
addition, \citet{kroch89} and \citet[270]{levhubook} point out that D-Linking amelioration effects
...
Rightward movement is traditionally regarded as being clause bounded. Such \emph{\isi{Right Roof
...
\eal \label{rrc}
\ex[*]{I [met a man [who knows \trace$_i$] yesterday] [all of your songs]$_i$.}
\itdobl{reference missing}
\ex[*]{[[That a review \trace$_i$ came out yesterday] is catastrophic] [of this
  \rep{article}{book}]$_i$.\footnote{
\added{\citew[\page 375]{rochemont}}}
}
\ex[*]{It was believed \trace$_x$ that $[$there walked into the room$]$ \trace$_y$ $[$by everyone$]_x$ $[$a man with  long blond hair$]_y$].\footnote{
\citew[\page 386]{rochemont}}}
\itdopt{Replace x and y by i and j?}
\zl
...
feature \textsc{extra(posed)} \citep{kellerverb,Keller95b,Bouma96,eynde96, Mueller99a,KimSag2005},\addpages
...
may escape its clause. However, the oddness of (\ref{rrc}) may not be due \add{to} any such syntactic
...
(Howard Lasnik 2007 course
handout\footnote{\url{http://ling.umd.edu/~lasnik/LING819\%202007/Multiple\%20Sluicing\%20819\%20.pdf};
  Retr.\ 2009.})
\itdobl{Turn into bibtex-ref or delete example.}
\zl



Further evidence against a syntactic account of RRC comes from corpora
\citep{Mueller2004d,Mueller2007c} and experimental findings \citep{SS2013b-u,strunk}, which confirm
that extraposition does not always obey island constraints.\itdobl{Strunk's and my stuff is about
  subjacency, it is not about the right roof constraint! The RRC is not violated in the examples.}  The counterexamples in
(\ref{nosubza}a--c) are adapted from \citet[\page 106, 109, 111]{SS2013b-u} and \citet{strunk}, and
those in (\ref{nosubza}d--f) are from \citet[863]{chavesrnr}.
...
\citet{stucky}, and later experimentally supported by \citet{levyted}, \citet{SS2013b-u}, and
\citet{strunk} seems to be on the right track. If so, then there is no syntactic constraint on {\sc
  extra}. Rather, RCC effects are to a large extent the result of difficulty in integrating the
...
\itdopt{You say they are hard to process but you give a star. Maybe you should say somewhere in the
  paper what the judgement markings mean.}
\zl
...
languages, including \ili{English} \citep{Ross67,chomsky73}\addpages, an effect referred to as a
...
\ex[*]{Which candidate were posters of  all over the town?\footnote{
\citew[\iaddpages]{lasnikpark}
}
}
\zl
...
(\ref{sie}b,c). Indeed, \citet{annerels} shows that extractions like those in (\ref{sie}c) are in
...
\citew[\iaddpages]{santorini}}
}
\zl

\noindent
\ili{English} exceptions to the SI constraint are not restricted to PP extractions,
however. Although \citet{Ross67}\addpages claimed NP extractions from NP subjects like (\ref{rosssi}) are
...
\citet[470, 471]{chavesextr}, and \citet{chavesjeruen}\addpages.
...
\ex[]{  [Which car]$_i$ did [some pictures of \trace$_i$] cause a scandal?\footnote{
 ...
 \ex[]{  [Which president]$_i$ would [the  impeachment of \trace$_i$] cause  outrage?\footnote{
 \citew[\iaddpages]{chavesextr}}
}
 
\ex[]{ I have a question$_i$ that [the probability of you knowing the answer to \trace$_i$] is zero.\footnote{
\citew[\iaddpages]{chavessubj}}
}
...
Incidentally, subject phrases are not extraposition islands either, as shown in (\ref{subxxx}).  See
also \citet{Gueron:May:84}. \changed{The oddness of examples like *\emph{$[$Pictures \trace$]$ frighten people $[$of
  John$]$} from \citet{drummond}\addpages is} more likely due to a digging-in effect, caused by speakers
assuming that the subject is syntactically and semantically complete by the end of the verb phrase.
...
\itdopt{This is very late for a subsection in a section. Put the stuff before in a subsection as well?}
...
\ex[]{ There are [people in this world]$_i$ that --  for me to describe \trace$_i$ as despicable 
 -- would be an understatement.\footnote{
\citew[471]{chavesextr}}
}
...
particularly hard to extract from, as shown in (\ref{ssc}).  \citet{Ross67}\addpages dubbed this extreme kind
of SI the \emph{Sentential Subject Constraint} (SSC). See also \citet{chomsky73}, \citet{huang82},
...
\citew[\iaddpages]{cowper76,gibson91}}
}

\ex[*]{That that Jill left bothered Sarah surprised Max.\footnote{
\citew[\iaddpages]{kimball}}
}
 
\ex[*]{That that the world is round is obvious is dubious.\footnote{
\citew[\iaddpages]{kuno74}} 
}
\zl
...
\itdopt{What are the markings for? Processability? Grammaticality?}
\zl
...
\eal  \label{kh}
\ex[]{ Which President would [the  impeachment of \spcs] cause  outrage?\footnote{
\citew[\iaddpages]{chavesextr}}
}
...
\citep{hiramatsu00,clausencuny,chavesjeruen,chavessubjexp}.\itdopt{Change order of pubs to be chronological?} This mixed evidence is consistent with
...
pied-piping examples studied by \citet{annerels}. The latter involve relative clauses, in which
subjects are not strongly required to be topics, in contrast to the subjects \rep{or}{of} main clauses.
...
repair an unspecified thing $x$ is connected to that attempt damaging a car.


\eal
\label{pgbq}
\ex[*]{What did [the attempt to repair \spcs] ultimately damage the car?}
\ex[]{What did [the attempt to repair \spcs] ultimately damage \spc?\footnote{
\citew[\iaddpages]{colinphillips}}
}
\zl
...
\citet{Ross67}\addpages and \citet{cattell} noted that adjunct phrases often resist extraction, as
illustrated in (\ref{tad}), a phenomenon usually referred to as \emph{The Adjunct Island Constraint}
(AIC).
\itdobl{The phenomenon is not called {The Adjunct Island Constraint}. The phenomenon is that extraction is impossible and
  a constraint is formulated to capture this.}

\eal  \label{tad}
\ex[*]{What$_i$ did John die [whistling \trace$_i$]? }
\ex[*]{What$_i$ did John build [whistling \trace$_i$]? }
\ex[*]{Which club$_i$ did John meet a lot of girls [without going to \trace$_i$]?\footnote{
\citew[38]{cattell}}
}
\ex[*]{Who$_i$ did Mary cry [after John hit \trace$_i$]?\footnote{
\citew[503]{huang82}}
}
\itdopt{Where are the other examples from?}
\itdobl{The example is only mildly better than ``John hit Mary.''. Is there an example you can
  replace it with? Or drop it?}
\zl
...
\citet{cinque}, \citet{pollardsag}, \citet[253]{culicover87}, and \citet{borg}.\addpages A sample of
...
Exceptions to the AIC include tensed adjuncts, as noted by \citet[88]{grosu81}, \citet[29]{deane},
\citet{kluender}\addpages, \citet[287]{levhubook}, \citet[144]{gold06}, \citet[471]{chavesextr}, \citet[175,
...
gap is rather dubious. See \citet{levhubook}\addpages for a more in-depth discussion of parasitism and
...
of referents. But as \citet[248ff.]{ginzsag}\itdobl{give exact range} note, there is no independent evidence for saliency
...
\emph{which}-phrases in examples like (\ref{which}).

\eal  \label{which}
\ex[]{ I don't know anything about cars. Do you have any suggestions about which car -- if any -- I should buy when I get a raise?}
\ex[]{ I don't know anything about cars. Do you have any suggestions about what  -- if anything -- I should buy when I get a raise?\footnote{
\citew[248]{ginzsag}}
\itdobl{both examples?}
}
\zl
...
\textsc{wh} value, the theory of extraction proposed in \citet{ginzsag}\addpages predicts that (\ref{whoavm}a)
...
\itdobl{source}
\ex[]{ Which shoes are you wondering whether you should buy \spc?\footnote{
\citew[\iaddpages]{chavesextr}}
}
\zl

As noted by \citet{kroch89},\addpages the reduced acceptability of an example like (\ref{kro}a) is better
...
\citet{Ross67}\addpages discovered that the leftmost constituent of an NP cannot be extracted, as in
(\ref{lbcv}), a constraint he dubbed the \emph{Left Branch Condition} (LBC).\footnote{As in other
  ...
  illustrated \changed{in the following example from \citew[303]{MRS}:}
...
\noindent
These facts are accounted for in versions of HPSG like \citet{cxsag07}\addpages where Determiner Phrases
(DPs) are not valents of the nominal head. \added{See also \crossrefchapterw[Section~\ref{np:sec-functor-basics}]{np}.} \changed{If the DP is not listed in the argument
...
\itdobl{ Why is \textsc{wh} a set and the others are lists? You cannot refer to \ibox{1} as a single
  element, since N$'$ may have several elements in \textsc{restr}. I suggest removing the curly
  brackets around \ibox{1}.  non-loc vs. nonloc}
\z

\noindent
\changed{Based on \citet[133]{cxsag07}, \citet[197, 198]{CP2020a-u} assume that} genitive DPs combine
...
\gll Combien$_i$  a-t-il  vendu  [~\trace$_i$  de  livres]?\\
     how.many     has-he  sold {} of  books\\\hfill\added{\ili{(French)}}
...
\citet{Abeille:Bonami:ea:04} for details.  Finally, \citet{Ross67}\addpages also noted that some languages do
...
\noindent
In \citet{bouma} and \citet{ginzsag}, extracted arguments are typed as \type{\rep{gap-ss}{gap-synsem}} rather than
\type{\rep{canon-ss}{canon-synsem}}. Only the latter are allowed to correspond to \emph{in situ} signs and to reside in
valence lists. However, subject extraction is different. If a subject phrase is extracted, then the
\changed{\textsc{subj} list} contains the respective \type{\rep{gap-ss}{gap-synsem}} sign. If one assumes that the lexical entry
for the complementizer \emph{that} requires S complements specified as $[$\textsc{subj}
$\langle \, \rangle]$ then the oddness of (\ref{subobjasymm2}) follows. For \citet{bouma}\addpages and
\citet{ginzsag}\addpages, the adverbial circumvention effect in (\ref{advcirc}) is the result of assuming
that the construction which allows the adverb to combine with the clause forces the mother node to
be [\textsc{slash} \eliste], a rather \emph{ad-hoc} account.\footnote{Except that in \ili{French}, when
  the subject of the complement CP is extracted, the complementizer is {\it qui} instead of {\it
    que}, which could easily be captured by such an account.}
\itdobl{I do not understand this explanation. If you provided page numbers I could have read up. How
can an empty SLASH list be helpful in licensing something that should be ruled out otherwise?}
