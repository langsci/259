...
The result of applying the revised Semantics Principle to \word{potentially controversial} is provided in Figure~\ref{pot-contr-sem} and the semantics of \word{controversial} and \word{potentially} are provided in (\ref{contr-sem}) and (\ref{pot-sem}), respectively. (The value of \attrib{arg} in Kasper's analysis of \word{controversial} corresponds to the syntactic and semantic properties of the modified constituent. The feature \attrib{arg} value is thus the equivalent of the \attrib{synsem} value  in current HPSG.) 
