...
Although prosodic phrasing is not generally believed to always align with syntactic phrasing, the fact that the coordinator prosodifies with the  coordinand suggests that it forms a unit with it.
 ...
 The analysis in (\ref{flat}) can be formalized in HPSG as
\inlinetodostefan{This reference went to Figure~3 but this did not make sense. Did you mean example (\ref{flat})? If so, your statement is not true since the coordination Coord is not covered by the schema.
}
...
In order to  account for the fact that different kinds of coordination strategies are possible, \citet[\page 89]{Mouret:05} 
\inlinetodostefan{Mouret has the first two implications but not the third one. I do not speak French, so maybe there is something in the paper. But I guess you misquoted him, please adapt text}
...
\inlinetodostefan{I redid (\mex{1}). Please check.}
...
\noindent
In French, as in other Romance languages, the conjunction itself can be reduplicated, and it is obligatory for some conjunctions (\emph{soit} `or' in French) \citep{Mouret:07, Bilbiie:17}:\todostefan{This is French, I cannot find the place. Please add page numbers.} 
...
more'. \citet{Abeille:06} and \citet{Abeille:Borsley:08} propose that they are  coordinate in some languages, 
 ...
 \inlinetodostefan{I reinserted the \emph{then}. We had a long discussion about this. I think it is
   correct with \emph{then}. Please check.}
...
\inlinetodostefan{Is finite a subtype of \type{verb} or should there be a \vform feature?}
\ea
\label{correlphrr}
...
\inlinetodostefan{What is \type{prs} in (\mex{1}b)? Present? This is tense information, usually not \vform. Do you mean \type{inf}?}
\eal
\label{vform}
\ex[*]{Tom [whistled]\sub{\vform \type{fin}}      and [singing]\sub{\vform \type{prp}}.}
\ex[*]{Sue [buy something]\sub{\vform \type{prs}} and [came home]\sub{\vform \type{fin}}.}
\zl
...
pragmatic in nature, as \citet{kehler}\addpages argues. 
...
In the following example, again from \ili{Portuguese}, the determiner and prenominal adjective agree with the first Noun\todostefan{noun?} (\ref{fo}a) and the postnominal adjective with the last Noun\todostefan{noun?} (\ref{fo}b).
...
\inlinetodostefan{(\mex{1}b,c) are not in \citet{Abeille:An:Shiraishi:18}, no source for (\mex{1}c). State sources in main text or provide source below (\mex{1}c).}
\begin{exe}
 \ex
\begin{xlista}
\ex  
...
\glt `certain regions and departments' \citep{Abeille:An:Shiraishi:18}\todostefan{This example is not in the paper. Chose one from the paper.}
...
While conjuncts have often been assumed to be phrasal (see for example \citet{Kayne:94}\addpages and \citet[Section~5.2]{bruening} a.o.), \citet{Abeille:06} gives several arguments in favor of lexical coordination.
...
\inlinetodostefan{\citet{hpsg1} talk about particles on p.\,176--177 and on p.\,181. They talk about the part of speech Part and its \lexv, not about projecting PPs. Do you mean a PartP? Maybe split (\mex{1}) into (\mex{1}a,b) and (\mex{1}c) as separate examples and mention them to make clear that (\mex{1}a,b) are based on PS claims and (\mex{1}c) is new.}
...
\inlinetodostefan{Your sentence ended in the middle. I guess you wanted to cite \citep[\page 14]{Abeille:06} here too.}
\eal
\label{fr1}
\ex[]{  
...
\textsc{arg-st} lists as it is for complex predicates, see \crossrefchaptert[Section~\ref{complex-predicates-sec-argument-attraction}]{complex-predicates}). It thus inherits all the dependents of the word it combines with.
...
regard such phenomena as base-generated\todostefan{strange term in a constraint-based paper} \citep{Mouret:06}.
...
\end{xlista}
\end{exe}
\inlinetodostefan{(\mex{0}c) example is not in \citet{Abeille:Godard:1996}. Please provide correct reference. And translate/gloss.}
