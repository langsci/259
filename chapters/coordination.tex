...
interpretations, as noted by \citet{ross67};\todostefan{check these} see also
...
\inlinetodostefan{17.04.2020: Just noticed that the general type implies a \phonv, which is probably wrong. It is way to general also the COORD value of \type{and} seems wrong. I guess \phon and COORD have to be removed.}
\begin{exe}
\ex 
\begin{avm}
...
\inlinetodostefan{actually it is a
  ...
  \citet{Miller92d-u}} Assuming a degree word \textit{the}, which can only appear as a specifier of
...
\inlinetodostefan{Cannot parse the following sentence. Why \emph{in}? Do they belong to this class or is Borsley claiming they do?}
...
\inlinetodostefan{Check my modification.}
\ea
\label{correlphrr}
...
\inlinetodostefan{\citet{binomial} is a book. I tried to find the place you refer to. I could not
  find it. Please provide a page number.}
\inlinetodostefan{
The conjuncts in (\ref{ex-case-match-coordination}) are \textbf{not} consistent.}
...
\inlinetodostefan{Isn't (\ref{cs1}b) grammatical if you see \emph{To him Fred gave a football} as a sentence that is coordinated with \emph{Kim gave me a book}? To indicate the problem, I would suggest brackets around the conjuncts like [X and Y].}
\begin{exe}
\ex \begin{xlista}
...
masculine, at least for animates \citep{Corbet91}\todostefan{This is a book. You have to add a page here.}. This is illustrated in (\ref{pt}), for Portuguese.
...
As observed by \citet{Corbet91},\addpages many languages including \ili{Romance}, \ili{Celtic}, \ili{Semitic} and \ili{Bantu} languages, also have another strategy, namely partial agreement with only one conjunct, the one closest to the target, called \emph{closest conjunct agreement} (CCA)\is{agreement!closes conjunct}. 
...
\inlinetodostefan{no source for (\ref{ft}c). State sources in main text or provide source below (\ref{ft}c).}
\begin{exe}
 \ex
\begin{xlista}
\ex  
...
\crossrefchapterw[Section~\ref{agreement-sec-index-concord}]{agreement}). \citet{Moosally}\todostefan{Do not have this paper, please add page numbers} proposes an account
...
\glt  `Gwenllian and I have not been paid.' \citep[\page 12]{Sadler2003a-u}\todostefan{It is example 12 but cannot be page 12. Add real page number, I do not have the paper.}
\end{exe}
...
complement (\ref{ex-turn-off}a), but not before (\ref{ex-turn-off}b); but coordination is possible, at least for some
speakers as the example in (\ref{ex-turn-off}c) from \citew[\page 23]{Abielle2006a} shows.
\eal
\label{ex-turn-off}
...
Analyzing the conjunction as a weak head (see above), the sub-type for lexical coordination has to allow for the coordination of items waiting for complements (this is done by concatenation of 
\textsc{arg-st} lists as it is for complex predicates, see \crossrefchaptert[Section~\ref{complex-predicates-sec-argument-attraction}]{complex-predicates}): the conjunction thus inherits all the dependents of the word it combines with. 
...
17.04.2020 Figure~\ref{light} does not explain what is going on here. The whole \argst trick has to be explained. And note that the lexical item is multiple ways ambiguous since the \argst of the embedded head can be split in various ways. The last paper you cite here is \citet{Abeille:06}, but Abielle does not work with \argst. She works with \subj and \comps. If you do this with \argst a lot of questions are open: How does linking work? The category of the head is not known. Do all arguments go to \ibox{1}? All to \ibox{2}? A lot of things are open here. Did somebody else do this in the literature before? If so cite her/him.}

\inlinetodostefan{I cannot fill in Anne's text here, since (16) is not a lexical entry. Do you mean (13)? This has to be revised.}
The entry for the conjunction \emph{and} in (\ref) will be revised as follows (with \type{lex-coord-lexeme} type instead of of \type{coord-lexeme}):
...
\inlinetodostefan{Where is (\ref{unlk}a) from? The URL of (\ref{unlk}c) from 2010 did not work any more. I got you a working one.}
\begin{exe}
\ex
\begin{xlista}
...
\inlinetodostefan{The sentences in (\ref{treesa}) are neither in \citet{Beavers} nor in \citet{chaves06} contrary to the claim.}
\noindent
...
\citet{Yatabe:04}, \citet{chavesrnr} and others. The rule in (\ref{rnrcx}) illustrates the account adopted by 
 \citet{chavesrnr}\todostefan{add pages, could not find them}  and \citet[\page 19]{aoi}
...
\citet{buitelaar1998corelex} does not contain the cited example. I added Artstein's page number. But the example in Artstein is longer and without caps. The example (\ref{badp2}f) from \citet[274]{chaveslp} starts with \emph{I} not with \emph{we}. I changed this.}
\begin{exe}
\ex \begin{xlista} 
...
\inlinetodostefan{The examples in (\ref{disrnr})  are from \citep[\page 868]{chavesrnr}. You may want to add this information.}
\begin{exe}
\ex \begin{xlista}
...
\inlinetodostefan{Where are these examples from? (\ref{gnc}b) is from Park (page 30) but Park is not cited. (\ref{gnc}c) is also from Park p. 31. The example by Park is an attested example, the source should be given and the attested example does not contain a dash!}
\begin{exe}
\ex 
\begin{xlista}
