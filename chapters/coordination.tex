...
See \citet{borsley94},\todostefan{check all these} 
\citet{Borsley2005a} %ok
and 
...
That said, there are some reasons for assuming that the coordinator does in fact combine with the coordinand, as in (\ref{ok}). First, in some  languages of the world the coordinator is a bound morpheme instead of a free morpheme. For example,  verbs are coordinated by adding one of a set of suffixes to one of the coordinands in \ili{Abelam} (Papua New Guinea),  usually the prior one.  Similarly, in Kanuri (Nilo-Saharan), verb phrases are coordinated by marking the first verb with a conjunctive form affix, and
 in languages like \ili{Telugu} (Dravidian), the coordination of proper names is marked  by the lengthening of their final vowels \citep[\page 111]{Drellishak:Bender:05}. The latter is illustrated in (\ref{telugu}) quoted from \citew[\page 111]{Drellishak:Bender:05}.
...
\inlinetodostefan{17.04.2020: Just noticed that the general type implies a \phonv, which is probably wrong. It is way to general also the COORD value of \type{and} seems wrong. I guess \phon and COORD have to be removed. answer: it is the entry for and just like 13}
\begin{exe}
\ex 
\begin{avm}
...
(see \crossrefchapterw[Section~\ref{sec:UDC:Middle}]{udc} and \crossrefchapterw[\page \pageref{page-hpsg-traceless-account-arg-st-extraction-conjuncts}]{islands}).
...
See \citet{Bilbiie:08}\addpages for arguments that both types are attested in \ili{Romanian}.
...
\inlinetodostefan{I changed the glosses according to the Leipzig Glossing Rules, please check. Put
  \textsc{ppart} into the abbrevs or use \textsc{ptcp} for participle.}
\begin{exe}
 \ex
\begin{xlista}
\ex 
...
feature in \citet{Bonami:2004} for \ili{French} \isi{liaison}).
\inlinetodostefan{actually it is a
  \textsc{left} feature in ABE2006 and an \textsc{edge} feature in Borsley. Borsley cites
  \citet{Miller92d-u}; answer:This is fine, LEFT was an abreviation for EDGE LEFT} Assuming a degree word \textit{the}, which can only appear as a specifier of
a comparative word, \citet[\page 13]{Borsley:11}  defines \textit{the}-clause as a subtype of \type{head-filler-phrase} with [\textsc{correl} \type{the}]; see also \citet[\page 527]{fgsag08}.

\inlinetodostefan{Cannot parse the following sentence. Why \emph{in}? Do they belong to this class or is Borsley claiming they do? answer:both: Borsley is claiming that and he is right}
...
\inlinetodostefan{Check my modification. fine}
\ea
\label{correlphrr}
...
[synsem|loc|cat|correl  \type{none}\\
 dtrs < [synsem|loc|cat|correl \type{corr-mrk} ], %\smallskip\\
...
\inlinetodostefan{\citet{binomial} is a book. I tried to find the place you refer to. I could not find it. Please provide a page number.}
\inlinetodostefan{The conjuncts in (\mex{1}) are \textbf{not} consistent.}
...
\inlinetodostefan{Isn't (\mex{1}b) grammatical if you see \emph{To him Fred gave a football} as a sentence that is coordinated with \emph{Kim gave me a book}? To indicate the problem, I would suggest brackets around the conjuncts like [X and Y].}
\begin{exe}
\ex \begin{xlista}
...
\inlinetodostefan{I fixed the page number in \citet[205]{levineetal}. (\mex{1}b) is not in this paper and it doesn't seems to be an example for what you are claiming. The NP is not the filler of the gap. \emph{which} is the gap and the other relative clause does not have a filler since you say that \emph{that} is not a relative pronoun.}
\eal
\label{syn}
...
\inlinetodostefan{This claim is not exact. They have a much larger hierarchy and the subhierarchy you cite here should have \type{scase} at the top. Alternatively you may reformulate the statement above.}
...
with inconsistent person, gender and number specifications are free to combine. But \citet[Chapter~2]{wechsler} have also argued that there should be a distinct feature called \textsc{concord}, which is morphosyntactic in nature (see \crossrefchapterw[Section~\ref{agreement-sec-index-concord}]{agreement}). The motivation for this move, is that there are languages, like \ili{Serbo-Croatian}, 
...
masculine, at least for animates \citep[186]{Corbet91}\todostefan{This is a book. You have to add a page here.}. This is illustrated in (\ref{pt}), for Portuguese.
...
As observed by \citet[186]{Corbet91},\addpages many languages including \ili{Romance}, \ili{Celtic}, \ili{Semitic} and \ili{Bantu} languages, also have another strategy, namely partial agreement with only one conjunct, the one closest to the target, called \emph{closest conjunct agreement} (CCA)\is{agreement!closes conjunct}. 
...
\inlinetodostefan{no source for (\mex{1}c answer: no need to put a source for every example). State sources in main text or provide source below (\mex{1}c).}
\begin{exe}
 \ex
\begin{xlista}
\ex  
...
\crossrefchapterw[Section~\ref{agreement-sec-index-concord}]{agreement}). \citet{Moosally}\todostefan{Do not have this paper, please add page numbers; dont have it either, sorry} proposes an account
...
\glt  `Gwenllian and I have not been paid.' \citep[\page 12]{Sadler2003a-u}\todostefan{It is example 12 but cannot be page 12. Add real page number, I do not have the paper.CAN we say ex. 12 ?}
\end{exe}

\inlinetodostefan{Add citation of \citet{Reape94a}. Crossref to order chapter: \crossrefchaptert{order}, I crossref from order to here.}
\noindent
...
\inlinetodostefan{I changed the order of the examples and referenced them in the text. Hope this is OK. Please check.}
...
Semantically, lexical coordination is more constrained than phrasal coordination. With \textit{and}, two lexical verbs, sharing a preverbal clitic in French, must share the same verbal root, and in Spanish, they must refer to the same event \citep{Bosque:86}.\todostefan{Could not find paper. Please add page numbers. answer:I dont have ot}

\eal
\ex[]{
\gll Je te  dis  et  redis  que  tu  as   tort. \\
     I  you tell and retell that you have wrong\\\hfill(Spanish)
...
\gll Paul cherche   le                ou la                responsable.\\
     Paul looks.for the.\textsc{m.sg} or the.\textsc{m.sg} responsible\\\hfill(French)
...
\noindent
The functor analysis of coordinands in (13) is compatible with lexical coordination, since the head-functor phrase in (14) has the same valence features as the head. The weak head analysis in (16) is also compatible since the conjunction inherits the complements expected byt the conjunct (this is done by concatenation of 
\textsc{arg-st} lists as it is for complex predicates, see \crossrefchaptert[Section~\ref{complex-predicates-sec-argument-attraction}]{complex-predicates}) 
...
17.04.2020 Figure~\ref{light} does not explain what is going on here. The whole \argst trick has to be explained. And note that the lexical item is multiple ways ambiguous since the \argst of the embedded head can be split in various ways. The last paper you cite here is \citet{Abeille:06}, but Abeille does not work with \argst. She works with \subj and \comps. If you do this with \argst a lot of questions are open: How does linking work? The category of the head is not known. Do all arguments go to \ibox{1}? All to \ibox{2}? A lot of things are open here. Did somebody else do this in the literature before? If so cite her/him. Answer: we go back to subj and comps now; no need for a special entry for lexical coordination}
...
\inlinetodostefan{This section is largely identical in text to parts of \citet[Section~1.1 and 1.2]{Chaves2013b-u}. This is self plagiarism, which is scientifically unethical. Apart from this there are copyright issues. The material is copyrighted by CSLI Publications. I noticed this by chance, but there may be issues with other parts in this chapter.}
...
\citet[\page 580]{bayer}, % Republican & proud of it
...
\inlinetodostefan{I could not find any of these examples in \citet{GKPS85a}. \citet{GKPS85a} discuss asymmetric coordination on page 174 though. I found the Africa example in \citet[\page 1327]{rodney}. The \emph{republican} is in Bayer. Where are the other examples from?}
\begin{exe}
\ex \begin{xlista}
...
\inlinetodostefan{I would read the following paragraph as if this is what Jacobson showed. But she did not discuss the examples you are discussing here. So this is misleading. Maybe cite her with page 417 and then explain that you are dicussing something else.}
\noindent
...
\inlinetodostefan{Where is (\mex{1}a) from? The URL of (\mex{1}c) from 2010 did not work any more. I got you a working one.}
\begin{exe}
\ex
\begin{xlista}
...
\inlinetodostefan{Where are the other examples from?}
\begin{exe}
\ex
\begin{xlista}
...
\inlinetodostefan{What does CRD stand for? Formally? It is small caps so this seems to point to a
  feature name. But this cannot be right. What is it? answer: crd is the same as in (12): the COORD value of any coordinand}
...
\inlinetodostefan{The sentences in (\mex{1}) are neither in \citet{Beavers} nor in \citet{chaves06} contrary to the claim.}
\noindent
...
\inlinetodostefan{Replaced \emph{some underspecified non"=headed construction} by \emph{the underspecified non"=headed construction} because of hypenation issues and since \type{ac-cx} was already introduced in the text.}
\noindent
...
\glt `Paul is planning to offer a record to Marie and a book to Jean.\\
}
\ex[*] {
...
\inlinetodostefan{add glosses and translations to all (!) examples what is \emph{pas}? Is it `not'?}
\begin{exe}
 \ex 
 \begin{xlista}
\ex[] {
...
\citet{bresnan74}, \citet[299]{wexlercull},  \citet[45]{grosu81}, \citet{mccawley},\todostefan{some have page numbers others do not. Consider adding numbers to all.} and \citet[382, footnote 30]{sab}
...
In our experience, informants do not systematically share this judgment.}\todostefan{Add page numbers to Steedman in fn. One source is a book.}
...
\inlinetodostefan{What about the other examples? Are there sources? All these examples appear in \citet[\page 867]{Chaves2014a-u}}
\begin{exe}
\ex \begin{xlista}
...
\citet{Yatabe:04}, \citet{chavesrnr} and others. The rule in (\ref{rnrcx}) illustrates the account adopted by 
 \citet[874]{chavesrnr}\todostefan{add pages, could not find them}  and \citet[\page 19]{aoi}
...
\inlinetodostefan{Isn't there any other reference? This is such a frequently discussed topic. Is there no publication since 1995 discussing this? Google scholar lists and others cite it as 1993 not 1995, but it is not available anymore.}
...
\inlinetodostefan{\citet[\page 936]{Milward94} contains the sentence \emph{*[John will] and [Mary built the] drive}, that is, with different proper names and without small caps.
\citet{buitelaar1998corelex} does not contain the cited example. I added Artstein's page number. But the example in Artstein is longer and without caps. The example (\mex{1}f) from \citet[274]{chaveslp} starts with \emph{I} not with \emph{we}. I changed this.}
\begin{exe}
\ex \begin{xlista} 
\ex[*] {Randy \textsc{saw} and Rene has \textsc{been} [flying planes].}\todostefan{give source?}
\citep{booij:85}
...
\end{figure}
\inlinetodostefan{I and phi are strange in Figure~\ref{rnrt}. There is no feature but a bracket for embedding. This needs
  explanation. It is formally incoherent.}



\inlinetodostefan{\emph{Politicians who fought \textsc{for}} was \emph{Politicians who have fought \textsc{for}}. I fixed this.}
\begin{exe}
\ex \begin{xlista}
...
In the example in Figure~\ref{rnrt}  the sub-list \avmbox{R$_3$} is resolved as the empty list, but this need not be so. When the latter sublist is not resolved as the empty list, we obtain discontinuous Right-Node Raising cases like (\ref{disrnr}), due to  \citet[238--240]{Whitman:09}
and \citet[868]{chavesrnr}, where the RNRaised expression is followed by extra material. 
\inlinetodostefan{The examples in (\mex{1})  are from \citep[\page 868]{chavesrnr}. You may want to add this information.}
\begin{exe}
\ex \begin{xlista}
...
\inlinetodostefan{Where are these examples from? (\mex{1}b) is from Park (page 30) but Park is not cited. (\mex{1}c) is also from Park p. 31. The example by Park is an attested example, the source should be given and the attested example does not contain a dash!}
\begin{exe}
\ex 
\begin{xlista}
