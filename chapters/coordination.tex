...
\inlinetodostefan{This reference went to Figure~3 but this did not make sense. Did you mean example (\ref{flat})? If so, your statement is not true since the coordination is not covered by the schema.
}
...
\inlinetodostefan{I redid this. Please check.}
...
\noindent
In French, as in other Romance languages, the conjunction itself can be reduplicated, and it is obligatory for some conjunctions (\emph{soit} `or' in French) \citep{Mouret:07, Bilbiie:17}:\todostefan{This is French, I cannot find the place.} 
...
feature in \citet{Bonami:2004} for \ili{French} \isi{liaison}).\todostefan{actually it is a
...
\inlinetodostefan{Isn't (\mex{1}b) grammatical if you see \emph{To him Fred gave a football} as a sentence that is coordinated with \emph{Kim gave me a book}? To indicate the problem, I would suggest brackets around the conjuncts.}
\begin{exe}
\ex \begin{xlista}
...
\glt  `Gwenllian and I have not been paid.' \citep[\page 12]{Sadler2003a-u}\todostefan{It is example 12 but cannot be page 12.}
\end{exe}

\inlinetodostefan{Reape was not cited here. Crossref to order chapter: \crossrefchaptert{order}, I crossref from order to here.}
\noindent
...
\inlinetodostefan{I could not find any of these examples in \citet{GKPS85a}. \citet{GKPS85a} discuss asymmetric coordination on page 174 though. I found the Africa example in \citet[\page 1327]{rodney}. The republican is in Bayer. Where are the other examples from?}
\begin{exe}
\ex \begin{xlista}
...
\inlinetodostefan{What about the other examples? Are there sources?}
\begin{exe}
\ex \begin{xlista}
...
See \citet{swingle}\todostefan{Isn't there any other reference? This is such a frequently discussed topic. Is there no publication since 1995 discussing this? Google scholar lists it as 1993, but it is not available anymore.} for more discussion. 
