...
\isi{Sign-based Construction Grammar} (cf.\ \crossrefchapteralt[Section~\ref{sec-sbcg}]{cxg}).
...
provides some discussion of some brief discussion of alternative approaches.
...
to the same kind of formal mechanisms as \feat{slash}.\footnote{The assumption that
  ...
  \cite{Van-Eynde:04} (\citeauthor{Van-Eynde:04} argues it should be treated as a local
  dependency).}
...
\avm{
  ...
  \ex\label{x:rc-18} \avm{\ibar{N}$_{\1}$ [rel & \{ \1 \} ]}
\end{xlist}
...
\lic{wh}-phrase.\footnote{We assume, for simplicity,
  ...
  \istype{scope-object}\type{scope-object}), like the \feat{que} and \feat{wh} attributes
...
variation (cf.\ for example, \citealt{Webelhuth92a-u}), but even in \ili{English} the data are
...
\begin{exe}
  ...
  \end{xlist}
\end{exe}
However, \cite[311]{NanniStillings78} give examples \pref{x:rc-231} and \pref{x:rc-232}
and argue that \lic{compared}, and \lic{seated} can be analysed as adjectives,
\cite[129]{Webelhuth92a-u} gives \pref{x:rc-233}, which uncontroversially involves an AP,
...
are far from common.\footnote{Examples like \pref{x:rc-234} appear often in discussions of
  ...
  God. \pref{x:rc-235} is from a legal judgement
...
\begin{exe}
    \ex[] {Light, [\sub{AdvP} faster than which] nothing can travel \trace, takes 412 years to get from here to the
...
\pref{x:rc-236}, and \cite[399]{Ishihara84} gives example \pref{x:rc-237}, both of which
seem fully acceptable.\footnote{Notice also that an analogue of \pref{x:rc-27} is
  grammatical in \ili{German}. See \cite{deKuthy99a}, \cite{HN99a} and \cite{Mueller99a}
  for discussion and HPSG analyses of the phenomena in \ili{German}. Some discussion of
  pied-piping in \ili{French} can be found in \cite{Godard92} and \cite{SG94}.}
\begin{exe}\ex\begin{xlist}
...
\feat{que}-inheritance (i.e.\ pied-piping in interrogatives).\footnote{See for example
  \cite[578--586]{Horvath05BSC}.} For example, consider the contrast in \pref{x:rc-2930},
...
\pref{x:rc-29}:\footnote{On \citegen{Ginzburg:Sag:00} account, \pref{x:rc-29} is excluded
  ...
  the initial element on the \feat{arg-st} of \lic{pictures}, and \lic{(of) whom} is
...
\cite[490--493]{Sag:10b} emphasises, though there are some ``\lic{wh}-expressions'' which
...
\lic{wh}-expression''):\footnote{See also \cite[81--85]{Mueller99b} on differences between
  ...
  (though it is fine as an interrogative, as can be seen in \pref{x:rc-33}). \cite[491,
...
\avm{
 ...
 \end{exe}
\begin{exe}
\ex\label{x:rc-38}\ibox{1} : \ensuremath{\{person(\ibox{1}),
...
\avm{
	[synsem &	[loc &	[cat &	[head & [mod & \upshape \ibar{N}$_{\1}$ ] ] \\
						cont &	[\type*{scope-obj}
								index & \1 \\
...
\begin{figure}
  \begin{forest} % qtree edges
   [\ibar{N}$_\ibox{1}$%, s sep=5em, baseline
...
The first argument of R is specified as [\feat{rel}~\{\ibox{1}\}]. Thus, it must contain
...
\pref{x:rc-8}, above); the [\feat{rel}~\{ \}] on the daughter S excludes the
...
\gll wajadtu    l-kitab-a    [{llaði} tuħib-\textbf{hu}     Hind-un] \\
     found.{\sc 1.sg} {\DEF-book-\ACC} \hphantom{[}{that.{\sc m.sg}}   like.{\sc 3.f.sg-3.m.sg} Hind-\NOM\\\jambox*{(\ili{Arabic})}
...
\avm{\emph{inf-hd-fill-ph} \impl
 ...
 \ex\label{x:rc-56}
   \istype{inf-hd-fill-rel-cl}
\avm{\emph{inf-hd-fill-rel-cl} \impl
   \istype{inf-hd-fill-phrase}
\emph{inf-hd-fill-ph} \&
	[non-hd-dtrs & < PP >]
}
 \end{xlist}
...
controversial, and an interesting construction in \ili{French}.\footnote{There are also cases which involve a \isi{relative pronoun} \emph{and} a
  ...
  \end{exe}
  \cite{Hinrichs:Nakazawa:02} analyse these as \lic{wh}-relatives,\is{relative
  ...
  dependencies in \ili{Irish}, \cite{Assmann:Heck:ea:10} discuss how \ili{Irish} relative
...
\pref{x:rc-59} (see, for example, \crossrefchaptert[\page \pageref{fig:UDC:59}]{udc} which
gives an appropriate lexical entry for \lic{that} on this approach).  However,
historically, this approach has not always been favoured: \cite{Pollard:Sag:94} treated some uses of
...
\cite[462--464]{Sag:97} prefered to treat \lic{that} as
...
is another issue: \cite{Sag:97} assumes that all and only \lic{wh}-relatives are NP
...
associated with a gap that is complement of a preposition. But this makes it difficult to
...
value along the lines of \pref{x:rc-72} (cf.\ also \cite{winckel2020french}).
\begin{exe}\ex\label{x:rc-72}
\avm{
	[\type*{synsem}
	loc &	[cat &	[head &	[\type*{c}
							mod & \upshape \ibar{N}$_{\1}$ ] \\
					comps &	<[loc &	[cat & \upshape S\subscr{fin} \\
									cont & \2 ] \\
							non-loc &	[slash & \{ [\type*{nprl}
														cat & \upshape PP\subscr{de \1} ] \} $\uplus$ \4 ] ] > ] \\
                        %   \upshape{NP\subscr{de}}\thinspace\subscr{\@1}
			cont & \2 ] \\
	non-loc|slash & \4  ]
}
\isfeat{local}\isfeat{head}\isfeat{mod}\isfeat{comps}\isfeat{non-local}\isfeat{cat}\isfeat{content}\istype{nprl}
\end{exe}
...
values, is inherited upwards).\footnote{\cite{AbeilleGodard07} assume that gaps and resumptive pronouns are
...
pronoun, and replace it with a gap dependency.\footnote{As \cite{AbeilleGodard07} point
  ...
  \begin{exe}\ex\label{x:rc-80}\gll
    ...
    spoken \hphantom{\trace} \hphantom{[}and that we to-it will.come.back more late\\\jambox*{(\ili{French})}
...
\gll Naomi-ga \trace\subscr{i} yon-da hon\subscr{i}\\
     Naomi-{\sc nom} {} read-\textsc{pst} book\\\jambox*{(\ili{Japanese})}
...
\begin{exe}\ex\label{x:rc-82}
 \gll [motwu-ka           [Kim-i       \trace\subscr{i} ilk-ess-ta-ko]  sayngkakha-nun] chayk\subscr{i} \\
      \hphantom{[}everyone-{\sc nom} \hphantom{[}Kim-{\sc nom}  {}     read-{\sc pst-decl-comp} think-{\sc prs.mod}   book\\\jambox*{(\ili{Korean})}
...
\avm{\emph{hd-relative-mod-phrase} \impl
	[head & noun \\
	slash &  \{ \} \\
	hd-dtr & \2 \upshape N$_{\1}$ \\
	non-hd-dtrs &	<S	[\punk{head|mod}{\2} \\
						slash & \{NP$_{\1}$ \}]> ]
}
...
\avm{\emph{non-wh-rel-cl} \impl
	[ %\asort{rel-cl} 
	head &	[mod &	[head & \upshape \ibar{N}$_{\1}$] ] \\
	slash & \{ \} \\
	hd-dtr &	[slash & \{NP$_{\1}$\}] ]
}
...
\begin{forest}  %qtree edges
	[\ibar{N}    , baseline
		...
		[%
		\avm{
			S [mod & \2 \upshape \ibar{N}$_{\ibox{1}}$ \\
			slash & \{\} ]
		}
			[NP
				[Kim]
			]
			[\avm{VP [slash & \{NP$_{\ibox{1}}$\}]}
				[bought ,roof]
			]
		]
	]
...
\avm{
	[head &	[\type*{verb}
			mod & \upshape \ibar{N}$_{\ibox{1}}$ ] \\
	comps & < \upshape PP > \\
	slash & \{ \upshape NP$_{\ibox{1}}$ \} ]
}
...
\begin{exe}\ex\label{x:rc-111}
  ...
  three people({\sc fem}) \hphantom{[}among which.{\sc fem} John {\sc aux} come\\\jambox*{(\ili{French})}
...
\is{relative clause!antecedent of}antecedents. \cite{Holler:03} provides an analysis of \ili{German} non-restrictive relatives which are adjoined to S, as
...
\cite{Arnold:04a} provides an analysis of \ili{English} non-restrictive relatives of all kinds. This
...
\cite{Arnold:Borsley:10}\label{page-relative-clauses:stranded-aux} look at NRCs where the antecedent\is{relative clause!antecedent of} is a VP, and where the gap is the
...
%                       The professor has been seen who smoke-{\sc imperf}\\\jambox*{ (\ili{French})}
...
\begin{exe}\ex\label{x:rc-135}
  \gll [ya indɛ mi wɛ gɔ]  yimaa   boli.\\
       \hphantom{[}yesterday person {\sc 1sg} see.{\sc pn.$\emptyset$} {\sc def} die.{\sc psp} go.{\sc pn.3sg}\\\jambox*{(\ili{Dogon})}
