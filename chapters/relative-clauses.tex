...
\abstract{ We provide an extended discussion of analyses of relative clauses
  ...
  ``pseudo'' relative clause\add{s}, and free (headless) relatives.
}
...
\isi{Sign-based Construction Grammar} (SBCG; cf.\ \crossrefchapteralt[Section~\ref{sec-sbcg}]{cxg}
\added{on SBCG and \crossrefchapteralt{evolution} on the evolution of HPSG}).

The basic organisation of the discussion is as follows\rep{.}{:} Section~\ref{sec:rc-approaches}
...
\hilite{provides some discussion of some brief discussion} of alternative approaches.
...
In examples like \pref{x:rc-3} the \lic{wh}-phrase must contain a \isi{relative pronoun}.
\itdopt{Stefan: ``the fronted phrase''? Otherwise it sounds strange since of course a
  \emph{wh}-phrase has to contain a \emph{wh}-element, otherwise it would not be one. Of course
  there are different \emph{wh}-elements, but still.} Here we
have another apparently \rep{unbounded}{nonlocal} dependency,\itdobl{It is bounded: Clause bounded. But it is a
  nonlocal dependency.} because the \isi{relative pronoun} can be embedded
...
\label{x:rc-12}
  ...
  \ex\label{x:rc-16} \changed{reports [the height [of [the lettering [on [the covers [of which]]]]]
...
percolation}'', or, following \citet{Ross67a}, ``\isi{pied-piping}''. We will talk about
\emph{\isi{relative inheritance}}.\itdopt{Stefan: I would avoid the term inheritance
  here. Non-formal CxG
...
exist in other languages, so it is reasonable to assume that \isi{relative inheritance} involves a set of some kind.\footnote{Examples of languages which allow
  ...
  relate to the same entity). \citet[\iaddpages]{Kayne17SomeEvenMoreUnusual} gives some \ili{English}
...
to the same kind of formal mechanisms as \feat{slash}.\footnote{The assumption that
  ...
  handled with a \feat{nonlocal} feature, like \feat{slash}), has been challenged in
...
\lic{wh}-phrase at the top of the relative clause\itd{But is in this \emph{wh}-phrase all the
  time. How can it reach the \emph{wh}-phrase? Do you mean: ``It reaches the top node of the \emph{wh}-phrase?''} (equivalently: a relative clause
introduces a non-empty \feat{rel} value on its \lic{wh}-phrase daughter that is inherited downwards till it is
realised as a \isi{relative pronoun}).\footnote{%
  Note that the relative word has its normal syntactic function as a
  determiner or a full NP. This is different from most approaches in \isi{Categorial Grammar}, which assume that the relative word is the functor taking a clause
  with a gap as argument \citep[\iaddpages]{Steedman97a}. As \citet{Pollard88a}
  pointed out pied-piping data like the one discussed in (\ref{x:rc-12})
  are problematic for \isi{Categorial Grammar}. These problems were addressed in
  ...
  a general comparison of Categorial Grammar and HPSG. \added{\citeauthor{chapters/cg} addresses pied-piping
  on p.\,\pageref{cg:page-pied-piping-start}--\pageref{cg:page-pied-piping-end}.}}
Within the \lic{wh}-phrase, \feat{rel} inheritance can
...
higher than the level at which the associated filler appears.\footnote{In case it is not
  ...
  beyond the relative clause \lic{to whom Kim spoke}, so that e.g.\ \lic{a person to whom
    Kim spoke} was marked as [\feat{rel}~\{\ibox{1}\}]. This phrase would be able to
  ...
  [\feat{index}~\ibox{1}] to produce something like *\lic{a person} [[\lic{a person to whom
...
value, roughly as in \pref{x:rc-17}, which we abbreviate to \pref{x:rc-18}.\footnote{Here, and
  ...
  an empty \changed{\feat{spr}} (\feat{specifier}) list (e.g.\ a combination of determiner and a \ibar{N}).
  ...
  VP for a phrase consisting of a verb with its complements, and S for a phrase consisting of
  a subject and a VP.\itdopt{This definition requires a projection of V to VP, which I would avoid
    in the definition. ``Kim laughs.'' would need a unary projection. The definition would not work
    for German and Icelandic which have subjectless clauses. I would define S as a fully saturated
    verbal projection. ``Go!'' would be an S as well. Similarly a VP should be defined as a verbal
    projection or lexical item with an empty \compsl and something in \spr/\subj.}}
\eal
\ex
\label{x:rc-17}  
...
\ex\label{x:rc-18} \changed{\ibar{N}[\textsc{rel} \{ \ibox{1} \}]\ind{1}}
\zl
...
\ibox{1} is the index that appears in the \feat{rel} value of the associated
\lic{wh}-phrase.\footnote{We assume, for simplicity,
  ...
  \citet[\iaddpages]{Pollard:Sag:94} and \citet[\iaddpages]{Sag:97}, but not with \citew[188]{Ginzburg:Sag:00}, who
  assume it is a set of \istype{parameter}\type{parameters}, that is, indices with restrictions (a kind of
  \istype{scope-object}\type{scope-object}), like the \feat{que} and \feat{wh} attributes\add{,}
...
the index of its antecedent. In terms of standard HPSG semantics,\itdopt{JP said that actor and
  undergoer would be standard.} this
...
\pref{x:rc-21}, abbreviated to  \pref{x:rc-22}:\footnote{In fact \pref{x:rc-21} is already somewhat abbreviated:
   ...
   \istype{naming}\type{naming} relation to the name \type{Kim} \citep[\page 27]{ps2}\com{added ref
     to ps94}.}
\begin{exe}\ex\begin{xlist}
  \ex\label{x:rc-21}
\avm{
...
variation (cf.\ for example, \citealt{Webelhuth92a-u}\addpages), but even in \ili{English} the data are
...
are far from common.\footnote{Examples like \pref{x:rc-234} appear often in discussions of
  ...
  God. \pref{x:rc-235} is from a legal judgement \added{at}:
  \url{http://www.worldcourts.com/pcij/eng/decisions/1927.09.07_lotus.htm}\added{, accessed 2021-02-04}.}
\begin{exe}\ex\begin{xlist}\label{x:rc-230}
  ...
  \end{xlist}\end{exe}
Examples involving \rep{Adverb Phrases}{adverb phrases} are rarely discussed, but they can also be found, though
...
\z
\itdopt{Is ``faster'' an adverb? In German grammar adverbs are things that do not inflect, while
  adjectives are things having a comparative. So I would call this thing adjective. There are
...
seem fully acceptable.\footnote{Notice also that an analogue of \pref{x:rc-27} is
  grammatical in \ili{German}. See \citew{deKuthy99a}, \citew{HN99a} and \citew[\iaddpages]{Mueller99a}
...
treat the phenomenon as involving restrictions on the way the \feat{rel} feature is
inherited (i.e.\ \isi{relative inheritance}, pied-piping in relative clauses) --- e.g.\ as
indicating that while \feat{rel}-inheritance from e.g.\ NP to PP (and through an upward
...
the Clausal \feat{rel} Prohibition of \citealt[220]{Pollard:Sag:94}, which requires the
\feat{rel} value of S to be empty,\itdopt{So the dependency is bounded!} correctly excluding examples like \pref{x:rc-28}, but
...
The second point is that it is worthwhile emphasising that restrictions on \feat{rel}, and
\feat{rel}-inheritance are different from the restrictions on \feat{que} and
\feat{que}-inheritance (i.e.\ pied-piping in interrogatives).\footnote{See for example
...
\pref{x:rc-29}:\footnote{On \citegen{Ginzburg:Sag:00} account, \pref{x:rc-29} is excluded
  ...
  [\feat{wh}~\{ \}], \feat{wh} corresponding to what we are here calling \feat{que} (the
  \feat{Wh}-Constraint, \citealt[189]{Ginzburg:Sag:00}). In \pref{x:rc-29} \lic{some} is
  the initial element on the \feat{arg-st} of \lic{pictures},\itdblue{Made my day: this means that
    one cannot assume the functor analysis of the NP. Just a note to me and you. No need to change
    anything, but you could.} and (\lic{of}) \emph{whom} is
...
cannot --- ones which can be interpreted as interrogative but not as \isi{relative
  pronoun}s (i.e.\ which have non-empty \feat{que} values, but empty \feat{rel} values),
and ones which can be interpreted as relative pronouns but not interrogatives (i.e.\ with
non-empty \feat{rel} values, but empty \feat{que} values). For example, \lic{how} and (in
...
\lic{wh}-expression''):\footnote{See also \citew[81--85]{Mueller99b} on differences between
  ...
  note 10]{Sag:10b} suggests that NP \lic{which} is only ever\itdopt{is only ever? I am not a
    native, maybe it is ok.} a \isi{relative pronoun} (an
...
\add{the} relation between the relative clause and the nominal it modifies (its antecedent).
...
\citet{Sag:97}, which does not.\footnote{\citet{Mueller99b} presents what might be
  considered a third approach, which resembles \citew{Sag:97} in avoiding empty elements,
  ...
  \citew{MuellerCurrentApproaches} also presents a rule schema for relative clauses
  (\emph{loc cit} Section 6.1). Rule schemas were a crucial piece of apparatus in the framework of
  \citet{Pollard:Sag:94}, but they have fallen out of favour with the rise of construction-based 
  analyses since \citew{Sag:97}. A rule schema is essentially just a phrasal type ---
  ...
  \citep[74]{Mueller99b}.
\itdobl{The schema in \citet{Mueller99b} is there because the whole
paper is in PS94 style. \citew{MuellerCurrentApproaches} have a schema but this schema is basically
a constraint on a type. And we state in the text that the type \type{relative-clause} inherits from
a type for head-filler-phrases. Even if it is only one schema, one can miss generalizations with
it. =:-)}}
\is{relative clause!antecedent of|)}
...
of \pref{x:rc-39} \citep[cf.][216]{Pollard:Sag:94}. Here XP~\ibox{4} is intended to
mean an XP whose \feat{local} value is \ibox{4}, and S:\ibox{3} means a clause (a
saturated \istype{verb}\type{verb}\itdobl{a saturated projection of type \type{verb}? For me a verb
  is always a lexical item.} -- i.e.\ one with empty \feat{subj} and \comps
specifications) whose \feat{content} is \ibox{3}. The \ibox{2} that appears in the
...
\avm{
	[synsem &	[loc &	[cat &	[head & [mod & \upshape \ibar{N}\ind{1} ] ] \\
						cont &	[\type*{scope-obj}
								index & \1 \\
								restr &  \2 $\cup$	\{[\type*{fact}
													prop|soa & \3]\} ] ] ] \\ 
	arg-st & < \cPile{{\upshape XP} \4 \\ [rel & \{ \1 \} ]}, 
               \cPile{{\upshape S}:\3 \\ [slash & \{ \4 \} ]} > ]
}
...
head-adjunct phrase to produce the structure in Figure~\ref{fig:rc-3}.\footnote{Here again
  we have used PP \ibox{4} to indicate a PP whose \feat{local} value is \ibox{4}.}
\begin{figure}
\changed{
\begin{forest}
sm edges
...
following \changed{way: the} first argument of R is specified as [\feat{rel}~\{\ibox{1}\}]. Thus, it must contain
...
like \feat{rel} and \feat{slash} (nonlocal features) the value on the mother is the
...
\feat{slash} value on its S sister (so that \ibar{R} is [\feat{slash}~\{ \}]), and we can
ensure that the topmost \ibar{N} is [\feat{rel}~\{ \}], so long as its head \ibar{N} daughter is specified as
binding-off the \feat{rel} value on RP. This specification can be imposed by stipulation
in the \feat{mod} value of R.  See \citew[Section~5.2.2]{Pollard:Sag:94} for details.
\is{relative inheritance|)}
...
of these is the standard construction for head-filler phrases.
\itd{Shouldn't it be: ``One of these is a subtype of head-filler phrases.''? Or something else? In
  any case I have a problem with this and the following sentence.}  The other involves a
...
Figure~\ref{fig:rc-42}.\footnote{See \citew[Chapter~11]{KimSellsIntroduction} for an introductory
  ...
  \citet[\iaddpages]{Pollard:Sag:94} in treating them differently (cf.\ footnote~\ref{fn:rc-1}; and see
...
\begin{figure}
  ...
  \caption{Type hierarchy for {\protect\istype{clause}\type{clause}}, based on \citew[\iaddpages]{Sag:97}}
  \label{fig:rc-42}
...
\begin{exe}\ex\label{x:rc-46}
  \istype{wh-rel-cl}
\avm{\emph{wh-rel-cl} \impl
	[head &	[mod \ibar{N}\ind{1}] \\
...
constraints from different dimensions
\itdobl{This was the case before already. But it is now used for the clausal domain.} (cf.\
\crossrefchapteralt[\pageref{properties:page-cross-classification-of-lexemes}]{properties}). As well
...
(\istype{hd-fill-phrase}\type{hd-fill-ph}), thus inheriting constraints as in \pref{x:rc-47}.\footnote{The
  ...
  except that it is undefined for pairs of sets that share common elements. Its use here
  is what ensures that the \feat{slash} value of the mother is the \feat{slash} value of the
  head daughter less the \feat{local} value of the non-head daughter.}
\begin{exe}\ex\label{x:rc-47}
  \istype{hd-fill-phrase}
\changed{\avm{\emph{hd-fill-ph} \impl
[slash  & \1 \\
 hd-dtr & [head & verbal \\
 	   slash & \{ \2 \} $\uplus$ \1 ] \\
...
values\itdopt{I stumbled here because I thought you were talking about a general constraint that blocks
  passing up \REL values. But what is required here is something that binds off \REL. So this is a
  crucial part of the relative clause analysis. You don't have to do anything here. I am just saying
  what happened to me.
} 
will license local trees like that in Figure~\ref{x:rc-48} for a finite relative
clause (\istype{fin-wh-rel-cl}\type{fin-wh-rel-cl}) like \pref{x:rc-3} (simplifying, and
disregarding most empty and irrelevant attributes\itdopt{What is an ``empty attribute''? A
  list/set-valued attribute with the value of an empty list/set?}).\footnote{\label{fn:rc-3}This
  assumption about \feat{rel} values is one of many minor technical differences between
  \citew{Sag:97} and \citew{Pollard:Sag:94}, where the non-empty \feat{rel} value is
  ...
  clause is a \feat{rel}\add{-}marked clause.}
...
(the PP) in \pref{x:rc-46}\itdopt{``(the PP) in \pref{x:rc-46}'' is confusing, since \pref{x:rc-46}
  does not contain a PP.} ensures the presence of a \lic{wh}-phrase, and the fact that this
is a head-filler phrase ensures that the \lic{wh}-phrase cannot be \emph{in situ} (cf.\
\pref{x:rc-8}\itdopt{replace by \pref{ex:a-person-Kim-spoke-to-whom-yetsterday}?}, above); the [\feat{rel}~\{ \}] on the daughter S excludes the
possibility of additional \isi{relative pronoun}s inside the S (i.e.\ the possibility of multiple
\isi{relative pronoun}s, cf.\ *(\emph{the person}) \emph{to whom Kim spoke about whom}). \feat{rel}
...
\feat{rel} values.\footnote{%
  ...
  apparatus that is now standardly assumed for propagation of all nonlocal features,
\itdopt{Not everybody assumes this mechanism. Not everybody assumes defaults. I do not. I guess the whole Tübingen/Frankfurt gang
  does not. I would change this to ``frequently'' or ``often''.}
  \feat{slash}, \feat{wh} (i.e.\ \feat{que}), and \feat{background}\itdopt{Is background a nonlocal
    feature? For me this sounds very strange. It is like claiming that the \textsc{rels} list of MRS
    is a nonlocal feature since its elements are passed up.}
...
\changed{\avm{
\emph{head-relative-phrase} \impl\\
[head & noun \\
 cont &	[index & \1 \\
         restr & \2 $\uplus$ \{[\type*{fact}
             		        prop & \3 ]\} ] \\
 hd-dtr & [index & \1 \\
           restr & \2 ] \\
...
\z
\itdobl{I asked for this in an earlier review but it was not addressed: What about \emph{Peter loves his red car and it is really red.}?
Here the disjoint set union would fail, wouldn't it? For semantics simple multi-sets would be
correct, wouldn't it? And you use $\cup$ in an earlier lexical item.
}
...
Figure~\ref{fig:rc-4}.\footnote{This is not the normal semantics associated with head-adjunct
  ...
  with content in this way:\itdopt{Difficult to find out what ``this way'' means.} \istype{head-adjunct-phrase}\type{head-adjunct-ph} itself would impose no constraints on
  content. Notice that we again follow \citet[\iaddpages]{Ginzburg:Sag:00} in taking restrictions to be
...
noun-modifying semantics appropriate for a relative clause \citep[522]{Sag:10b}.
\itdobl{This is what I did all the years. Add
  \parencites{Mueller99b}[Section~6.1]{MuellerCurrentApproaches} somewhere here.}
\is{relative clause!wh-relative@\lic{wh}-relative|)}
\is{relative clause!construction|)}
...
different (e.g.\ in the use of phonologically empty lexical heads vs.\ the use of \add{phrasal} constructions), and there
...
analysis \citep[see \emph{inter alia}][]{Schachter73a-u,Vergnaud74a-u,Kayne94a-u}\addpages, and the
\emph{matching} analysis \citep[see \emph{inter
  alia}][]{Chomsky65a,Lees61Constituent,Sauerland98MeaninChain}\addpages. Under the \is{relative
...
in HPSG, see \crossrefchapteralt{arg-st} \added{for argument structure and
  \crossrefchapteralt{binding} for binding in HPSG}). More important, as discussed in
\citew{Webelhuth18Idioms}\addpages, both face numerous empirical difficulties and miss important
...
here.\footnote{For example, both analyses treat \lic{wh}-words like
  ...
  never are \citep[see][\iaddpages]{Webelhuth18Idioms}.}
\is{relative clause!wh-phrase@\lic{wh}-phrase in|)}
...
extended to deal with other kinds of relative clause\add{s} in a variety of
...
\gll wajadtu    l-kitab-a    [{llaði} tuħib-\textnobf{hu}     Hind-un] \\
     found.\textsc{1.sg} {\DEF-book-\ACC} \hphantom{[}{that.\textsc{sg.m}}   like.\textsc{\changed{3sg.f}-\changed{3sg.m}} Hind-\NOM\\\jambox*{(\ili{Arabic})}
...
\lic{wh}-relative in \changed{Figure~\ref{fig-wh-relative-clause}} above only in the presence of the \isi{PRO} on the
\feat{subj} list.\footnote{The use of S\subscr{inf} in Figure~\ref{fig:rc-5} is an
  ...
  as is often assumed in HPSG analyses (e.g.\ \citealt[51--52]{Ginzburg:Sag:00}\added{;
    \citealt{Levine2012a-u}; \citealt[\page 89]{Sag2020a}}).\itdopt{added Levine and Sag 2020}
 S and CP are
  not very different (both \istype{verb}\type{verb} and \istype{comp}\type{comp} are
  subtypes of \istype{verbal}\type{verbal}), but \citet[\page 458]{Sag} is careful to treat \lic{to} as a
...
[%
\avm{S$_{inf}$
  [head & \1	\upshape ![\textsc{mod} \ibar{N}\ind{2}]! \\
   cont & \3 \\ % \[ \asort{fact} prop\|soa & \@4 \]\\
   slash & \{ \}\\
   rel & \{ \}\\
   subj & < \4 > ]}
...
\eal
\label{x:rc-55}
\ex \istype{inf-hd-fill-phrase}
\avm{\emph{inf-hd-fill-ph} \impl\\
	[hd-dtr &	[head &	[vform & non-finite] \\
				subj & < PRO >] ]
} 
\isfeat{head-daughter}\isfeat{head}\isfeat{subj}\isfeat{vform}\istype{non-finite}\is{PRO}
\ex\label{x:rc-56}
\istype{inf-hd-fill-rel-cl}\istype{inf-hd-fill-phrase}
\avm{\emph{inf-hd-fill-rel-cl} \impl\\
\emph{inf-hd-fill-ph} \&
	[non-hd-dtrs & < PP >]
}
\itdobl{Mentioning the type \type{inf-hd-fill-ph} is redundant, since this is part of the type
  hierarchy anyway.}
\zl
...
\subsection{Complementi\changed{s}er relatives}
...
controversial, and an interesting construction in \ili{French}.\footnote{There are also cases which involve a \isi{relative pronoun} \emph{and} a
...
\itdopt{``but where it is controversial, and an interesting construction in French''. I get a garden
  path here.}
\is{relative clause!construction|)}
...
\end{exe}
\ea\label{x:rc-58}
\added{Arabic complementiser \emph{ʔallaði} `that' adapted from \citet[\page 42]{Alqurashi:Borsley:12}:}\\
\avm{
	[\type*{synsem}
	loc &	[cat &	[head &	[\type*{c} 
...
\end{exe}
According to this, \emph{ʔallaði} \added{`that'} will combine with a slashed finite sentential
...
\emph{ʔallaði} \added{`that'} itself (cf.\ its own empty \feat{slash} specification). That is,
...
purpose clauses and \lic{tough}-constructions).\footnote{\citet[42]{Alqurashi:Borsley:12}
  assume that \del{the} \feat{slash} inheritance is governed by a default principle, so the
  empty \feat{slash} specification on \emph{ʔallaði} \added{`that'} prevents upward
  inheritance. The same effect could be achieved in other ways (e.g.\ with an appropriate \feat{to-bind}
  specification). }


Since \emph{ʔallaði} \added{`that'} shows inflections agreeing with the \is{relative clause!antecedent of}antecedent NP for
...
alternative.\footnote{\ili{Arabic} also has finite relatives that do not have an overt
  ...
  \citet{Haddar:Boukedi:09,Zalila:Haddar:11}, \citet{Hahn:12}, and \citet{Crysmann:Reintges:14}\iaddpages
...
\pref{x:rc-59} (see, for example, \crossrefchaptert[\page \pageref{fig:UDC:59}]{udc} \changed{for an}
...
a \isi{relative pronoun}.\footnote{\label{fn:rc-4}\citet[\iaddpages]{Pollard:Sag:94} treat instances of 
...
non-empty \feat{rel} value on the relative clause (cf.\ Figure~\ref{fig:rc-3}). The fact that it
...
to have empty \feat{rel} values (cf.\ above Section~\ref{sec:rc-sag-1997}, especially
footnote~\ref{fn:rc-3}) so similarity of \feat{rel} values is not an issue. However, there
...
footnote~\ref{fn:rc-2}). Since coordination involves identity of \feat{mod} values, data
...
associated with a gap that is \add{the/a} complement of a preposition. But this makes it difficult to
analyse it as a filler in a head-filler phrase, where \feat{slash} inheritance ensures
identity between the \feat{local} values of filler and gap (including, of course
\feat{case}): if \lic{that} is nominative, then it should not be
compatible with non-nominative gaps, such as we see in \pref{x:rc-64}.\itdopt{Is ``such as we see'' English?} But
if it is not a filler, then it must be a head (or marker).

Treating \lic{that} as a head, presumably a  complementiser, is in some respects straightforward (cf.\ the
lexical entry in \crossrefchaptert[\page \pageref{fig:UDC:59}]{udc} is a starting point\itdopt{``Cf''
  seems to be incompatible with a following sentence. I always thought it means ``compare the following''.}),
...
as the head should be imposed.\footnote{It also ignores the analysis of \lic{who}, which one
  ...
  that a treatment of other filler-gap mismatches will provide an account of why \pref{x:rc-68}
...
something of a challenge, which is addressed in \citew{AbeilleGodard07}. \lic{Dont} is
generally analysed as a complementiser introducing finite relatives
\citep{Godard92a-u}\addpages. It can introduce a relative with a PP\sub{\lic{de}} gap (i.e.\ a gap
...
\citeauthor{AbeilleGodard07}\addpages suggest a lexical entry for \lic{dont} with a \feat{synsem}
value along the lines of \pref{x:rc-72} (cf.\ also \citew{WA2020a-u}\addpages).
\ea
\label{x:rc-72}
\added{Lexical entry for the French complementiser \lic{dont}:}
\avm{
	[\type*{synsem}
	loc &	[cat &	[head &	[\type*{c}
...
\itdobl{Shouldn't the semantics of the complemntizer incorporate the semantics of the modified noun?}
...
content (its \feat{content} is just that of its complement S). The complement S has
a \feat{slash} value that contains a PP\sub{\lic{de}} which is
co-indexed with the \is{relative clause!antecedent of}antecedent nominal, as specified in the \feat{mod} value. This \feat{slash} element is non-pronominal
(\istype{nprl}\type{nprl}) --- that is, a genuine gap, rather than a \isi{resumptive
  pronoun}, and is not inherited upwards (only \ibox{4}, the remaining set of \feat{slash}
values, is inherited upwards).\footnote{\citet[\iaddpages]{AbeilleGodard07} assume that gaps and resumptive pronouns are
...
\itdobl{add translation}
    \ex[*]{\gll
      ...
      a problem   of.which \hphantom{[}Paul  is   sure that everything goes itself
      to.solve\\\jambox*{(\ili{French})}}\label{x:rc-74}
  \end{exe}%
\itdobl{Shouldn't (ii) be a minimal pair with (44)? Also add brackets like in (44).}
  }
\begin{exe}\ex\label{x:rc-76}
  \gll
  un problème dont	[Paul est certain [qu' on \textnobf{le} résoudra]]\\
...
\begin{exe}
  \ex{\gll
    un problème dont [Paul est certain [qu' {il y a}  [quelqu’un  qui \textnobf{le} résoudra]]]\\
    a problem of.which \spacebr Paul is sure \spacebr that {there is}  \spacebr someone
    that it will.solve\\\jambox*{(\ili{French})}\label{x:rc-78}
...
\begin{exe}
  \ex[*]{\gll
    un problème dont {il y a} [quelqu’un qui est certain qu’ on \changed{\textnobf{le}} résoudra]\\
    a problem of.which {there is} \spacebr someone who is sure that one it will.solve\\\jambox*{(\ili{French})}
  }\label{x:rc-171}
\end{exe}
...
involves inheritance of a resumptive element,\itdobl{Not the resumptive element is inherited in the dependency.}
...
\feat{slash} value containing a pronominal (\istype{prl}\type{prl}) element (that is, a CP containing a resumptive pronoun). The effect of the rule is to provide a lexical entry that
...
pronoun, and replace it with a gap dependency.\footnote{As \citet{AbeilleGodard07} point
  ...
  \ea\label{x:rc-80}
  \gll
    un problème dont     Paul est certain \added{[}[que nous avons parlé  \trace] [et que nous \textnobf{y} reviendrons plus tard]\added{]}\\
    a  problem  of.which Paul is  sure    \hphantom{[[}that we have   spoken {} \hphantom{[}and that we to.it will.come.back more late\\\jambox*{(\ili{French})}
  \glt Lit: `a problem of which Paul is sure that we have  spoken and that he is sure that we will come back to it later'
  \z
\itdopt{I added brackets to make the structure clearer.}
  Dealing with this
  involves a formal complication that we leave aside here. See \citew[\iaddpages]{AbeilleGodard07}.}
...
are heads of verbal \rep{predications}{predicates}, and operate via ``predicate composition'' --- by
...
nominal with the corresponding index).\itdopt{Like \citet{Mueller2002b} for prenominal adjectives
  in German.} 
...
nonlocal relativization, like \pref{x:rc-85}, should be treated as involving
...
\itdopt{But then a special version of the morpheme would be needed for these cases. And how is
  itguaranteed that it does not apply to local cases? How do we assure that these empty elements do
...
\itdobl{Specification of HEAD noun seems superfluous, since the head daughter is specified to be a noun.}
...
appropriate \feat{slash} values is required. In \citew{Pollard:Sag:94} this was the role
...
In \citew{Sag:97} the task of licensing such bare relatives is carried out by an \added{unary
...
This licenses structures like that in Figure~\ref{fig:rc-8}.\footnote{Sag also proposes a
  ...
  \end{exe}
  \itdopt{See also \citet{Mueller2002b} for modal infinitives in German. \emph{ein zu lesendes Buch}
  a to read book}
...
\end{forest}}
\itdopt{I made this more similar to the previous figure. Top-node is now \ibar{N}\ind{1} as
  well. The S is MOD \ibox{2} and the \nbar is to the left after the \ibox{2}.}
...
This differs from Kim's proposal for \ili{Korean} in \add{the way} which the \feat{slash} value is
bound off: in particular, where Kim's analysis involves a nominal and a slashed S, Sag's
involves a nominal and an \emph{un}slashed S --- the clause is [\feat{slash}~\{ \}], it
is the VP which is [\feat{slash}~\{NP\}]. This reflects the fact that in \ili{English} the gap in the
...
\itdobl{This should be without the PP to be compatible with the examples in Figure~\ref{fig:rc-8}
  and (\mex{1}).}
\end{exe}
...
\itdobl{But isn't this how slash amalgamation works? You remove arguments from the \argst and put
  ...
  set collected from the \argstl without putting something into \slasch. The solution would be (\mex{-1}) with an empty \slasch set.
}
...
\rep{their heads}{the heads they modify} in two semantically distinct ways, either restrictively (in the normal way) or
...
The approach to NRCs developed in \citew{Arnold:04a} is \emph{syntactically integrated} --- NRCs
...
\citet{Arnold:Borsley:10}\label{page-relative-clauses:stranded-aux} look at NRCs where the antecedent\is{relative clause!antecedent of} is a VP, and where the gap is the
...
clause, without further stipulation. \added{See also \crossrefchapterw{ellipsis},
  \crossrefchapterw{negation}, \crossrefchapterw{udc} and \citew{Sag2020a}\addpages for further discussion.}
  
NRCs normally follow their \is{relative clause!antecedent of}antecedents. However, as \citet{Goldman12Supplemental} observes,
...
\itdopt{Change the order of examples so that b is mentioned in the text before c? I see that they
  were in a different order \ldots} 
...
feature \feat{relzr}, introduced by \citet{Sag:10b}, which is shared between a relative
...
for details. \added{\citeauthor{Kathol:Pollard:95}'s approach is discussed in more detail in \crossrefchapterw[Section~\ref{sec-partial-compaction}]{order}.}

A second approach treats extraposition as involving a nonlocal dependency, introducing a
nonlocal feature, typically called something like \feat{extra}, which functions much
like other nonlocal features (e.g.\ \feat{slash}).  The idea is that a relative
...
properties (e.g.\ the \feat{local} features) are added to the \feat{extra} \rep{list}{set}
\itdopt{Surprising
  for readers who may expect a set here as well. Berthold has sets in his chapter on udcs.} of the
head, and inherited up the tree until they are discharged from the \feat{extra} \rep{list}{set} by
...
semantics, so long as it is pushed onto the \feat{extra} \rep{list}{set} of the VP, from where it
...
and \citew{Crysmann2013a}. \added{Extraposition is also discussed in \crossrefchapterw[Section~\ref{sec:UDC:Extraposition}]{udc}.}
...
\citew{Walker2017}. This
approach exploits the more flexible approach to semantic composition provided by \isi{Minimal
  Recursion Semantics} (MRS, \citealt{CFPS2005a}), in the case of \citew{Kiss2005a}, and
\isi{Lexical Resource Semantics} (LRS, \citealt{richtersailer-lrs04}) in
\citew{Walker2017}. \added{See also \crossrefchapterw{semantics} for a discussion of both of these
...
\feat{anchors} feature, which is inherited upwards in the manner of a nonlocal
...
interpretations can be obtained. \added{See also \crossrefchapterw[Section~\ref{udc:sec-generalised-modification}]{udc} for a more detailed discussion of
\citegen{Kiss2005a} approach.}
\is{Minimal Recursion Semantics|)}
\is{Lexical Resource Semantics|)}
...
\itdobl{Müller (2004) compares grammars with continuous constituents with grammars using
  discontinuous constituents. I actually argue for discontinuous constituents, e.g. Kathol \&
  Pollard. Later I abandoned discontinuous constituents. What is correct though is that I pointed
  out that Kiss cannot account for all extraposition facts with generalized modification. Berthold
  describes this correctly in the UDC chapter.}
terms of theoretical costs and benefits there seems to be little to choose between these
alternatives\itdopt{Berthold draws other conclusions. Maybe a consensus is not necessary. He argues
  for EXTRA because the mechanism of liberation of domain objects is not used in grammars of
  English. Coordination seems to do without linearization domains as well, as Anne seems to suggest.} --- the first and third approaches rely on particular approaches to constituent order
...
\rep{lists}{sets}). Empirically, there are several issues that all approaches deal with more or less
successfully (for example, the Right Roof Constraint from \citealt[Section~5.1.2]{Ross67a} that prevents
...
examples.\footnote{Another case where a relative clause should be analysed as a complement
  is discussed in \citew{Arnold:Lucas:16}.\itdopt{seems rather unspecific \ldots.}}
\begin{exe}\ex\begin{xlist}
...
\ex\label{x:rc-132} 
\gll On l' a  vu \emph{qui} \emph{s'enfuyait}\add{.}\\
     we him have seen who run.away.\changed{\textsc{ipfv}}\\\jambox*{(\ili{French})}
\glt `We saw him running away\add{.}'
...
\citealt{Walker2017}\addpages).\is{relative clause!as complement!of \lic{diejenigen}}%
...
The \ili{French} example in \pref{x:rc-132} contains a so-called \emph{predicative
  relative clause} (PRC).\footnote{The \ili{French} term is \emph{proposition relative dépendante attribut} \citep[\iaddpages]{Sandfeld65Syntaxe}.} Such clauses have the superficial form of a finite relative
clause, but differ from them syntactically, semantically, and pragmatically.
\citet{KL99a-u} analyse them as a form of \is{secondary predicate}\emph{secondary predicate} (cf.\
\lic{running away} in \ili{English} \lic{We saw them running away}). Syntactically, they are
restricted to post-verbal positions, and are only permitted with certain kinds of verb\itdopt{kinds
  of verbs, maybe ``kinds of verb'' is correct. If so, other instances with plural are in the text
...
\end{exe}
\itdobl{What does $\emptyset$ stand for? Should be explained or mentioned in the abbreviations at
  the end.}
...
clause as a complement.  \citet[234]{Pollard:Sag:94} suggest (following \citealt[\iaddpages]{Culy:90}) that NPs
...
the index of the whole NP. (This ignores a number of technical and empirical issues that
have to do
with the inheritance and binding-off of \feat{rel} values.)
\itdobl{What is ``this'' referring to? ``Pollard \& Sag'' or ``This brief summary of P\&S''?}}
\is{relative clause!as complement!in dependent noun construction|)}
...
affixes are allowed
on the verb (here only \lic{-nun}); and only a limited range of nouns allow this kind
...
distribution of NPs or PPs\itdopt{I would say the \emph{put} selects a location rather than a
  specific part of speech. \emph{there} and \emph{where} are adverbs. Adverbs are different from
  preposition in not requiring an argument.
} rather than clauses (for example in \pref{x:rc-138} \lic{what I
...
possible. This is the so-called \emph{matching effect} in free relatives.\footnote{In
  fact, things are more complicated. For example, in \lic{He walked to} [\emph{where his horse
    was waiting}]\lic{.}
\itdopt{This is explained if ``to'' and ``put'' do not care for part of speech but select a location.} we have a free relative with \lic{where} in an NP position (object of a
  preposition) rather than a PP position. See e.g.\ \citew[\iaddpages]{KimMixed2017} for discussion.}
...
\itdopt{But beginnen selects a mit-PP, doesn't it?}
}
...
\is{relative clause!free|)}
\itdopt{OK, one can mix the two approaches and this would work for English. But it would not work
  for German, since acc RCs can fill slots for nom NPs. You did not mention the non-matching
  relative clauses. These are crucial and my proposal is the only one that works here. One has to
  have a place in the analysis where the connection between the selection of the lower verb and the
  higher verb is interrupted, that is, a relative clause with an accusative relative phrase is
  projected into an NP with nominative case.
}
...
relatives\is{relative clause!free}, as in \citew{Gerbl2007a}\addpages.\footnote{It can be
...
\istype{head-adjunct-phrase}\type{head-adjunct-ph}\itdopt{can I turn all these \type{-ph} into
  \type{-phrase}? \type{ph} looks so stingy. =:-)} structures, but
...
We are grateful to the editors of this volume\itdopt{name them?} and two anonymous referees for their careful
