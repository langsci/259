...
\feat{rel} value, which will be inherited upwards until it reaches the top node of the \emph{wh}-phrase (equivalently: a relative clause
...
realised as a \isi{relative pronoun}).\footnote{%
  ...
  determiner or a full NP. This is different from most approaches in \isi{Categorial Grammar}, which
...
value, roughly as in \pref{x:rc-17}, which we abbreviate to \pref{x:rc-18}.\footnote{Here, and
  ...
  an empty \feat{spr} (\feat{specifier}) list (e.g.\ a combination of determiner and a \ibar{N}).
...
mean an XP whose \feat{local} value is \ibox{3}, and S:\ibox{4} means a clause (a
saturated projection of type \istype{verb}\type{verb} -- i.e.\ one with empty \feat{subj} and \comps
specifications) whose \feat{content} is \ibox{4}. The \ibox{2} that appears in the
value of \feat{restr} is identical to the \feat{restr} set of the antecedent\is{relative clause!antecedent of} nominal.
\ea
\label{x:rc-39}
...
following way: the first argument of R is specified as [\feat{rel}~\{\ibox{1}\}]. Thus, it must contain
...
is a sub-type of \type{head-filler-phrase} which takes care of the relationship between the preposed wh-phrase
...
constraints from different dimensions \crossrefchapteralt[\pageref{properties:page-cross-classification-of-lexemes}]{properties}). As well
...
The \feat{rel} specification on the non-head daughter in \pref{x:rc-46}, which corresponds to the PP in Figure~\ref{x:rc-48},
...
\pref{ex:a-person-Kim-spoke-to-whom-yetsterday}, above); the [\feat{rel}~\{ \}] on the daughter S excludes the
...
\feat{subj} list.\footnote{The use of S\subscr{inf} in Figure~\ref{fig:rc-5} is an
  ...
  and here there is a non-empty \feat{subj}. Second, Sag would have CP instead of S here,
...
\emph{ʔallaði} `that' itself (cf.\ its own empty \feat{slash} specification). That is,
...
purpose clauses and \lic{tough}-constructions).\footnote{\citet[42]{Alqurashi:Borsley:12}
  assume that \feat{slash} inheritance is governed by a default principle, so the
  empty \feat{slash} specification on \emph{ʔallaði} `that' prevents upward
...
\citet[\page 54]{AbeilleGodard07} suggest a lexical entry for \lic{dont} with a \feat{synsem}
...
\begin{exe}
  \ex[*]{\gll
    un problème dont {il y a} [quelqu’un qui est certain qu’ on \textnobf{le} résoudra]\\
...
lexical rule along the lines of \pref{x:rc-79}, where $\oplus$ signifies the ``append'' relation --
...
pronoun, and replace it with a gap dependency.\footnote{As \citet{AbeilleGodard07} point
  ...
  \ea\label{x:rc-80}
  \gll
    un problème dont     Paul est certain [[que nous avons parlé  \trace] [et que nous \textnobf{y} reviendrons plus tard]]\\
...
This differs from Kim's proposal for \ili{Korean} in how the \feat{slash} value is
...
properties (e.g.\ the \feat{local} features) are added to the \feat{extra} set of the
head, and inherited up the tree until they are discharged from the \feat{extra} set by
...
semantics, so long as it is pushed onto the \feat{extra} set of the VP, from where it
...
\citew{Walker2017}. This approach exploits the more flexible approach to semantic composition provided by \isi{Minimal
...
\feat{extra}-style apparatus (e.g.\ \citealt{Crysmann2013a},
...
\citealt{Mueller2004b} looks at computer processability of linearization-based grammars vs.\
  grammars with continuous constituents, that is, grammars using the \textsc{extra} mechanism. He
  prefers the linearization approach for computational reasons. However, the linearization approach
...
Such clauses have the superficial form of a finite relative
...
external subject (instances of \istype{head-subject-phrase}\type{head-subject-phrase} which have a non-empty
