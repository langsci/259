...
\ChapterDOI{10.5281/zenodo.13644934}
...
\author{Doug Arnold\orcid{0000-0002-3685-8450}\affiliation{University of Essex} and Danièle Godard\orcid{0000-0001-7354-6264}\affiliation{Université de Paris, Centre national de la recherche scientifique (CNRS)}}
...
value, roughly as in \pref{x:rc-17}, which we abbreviate to \pref{x:rc-18}.\footnote{Here, and
  ...
  subscripts on nouns and NPs. We use \ibar{N} to indicate a noun with a determiner in \spr and an empty \comps
  list, i.e.\ one which has combined with its complements, if any, and NP for a fully saturated nominal sign with
  an empty \attrib{spr} (\attrib{specifier}) list and an empty \compsl (e.g.\ a combination of determiner and a \ibar{N}).
...
\ex\label{x:rc-18} NP[\textsc{rel} \{ \ibox{1} \}]\ind{1}
\zl
...
head-adjunct phrase to produce the structure in Figure~\ref{fig:rc-3}.\footnote{Here again
  we have used PP \ibox{3} to indicate a PP whose \attrib{local} value is \ibox{3}.}
\begin{figure}
\begin{forest}
sm edges
   [\ibar{N}\ind{1}
