...
\feat{subj} list.\footnote{The use of S\subscr{inf} in Figure~\ref{fig:rc-5} is an
  ...
  \istype{verb}\type{verb} with empty \feat{subj} and \feat{comps} values,
...
alternative.\footnote{\ili{Arabic} also has finite relatives that do not have an overt
  ...
  relatives, which have received some attention. See \citep{Melnik:06},
  \citep{Haddar:Boukedi:09,Zalila:Haddar:11}, \citep{Hahn:12}, and \citep{Crysmann:Reintges:14}
...
where $\oplus$ signifies the `append' relation -- in combination with the ellipsis it allows the possibility that the \feat{comps} list may contain additional elements.
