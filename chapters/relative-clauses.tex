...
are far from common.\footnote{Examples like \pref{x:rc-233} appear often in discussions of
...
\begin{exe}\ex\begin{xlist}
  ...
  \end{xlist}
\end{exe}
However, while finite VPs as in \pref{x:rc-281} seem genuinely impossible, non-finite VPs
...
\lic{wh}-expression''):\footnote{See also \cite[81--85]{Mueller99b} on differences between
  ...
  \ili{English} dialects allow NP \lic{what} as a \isi{relative pronoun} like \lic{which}
...
\avm{
	...
	loc &	[cat &	[head &	[\type*{c}
					...
					comps &	<[loc &	[cat & \upshape S\subscr{fin} \\
									cont & \2 ] \\
							non-loc &	[slash & \{ \3	[\type*{nprl}
														cat & \upshape PP\subscr{de \1} ] \} ] ] > ] \\
                        %   \upshape{NP\subscr{de}}\thinspace\subscr{\@1}
			cont & \2 ] \\
	to-bind & \{ \3 \} ]
}
\isfeat{local}\isfeat{head}\isfeat{mod}\isfeat{comps}\isfeat{non-local}\isfeat{cat}\isfeat{to-bind}\isfeat{content}\istype{nprl}
\end{exe}
...
\feat{slash} value containing pronominal (\istype{prl}\type{prl}) elements (that is, a CP that can
...
\begin{exe}\ex\label{x:rc-82}
 \gll [motwu-ka           [Kim-i       \trace\subscr{i} ilk-ess-ta-ko]  sayngkakha-nun] chayk\subscr{i} \\
      \hphantom{[}everyone-{\sc nom} \hphantom{[}Kim-{\sc nom}  {}     read-{\sc pst-decl-comp} think-{\sc pres.mod}   book\\\jambox*{(\ili{Korean})}
...
condition that requires the relative clause be ``about'' the modified noun.
 \begin{exe}\ex\label{x:rc-85}
   \gll [Ken-ga                                  [Eiko-ga     \trace\subscr{i} yon-da] to sinzitei-ru] hon\subscr{i}\\
        \hphantom{[}Ken-{\sc nom} \hphantom{[}Eiko-{\sc nom} {}        read.\textsc{past} \textsc{comp}
...
\begin{exe}\ex\label{x:rc-135}
  \gll [ya indɛ mi wɛ gɔ]  yimaa   boli.\\
       \hphantom{[}yesterday person 1sg see.PN.$\emptyset$ DEF die.PSP go.PN.3sg\\\jambox*{(\ili{Dogon})}
