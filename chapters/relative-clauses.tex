...
%%\input{../hpsg-handbook-master/localpackages.tex}
%%\input{../hpsg-handbook-master/localcommands.tex}
...
\author{%
 ...
 Dani\`{e}le Godard\affiliation{University de Paris Diderot}
}
...
\isi{Sign-based Construction Grammar} (cf.\ \crossrefchaptert{cxg}).
...
clause with a missing constituent --- a gap (the complement of \lic{speak}: \lic{Kim spoke
  \uGap yesterday}). This is often called the \is{relative clause!relativised constituent}\emph{relativised constituent}.
...
\feat{select} (cf.\ \crossrefchaptert{ChXXX} on the treatment of adjuncts). That is,
...
subject-verb agreement, as in \pref{x:rc-166}, and binding, as in \pref{x:rc-166}:
\begin{exe}\ex\begin{xlist}
...
standard \feat{slash} inheritance techniques (see \crossrefchaptert{udc}), so that these
...
arbitrarily deeply inside the \lic{wh}-phrase (example
  ...
  \ex\label{x:rc-13} the person  [to [whose friends]] Kim spoke \uGap   
  \ex\label{x:rc-14} the person [to [[whose children's] friends]] Kim spoke \uGap   
  \ex\label{x:rc-15} the person [to [the children [of [whose friends]]] Kim spoke \uGap   
  \ex\label{x:rc-16} books [the height [of [the letters [on [the covers [of which ]]]]]
  the government regulates \uGap
\end{xlist}\end{exe}
This dependency between a \isi{relative pronoun}
and the phrase that contains it is often called ``\lic{wh}-percolation'', ``relative
percolation'', or, following \cite{Ross67a-Eng}, ``pied-piping''. We will talk about
\emph{\isi{relative inheritance}}.

Notice that as well as being unbounded, \isi{relative inheritance} resembles \feat{slash}
inheritance in that the ``bottom'' of the inheritance path (i.e. the actual \isi{relative pronoun}, or the gap in a filler-gap a dependency) is typically not a head (e.g.\
...
exist in other languages, so it is reasonable to assume that \isi{relative inheritance} involves a set of some kind.\footnote{Examples of languages which allow
  ...
  \citep[e.g.][Ch7]{dhongde2009marathi}. See \cite[227--232]{Pollard:Sag:94} for HPSG
...
higher than the level at which the associated filler appears.\footnote{In case it is not
  obvious why further upwards inheritance of a \feat{rel} value would be problematic,
  ...
  beyond the relative clause \lic{to whom Kim spoke}, so that e.g.\ \lic{a person to whom
    Kim spoke} was marked as [\feat{rel}~\setof{\idx{1}}]. This phrase would be able to
  function as the \lic{wh}-phrase in a relative clause like *\lic{[a person to whom Kim
    spoke] Sam recognised \uGap}, which would be able to combine with a noun specified as
  [\feat{index}~\idx{1}] to produce something like *\lic{a person [[a person to whom
    Kim spoke] Sam recognised \uGap]}.}
\is{relative inheritance|)}
...
value, roughly as in \pref{x:rc-17}, which we abbreviate to \pref{x:rc-18}.\footnote{Here, and
  ...
  subscripts on nouns and NPs. We use \ibar{N} to indicate a noun with an empty \feat{comps}
  ...
  Similarly, we use PP to abbreviate a phase consisting of a preposition and its complement,
...
\lic{wh}-phrase.\footnote{We assume, for simplicity,
  ...
  \istype{scope-object}\istype{scope-object}\type{scope-object}), like the \feat{que} attribute which is used for
...
\begin{exe}\ex\begin{xlist}
  ...
  \begin{avm}
   ...
   \end{avm}
   \ex\label{x:rc-22} \itshape{speak\_to(Kim,\idx{1})}
 \end{xlist}
...
\ili{English}, but VPs and Ss are not.\footnote{This is a considerable simplification: e.g.\
  ...
  \cite{Mueller99a-unlinked} for discussion and HPSG analyses.}
\begin{exe}\ex\begin{xlist}\label{x:rc-23}
\ex[] {the person  [\sub{NP} who] we want Kim to speak to \uGap}  \label{x:rc-24}
\ex[] {the person  [\sub{PP} to whom] we want Kim to speak \uGap} \label{x:rc-25}
\ex[*] {the person [\sub{VP} speak to whom] we want Kim to \uGap}\label{x:rc-26}
\ex[*] {the person [\sub{VP} to speak to whom] we want Kim \uGap}\label{x:rc-27}
\ex[*] {the person [\sub{S} Kim to speak to whom] we want \uGap} \label{x:rc-28}
\end{xlist}\end{exe}
...
is permitted, it is blocked by VP and S nodes. The can be achieved by requiring that 
the \feat{rel} value on VP and  S nodes to be empty (cf.\ the Clausal \feat{rel}
...
\begin{exe}
  \ex\label{x:rc-29} *I wonder [some pictures of whom] they were admiring.  
  \ex\label{x:rc-30} the children [some pictures of whom] they were admiring
\end{exe}
...
but not \isi{relative pronoun}s, as the following show (as \citealt[493]{Sag:10b} puts it, there
is ``no morphological or syntactic unity underlying the concept of an \ili{English}
\lic{wh}-expression):\footnote{See also \cite[81--85]{Mueller:99b} on differences
...
The combination of a relative clauses and the nominal it modifies is traditionally
...
\begin{exe}
  ...
  \ex\label{x:rc-36}~\idx{1} : \setof{person(\idx{1})}
\end{exe}
\begin{exe}
  \ex\label{x:rc-37}
  \begin{avm}
 ...
 \end{exe}
\begin{exe}
     \ex\label{x:rc-38}~\idx{1} : \setof{person(\idx{1}),
        speak\_to(Kim,\idx{1})}
\end{exe}
...
clauses like \pref{x:rc-3} using HPSG apparatus. In the following sections we will make this
...
\cite{Sag:97}, which does not.\footnote{\cite{Mueller:99b} presents what might be
  considered a third approach, which resembles \cite{Sag:97} in avoiding empty elements,
  but uses a rule schema for \ili{German} relative clauses rather than constructional apparatus
  of phrasal types (see \citealt[95]{Mueller:99b} for details). The overview of HPSG in
  ...
  \cite{Pollard:Sag:94}, but they have fallen out of favour with the rise of construction
  based analyses since \cite{Sag:97}. A rule schema is essentially just a phrasal type ---
  ...
  analyses. This is not an issue for \citeauthor{Mueller:99b}, who claims that a
  description of restrictive relatives in \ili{German} requires only a single schema
  \citep[74]{Mueller:99b}.}
\is{relative clause!antecedent of|)}
...
saturated \istype{verb}\type{verb} -- i.e.\ one with empty \feat{subj} and \feat{comps}
...
\is{relative inheritance|(}
This leaves the question of how upwards inheritance of the \feat{rel} and \feat{slash}
...
gap, \cite[174]{Pollard:Sag:94}, so that R combines with a VP rather than an S. But this
...
``\isi{Trace-Principle}'', \cite[172]{Pollard:Sag:94}). Hence we ignore it here.}
...
Figure~\ref{fig:rc-42}.\footnote{See \cite[Chapter~11]{KimSellsIntroduction} for an introductory
  ...
  semantics (which is formulated using conventional $\lambda$-calculus apparatus), it is
...
\begin{figure}
  ...
  \caption{Type hierarch for {\protect\istype{clause}\type{clause}}, based on \cite{Sag:97}.}
  \label{fig:rc-42}
...
\is{relative clause!antecedent of}antecedent.\footnote{\label{fn:rc-2}For simplicity and to avoid distractions, we have
  ...
  \cite[465--469]{Sag:97}. \cite[293--294]{Kiss2005a} gives a number of arguments in
...
constraints from different dimensions (cf.\ \crossrefchaptert{ChXXXX}). As well as inheriting
...
(\istype{hd-fill-phrase}\type{hd-fill-ph}), thus inheriting constraints as in \pref{x:rc-47}.\footnote{The
  ...
  intention is that restrictions are to distributed between the noun and
...
\begin{figure}
  \newcommand{\MotherDA}{S
      \begin{avm}
   \[ head & \@4 \[ mod&\upshape{\ibar{N}\subtag{1}} \]\\
...
relative clause is used to modify. Further upwards \is{relative inheritance}inheritance of this \feat{rel} value
...
\feat{rel} values.\footnote{%
  ...
  \citep[Ch5]{Ginzburg:Sag:00}. Upwards inheritance is handled by a constraint on
  ...
  ``\isi{Wh-Inheritance Principle}'', WHIP), see \cite[449]{Sag:97}.  Since these are only
...
subcategorisation requirements of \lic{speak} can be satisfied, and the right content
...
\newcommand{\NContentDA}{
 ...
 \newcommand{\RelContentDA}{
   ...
   \end{forest}
   \caption{\citegen{Sag:97} analysis of a relative clause plus its \is{relative clause!antecedent of}antecedent.}
  \label{fig:rc-4}
...
mother. Perhaps neither argument is compelling --- in fact, the discussion of relative
clauses in \cite{Sag:10b} suggests essentially this approach \citep[522]{Sag:10b}.
\is{relative clause!wh-relative@\lic{wh}-relative|)}
\is{relative clause!construction|)}
...
assumptions (e.g. arguments from binding theory which taken be taken as indicating the
presence of \lic{wh}-phrase inside the relative clause fall out naturally without this
assumption given the argument-structure based account of binding theory which is standard
in HPSG, see \crossrefchaptert{arg-st}). More important, as discussed in
...
here.\footnote{For example, for example, both analyses treat \lic{wh}-words like
  \lic{who}, \lic{what} and \lic{which} and their equivalents as determiners, whereas in
...
languages.\footnote{In addition to the phenomena and languages we discuss, the HPSG
  ...
  \citep{Mueller:99b,Mueller99a-unlinked,MuellerCurrentApproaches}, \ili{Hausa}
...
\gll wajadtu    l-kitab-a    [{lla\textipa{D}i} tu{\textcrh}ib-\textbf{hu}     Hind-un] \\
...
\end{exe}
The analysis of resumptive pronouns is discussed elsewhere in this volume \crossrefchapterp{ChYYYY}, and while they
...
\feat{subj} list.\footnote{The use of S\subscr{inf} in Figure~\ref{fig:rc-5} is an
  ...
  \istype{verb}\istype{verb}\type{verb} with empty \feat{subj} and \feat{comps} values,
...
\emphtipa{{P}allaDi} `that' (transliterated as \emphtipa{llaDi} in \pref{x:rc-57}, from
...
complementiser, with a \feat{synsem} value roughly as in \pref{x:rc-58}.\footnote{Here S\subscr{fin}  means a
  finite clause (a \istype{verb}\type{verb} which is \feat{comps} and \feat{subj} saturated)
...
\gll jaa\textipa{P}a l-walad-u   lla\textipa{D}i qaabala l-malik-a.\\
...
\end{exe}
According to this, \emphtipa{{P}allaDi} will combine with a slashed finite sentential
...
Notice that there no role for a \feat{rel} feature here (obviously, since there is no
...
\emphtipa{{P}allaDi} itself (cf.\ its own empty \feat{slash} specification). That is,
...
purpose clauses and \lic{tough}-constructions).\footnote{\cite[42]{Alqurashi:Borsley:12}
  ...
  empty \feat{slash} specification on \emphtipa{{P}allaDi} prevents upwards
  inheritance. The same effect could be achieved with an appropriate \feat{to-bind}
  specification. }


Since \emphtipa{{P}allaDi} shows inflections agreeing with the \is{relative clause!antecedent of}antecedent NP for
...
\emphtipa{llaDi} in \pref{x:rc-57} will add to \pref{x:rc-58} the additional requirement that
...
alternative.\footnote{\ili{Arabic} also has finite relatives that do not have an overt
  ...
  relatives, which have received some attention. See \cite{Melnik:06};
  \cite{Haddar:Boukedi:09,Zalila:Haddar:11}; \cite{Hahn:12}; \cite{Crysmann:Reintges:14}
...
\begin{exe}\ex\begin{xlist}\label{x:rc-59}
  \ex\label{x:rc-60}  person that \uGap admires Kim
  \ex\label{x:rc-61} person that everyone thinks \uGap admires Kim
\end{xlist}\end{exe}
...
both conjuncts contain \isi{relative pronoun}s, of course).\footnote{The same argument can be
  ...
  \isi{relative pronoun} (namely, \lic{that}).XXXXX}
\begin{exe}\ex\label{x:rc-62}
 a book [that/which you own or that/which you can borrow]
\end{exe}
Potential evidence against this, and in favour of a complementiser (or perhaps marker)
style analysis would be that \lic{that} differs from normal \isi{relative pronoun}s in not
allowing pied-piping, cf.\ \pref{x:rc-65}.
\begin{exe}\ex\begin{xlist}\label{x:rc-63}
  \ex[] {the person that I spoke to \uGap}\label{x:rc-64}
  \ex[*]{the person to that I spoke \uGap}\label{x:rc-65}
\end{xlist}\end{exe}
...
\begin{exe}\ex\begin{xlist}\label{x:rc-66}
  \ex [] {the person who I spoke to \uGap}\label{x:rc-67}
  \ex [*]{the person to who I spoke \uGap}\label{x:rc-68}
\end{xlist}\end{exe}
...
\citegen{Sag:97} analysis might involve creating a new subtype of \istype{rel-cl}\type{rel-cl} for
...
\begin{exe}\ex\begin{xlist}\label{x:rc-168}
  ...
  \ex[*]{ \gll On  	{résoudra}  d' un problème.\\
           One  will.resolve of a problem\\\jambox*{(\ili{French})}}\label{x:rc-170}
\end{xlist}\end{exe}
...
(\istype{nprl}\type{nprl}) --- that is, a genuine gap, rather than a \isi{resumptive pronoun}.\footnote{\cite{AbeilleGodard07} assume that gaps and resumptive pronouns are
  associated with distinct subtypes of \istype{local}\type{local} value: \istype{plr}\type{plr} (pronominal) for
...
providing the \is{relative clause!relativised constituent}relativised constituent is
...
licensing predicate and the \is{relative clause!relativised constituent}relativised constituent involves inheritance of a resumptive element, that
...
pronoun, and replace it with a gap dependency.\footnote{As \cite{AbeilleGodard07} point
  ...
  \begin{exe}\ex\label{x:rc-80}\gll
    ...
    spoken \hspaceThis{\uGap} \hspaceThis{[}and that we to-it will.come.back more late\\\jambox*{(\ili{French})}
...
\gll Naomi-ga \uGap\subscr{i} yon-da hon\subscr{i}\\
     Naomi-{\sc nom} {} read-\textsc{past} book\\\jambox*{(\ili{Japanese})}
\glt `the book (that) Naomi read'
\end{exe}
\begin{exe}\ex\label{x:rc-82}
 \gll [motwu-ka           [Kim-i       \uGap\subscr{i} ilk-ess-ta-ko]  sayngkakha-nun] chayk\subscr{i} \\
...
condition that requires the relative clause be ``about'' the modified noun.
 \begin{exe}\ex\label{x:rc-85}
   \gll [Ken-ga                                  [Eiko-ga     \uGap\subscr{i} yon-da] to sinzitei-ru] hon\subscr{i}\\
...
resembles \citegen{Sag:97} analysis of \ili{English} --- see also \cite{kim1998head-driven-KoreanRC};
...
upwards. The \feat{slash} value on the S daughter ensures the presence of an appropriate gap, the
  ...
  \begin{forest}  %qtree edges
   [{N\subtag{1}}  ,baseline
      [{\TnodeDA{S}{\[mod&\@2\\slash&\setof{\@3 \mathrm{NP\subscr{\@1}}}\]}} 
         [{NP}  [ {\cPile{motwu-ka\\everyone-{\sc nom}}} ] ]
         [{\TnodeDA{VP}{\[mod&\@2\\slash&\setof{\@3}\]}}   
            [{\TnodeDA{S}{\[slash&\setof{\@3}\]}} 
               [{\TnodeDA{NP}{}}  [ {\cPile{Kim-i\\Kim-{\sc nom}}} ] ]
               [{\TnodeDA{VP}{\[slash&\setof{\@3}\]}}    
                  [{\TnodeDA{V}{\[slash&\setof{\@3}\]}}  [ {\cPile{ilk-ess-ta-ko\\read-{\sc pst-decl-comp}}} ] ]
               ]
            ]
            [{\TnodeDA{V}{\[mod&\@2\]}}  [ {\cPile{sayngkakha-nun\\think-{\sc mod}}} ]   ]
         ]
      ]
      [{\idx{2}N\subtag{1}}  [ {\cPile{chayk\\book}} ] ]
      ]
   \end{forest}
   \caption{A \ili{Korean} relative clause, based on \cite[295]{Kim16SyntacticStrKorean}. }
   \label{fig:rc-6}
 \end{figure}
\is{relative clause!headed by verb|)}
\is{relative clause!bare!in Korean|)}
...
\begin{figure}
   ...
   \end{forest}
   \caption{A \cite{Pollard:Sag:94} style structure for an \ili{English} bare relative.}
   \label{fig:rc-7}
\end{figure}
...
\begin{figure}
  ...
  \newcommand{\SlashModDA}{\begin{avm}\[mod&\@2 \upshape{\ibar{N}}\subscr{\idx{1}}\\slash&\setof{}\]\end{avm}}
   ...
   \end{forest}
   \caption{A \cite{Sag:97} style structure for an \ili{English} bare relative.}
   \label{fig:rc-8}
   \isfeat{slash}\isfeat{mod}
 \end{figure}
 
This differs from Kim's proposal for \ili{Korean} in where the \feat{slash} value is
...
like that in Figure~\ref{fig:rc-8}, except that the upwards inheritance of the \feat{slash} value will not be
...
\begin{exe}\ex\label{x:rc-98}
  *That book\subscr{i}, I enjoyed [ the book\subscr{i}  Kim read \uGap\subscr{i} ]
\end{exe} 
...
\istype{fin-wh-rel-cl}\type{fin-wh-rel-cl}).\footnote{As stated, given \citegen{Sag:97}  assumption that
  ...
  supplemental \lic{that}-relatives should normally be allowed. One way round this is to
  adopt an different analysis of \lic{that}, but \cite{Arnold:04a} also considers an analysis whereby
...
\begin{exe}\ex
  \label{x:rc-115}Kim has ridden a camel, which Sam never would \uGap.   
\end{exe}
...
an auxiliary verb being omitted from its \feat{comps} list. The idea is that auxiliary
...
\isi{linearisation} originally proposed by \citeauthor{Reape94-ohne-crossref} (e.g.\
\cite{Reape94-ohne-crossref}, and \crossrefchaptert{order}). The relevant construction
...
\ref{sec:rc-other-functions} will overview HPSG approaches on cases where clauses resembling relative
...
Section~\ref{sec:rc-non-restr-suppl} above (and see \crossrefchaptert{order}). The idea is
...
\pref{x:rc-121}.  See e.g.\ \cite{Nerbonne94-ohne-crossref}, and \cite{Kathol:Pollard:95}
for details.
...
downwards, this is equivalent having a construction which allows a relative clause to
...
\cite{Keller:95c}; \cite{Bouma96};  \cite{Mueller:99b}; \cite{Mueller:04c}; \cite{Crysmann2005a-u};
\cite{Crysmann2013a}. 
...
\feat{extra}-style apparatus (e.g.\ \cite{Mueller:04c} and \cite{Crysmann2013a}), but in
...
and semantic composition, while \feat{extra}-style analyses involve only on the more
...
\cite{Walker2017}).\is{relative clause!as complement!of \lic{diejenigen}}%
...
being the existence of an unbounded dependency). One problem is that is not clear how this
...
\begin{exe}\ex\label{x:rc-134}
 It was Sam [on whom she particularly focused her attention \uGap].
\end{exe}
\is{relative clause!as complement!in cleft construction|)}
...
between the perception/discover event and the event reported in the relative clause, and
...
\begin{exe}\ex\label{x:rc-135}
  \gll [ya ind\textipa{E} mi w\textipa{E} g\textipa{O}]  yimaa   boli.\\
...
\end{exe}
Here we have a determiner \lic{g\textipa{O}} preceded by a clause containing what would be
the external head of a standard relative clause (in this case \lic{ind\textipa{E}} `person'). The key
difference between this and the \ili{Korean} case is the absence here of any obvious clause
external nominal like \lic{kes} which can be treated as the head which takes the relative
...
is identified with the index of one of the NPs, here the index of \lic{ind\textipa{E}}
`person': the effect of this is that the index of \lic{ind\textipa{E}} `person' becomes
...
\emph{pseudo-relative} construction, exemplified in \pref{x:rc-136}.\footnote{A similar
  construction can be found in \ili{Japanese}, cf.\ \cite{kikuta1998multiple,kikuta2001japanese,Kikuta2002a-u,Chan:Kim:03}.}
...
\cite{kim1998pseudoRC}; \cite{Yoon:93}; \cite{Chan:Kim:03}; \cite{Cha:05}; \cite{Kim16SyntacticStrKorean}.
\is{relative clause!as complement!in pseudo-relative construction|)}
...
gap, and an initial \is{relative clause!wh-phrase@\lic{wh}-phrase in}\lic{wh}-phrase in which is interpreted as filling the gap.  They
...
distribution of NPs or PPs rather than clauses (for example in \pref{x:rc-138} \lic{what I
  suggested} is the complement of \lic{eat}, and in \pref{x:rc-140} \lic{where suggested}
...
(determiners and genitive NPs), as in \pref{x:rc-149}, and \pref{x:rc-155} below.\footnote{Other
  languages are less restrictive, e.g. \cite[57]{Mueller:99b} gives \ili{German} examples
...
apart.\footnote{\label{fn:rc-5}\cite[90]{Mueller:99b} discusses the following \ili{German} example
  ...
  the variant where the parenthesised instance of \lic{beginnen} is present, the within
  clause role is also that of a PP.  However, \lic{mit} here is a non-predicative
  ...
  \lic{wh}-phase and the properties of the \lic{wh}-word it contains.
}



Following \cite{Mueller:99b} on \ili{German}, free relatives have received considerable
...
suggested to some (e.g. \cite{Kubota:03}) that the \is{relative clause!wh-phrase@\lic{wh}-phrase in}\lic{wh}-phrase should be the head
...
the filler is \emph{non}-head. If, instead, we assume that \lic{what} is primarily the
filler of the gap in the free relative, then we should assume that the clause \lic{I
  suggested \uGap} is the head of the free relative --- and the distributional properties
...
\begin{exe}\ex\begin{xlist}\label{x:rc-163}
 \ex\label{x:rc-164} She replied in [what anyone would consider \uGap a belligerent tone].
 \ex\label{x:rc-165} Her reply was [what anyone would consider \uGap belligerent].  
\end{xlist}\end{exe}
...
relatives\is{relative clause!free}, as in \cite{Gerbl2007a}.\footnote{It can be
  ...
  proposition --- it cannot be paraphrased as ``It is mess that she is wearing'' or ``She is
...
using existing apparatus with only minor adjustments.
\is{specificational pseudo-cleft|)}
\is{relative clause!transparent free|)}
...
notably in the development of constructional approach involving inheritance from cross
classifying dimensions of description. Empirically, relative clauses have been the focus
