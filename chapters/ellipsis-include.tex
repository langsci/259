...
Based on the type of analysis they receive in HPSG, elliptical phenomena can be broadly divided into three types:
         nonsentential utterances (NSU), predicate ellipsis (PAE), and non-constituent coordination.\footnote{For more detailed discussion, see \citet{Kim2020}.}
...
We focus on three instances of non-constituent coordination ---  gapping (\citealt{Ross1967}), right node raising (RNR), argument cluster coordination (ACC) --- illustrated in (\ref{9}), (\ref{8}), and (\ref{acc}), respectively.
...
either as an overt PP or a \textit{pro} expression. In the framework of HPSG, this optionality of an argument to be either realized as a complement or not expressed is represented as the Argument Realization Principle  (\citealt[171]{Ginzburg:Sag:00}; \citealt[11]{Bouma:Malouf:Sag:01}; \crossrefchapteralt[\page \pageref{page-argument-realization-principle}]{properties}):
%
\inlinetodostefan{AVM: Stefan, take care of the space after $\ominus $}

\inlinetodoobl{Both G\&S and BMS have gap-ss rather than non-canonical}
\ea
\label{arp}
...
\avmtmp{
[cat    & [ subj  & \1\\
            comps & \2 $\ominus$ !$list(non-canonical)$!]\\
 arg-st & \1 \+ \2 ]
}
%% \begin{avm}
...
COMPS element and further that a non-canonical  element in the argument structure, of which \textit{pro} is a subtype, need not be realized in the syntax, permitting mismatch between argument structure and syntactic valence features (see Section 5). 
