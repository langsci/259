...
\z
%
In this dialogue, the fragment \emph{The mike} functions as the SAL-UTT. Thus  
...
of the lexical heads \textit{wait} and \emph{losses}.  We illustrate this idea for the verb \emph{wait}. Adopting the analysis of \citet{Ginzburg:Sag:2000}, \citet{Kim2015} suggest the following way of analyzing such sprouting
...
As mentioned in connection with examples (\ref{14}) and (\ref{15}), sprouting requires the presence of prepositions in NSUs. That is, the NSUs in (\ref{spr1}) and (\ref{spr2}) may not be NPs consisting of the wh-phrase \emph{what} alone.  
...
\emph{for} and its object NP), can supply
two possible SAL-UTTs: a PP or an NP. An NSU whose CAT feature matches either of these two SAL–UTTs is licensed by (\ref{hf-cx}). In contrast, the verb \emph{wait} in (\ref{wait}) selects an optional PP as its
...
The \emph{pro} analysis outlined above expects
...
valence (SUBJ and COMPS) and nonlocal (SLASH) features while
 ...
 examples like (\ref{65}). On this analysis, the gapped conjunct \emph{Chris an incredible
...
%joanna coordinator missing here?
 ...
 \ea
\label{gap-hf-con}
Syntactic constraints on \emph{head-fragment-ph} (\citealt[(53)]{Abeille2014}):\\
