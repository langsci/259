...
This section introduces utterances smaller than a sentence, which we refer to as \emph{\isi{nonsentential utterances}} (NSUs). These range from \emph{\isi{Bare Argument Ellipsis}} (BAE)\footnote{This term is used in \citet[\iaddpages]{CJ2005a}.} as in (\ref{1}), through fragment answers as in (\ref{2})
...
%
As illustrated by these examples, sluicing involves stranded \emph{wh}-phrases and has the function of an interrogative clause, while \changed{Bare Argument Ellipsis} involves XPs representing various syntactic categories and typically has the function of a clause (\citealt[313]{Ginzburg:Sag:2000}, \citealt[233]{CJ2005a}).\footnote{Several subtypes of nonsentential utterances (\changed{nonsentential utterance}s) can be distinguished, based on their contextual functions, an issue we leave open here (for a recent taxonomy, see \citealt[217]{Ginzburg2012}).}

The key theoretical question \changed{nonsentential utterance}s raise is whether they are, on the one hand, parts of larger sentential structures or, on the other, nonsentential structures whose semantic and morphosyntactic features are licensed by the surrounding context. To adjudicate between these views, researchers have looked for evidence that \changed{nonsentential utterance}s in fact behave as if they were fragments of sentences. As we will see in Section~\ref{sec-evidence-for-invisible-material}, there is evidence to support both of these views. However, HPSG doesn't assume that \changed{nonsentential utterance}s are underlyingly sentential structures.
...
constructions are \emph{\isi{Post-Auxiliary Ellipsis}} (PAE),\footnote{The term \changed{Post-Auxiliary Ellipsis} was introduced by
  \citet[\iaddpages]{Sag1976} and covers cases where a non-VP element is elided after an auxiliary verb, as in \emph{You think
...
\emph{pro}-drop). \changed{Post-Auxiliary Ellipsis} features stranded auxiliary verbs as in (\ref{5}), while pseudogapping, also
...
%
\changed{Null Complement Anaphora} is characterized by omission of complements to some lexical verbs as in (\ref{6}), while argument drop refers to omission of a pronominal subject or an object argument, as illustrated in (\ref{7}) for \ili{Polish}.

\ea A: I didn't ask George to invite you.\\B: Then who did? (\changed{Post-Auxiliary Ellipsis}) \label{5}\z
...
\ea Some mornings you can't get hot water in the shower, but nobody complains. (\changed{Null Complement Anaphora}) \label{6} \z
...
This section is concerned with \changed{nonsentential utterance}s and \changed{Post-Auxiliary Ellipsis}, since this is where the contentious issues arise of
...
Connectivity effects refer to parallels between \changed{nonsentential utterance}s and their counterparts in sentential structures, thus speaking in favor of the existence of silent sentential structure. We focus on two kinds here: case-matching effects and preposition-stranding effects (for other examples of connectivity effects, see \citealt{Ginzburg2018}). It's been known since \citet{Ross1967} that \changed{nonsentential utterance}s exhibit case-matching effects, that is, they are typically marked for the same case that is marked on their counterparts in sentential structures. (\ref{10}) illustrates this for \ili{German}, where case matching is seen between a \emph{wh}-phrase functioning as a \changed{nonsentential utterance} and its counterpart in the antecedent (\citealt[663]{Merchant2005-proc}):
...
Case-matching effects are crosslinguistically robust in that they are found in the vast majority of languages with overt case marking systems, and therefore, they have been taken as strong evidence for the reality of silent structure. The argument is that the pattern of case matching follows straightforwardly if a \changed{nonsentential utterance} is embedded in silent syntactic material whose content includes the same lexical head that assigns case to the \changed{nonsentential utterance}'s counterpart in the antecedent clause (\citealt{Merchant2001, Merchant2005a}). However, a language like \ili{Hungarian} poses a problem for this reasoning \citep{Jacobson2016}. While \ili{Hungarian} has verbs that assign one of two cases to their object NPs in overt clauses with no meaning difference, case matching is still required between a \changed{nonsentential utterance} and its counterpart, whichever case is marked on the counterpart. To see this, consider (\ref{11}) from \citet[356]{Jacobson2016}. The verb \emph{hasonlit} `resembles' assigns either sublative (\textsc{subl}) or allative (\textsc{all}) case to its object, but if  the sublative is selected for a \changed{nonsentential utterance}'s counterpart, the \changed{nonsentential utterance}  must match this case.
...
\z
%
\citet{Jacobson2016} notes that there is some speaker variation regarding the (un)ac\-cepta\-bi\-li\-ty of case mismatch here, while all speakers agree that either case is fine in a corresponding nonelliptical response to (\ref{11}A). This last point is important, because it shows that the requirement of---or at least a preference for---matching case features applies to \changed{nonsentential utterance}s to a greater extent than it does to their nonelliptical equivalents, challenging connectivity effects.

Similarly problematic for case-based parallels between \changed{nonsentential utterance}s and their sentential counterparts are some \ili{Korean} data. \ili{Korean} \changed{nonsentential utterance}s can drop case markers more freely than their counterparts in nonelliptical clauses can, a point made in \citet{Morgan1989} and \citet{Kim2015}. Observe the example in (\ref{12}) from \citet[237]{Morgan1989}.
...
\z
%
When a \changed{nonsentential utterance}  corresponds to a nominative subject in the antecedent (as in (\ref{12}B)), it can either be marked for nominative or be caseless.
However, replacing the same \changed{nonsentential utterance}  with a full sentential answer, as in (\ref{12}B$'$), rules out case drop from the subject. This strongly suggests that the case-marked and caseless \changed{nonsentential utterance}s couldn't have identical source sentences if they were to derive via PF-deletion (deletion in the phonological component).\footnote{Nominative (in \ili{Korean}) differs in this respect from three other structural cases in the language---dative, accusative, and genitive---in that these three may be dropped from nonelliptical clauses \citep[see][]{Morgan1989, Lee2016, Kim2016}. However, see \citet{Mueller2002b} for a discussion of \ili{German} dative and genitive as lexical cases.}  Data like these led \citet{Morgan1989} to propose that not all \changed{nonsentential utterance}s have a sentential derivation, an idea later picked up in \citet{Barton1998}.

%\iffalse{
The same pattern is associated with semantic case. That is, in (\ref{13}), if a \changed{nonsentential utterance} is case-marked, it needs
to be marked for comitative case like its counterpart in the A-sentence, but it may also simply be caseless. However,  
being caseless is not an option for the \changed{nonsentential utterance}'s counterpart in a sentential response to A \citep{Kim2015}\addpages.
...
%
The generalization for \ili{Korean} is then that \changed{nonsentential utterance}s may be optionally realized as caseless, but may never be marked for a different case than is marked on their counterparts.
%}\fi 

Overall, case-marking facts show that there is some morphosyntactic identity between \changed{nonsentential utterance}s and their antecedents, though not to the extent that \changed{nonsentential utterance}s have exactly the features that they would have if they were constituents embedded in sentential structures. The \ili{Hungarian} facts also suggest that those aspects of the argument structure of the appropriate lexical heads present in the antecedent that relate to case licensing are relevant for an analysis of \changed{nonsentential utterance}s.\footnote{\ili{Hungarian} and \ili{Korean} are not the only problematic languages; for a list, see \citet{Vicente2015}.}

The second kind of connectivity effects goes back to \citet{Merchant2001, Merchant2005a} and highlights apparent links between the features of \changed{nonsentential utterance}s and \emph{wh}- and focus movement (leftward movement of a focus-bearing expression). The idea is that prepositions behave the same under \emph{wh}- and focus movement as they do under clausal ellipsis, that is, they pied-pipe or strand in the same environments. If a language (e.g., \ili{English}) permits preposition stranding under \emph{wh}- and focus movement (\emph{What did Harvey paint the wall with?} vs.\ \emph{With what did Harvey paint the wall?}), then \changed{nonsentential utterance}s may surface with or without prepositions, as illustrated in (\ref{14}) for sluicing and \changed{Bare Argument Ellipsis} (see Section~\ref{sec-analyses-of-NSUs} for a theoretical analysis of this variation).
%
\ea A: I know what Harvey painted the wall with.\\B: (With) what?/(With) primer.\label{14}\z
%
If there were indeed a link between preposition stranding and \changed{nonsentential utterance}s, then we would expect prepositionless \changed{nonsentential utterance}s to only be possible in languages with preposition stranding. This expectation is, however, disconfirmed by an ever-growing list of non-preposition stranding languages that do feature prepositionless \changed{nonsentential utterance}s: Brazilian \ili{Portuguese} (\citealt{AlmeidaYoshida2007}), \ili{Spanish} and \ili{French} (\citealt{Rodrigues2006}), \ili{Greek} (\citealt{Molimpakis2019a}), \ili{Bahasa Indonesia} (\citealt{Fortin2007}), %Emirati \ili{Arabic} \citep{Leung2014},
...
\ili{Arabic} \citep{Leung2014, Alshaalan2020}. A few of these studies have presented experimental evidence that prepositionless \changed{nonsentential utterance}s are acceptable, though for reasons still poorly understood, they typically do not reach the same level of acceptability as their variants with prepositions do (see \citealt{Nykiel2013} for \ili{Polish}, \citealt{Molimpakis2019a} for \ili{Greek}, and \citealt{Alshaalan2020} for Saudi \ili{Arabic}). It is worth noting in this regard that the work on connectivity effects that follows the HPSG tradition is based on a solid foundation of empirical evidence to a larger extent than work grounded in the Minimalist tradition (see \citealt{Sag2011, Nykiel2013} for experimental work on \ili{Polish}, and \citealt{Nykiel2015, Nykiel2017, Nykiel2020} for corpus work on \ili{English}).
...
It is evident from this research that there is no grammatical constraint on \changed{nonsentential utterance}s that keeps track of what preposition-stranding possibilities exist in any given language. On the other hand, it does not seem sufficient to assume that \changed{nonsentential utterance}s can freely drop prepositions, given examples of sprouting like (\ref{15}), in which prepositions are not omissible (see \citealt{Chung1995}).\footnote{However, as noted by \citet[\iaddpages]{Ginzburg:Sag:2000} and \citet{Hardt2020}, there are cases where prepositions are dropped from \changed{nonsentential utterance}s that serve as adjuncts rather then arguments, as in \textit{A: I am going to the concert tonight. B: (At) What time?}}  As noted by \citet{Chung1995}\addpages,
the difference between the
merger type of sluicing (\ref{14}) and the sprouting type of sluicing (\ref{15}) is that there is an explicit phrase that the \changed{nonsentential utterance} corresponds to in the former but not in the latter (in the HPSG literature, this phrase is termed a Salient Utterance by \citealt[313]{Ginzburg:Sag:2000} or a Focus-Establishing Constituent by \citealt{Ginzburg2012}\addpages). 
...
the \changed{nonsentential utterance} is a PP matching the implicit PP argument in 
...
One of the predictions from the view that \changed{nonsentential utterance}s are underlyingly sentential is that they should respect \isi{island} constraints on long-distance movement (see \crossrefchapteralt{islands} for a discussion of islands in HPSG). But as illustrated below, \changed{nonsentential utterance}s (both sluicing and \changed{Bare Argument Ellipsis}) exhibit island-violating behavior.%joanna Why this footnote?
\footnote{As hinted earlier, the derivational approaches
need to move a remnant or \changed{nonsentential utterance} to the sentence initial position and delete a clausal constituent since only constitutents can be deleted. See \citet{Merchant2001,Merchant2010} for details.} The \changed{nonsentential utterance}  in (\ref{16}) would be illicitly extracted out of an adjunct (\textit{*Where does Harriet drink scotch that comes from?}) and the \changed{nonsentential utterance}  in (\ref{17}) would be extracted out of a complex NP (\textit{*The Gay Rifle Club, the administration has issued a statement that it is willing to meet with}).\footnote{\citet{Merchant2005a} argued that \changed{Bare Argument Ellipsis}, unlike sluicing, does respect island constraints, an argument that was later challenged (see, e.g., \citealt[239]{CJ2005a}; \citealt{Griffiths2014}). However, \citet{Merchant2005a} focused specifically on pairs of \emph{wh}-interrogatives and answers to them, running into the difficulty of testing for island-violating behavior, since a well-formed \emph{wh}-interrogative antecedent could not be constructed.}
...
Among \citeauthor{CJ2005a}'s (\citeyear[245]{CJ2005a}) examples of well-formed island-violating \changed{nonsentential utterance}s are also sprouted \changed{nonsentential utterance}s (those that correspond to implicit phrases in the antecedent) like (\ref{18})--(\ref{19}).
...
Other scholars assume that sprouted \changed{nonsentential utterance}s are one of the two kinds of \changed{nonsentential utterance}s that respect island constraints, the other kind being contrastive \changed{nonsentential utterance}s, illustrated in (\ref{20}) \citep{Chung1995, Merchant2005a, Griffiths2014}.
...
\z
%
\citet{Schmeh2015} further explore the acceptability of \changed{nonsentential utterance}s preceded by the response particle \textit{no} like those in (\ref{20}) compared to \changed{nonsentential utterance}s introduced by the response particle \textit{yes}, depicted in (\ref{21}). (\ref{20}) and (\ref{21}) differ in terms of discourse function in that the latter supplements the antecedent rather than correcting it, a discourse function signaled by the response particle \textit{yes}.
...
supplementations with \textit{yes} and propose that this follows from the fact that corrections induce greater processing difficulty than supplementations do, hence the acceptability difference between (\ref{20}) and (\ref{21}). This finding makes it plausible that the perceived degradation of island-violating \changed{nonsentential utterance}s could ultimately be attributed to nonsyntactic factors, e.g., the difficulty of successfully computing a meaning for them.

In contrast to \changed{nonsentential utterance}s, many instances of \changed{Post-Auxiliary Ellipsis} appear to respect island constraints, as would be expected if there were unpronounced structure from which material was extracted. An example of a relative clause island is depicted in (\ref{22}) (note that the corresponding sluicing \changed{nonsentential utterance}  is fine).
...
\zl
%
As \citet{Ginzburg2018} rightly point out, we do not yet have a complete understanding of when or why island effects show up in \changed{Post-Auxiliary Ellipsis}. Its behavior is at best inconsistent, failing to provide convincing evidence for silent structure.


\subsection{Structural mismatches}
\label{sec-structural-mismatches}

Because structural mismatches are rare or absent from \changed{nonsentential utterance}s \citep[see][]{Merchant2005a, Merchant2013},\footnote{Given the assumption that canonical sprouting \changed{nonsentential utterance}s have VP antecedents, as in (\ref{15}), \citet[95]{Ginzburg2018} cite examples---originally from \citet[13]{Beecher2008}---of sprouting \changed{nonsentential utterance}s with nominal, hence mismatched, antecedents, e.g., (i).
	\ea We're on to the semi-finals, though I don't know who against.\z
%
	Further examples where \changed{nonsentential utterance}s refer to an NP or AP antecedent appear in COCA (Corpus of Contemporary American \ili{English}):
%
...
%
	The \changed{nonsentential utterance}s in (ii)--(iii) repeat the lexical heads whose complements are being sprouted (\textit{defense} and \textit{fallen}), that is, they contain more material than is usual for \changed{nonsentential utterance}s (cf. (i)). It seems that without this additional material it would be difficult to integrate the \changed{nonsentential utterance}s into the propositions provided by the antecedents and hence to arrive at the intended interpretations.} this section focuses on \changed{Post-Auxiliary Ellipsis} and developments surrounding the question of which contexts license it. In a seminal study of anaphora, \citet{Hankamer1976} classified \changed{Post-Auxiliary Ellipsis} as a surface anaphor with syntactic features closely matching those of an antecedent present in the linguistic context. They argued in particular that \changed{Post-Auxiliary Ellipsis} is not licensed if it mismatches its antecedent in voice. Compare the following two examples from \citet[327]{Hankamer1976}.
...
There are two opposing views that have emerged from the empirical work regarding the acceptability and grammaticality of structural mismatches in \changed{Post-Auxiliary Ellipsis}. The first view takes mismatches to be grammatical and connects degradation in acceptability to violation of certain independent constraints on discourse \citep{Kehler2002, Miller2011, %Kertz2013,
Miller2014, Miller2014a, Miller2014b} or processing \citep{Kim2011}. Two types of \changed{Post-Auxiliary Ellipsis} have been identified on this view through extensive corpus work---auxiliary choice \changed{Post-Auxiliary Ellipsis} and subject choice \changed{Post-Auxiliary Ellipsis}---each with different discourse requirements with respect to the antecedent \citep{Miller2011, Miller2014a, Miller2014b}. The second view assumes that there is a grammatical ban on structural mismatch, but violations may be repaired under certain conditions; repairs are associated with differential processing costs compared to matching ellipses and antecedents \citep{Arregui2006, Grant2012}. If we follow the first view, it is perhaps unexpected that voice mismatch should consistently incur a greater acceptability penalty under \changed{Post-Auxiliary Ellipsis} than when no ellipsis is involved, as recently reported in \citet{Kim2018}.\footnote{But see \citet{Abeille2016} for experimental results
that show no acceptability penalty for voice mismatch in \ili{French} Right Node Raising.} \citet{Kim2018} stop short of drawing firm conclusions regarding the grammaticality of structural mismatches, but one possibility is that the observed mismatch effects reflect a construction-specific constraint on \changed{Post-Auxiliary Ellipsis}. HPSG analyses take structurally mismatched instances of \changed{Post-Auxiliary Ellipsis} to be unproblematic and fully grammatical, while also recognizing construction-specific constraints: discourse or processing constraints formulated for \changed{Post-Auxiliary Ellipsis} may or may not extend to other elliptical constructions, such as \changed{nonsentential utterance}s (see \citealt{Abeille2016,Ginzburg2018} for this point).


\subsection{Nonlinguistic antecedents}
\label{sec-nonlinguistic-antecedents}

Like structural mismatches, the availability of nonlinguistic (situational) antecedents for an ellipsis points to the fact that it need not be interpreted by reference to and licensed by a structurally identical antecedent. Although this option is somewhat limited, \changed{Post-Auxiliary Ellipsis} does tolerate nonlinguistic antecedents, as shown in (\ref{30})--(\ref{31}) \citep[see also][]{Hankamer1976, Schachter1977}.
...
%
\citet{Miller2014b} note that such examples are exophoric \changed{Post-Auxiliary Ellipsis}
involving no linguistic antecedent for the ellipsis but 
just a situation where the speaker articulates their opinion about the action involved. \citet{Miller2014b} provide an extensive critique of the earlier work on the ability of \changed{Post-Auxiliary Ellipsis} to take nonlinguistic antecedents, arguing for a streamlined discourse-based explanation that neatly captures the attested examples as well as examples of structural mismatch like those discussed in Section~\ref{sec-structural-mismatches}. The important point here is again that \changed{Post-Auxiliary Ellipsis} is subject to construction-specific constraints which limit its use with nonlinguistic antecedents.

\changed{nonsentential utterance}s appear in various nonlinguistic contexts as well. \citet{Ginzburg2018} distinguish three classes of such \changed{nonsentential utterance}s: sluices (\ref{32}), exclamative sluices (\ref{33}), and declarative fragments (\ref{34}).
...
%
In addition to being problematic from the licensing point of view, \changed{nonsentential utterance}s like these have been put forward as evidence against the idea that they are underlyingly sentential, because it is unclear what the structure that underlies them would be. There could be many potential sources for
these \changed{nonsentential utterance}s 
\citep[see][306]{CJ2005a}.%
%joanna For instance, the \changed{nonsentential utterance} in (\ref{34}) has no determined source sentence, but could have more than one source sentences.
...
\section{Analyses of \changed{nonsentential utterance}s}
\label{sec-analyses-of-NSUs}

It is worth noting at the outset that the analyses of \changed{nonsentential utterance}s within the framework of HPSG are based on an elaborate theory of dialog  and on a wider range of data than is common practice in the ellipsis literature\itdopt{
You said this already \ldots} \citep{Ginzburg1994, Ginzburg2004, Ginzburg2014a, Larsson2002, Purver2006, Fernandez2006, Fernandez2002, Fernandez2007, Ginzburg2010, Ginzburg2014b, Ginzburg2012, Ginzburg2013, Kim2019}. Existing analyses of \changed{nonsentential utterance}s go back to \citet{Ginzburg:Sag:2000}, who recognize declarative fragments as in (\ref{34a}) and two kinds of sluicing \changed{nonsentential utterance}s: direct sluices as in (\ref{35}) and reprise sluices as  in (\ref{36}) (the relevant fragments are bolded). The difference between direct and reprise sluices lies in the fact that the latter are requests for clarification of any part of the antecedent. For instance, in (\ref{36}), the referent of \textit{that} is unclear to the interlocutor.
...
\ea ``Can we please not say a lot about that?'' ``\textbf{About what?}'' (COCA FIC: The chance, 2014) \label{36} 
\z
%
These different types of \changed{nonsentential utterance}s are derived from the \citet[333]{Ginzburg:Sag:2000} hierarchy of clausal types depicted in Figure~\ref{fig-cltypes}.
%
\begin{figure}
\centering
\begin{forest}
...
\changed{Nonsentential utterance}s like declarative fragments (\type{decl-frag-cl}) are subtypes of \type{hd-frag-ph} (headed"=fragment phrase) and \type{decl-cl} (declarative clause), while direct sluices (\type{slu-int-cl}) and reprise sluices (\type{dir-is-int-cl}) are subtypes of \type{hd-frag-ph} and \type{inter-cl} (interrogative clause). The type \type{slu-int-cl} is permitted to appear in independent and embedded clauses, hence it is underspecified for the head feature \textsc{ic} (independent clause). This specification contrasts with that of declarative fragments and reprise sluices, which are both specified as [\textsc{ic}+]. \citet[305]{Ginzburg:Sag:2000} use [\textsc{ic} +]  to block declarative fragments and reprise sluices from appearing in embedded clauses (e.g., \textit{A: What do they like? B: *I doubt bagels}).\footnote{This feature specification, however, needs to be remedied for speakers who accept examples like \textit{A: What does Kim take for breakfast? B: Lee says eggs.}}
%
...
(we have added information about the \textsc{max-qud} to generate \changed{nonsentential utterance}s):
%\footnote{
...
any non-verbal phrasal category (NP, AP, VP, PP, AdvP) to be mapped onto a sentential utterance as long as it corresponds to a Salient Utterance (\textsc{sal-utt}).\footnote{\citet{Ginzburg2012} uses the notion of a \isi{Dialogue Game Board} (DGB) to keep track\itdopt{He does not use the ``notion'' to keep track but he does use the ``board'', doesn't he?} of all information relating to the common ground between interlocutors. The DGB is also the locus of contextual updates arising from each newly introduced question-under-discussion. \added{See \crossrefchapterw{pragmatics} for more on Dialogue Game Boards.}
% Yes, whole chapter.
}
 This means that
...
corresponds to the \textsc{sal-utt} \emph{what}. Thus  
  the constructional constraint in (\ref{hf-cx}) would license a \changed{nonsentential utterance} structure
...
the value of the variable \textit{i} linked to the object that Barry broke. The \changed{nonsentential utterance} 
\textit{The mike} matches that value.
The structured dialogue thus plays a key role in the retrieval of the propositional semantics for the \changed{nonsentential utterance}. %for the unexpressed expression. 

This constructional approach
has the advantage that it gives us a way of capturing the problems that \citet{Merchant2001, Merchant2005a} faces with respect to misalignments between preposition stranding under \emph{wh}- and focus movement and the realization of \changed{nonsentential utterance}s as NPs or PPs discussed in Section~\ref{sec-connectivity-effects}. Because the categories of \textsc{sal-utt} discussed in \citet{Ginzburg:Sag:2000} are limited to nonverbal, \textsc{sal-utt}s can surface either as NPs or PPs. As long as both of these syntactic categories are stored in the updated contextual information, a \changed{nonsentential utterance}'s \textsc{cat} feature will be able to match either of them (See \citealt{Sag2011} for discussion of this possibility with respect to \ili{Polish} and \citealt{Abeille2019} with respect to \ili{Mauritian}).    

 Another advantage of this analysis of \changed{nonsentential utterance}s is that the content of \textsc{max-qud} can be supplied by either linguistic or nonlinguistic context. \textsc{max-qud} provides the propositional semantics for a \changed{nonsentential utterance}  and is, typically, a unary question. In the prototypical case, \textsc{max-qud} arises from the most recent \emph{wh}-question uttered in a given context,
...
\z
%
The analysis of such direct sluices differs only slightly from that illustrated for (\ref{37}), and in fact all existing analyses of \changed{nonsentential utterance}s \citep{Sag2011,Ginzburg2012, Abeille2014, Kim2015, Abeille2019, Kim2019} are based on \added{the Head-Fragment Schema in} (\ref{hf-cx}). The direct sluice would have the structure given in Figure~\ref{fig-slu}.
%
...
suggests that a cleft-source analysis does not \rep{hold}{work} for languages like\itdopt{``does not hold''? I think a statement can hold, but an analysis?}
...
as \citet[329]{Ginzburg:Sag:2000} do.\itdopt{Replace ``as GS do'' by ``as is analyzed by GS'' or ``covered by''.} } 

This construction-based analysis, in which dialogue updating plays
a key role in the licensing of \changed{nonsentential utterance}s, also offers a direction
...
former includes an overt correlate for the \changed{nonsentential utterance}, but in (\ref{paint-spr}), 
...
either as an overt PP or a \textit{pro} expression. In the framework of HPSG, this optionality of an argument to be either realized as a complement or not expressed is represented as the Argument Realization Principle  (ARP; \citealt[\page 101]{ginzburg-miller-ellipsis-handbook}; \crossrefchapteralt[\page \pageref{page-argument-realization-principle}]{properties}):\footnote{This ARP is an extended version of \citet[171]{Ginzburg:Sag:2000} and \citet[11]{Bouma:Malouf:Sag:01}.}
%
...
\itdobl{Stefan: BMS and GS have an $\ominus$. This is also what we have in Chapter 1. Your formulation predicts that only elements from the end of \argst may be omitted.}
%% \begin{avm}
...
\end{figure}
%
Let us consider the \changed{nonsentential utterance} with the merger
antecedent in (\ref{paint-mer}). In this case, 
the \changed{nonsentential utterance} can be either the NP \emph{What?} or the PP
...
internal structure, including the NP within it. The \changed{nonsentential utterance} can thus be anchored to
...
\changed{nonsentential utterance} in accordance with (\ref{hf-cx}). Since the overt PP also activates
\rep{its prepositional NP object}{the NP object of the preposition}, the discourse can supply the NP
as another possible \textsc{sal-utt} value, as in (\ref{np-dgb}).
This information then projects \emph{What?} as a well-formed \changed{nonsentential utterance} in accordance with (\ref{hf-cx}).
...
the \textsc{sal-utt}. This is why the sprouting example allows only a PP as a possible \changed{nonsentential utterance}. Thus the key difference between
...
%
%
%The notable property, as mentioned, sprouting requires the presence of %prepositions in \changed{nonsentential utterance}s. That is, the \changed{nonsentential utterance}s in (\ref{paint-spr}) may not be NPs %consisting of the wh-phrase \emph{what} alone. The HPSG analysis presented %above offers a solution for
...
The advantages of the discourse-based analyses sketched here thus follow from their ability to capture limited morphosyntactic parallelism between \changed{nonsentential utterance}s and \textsc{sal-utt} without having to account for why \changed{nonsentential utterance}s behave differently from constituents of sentential structures. The island-violating behavior of \changed{nonsentential utterance}s is unsurprising on this analysis, as are attested cases of structural mismatch and situationally controlled \changed{nonsentential utterance}s.\footnote{The rarity of \changed{nonsentential utterance}s with nonlinguistic antecedents can be understood as a function of how hard or how easily a situational context can give rise to a \textsc{max-qud} and thus license ellipsis. See \citet{Miller2014b} for this point with regard to \changed{Post-Auxiliary Ellipsis}.}
...
The first issue in the analysis of \changed{Post-Auxiliary Ellipsis} is the status of the elided expression. It is assumed to be a
\textit{pro} element due to its pronominal properties \citep[see][]{Lobeck1995, Lopez2000, Kim2006,
  Aelbrecht2015, Ginzburg2018}. For instance, \changed{Post-Auxiliary Ellipsis} applies only to phrasal categories (\ref{43}),
...
One way to account for \changed{Post-Auxiliary Ellipsis} closely tracks analyses of \emph{pro}-drop phenomena. We do not need to posit a phonologically empty pronoun if a level of argument structure is
available where we can encode the required pronominal properties \citep[see][]{Bresnan1982a,Ginzburg:Sag:2000}.\itdobl{Add pages to both references} As we have seen, the \changed{Argument Realization Principle} in (\ref{arp}) 
allows an 
...
verb \textit{can}, bearing the feature AUX, has a \textit{pro} VP as its second argument in a sentence like \textit{John can't dance, but Sandy can}, that is, this VP is not instantiated as a syntactic complement of the verb.\footnote{The rich body of HPSG work on \ili{English} auxiliaries takes them to be not special Infl categories, but verbs \rep{bearing the feature AUX}{having the \textsc{aux} value $+$}.\itdobl{All verbs bear the feature AUX but some have the value $+$.} See \citet{Kim:00, KS2002a, Sag:Wasow:Bender:2003, Sag2020a, kimmichaelis:2020}.} This possibility is represented formally in (\ref{51}) 
...
%
\iffalse{
Given this, \ili{English} \changed{Post-Auxiliary Ellipsis} can be analyzed as a language-particular VP \textit{pro} drop phenomenon, triggered
...
%\zl 
%
because the relevant antecedent information is the information that the Dialog Game Board provides via the \textsc{max-qud} in each case, and hence no structural-match requirement is enforced on \changed{Post-Auxiliary Ellipsis}.\footnote{In the derivational
...
VP is linked to its antecedent under the IP.} This means in turn that HPSG analyses of \changed{Post-Auxiliary Ellipsis} do not face the problem of having to rule out, or rule in, cases of structural mismatch or nonlinguistic antecedents, because their acceptability can be captured as reflecting discourse-based and construction-specific constraints on \changed{Post-Auxiliary Ellipsis}.
...
Constructions such as gapping, \changed{Right Node Raising}, and \changed{Argument Cluster Coordination} have also often been taken to be elliptical
...
discussion to \crossrefchapteralt{coordination} and references therein.\footnote{We also leave out
  discussion of HPSG analyses for pseudogapping: readers are referred to \citet{Miller92d-u,
    Kim2020} and \crossrefchapteralt[Section 4]{control-raising}.\inlinetodoobl{Add latex ref once
...
%
%
HPSG analyses of gapping fall into two kinds: one kind draws on \citeauthor{Beavers2004}'s (\citeyear{Beavers2004}) deletion-like analysis of non-constituent coordination \citep{Chaves2009} and the other on \citeauthor{Ginzburg:Sag:2000}'s (\citeyear{Ginzburg:Sag:2000}) analysis of \changed{nonsentential utterance}s \citep{Abeille2014}.\footnote{For a semantic approach to gapping, the reader is referred to \citet{Parketal2019}, who offer an analysis of scope ambiguities under gapping where the syntax assumed is of the \changed{nonsentential utterance}  type and the semantics is cast in the framework of Lexical Resource Semantics. \added{For more on Lexical Resource Semantics see \crossrefchapterw[Section~\ref{semantics:sec-lrs}]{semantics}.}} The latter analyses align gapping with analyses of \changed{nonsentential utterance}s, as discussed in Section~\ref{sec-analyses-of-NSUs}, more than with analyses of non-constituent coordination, and for this reason gapping could be classified together with other \changed{nonsentential utterance}s. We use the analysis in \citet{Abeille2014} for illustration below.
...
\changed{nonsentential utterance}.  %type of fragment.
...
valence (\textsc{subj} and \textsc{comps}) and nonlocal (\textsc{slash}) features, while
 ...
 To illustrate, the gapped conjunct \emph{Chris an incredible bore} in (\ref{65}) is a \changed{nonsentential utterance} with a
...
feature in Figure~\ref{fig-gapping} and in the constraint in (\ref{gap-hf-con}) below).\footnote{The
  ...
  \citet{Mouret2006}'s analysis of \changed{Argument Cluster Coordination}. \changed{See \crossrefchapterw[\pageref{cg:page-ACC-semantics}]{cg} on the semantics of Argument Cluster Coordinations}.} This cluster phrase then serves as the head daughter of a head-fragment phrase, whose syntactic category is also underspecified. This means that there is no unpronounced verbal head in the phrase to which gapping remnants belong. The meanings of the gapping remnants are computed from the meaning of the leftmost nonelliptical verbal conjunct. %as represented by the Source feature in (\ref{gap-hf-con}). %Finally, the conjuncts are linked by a symmetric discourse relation (i.e.,  parallelism or contrast) that is part of the Background feature in (\ref{gap-hf-con}). 
In sum, the \changed{nonsentential utterance} that consists of the gapped conjunct in Figure~\ref{fig-gapping} has a single
daughter, a cluster phrase, which in turn has two daughters.
\itdobl{JP: I think a reference to the coordination chapter for more details on the feature CLUSTER is enough to at least orient the reader to what the feature is and its motivation.}  
    
The constraint in (\ref{gap-hf-con}) illustrates how syntactic parallelism between gapping remnants and their correlates in the leftmost conjunct is operationalized. We saw above that \citet{Abeille2014} ensure that gapping remnants are arguments of a verbal head located in the leftmost conjunct. They do so by adopting the contextual attribute \textsc{sal-utt}, which is introduced for all \changed{nonsentential utterance}s, as in (\ref{gap-hf-con}) (\citealt[259]{Abeille2014}) (for the definition of \textsc{sal-utt}, see Section~\ref{sec-analyses-of-NSUs}).
 %\footnote{The interpretation of the remnants
 %follows from the high order unification algorithm.}

 \ea
\label{gap-hf-con}
Syntactic constraints on \emph{head-fragment-ph\added{rase}}:\\
\type{head-fragment-ph\added{rase}} \impl
\avm{
...
In typical examples of \changed{Right Node Raising}, as shown below, the element to the immediate right of a parallel structure is shared with the left conjunct:
...
\changed{Right Node Raising} has consistently attracted HPSG analyses involving silent material (a detailed discussion of these can be found
in \crossrefchapteralt[\pageref{coordination:page-rnr-start}--\pageref{coordination:page-rnr-end}]{coordination}). 
\itdobl{Stefan: What I added as page references is referring to RNR in general. They do not discuss HPSG approaches assuming empty elements. If I missed something, you have to tell me where they do.}
All existing analyses of \changed{Right Node Raising} \citep{Abeille2016, Beavers2004, Chaves2014, Crysmann2003, Shiraishi2019, Yatabe2001, Yatabe2012} agree on this point,
although some of them propose more than one mechanism for accounting for different kinds of non-constituent coordination \citep{Chaves2014, Yatabe2001, Yatabe2012, Yatabe2019}. One strand of research within the \changed{Right Node Raising} literatures adopts a linearization-based approach employed more generally in analyses of non-constituent coordination (NCC) (see \citealt{Yatabe2001, Yatabe2012}, for a general introduction to order domains see \crossrefchapteralt[Section~\ref{sec-domains}]{order}) and another proposes a deletion-like operation \citep{Abeille2016, Chaves2014, Shiraishi2019}.
%, the
%latter of which is our focus here.

%
% We focus on the latter here, along with the question that it has raised, that is, what kind % of identity constraints must be satisfied before \changed{Right Node Raising} can apply.

The kind of material that may be \changed{Right Node Raised} and the range of structural mismatches permitted between the left and right conjuncts have been the subject of recent debate.\footnote{Although we refer to the material on the left and right as conjuncts, it is been known since \citet{Hudson1976,Hudson1989} that \changed{Right Node Raising} extends to other syntactic environments than coordination (see \citealt{Chaves2014}).} For instance, \citet[839--840]{Chaves2014} demonstrates that, besides more typical examples like (\ref{ex-kim-prepares-and-Lee-eats-Kim-played-and-Lee-sang}),
 there is a range of phenomena classifiable as \changed{Right Node Raising} that exhibit various argument-structure mismatches as in (\ref{ellipsis:ex-RNR}a,b), and that can target material below the word level as in (\ref{ellipsis:ex-RNR}c,d).
%
...
%
Furthermore, \changed{Right Node Raising} can target strings that are not subject to any known syntactic operations, such as rightward movement, as illustrated in (\mex{1}) \citep[865]{Chaves2014}.
...
\changed{Right Node Raised} material can also be discontinuous, as in (\ref{ex-rnr-discontinuous}) (\citealt[868]{Chaves2014}; \citealt[238--240]{Whitman2009}).
...
%
This evidence leads \citet{Chaves2014} to propose that \changed{Right Node Raising} is a nonuniform phenomenon, comprising extraposition,  VP- or N$'$-ellipsis, and true \changed{Right Node Raising}.
% besides 'true' RNR
% and requiring different kinds of theoretical analyses.
%
%
%
Of the three, only true \changed{Right Node Raising} should be accounted for via the mechanism of optional surface-based deletion that is sensitive to morph form identity and targets any linearized strings, whether constituents or otherwise.\footnote{Whenever \changed{Right Node Raising} can instead be analyzed as either VP or N$'$-ellipsis or extraposition, \citet{Chaves2014} proposes separate mechanisms for deriving them. These are the direct interpretation line of analysis described in the previous sections for \changed{nonsentential utterance}s and predicate/argument ellipsis and an analysis employing the feature \textsc{extra} to record extraposed material along the lines of \citet{KimSag2005} and \citet{Kay2012}. \added{See also \crossrefchapterw[Section~\ref{udc:sec-extra-feature}]{udc} on extraposition and the \textsc{extra} feature.}} \citeauthor{Chaves2014}' (\citeyear[874]{Chaves2014}) constraint licensing true \changed{Right Node Raising} is given informally in (\ref{63}) (where $\alpha$ means a morphophonological constituent\added{, $^{*}$ the Kleene star (operator)}, and $^{+}$ the Kleene \rep{star}{plus} (operator)):
...
\z
(\ref{63}) takes the morphophonology of a phrase to be computed as the linear combination of the morphophonologies of the daughters, allowing deletion to apply locally.\footnote{For further detail on linearization-based analyses of \changed{Right Node Raising}, the interested reader is referred to \citet{Yatabe2001, Yatabe2012} and to \crossrefchapteralt[Section~\ref{sec-domains}]{order} for details of linearization-based approaches in general.}
%, who distinguish between syntactic \changed{Right Node Raising} and phonological \changed{Right Node Raising}, based on the %amount of morphosyntactic identity holding between \changed{Right Node Raising}aised material and %the requirements imposed on the slots it occupies in the structure, and %represent this distinction by treating the \changed{Right Node Raising}aised material as either a %separate domain object on the mother's \textsc{dom} list (syntactic \changed{Right Node Raising}) or %embedded in a larger domain object corresponding to the right conjunct %(phonological \changed{Right Node Raising}).}



% \iffalse{
% \citeauthor{Chaves2014}' (\citeyear[874]{Chaves2014}) constraint licensing true \changed{Right Node Raising} is given in (\ref{bpd}). It permits the M(orpho)P(honology) feature of the mother to contain only one instance (represented as $L_{3}$ in (\ref{63})) of the two morphophonologically identical sequences [FORM $F_{1}$], \ldots, [FORM $F_{n}$] present in the daughters; the leftmost of these sequences undergoes deletion. The final list in the mother, $L_{4}$, may be empty or nonempty, depending on whether \changed{Right Node Raised} material is discontinuous.
% %
% %\fi
...
Another deletion-based analysis of \changed{Right Node Raising} is due to \citet{Abeille2016, Shiraishi2019}, differing from \citet{Chaves2014} in terms of identity conditions on deletion. \citet{Abeille2016} argue for a finer-grained analysis of \ili{French} \changed{Right Node Raising} without morphophonological identity. Their empirical evidence reveals a split between functional and lexical categories in \ili{French} such that the former permit mismatch between the two conjuncts (where determiners or prepositions differ) under \changed{Right Node Raising}, while the latter do not. \citet{Shiraishi2019} provide further corpus and experimental evidence that morphophonological identity is too strong a constraint on \changed{Right Node Raising}, given the range of acceptable mismatches between the verbal forms of the material missing from the left conjunct and those of the material that is shared between both conjuncts.
...
\changed{Argument Cluster Coordination} is a type of non-constituent coordination (NCC), as
...
\zl
%
%Our focus here is on HPSG analyses of \changed{Argument Cluster Coordination}, which departs from those
...
%coordination, 
As for the treatment of \changed{Argument Cluster Coordination}, the existing HPSG analyses have articulated two main views: ellipsis
(\citealt{Yatabe2001, Crysmann2003, Beavers2004}) and non-standard constituents (\citealt{Mouret2006}). For discussion of
the nonelliptical view, which takes \changed{Argument Cluster Coordination} to be a special type of coordination,
...
from examples like  (\ref{ex-acc-eliptical}).%\footnote{For an example of an analysis of \changed{Argument Cluster Coordination} that coordinates noncanonical constituents and doesn't posit the existence of silent material, see \citep{Mouret2006}.}
...
the mother's \textsc{dom} list (\citealt{Yatabe2001,Crysmann2003a}).\footnote{For detailed discussion of the feature \textsc{dom}, see \crossrefchapterw[Section~\ref{sec-domains}]{order}.} 
%
...
\z
\itdobl{Stefan: Adapt to geometry used here: Get rid of MOTHER, rename cxt to phrase.}
%
%
% \ea
%\label{conj-cxt}
...
\itdobl{Stefan: Unclear. ``there are two conjuncts with the \textsc{dom} value.'' What is this supposed to mean? ``the'' presupposes uniqueness. Maybe: ``there are two constituents contributing a DOM value'' And note that it is not the conjuncts, one of the daughter contains the coordiantion.}
...
%
%
% To make this more precise, consider the \citep{Beavers2004} schema in (\ref{acc2}), which % derives not just \changed{Argument Cluster Coordination}, but also \changed{Right Node Raising} and constituent coordination.
%
%
%
...
%The schema in (\ref{acc2}) also permits derivation of \changed{Right Node Raising}, provided the right-most element %on the mother's \textsc{dom} list (the correspondent of the material present overtly only in the %right conjunct) is not empty. We now take a closer look at analyses of \changed{Right Node Raising} in the next %section.
