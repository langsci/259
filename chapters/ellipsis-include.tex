...
Overall, case-marking facts show that there is some morphosyntactic identity between NSUs and their antecedents, though not to the extent that NSUs have exactly the features that they would have if they were constituents embedded in sentential structures. The \ili{Hungarian} facts also suggest that those aspects of the argument structure of the appropriate lexical heads present in the antecedent that relate to case licensing are relevant for an analysis of NSUs.\footnote{\ili{Hungarian} and \ili{Korean} are in fact not the only problematic languages; for a list, see \citet{Vicente2015}.}
...
\ea A: John met a woman who speaks \ili{French}.\\B: With an English accent?\label{18}\z
...
%
\eal
\ex{He managed to find someone who speaks a \ili{Romance} language, but a Germanic language, he didn't [\sout{manage to find someone who 
...
only in the value of the feature \textsc{cont} (Content): in the former it is a proposition and in the latter a question.\footnote{In-situ languages like \ili{Korean} and \ili{Mandarin} allow pseudosluices (sluices with a copula
...
\iffalse{
\citet{Abeille2014} offer additional evidence from \ili{Romance} (e.g., case mismatch between gapping remnants and their counterparts and even more possibilities of ordering remnants than is the case in English) to strengthen their point that syntactic identity is relaxed under gapping.\footnote{As
