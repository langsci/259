...
analoguous motivation for scrambling, where only union and total
compaction are used.\footnote{In linearisation-based HPSG, domain union
  creates an extended order domain, whereas compaction closes the
  domain by collapsing the list of domain objects into a single
  one. See \crossrefchapterw[Section~\ref{sec-partial-compaction}]{order} for a more detailed
  discussion of partial compaction and extraposition.} Once we approach 
...
leftward extraction in this respect, which underlines the
