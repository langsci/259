...
\inlinetodoobl{Stefan: I do not understand why there is so much space after ``Where''.}
\ea
\label{ex:UDC:6}
      % \avmtmp{
      % 	\{\normalfont Where \\ \normalfont When \\ \normalfont How \\ \normalfont Why & \}
...
\inlinetodoobl{Stefan: Do we need bold here?}
\ea
\label{ex:UDC:7}
...
In Nias \citep{Crysmann:09}, we find agreement with \textsc{subj} in the realis, and with
...
% an approach will miss a generalisation in the case of Welsh. In Welsh
...
% must be empty categories in this language.\footnote{\citet{Borsley:16:Archi} argues that an AGR-ST based approach to agreement will also miss a generalisation in the case of the Caucasian language Archi and proposes that agreement in this language involves constraints on constituent structure. This also means that gaps must be empty categories.}
...
\begin{exe}
  \ex \label{ex:HebATB}{
\gll kol profesor$_i$ še dani roce lehazmin \trace{}$_i$ aval lo maarix \textbf{ʔoto}$_i$ maspik\footnotemark\\
...
In his work on Welsh unbounded dependencies, \citet{Borsley.2010}
...
\ex{
\gll mutā̀nên dà sukà ƙi sayar \textbf{musù} dà àbinci sukà fìta\footnotemark\\
...
\gll Gā̀      tābōbîn$_j$ dà          Àli ya                 san  mùtumìn$_i$ dà           zâi$_i$            yī \textbf{musù}$_j$ /  *{wà \trace{}$_j$}   kwālī\footnotemark\\
...
\ea{
\label{ex:HauATB}
\gll [àbōkī-n-ā]{$_i$} dà [[na zìyartā̀ \trace{}$_i$] àmmā [bàn sā̀mē \textbf{shì}$_i$ à gidā  ba]]\footnotemark\\
...
stranding in German, French, and many other languages, but it is also
...
restricted
pragmatically. %\pdfmargincomment{BC: We should cite some work on French here.}

However, \emph{wh} in situ may even be an unmarked, or even the default
option for the expression of \emph{wh}"=interrogatives:
\citet[Section~\inlinetodoobl{add sec}]{Johnson:Lappin:97}, studying Iraqi Arabic, made the important
...
\zl
\inlinetodoobl{Stefan: Add page numbers for (\mex{1}).}
\eal
\ex[]{
\gll Ich habe [die Frau \_$_i$ ] getroffen, [die das Stück gelesen hat]$_i$.\footnotemark\\
     I   have \spacebr{}the woman {} {} met \spacebr{}who the play read has\\
\footnotetext{\citep[G.][]{mueller96:_extrap_and_succes_cyclic}}
\glt `I met the woman who has read the play.'}
\ex[*]{
\gll [die das Stück gelesen hat]$_i$, habe ich [die Frau \_$_i$ ] getroffen.\footnotemark\\
      \spacebr{}who the play read has have I \spacebr{}the woman {} {} met \\
\footnotetext{\citep[G.][]{mueller96:_extrap_and_succes_cyclic}}
}
\zl
...
\footnotetext{(\citealp{wiltschko94:_extrap_in_german}; \citealp[11]{Keller:94a})}
...
\inlinetodoobl{JP: So we have Keller's formulation even though we don't use \subcat throughout the
  book? Stefan: Can we use \argst here? Or \comps. \comps would work for German in my world.}
