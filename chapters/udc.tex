...
\citep{Henri2010a-u}\addpages. Thus, if nonlocal realisation corresponds to lexical
...
\inlinetodoobl{JP: Is the difference between lexemes and words
  explained anywhere? BC: It should be, but there must be a better
  place than the chapter on UDCs!}
...
\inlinetodoobl{BC: $\slash$ does not display in pdf.}
\ea
\label{fig:UDC:32}
\textsc{slash} Amalgamation Principle:\\
   \textit{word} \impl
\avm{
	$\slash$	[synsem & [nonloc & [slash & \1 $\cup$ \ldots{} $\cup$ \tag{n}]]\\
				arg-st & <[nonloc & [slash & \1], \ldots{}
                                [nonloc & [slash & \tag{n} ]] > ] ]
}
\z
...
Feature Principle for this purpose. This says that a headed phrase and
...
\emph{aL} only occurs between a filler and a gap. The following \citep[26]{BMS2001a}
...
constituent.\footnote{Non-restrictive relatives can also modify various
  ...
  \citet{Arnold:Borsley:08}.% \inlinetodoopt{Stefan: And there are also free relatives functioning as arguments.}
...
\ex[*]{\gll Ko si koga mislio da je voleo?\\
     who  \textsc{cl.2s} whom thought Comp \textsc{cl.3s} loved\\
     \glt
   }
\zl
