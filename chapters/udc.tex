...
\crossrefchaptert{islands}. \marginpar{Find reference to island
  chapter. }
...
further aspects of wh-interrogatives, including
...
\begin{figure}[htb]
  ...
  \caption{HPSG feature geometry: nonlocal and local features }
  \label{fig:UDC:9}
...
sentence. Thus, we have the schematic structure illustrated in Figure
\ref{fig:UDC:11}. 
...
%  \includegraphics{chapters/BB-udc-basic-tree-crop}
  ...
  \caption{Extraction by \textsc{slash} feature percolation}
  \label{fig:UDC:11}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Representation of gaps}
  \label{fig:UDC:12}
\end{figure}
...
mean that it has the following more elaborate analysis:
 ...
 table}.{PP\\\begin{avm}
   ...
   \caption{Head-driven \textsc{slash} feature percolation}
   \label{fig:UDC:14}
 \end{figure}
...
\begin{figure}[htb]
 ...
 \end{avm}
  ...
  \caption{Head-filler phrase (singleton \textsc{slash} set)}
  \label{fig:UDC:16}
\end{figure}
...
\begin{figure}[htb]
  ...
  \caption{Representation of a slashed preposition (traceless)}
  \label{fig:UDC:20}
\end{figure}
...
\begin{figure}[htb]
  ...
  \caption{Representation of a slashed PP (with trace)}
  \label{fig:UDC:21}
\end{figure}
...
which we can formulate as follows, following 
\citet[199]{Ginzburg:Sag:01}:
...
\begin{figure}[htb]
  \centering
   ...
   \begin{avm}
     $\slash$\[slash & \@1 $~\cup$ \dots $~\cup$ \@n\\
       arg-st & \<\[slash & \@1\] \dots \[slash & \@n \] \>\]
   \end{avm}
   
  
  \caption{\textsc{slash} Amalgamation Principle}
  \label{fig:UDC:32}
\end{figure}
...
{\ldots{}} ].{
  ...
  \caption{Across-the-board (ATB) extraction: conflation of \textsc{slash} values}
  \label{fig:UDC:36}
\end{figure}
...
\begin{figure}[htb]
  ...
  \includegraphics{chapters/BB-head-fill-hier-crop}
  \caption{Hierarchy of head-filler phrases}
  \label{fig:UDC:48}
\end{figure}
...
\begin{figure}[htb]
  ...
  \includegraphics{chapters/BB-extraction-function-hier-crop}
  \caption{Hierarchy of extraction clause types (preliminary)}
  \label{fig:UDC:49}
\end{figure}
...
\begin{figure}[htb]
  \centering
  \includegraphics{chapters/BB-extraction-function-hier-full-crop}
  \caption{Hierarchy of extraction clause types (final)}
  \label{fig:UDC:50}
\end{figure}
%
%
...
the lexical description in Figure \ref{fig:UDC:53} in order to account for \emph{hard} in examples like (\ref{ex:UDC:51})
and (\ref{ex:UDC:52}):
...
\begin{figure}[htb]
  ...
  \begin{avm}
    ...
    \caption{Lexical representation of \textit{tough} adjectives (preliminary)}
    \label{fig:UDC:53}
\end{figure}
...
\begin{figure}[htb]
  \centering
  \resizebox{\linewidth-1pt}{!}{\begin{avm}
    \[synsem  \[local|cat & \[head & adj\\
          subj & \< NP \[index \@i\] \>\\
          comps & \< VP\[vform & inf\\ slash & \{ NP \[index &
              \@i\] \} $~\cup$ \@1 \]\>\]\\
  ...
  \caption{Lexical representation of \textit{tough} adjectives (final)}
  \label{fig:UDC:54}
\end{figure}
...
\begin{figure}[htb]
  ...
  \caption{Lexical representation of relative complementiser
    \textit{that}}
  \label{fig:UDC:59}
\end{figure}
...
\begin{figure}[htb]
  ...
  \includegraphics{chapters/BB-filler-phrase-crop}
  \caption{Hierarchy of filler phrases}
  \label{fig:UDC:67}
\end{figure}
...
some free relatives in which the case of the wh-element differs from
    ...
    \Tree [ 
{
  ...
  { \avmbox{3}NP$_i$ } { ... }  
    ...
    \caption{Representation of Welsh resumptives}
    \label{fig:WelshResump}
\end{figure}
\noindent
...
dependencies, as observed with wh-fronting, give rise to a matching
...
\begin{figure}[htb]
  ...
  \caption{Hierarchy of \textit{local} \citep{Crysmann:12}}
  \label{fig:local}
\end{figure}
...
\end{tree}
  ...
  \caption{Hierarchy of \textit{synsem} objects \citep{Crysmann:12}}
  \label{fig:synsem}
\end{figure}
...
strong UDC with e.g.\ a wh-filler. Conversely, islands can narrow down
...
\section{More on wh-interrogatives}
...
\noindent
In (\ref{ex:WhPiedPiping}) the wh-word, a pronoun or determiner, that
marks the (embedded) wh-interrogative clause may be arbitrarily deeply embedded inside the filler.  
...
\noindent
In order to capture  the fact that the filler of a wh-clause must
contain a wh-word, or that the relative pronoun contained within the
...
\subsection{Multiple wh-questions}

% G&S on English

While in languages such as English only one wh-phrase may be fronted per interrogative clause (and typically one phrase is indeed fronted), it is nevertheless possible to ask multiple questions, with additional wh-phrases remaining in situ, as witnessed by \textit{what} in (\ref{ex:WhMult}).  
...
\textsc{wh}  feature thus not only ensures that a wh-interrogative is
marked as such by a filler containing a wh-word, but it also fixes the
semantic scope of ex-situ % \pdfcomment{BC: The term is quite frequently
  % used in the focus literature. }
wh-phrases to their syntactic
scope.\footnote{\citet{kathol:scope-marking} uses the \textsc{que}
  feature in his analysis of partial wh-fronting in German. } In situ wh-quantifiers, by contrast, are permitted to take arbitrarily wide scope. 

In Slavic languages such as Russian or Serbo-Croatian \citep{Penn:99},
there does not appear to be a constraint on the number of simultaneously fronted wh-phrases, as illustrated by the example in (\ref{ex:WhMultFront}). 
...
and \textsc{que/wh}, are set valued, multiple wh-fronting is a rather
...
The point where Slavic multiple fronting poses a challenge is its interaction with second position clitics:  it seems, as witnessed by the contrast in (\ref{ex:WhMultFront}) that multiple fronted wh-phrases are treated as a constituent, as far as linearisation is concerned. \citet{Penn:99} proposes a topological analysis based on extended word order domains \citep{Reape:90,kathol_a00} in order to reconcile multiple fronted constituents with the second position property: in essence, 
...
In languages such as English, where  standard wh-interrogatives are signalled by a wh-phrase ex situ (i.e. by a wh-filler),   
\citet{Ginzburg:Sag:01} identify two types of in situ wh-questions in English: so called reprise (or "echo") questions, which typically mimic
...
However, wh in situ may even be an unmarked, or even the default option for the expression of wh-interrogatives: 
\citet{Johnson:Lappin:97},  studying Iraqi Arabic, made the important observation that wh-fronting is optional in this language, posing a challenge for transformational models at the time.   
In Iraqi Arabic, a wh-interrogative may be realised ex situ, as in (\ref{ex:Iraqi}a) or in situ, as in (\ref{ex:Iraqi}b). 
...
They propose a straightforward analysis within HPSG, suggesting to drop what can be regarded as a parochial constraint of English and related languages an allow  \textsc{que} feature percolation from the right clausal daughter. 
 ...
 What is more, they note that wh in situ and ex situ strategies do observe different locality restrictions, thereby lending further support to a difference in the type of nonlocal feature involved. While nonlocal feature percolation for in situ wh-constructions cannot escape finite clauses (cf. the contrast in (\ref{ex:IraqiLoc}a,b).
...
\noindent
However, ex situ wh-interrogatives, involving a \textsc{slash} dependency, are obviously not subject to this restriction, as witnessed by (\ref{ex:IraqiLoc}c).  

Yet, even this constraint, while valid for Iraqi Arabic, must be considered language-specific: \citet{Crysmann:Reintges:14} study Coptic Egyptian, where wh in situ is the norm. They observe that the scope of an in situ wh-phrase is determined by the position of a relative complementiser and note that it can easily escape finite clauses, as shown in (\ref{ex:Coptic}). 
...
structure for (\ref{ex:UDC:ThatHeMightBeWrong}).
      ...
      \begin{figure}[htb]
        ...
        \caption{``Dishonest'' gap }
        \label{fig:UDC:Tree:ThatHeMightBeWrong}
      \end{figure}
...
hence not the \textsc{wh} feature in wh-interrogatives. However, the framework
...
% \begin{itemize}
% \item Maybe something on the main ways in which SBCG is
%     different in this area

%   \item A distinction between signs and constructions -- a
%     slimmed down notion of sign -- no daughters -- \emph{synsem} and
%     \emph{local} objects dispensed with -- \textsc{arg-st} and \textsc{valence} taking a
%     list of signs and not \emph{synsem} objects as their value -- GAP
%     taking a list of signs and not \emph{local} objects as its value

%   \item Bottom of dependency -- analysis allows a lexical head to
%     appear without a valent (subject, object, or other complement)
%     just in case its GAP list contains an element corresponding to
%     that valent. That is, a word's VAL list is shortened just in case
%     its GAP list is expanded.
