...
\label{chap-udc}
  ...
  \begin{exe}
    \ex \label{ex:UDC:basic} \begin{xlist} \ex[]{What did you put
        \trace{} on the
        table?}  % \ex[]{Who did you talk to \trace{}?}

      \ex[*]{You put \trace{} on the
        table?}  % \ex[*]{You talked to \trace{}.}
      \ex[*]{What did you put it on the table?}  % \ex[*]{Who did you
      % talk to him?}
    \end{xlist} \end{exe}
  ...
  \begin{exe}
    \ex \begin{xlist} \label{ex:UDC:3} \ex[]{ {[}\textsubscript{NP}
        Who{]} did Kim talk to \trace{} (NP)?}  \ex[]{
        {[}\textsubscript{PP} To whom{]} did Kim talk \trace{} (PP)?}
      \ex[]{ {[}\textsubscript{AP} How long{]} is a piece of string
        \trace{} (AP)?}  \ex[]{ {[}\textsubscript{AdvP} How quickly{]}
        did you do it \trace{}
        (AdvP)?}  \end{xlist} \end{exe} \noindent They typically match
...
\z
  ...
  have a so-called resumptive pronoun (RP). The following \ili{Welsh}
...
\begin{exe} \ex \begin{xlist} \label{ex:UDC:8} \ex[]{
      Kim will sing, which Lee won't \trace{}.}  \ex[*]{Which won't
      Lee \trace{}?}  \end{xlist} \end{exe}
...
relativisation with resumption in \ili{Hausa} or Modern Standard Arabic.
...
\citet[Section~4.2]{Borsley:13}; \citet{Mueller:14b}. On
...
categories.\footnote{Coordination is a problem for any empty category,
  ...
  propose that \ili{African American Vernacular English}\il{English!African American Vernacular} has a
...
with intransitives, like \ili{Hausa} \citep{crysmann_b04yom} or Mauritian
Creole \ili{French} \citep{Henri10}. In \ili{Hausa} and Mauritian, verbs
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \gll Sun rāzànā.\footnotemark\\
         ...
         \footnotetext{\citep[632]{newman_p00}}
...
valence reduction, the \ili{Hausa} (and Mauritian) facts are
...
\inlinetodoobl{JP: Shouldn't we say something about ``flattening'' of feature geometry throughout
  the chapter? Stefan: Yes, and point to the Intro: \crossrefchaptert{properties}.}
\ea
\label{fig:UDC:32}
...
complement is the top of the dependency.\footnote{The
  ...
  peculiarity of English: similar constructions in \ili{German} and \ili{French}
  do exist, but they feature local (passive-like) dependencies. See
  \citet{abeille_a-godard_d-miller_p-sag_i95} and
  \citet{aguila-multner18} for \ili{French}, as well as \citet{Mueller:02b}
  for \ili{German}. Even for English, the unboundedness of the construction
...
\ex[]{  someone {[}who I should talk to \trace{}{]}}

\ex[]{ someone {[}to whom to talk \trace{}{]}}
\end{xlist}
\end{exe}
...
\emph{the}-clauses (\citealp{Borsley:04}; \citealp[490--4,524--7]{Sag:10a}, \citealp{Borsley:11}), the components of
...
assumed that the initial constituent is a filler and not a head
...
\footnotetext{\citep[78]{sells_p84}}
...
\footnotetext{\citep[91]{Borsley.2010}}
...
\footnotetext{\citep[92]{Borsley.2010}}
...
\ili{Hausa} patterns with a number of resumptive languages, including \ili{Welsh}, in that use of a resumptive is obligatory for complements of a preposition or the possessor of a noun. With direct and indirect objects, however, both resumptives and gaps are possible: 

\eal
\label{ex:HauIO}
\ex{
\gll mutā̀nên dà sukà ƙi sayar wà \trace{} dà àbinci sukà fìta\footnotemark\\
     men \textsc{rel} \textsc{3.p.cpl} refuse sell to {} with food \textsc{3.p.cpl} left\\
\footnotetext{\citep[534]{jaggar01:_hausa}}
\glt `The men they refused to sell food to left.'} 
\ex{
\gll mutā̀nên dà sukà ƙi sayar \textnobf{musù} dà àbinci sukà fìta\footnotemark\\
...
\footnotetext{\citep[84]{tuller_l86}}
...
\footnotetext{\citep[539]{newman_p00}}
...
within the filler of a relative must structure-share its
...
languages like English, and even more in \ili{French}, we do find
...
\ili{Japanese} or Coptic Egyptian, in situ realisation is the norm, rather
...
\gll Mona shaafat meno?\footnotemark\\
     Mona saw whom\\
\footnotetext{\citep[318]{Johnson:Lappin:97}}
\glt `Who did Mona see?'}
\ex[]{
\gll Meno shaafat Mona?\footnotemark\\
     who saw Mona\\
\footnotetext{\citep[320]{Johnson:Lappin:97}}
\glt `Who did Mona see?'}
\zl
...
\ex[]{
\gll Mona raadat tijbir Su'ad tisa'ad meno?\footnotemark\\
      Mona wanted to.force Su'ad to.help who\\
\footnotetext{The examples in (\ref{ex:IraqiLoc}) are from \citew[318]{Johnson:Lappin:97}}
...
\footnotetext{\citep[72]{Crysmann:Reintges:14}}
...
such as the Complex NP Constraint \citep{ross_j67}, clearly do not hold
...
\footnotetext{\citep[4]{Keller:94a}}
...
\footnotetext{\citep[11]{Keller:94a}}}
\zl
\eal
\ex[]{
...
\footnotetext{\citep[G.][\page 219]{mueller96:_extrap_and_succes_cyclic}}
...
\footnotetext{\citep[282]{kiss_t02nllt}}
...
\footnotetext{\citep[259]{haider96:_downr_down_to_right}}
\glt `People have scolded the wife of the messenger who delivered the order.'}
\zl

\inlinetodoobl{JP: Not clear that (\mex{1}b) involves complement extraposition.}
...
\footnotetext{\citep[283]{kiss_t02nllt}}
...
\footnotetext{\citep[285]{kiss_t02nllt}}
...
\footnotetext{\citep[261]{haider96:_downr_down_to_right}}
...
{\gll Ich habe [von [dem Versuch [eines Beweises [der Vermutung
      \_$_i$ ]]]] gehört, [daß es Zahlen gibt, die die folgenden
      Bedingungen erfüllen]$_i$.\footnotemark\\
      ...
      fulfil\\
\footnotetext{\citep[St.][223]{mueller_s02lc}}
...
\footnotetext{\citep[381]{Crysmann2013a}}
...
\footnotetext{\citep[148]{kuthy01}}
...
are lacking in construction-based HPSG. Hence, they are
