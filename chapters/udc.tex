...
\abstract{Unbounded dependencies of the kind that are found in
  \emph{wh}-interrogatives, relative clauses, and other constructions
  have been a major focus of research in HPSG. They typically involve
  a gap of some kind and some distinctive higher structure, often
  involving a filler in a non-argument position with the properties of
  the gap. HPSG has developed detailed proposals about the bottom of
  the dependency, the middle, and the top. In the case of the top of
  the dependency, complex hierarchies of phrase types have been
  employed to handle the distinctive properties of the various
  unbounded dependency constructions. Analyses have also been
  developed for unbounded dependencies with a resumptive pronoun, the
  special properties of \emph{wh}-interrogatives, extraposition
  phenomena, and filler-gap mismatches.}
...
\ex[]{ [\textsubscript{AP} How long] is \rep{a}{this} piece of string \trace{} (AP)?}  
...
constituent.\footnote{The basic approach derives from the earlier
  ...
  category Y.} 
Much HPSG work assumes the feature geometry \add{in} (\ref{ex:UDC:9}), following \citet[Chapter~4]{Pollard:Sag:94}:
...
\noindent
Because the V contains an NP gap,\itdopt{this will be unusual for most readers. The verb contains a
  gap? Maybe say more/things differently?} it will be [\textsc{slash} \{NP\}], and so
...
\textsc{arg-st} lists.\itdopt{
Explain what \argst stands for. Crossref to basic properties and arg-st chapter.} Thus, the verb \emph{put} in (\ref{ex:UDC:10})
 has a gap in its
...
\itdopt{It may be helpful to visualize the flow of information.}   
\end{figure}
...
\added{We use shorthands rather than full AVMs. For example \SLASH is located under
...
%   \small
\emph{head-filler-ph\added{rase}} \impl\\
\avm{
	[slash & \{ \} \\
	hd-dtr & \1	[comps & < >\\
                         slash & \{ \2 \} ]\\
...
\ea
\gll Sun hūtā̀.\\
  \textsc{3pl.cmpl} rest.\textsc{a}\\\hfill\added{(Hausa)}
...
\label{ex:UDC:Hau:tr}
    ...
    \gll Sun rāzànā.\footnotemark\\
         \textsc{3pl.cmpl} terrorise.\textsc{a}\\\hfill\added{(Hausa)}
...
\added{Lexemes are basic lexical items. Lexemes of inflectable parts of speech are mapped to
...
by \emph{that}.\itdopt{Say that this is the that trace effect? To make Minimalists feel welcome?} In contrast, the examples in (\ref{ex:UDC:26}) suggest
...
\avm{
[synsem & [nonloc & [slash & \1 $\cup$ \ldots{} $\cup$ \tag{n}]]\\
...
Feature Principle for this purpose.\footnote{
  \added{See also \crossrefchapterw[\pageref{properties:ex-generalized-head-feature-principle}]{properties} for an explicit formulation of the constraint.}
...
\ex[]{
\gll \rep{A}{a}n fear aL shíl mé aL bheadh ann\del{.}\\
     the man \textsc{vpart} thought I \textsc{vpart} would.be there\\
\glt `the man that I thought would be here'}
\itdobl{The original was small and without dot. And it is an NP so no punctuation and no capital at
  the beginning.}
\zl
...
is standardly said to be a case of \isi{across-the-board extraction} \added{\citep{Ross67a,Williams78a}}. (\ref{ex-which-person-did-you-invite-without-thinking}) is traditionally seen as involving an
...
\added{See also \crossrefchapterw{relative-clauses} for a more detailed discussion of relative clauses.}} as two examples of
...
However, for other work, this is a filler just like the
  \emph{wh}-elements in the text.
\itdopt{Which text? Replace by ``just like the \emph{wh}-elements in examples like (someref)'' or
  ``just like the \emph{wh}-elements in the examples discussed so far''?}} while the later sometimes do but
...
\citealp{Borsley:11}; \added{\crossrefchapteralt[Section~\ref{coord:sec-comparative-correlatives}]{coordination}}), the components of
...
be a subtype of \emph{head-filler-ph\added{rase}} but also a subtype of
...
\itdobl{Redraw with forest, use head-filler-phrase as every where else in the paper. Stefan: I can
  do this.}
...
constituent.\footnote{Non-restrictive relatives can also modify various
  ...
  \citew{Arnold:Borsley:08}\added{, and \crossrefchapterw[Section~\ref{sec:rc-non-restr-suppl}]{relative-clauses}}.% \inlinetodoopt{Stefan: And there are also free relatives functioning as arguments.}
...
\emph{head-filler-ph\added{rase}}, \emph{standard-head-fill-ph}, with
...
\itdopt{Explain why NP rather than N$'$? Cite Kiss?}
\z
...
is coindexed with the antecedent noun\itdopt{Here you talk about noun rather than NP} selected via \textsc{mod}, and that any additional members of the
...
different case requirements.}%
\itdopt{You may leave this the way it is, but defaults will not work, since the case information is
  needed. What you suggest is defaults in syntax. This leads directly to hell. This is not done what
  is done elsewhere. All other defaults can be replaced by non-default representations. One has to
  know what the upstairs case can be. And in the case of free relatives this is just the cases of
  less oblique grammatical functions, \eg for a PP relative phrase the upstairs function can be
  NP-nom and NP-acc. No way you do this with defaults.}
...
\gll Dyma ’r dyn y credodd Dafydd [y si [y gwelodd Mair (o)]].\footnotemark\\
     here.is the man \textsc{prt} believe.\textsc{past.3\changed{sg}} Dafydd \spacebr{}the rumour
     \spacebr{}\textsc{prt} see.\textsc{past.3\changed{sg}} Mair \hphantom{(}he\\
...
\gll Dyma ’r dyn y credodd Dafydd [y si [y cest ti ’r llythyr ’na ganddo (fo)]].\footnotemark\\
     ...
     \spacebr{}\textsc{prt} get.\textsc{past.2sg} you the letter \textsc{dem} with.\textsc{3\changed{sg}.m} \hphantom{(}him\\
...
\gll Dyma ’r ffenest darais i [’r bachgen [dorrodd (hi) ddoe]].\footnotemark\\
     that.is the window hit.\textsc{past.1\changed{sg}} I \spacebr{}the boy \spacebr{}break.\textsc{past.3\changed{sg}} \hphantom{(}she yesterday\\
\footnotetext{\citet[92]{Borsley.2010}}
    }
\zl
...
pronominal argument coindexed with its slashed value''.\itdopt{What is a ``slashed value''? Its
  ``\SLASH value''? If you change things, please remove quotation marks since it is not a literal
  quote any longer.} Consequently,
...
\avm{
   [head & \1 prep $\lor$ noun \\
	...
	[\ldots]]
\end{forest}
\itdobl{Without the \ldots{} I added the tree suggests that the resumptive is always the first
  daughter which does not seem to be the case in (\ref{ex-dymar-r-dyn-resumptive}).}
...
\textit{synsem},
\itdobl{There seems to be a problem with formulation here. How can a type be appropriate for another
  type? Replace ``which is appropriate for synsem'' by ``for which \textsc{cat} is appropriate''?}
this includes categorial and full semantic
...
\itdobl{refer to figure in the text}
...
\citet[Chapter~7]{Ginzburg:Sag:01} for \add{a} head-driven formulation of nonlocal feature percolation.
...
\itdobl{(\mex{1}a) is not mentioned in the text.}
...
\ili{English}.\itdopt{Cite \citet{Riemsdijk78a}?} This is supported not only by the ban on preposition
...
\added{See also \crossrefchapterw[Section~\ref{rc:sec-internal-structure}]{relative-clauses} on pied
  piping.}
}
...
situ \emph{wh}"=phrases, by contrast, are still\itdopt{why ``still''?} quantifiers, so they
...
\itdopt{Why is this footnote here? To contrast the feature name WH with QUE? Maybe ``uses the
  feature QUE'' would be better.}
 }
...
\gll Ko koga si mislio da je voleo?\\
     who whom \textsc{cl.2sg} thought \textsc{comp} \textsc{cl.3sg} loved\\\hfill\added{(Serbo-Croatian)}
...
\itdopt{changed s to sg for LGR compliance.}
\zl
...
(\citealt{Reape:90,Reape94a,kathol_a00}\added{; \crossrefchapteralt[Section~\ref{sec-domains}]{order}}) in order to reconcile multiple fronted
...
\gll Mona shaafat meno?\footnotemark\\
     Mona saw whom\\\hfill\added{(Iraqi\il{Arabic!Iraqi|(} Arabic)}
...
\gll Mona raadat tijbir Su'ad tisa'ad meno?\\
      Mona wanted to.force Su'ad to.help who\\\hfill\added{(Iraqi\il{Arabic!Iraqi|(} Arabic)}
...
\gll ere əm=mɛɛʃe tʃoː əmmɔ=s [tʃe ang nim]?\footnotemark\\
     \textsc{rel} \textsc{def.pl}=crowd say \textsc{prep=3f.sg} \spacebr{}that I who\\\hfill\added{(Coptic Egyptian\il{Egyptian!Coptic})}
...
\gll Planck hat [die Entdeckung \_$_i$] gemacht, [daß Licht Teilchennatur hat.]$_i$\footnotemark\\
     Planck has \spacebr{}the discovery {} made \spacebr{}that light particle.nature has\\\hfill\added{(German)}
...
\gll Was$_i$ hat Hans gesagt, [daß           wir \_$_i$ kaufen sollten]?\\
     what    has Hans said    \spacebr{}that we {} buy should\\\hfill\added{(German)}
...
\footnotetext{\cites[\page 11]{wiltschko94:_extrap_in_german}[\page 10]{Keller:94a}}
...
\flushright\avm{
[loc|cont \3\\
 nonloc|extra & \1 $\cup$ \{[cat & [head & [\type*{prep $\lor$ rel}
                                                  mod|loc & \2 ] \\
                                   cont & \3 ] ] \} ]
}
\zs
...
The complement extraposition rule is straightforward: it removes \rep{a
valency}{an argument} from the \textsc{comps} list and inserts its \textsc{local}
...
\itdobl{loc is not used any longer in your schema.}}
\begin{exe}
...
%        }
	...
	[synsem & [nonloc|extra & \tag{x} ] \\
         ...
         $\cup$ \tag{x}] \\
	 extra-dtrs & < [synsem|loc & \tag{1}], \ldots{}, [synsem|loc & \tag{n}]> ] ]
}
\itdobl{Well, if you modify it anyway, you can go to NON-HEAD-DTRS and no DTRS right away.}
\end{exe}
...
\gll Man hat [die Frau [des Boten \_$_i$]] beschimpft, [der den Befehl überbrachte]$_i$.\footnotemark\\
     one has \spacebr{}the wife \spacebr{}of.the messenger {}  scolded \spacebr{}who the order delivered\\\hfill\added{(German)}
...
\glt \added{Intended:} `He laughed when he left the dungeons.'
}
\z
...
target any referential index introduced within the clause the relative \added{clause}
...
adjoin to.\footnote{See
  ...
  approach.\itdopt{Does the approach assume something? Maybe ``in Kiss' appraoch''.}}$^,$\footnote{\citet{crysmann_b04rlc} proposes to
...
\caption{Captain: add some caption}
\itdobl{add caption}
\itdopt{can we use [1] and [2]? The i and j looks ugly. In any case the $_j$ is missing at theory.}
\end{figure}
...
``dishonest gap''\is{gap!dishonest}, one whose \textsc{local} value and \textsc{slash}
value\itdopt{should be \SLASH element, since the \SLASH value is a set. also below ``value of \SLASH''} do not match.
...
result is an auxiliary complement ellipsis sentence. When \textsc{slash} \rep{has the
nominal value}{contains a nominal element}, we have a dishonest gap, because the value of \textsc{local} is
whatever the auxiliary requires, normally a VP of some kind, and the
result is an ASRC.
\itdopt{Since ASRC is used only once, you may give the full name rather than the abbreviation.}
...
\added{See \citew[Section~\ref{prop:sec-sbcg}]{chapters/properties} and \citew[Section~\ref{cxg:sec-sbcg}]{chapters/cxg} for a general comparison of
...
At the bottom of \add{the} dependency, things are rather different. The SBCG
...
\itdopt{Here, there is an accumulation of sentence initial ``here'', which you may want to change.}
...
\itd{Stefan: Add missing abbreviations}
...
Stefan Müller was a real pain in the neck since he kept on asking for changes. But we want to thank
him wholeheartedly for reminding us to put something into the acknowledgements section.

\itd{Stefan: Add something?}
...
%      <!-- Local IspellDict: en_GB-w_accents -->
