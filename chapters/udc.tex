...
\abstract{Unbounded dependencies of the kind that are found in
  \emph{wh}-interrogatives, relative clauses, and other constructions have
  been a major focus of research in HPSG. They typically involve a gap
  of some kind and some distinctive higher structure, often involving
  a filler in a non-argument position with the properties of the
  gap. HPSG has developed detailed proposals about the bottom of the
  dependency, the middle, and the top. In the case of the top of the
  dependency, complex hierarchies of phrase types have been employed
  to handle the distinctive properties of the various unbounded
  dependency constructions. Analyses have also been developed for
  unbounded dependencies with a resumptive pronoun, the special
\inlinetodoopt{Stefan: Minor point: Analyses for special properties?}
  properties of \emph{wh}-interrogatives, extraposition phenomena, and
...
properties of the associated gap. Thus, in the following, \inlinetodoopt{Stefan: insert ``the''?}filler and
...
\ex[]{ [\textsubscript{AP} How long] is a piece of string \trace{} (AP)?}  
...
\inlinetodoopt{Stefan: Why don't you give the whole sign rather than just synsem?}
\ea
\label{ex:UDC:9}
...
\textsc{arg-st} lists. Thus, the preposition \emph{to} in (\ref{ex:UDC:10})
\inlinetodoobl{Stefan: There is no preposition \emph{to} in (\ref{ex:UDC:10}).} has a gap in its
...
\ea
\label{ex:UDC:12}
\inlinetodoobl{Stefan: It is on page 161 and it is the whole sign. You may say ``according to
  Pollard \& Sag''.}
...
list 
\inlinetodoobl{JP: I find this confusing. Maybe NP LOCAL. Stefan: ``where \ibox{1} is the \localv of
  an NP.''}
where \ibox{1} is NP.
...
arguments.\inlinetodoobl{Stefan: What about adjuncts? BMS had a \deps list.}
...
\ea
%\label{fig:UDC:16}
\inlinetodoobl{Stefan: Shouldn't we have \synsem in the daughters? And maybe full pathes in schemata
  to be precise?}
...
\inlinetodoobl{Stefan: Check whether you can cite \crossrefchaptert{islands} here. I could not find
  the section/page to cite, but I guess there is something to cite.}
...
\begin{figure}
\inlinetodoopt{Stefan: Why is this a tree? Turn into a lexical item with a description saying that
  it is \emph{put}?}
\centering
\begin{forest}
sm edges without translation
...
sm edges without translation
	...
	\avmtmp{
		...
		\avmtmp{
			...
			[\trace]]
		[%
		\avmtmp{[synsem & \3 ]} [on the table, roof]] ]
\end{forest}
}
...
with intransitives, like \ili{Hausa} \citep{crysmann_b04yom} or \ili{Mauritian}
Creole \ili{French} \citep{Henri10}. In \ili{Hausa} and \ili{Mauritian}, verbs
...
the following examples from \ili{Hausa}:
\inlinetodoobl{Stefan: consider adding ``taken from \ldots'' before the examples. (\mex{1}) and
  (\mex{2}c) do not have a source.}
...
\noindent
\ili{Hausa} verbs are lexically transitive or intransitive, and they are
...
direct object: if used intransitively, they pattern with intransitive\inlinetodoobl{Stefan:
  ``verbs''? or use plural ``intransitives''}
...
there. Similar observations appear to hold for \ili{Mauritian}
\citep{Henri10}\addpages. Thus, if nonlocal realisation corresponds to lexical
valence reduction, the \ili{Hausa} (and \ili{Mauritian}) facts are
...
valence lists of word-level signs. The theory of ergativity proposed
by \citet[Section~5.2]{Manning:Sag:99} in terms of
\inlinetodoobl{JP: The traditional term ``linking'' refers to something else. Stefan: Maybe use
  mapping. !several occurances of ``linking''!}
...
languages.\footnote{\citet{Crysmann:09} exploits the
  fact that extracted arguments do not appear on the valence lists of
  word-level signs and formulates local case assignment for \ili{Nias} as a
...
\inlinetodoobl{JP: Is the difference between lexemes and words explained anywhere?}
...
% agreement in \ili{Welsh} is controlled by the first element on
...
% so does agreement: for \ili{Welsh}, \citet{Borsley89} argued that the
...
% finite verbs and prepositions. In \ili{Welsh}, prepositions

% Borsley's \citeyear{Borsley89} theory of \ili{Welsh}
...
% \ex[]{ Which student do you think \trace{} knows the answer?}

% \ex[]{ Which students do you think \trace{} know the answer?}
% \end{xlist}
% \end{exe}
...
% an approach will miss a generalisation in the case of \ili{Welsh}. In \ili{Welsh}
...
% \noindent
% An approach to \ili{Welsh} agreement which refers to \textsc{arg-st} lists
...
% cases. \ili{Welsh} has other cases of agreement. For example, most
...
% order domains in \ili{Welsh}. As he shows, this proposal entails that gaps
% must be empty categories in this language.\footnote{\citet{Borsley:16:Archi} argues that an AGR-ST based approach to agreement will also miss a generalisation in the case of the Caucasian language \ili{Archi} and proposes that agreement in this language involves constraints on constituent structure. This also means that gaps must be empty categories.}
...
% agreement in \ili{Nias}, building on the theory of ergativity of
...
% may indeed target valence lists, the \ili{Welsh} data above could just as well
...
% constraints on order domains in \ili{Welsh}, and \citet{Borsley:16:Archi} argues that
% agreement in the Caucasian language \ili{Archi} involves constraints on
...
\inlinetodoobl{JP: Has some comment which Stefan cannot read: That's \ldots????}
\begin{exe}
\ex \label{ex:UDC:27}
...
\inlinetodoobl{JP: Shouldn't we say something about ``flattening'' of feature geometry throughout
  the chapter? Stefan: Yes, and point to the Intro: \crossrefchaptert{properties}. Stefan: Actually,
I would prefer precision.}
\ea
\label{fig:UDC:32}
\textsc{slash} Amalgamation Principle:\\
   \textit{word} \impl
\avmtmp{
	$\slash$	[slash & \1 $\cup$ \ldots{} $\cup$ \tag{n}\\
				arg-st & <[slash & \1], \ldots{} [slash & \tag{n} ] > ]
}
\z

\noindent
This is a default constraint, as indicated by the `$/$'\is{$/$}. Essentially,
...
itself since the 
\inlinetodoobl{JP: It should be the subject rather than the object.}
complement is the top of the dependency.\footnote{The
...
\inlinetodoobl{Stefan: Reference for the examples is missing.}
\eal
\label{ex:UDC:34}
\ex[]{
...
\ex[]{
\gll An fear aL shíl mé aL bheadh ann.\\
...
\textsc{slash} by means of the \isi{Nonlocal Feature Principle}, and related
...
\ex[]{ Which person did you [invite \trace{} [without thinking \trace{} would
actually come]]?}
\end{xlist}
\end{exe}
...
is standardly said to be a case of \isi{across-the-board extraction}. (\ref{ex:UDC:37}b) is traditionally seen as involving an
...
constituent.\footnote{Non-restrictive relatives can also modify various
  ...
  \citet{Arnold:Borsley:08}.\inlinetodoopt{Stefan: And there are also free relatives functioning as arguments.}} Finally, constraints on \emph{decl-cl}
...
\inlinetodoobl{Stefan: Even though the example is well-known, a source should be given. \citet[\page
  169]{ps2} have the following example: \emph{[A violin this well crafted]1, even [the most
    difficult sonata]2 will be easy to play -2 on -1·}. Take this example?}
...
\{\}].\footnote{This is essentially the approach that is taken to
  relatives in Modern Standard \ili{Arabic} in \citet{Alqurashi:Borsley:12}.}
...
\inlinetodoobl{Stefan: Where is this example from?}
\eal
...
which \inlinetodoopt{Stefan: What do I know about English. But can one ``undergo agreement''?}undergo agreement with the antecedent.  
...
\type{filler-phrase}, as shown in Figure~\ref{fig:UDC:67}.
...
identifying the second daughter as the head, while \type{free-relative} will
...
Naturally, there may be complications here. \ili{German}, for example, has
...
\noindent
Subsequent work on \ili{Persian} \citep{taghvaipour:phd:05}, \ili{Welsh}
\citep{Borsley:13} and \ili{Hausa} \citep{Crysmann:12} essentially follows
...
In his work on \ili{Welsh} unbounded dependencies, \citet{Borsley.2010}
\inlinetodoobl{JP: insert ``pronoun'' after ``resumptive''. I never saw ``resumptive'' used as a noun.}
observes that the choice between gap and resumptive pronoun is essentially
determined by properties of the immediate environment of the bottom of the
...
\eal
\ex[]{
\gll Dyma ’r dyn y credodd Dafydd [y si [y gwelodd Mair (o)]].\footnotemark\\
     here.is the man \textsc{prt} believe.\textsc{past.3s} Dafydd \spacebr{}the rumour
     \spacebr{}\textsc{prt} see.\textsc{past.3s} Mair \hspaceThis{(}he\\
\footnotetext{\citet[91]{Borsley.2010}}
\glt `Here’s the man who David believed the rumour that Mair saw.'}
\ex[]{ 
\gll Dyma ’r dyn y credodd Dafydd [y si [y cest ti ’r llythyr ’na ganddo (fo)]].\footnotemark\\
     here.is the man \textsc{prt} believe.\textsc{past.3sg} Dafydd \spacebr{}the rumour
     \spacebr{}\textsc{prt} get.\textsc{past.2sg} you the letter \textsc{dem} with.\textsc{3s.m} \hspaceThis{(}him\\
\footnotetext{\citet[92]{Borsley.2010}}
\glt `Here’s the man who David believed the rumour that you got that letter from.'}
\ex[*]{ 
\gll Dyma ’r ffenest darais i [’r bachgen [dorrodd (hi) ddoe]].\footnotemark\\
     that.is the window hit.\textsc{past.1s} I \spacebr{}the boy \spacebr{}break.\textsc{past.3s} \hspaceThis{(}she yesterday\\
\footnotetext{\citet[92]{Borsley.2010}}
    }
\zl
...
resumptive\inlinetodoopt{Stefan: JP suggested adding ``pronoun'' above.} is obligatory for
...
\gll Gā̀      tābōbîn$_j$ dà          Àli ya                 san  mùtumìn$_i$ dà           zâi$_i$            yī \textnobf{musù}$_j$ /  *{wà \trace{}$_j$}   kwālī\footnotemark\\
     here.is cigarettes \textsc{rel} Ali \textsc{3.s.m.cpl} know man         \textsc{rel} \textsc{3.s.m.fut} do to.them           {} \hspaceThis{*}to     box\\
\footnotetext{\citet[84]{tuller_l86}}
    \glt `Here are the cigarettes that Ali knows the man that (he) will make a box for.'
\z

\noindent
Crysmann\inlinetodoobl{Stefan: Provide exact reference.} further emphasises that relativisation (which may escape
...
\inlinetodoobl{Stefan: \citet{Crysmann:12} does not have \type{weak-local}, add \citet{Crysmann:16}
  here and everything is fine. I changed the references in the captions below.}
...
dependency 
\inlinetodoobl{JP: delete ``easily''.}
easily permits mixing of gap and
...
\inlinetodoopt{Stefan: The three subsections should be introduced/mentioned here.}
...
pied-piping,\inlinetodoobl{Stefan: Cite Ross? The first examples do not show pied piping contrary to
  the claim in the text.} as illustrated in (\ref{ex:WhPiedPiping}) and
(\ref{ex:RelPiedPiping}), taken from \citet[184]{Ginzburg:Sag:01}.
...
\gll das Buch [[\textit{dessen} / *\textit{deren} Rezension] mir gefiel]\\
     \textsc{def.n.s} book(\textsc{n})\textsc{.s} \hspaceThis{[[}\textsc{rel.n.s.poss} {} \hspaceThis{*}\textsc{rel.f.s.poss} review(\textsc{f}).\textsc{s} me pleased\\
\glt `the book the review of which I liked'
\ex
\gll die Autorin [[\textit{deren} / *\textit{dessen} Roman] mir gefiel]\\
     \textsc{def.f.s} author(\textsc{f})\textsc{.s} \hspaceThis{[[}\textsc{rel.f.s.poss} {} \hspaceThis{*}\textsc{rel.m.s.poss} novel(\textsc{m}) me pleased\\
...
within the filler of a relative\inlinetodoopt{Stefan: Insert ``clause''?} must structure-share its
...
a single \isi{Nonlocal Feature Principle} that generalises from
...
daughters. See, however, \citet{Sag:97}\addpages and \citet{Ginzburg:Sag:01}\addpages
...
\ili{English}. This is supported not only by the ban on preposition
...
\inlinetodoopt{Stefan: Do people know what a parameter is?}
...
\inlinetodoobl{Stefan: Glosses missing (in original as well). Changed dash to dot, since this corresponds to the LGRs.}
\eal
...
\citet[Section~\inlinetodoobl{add sec}]{Johnson:Lappin:97}, studying Iraqi\il{Arabic!Iraqi|(} Arabic, made the important
...
with relative clause extraposition.

\inlinetodoobl{Stefan: Only the second set is a relative clause. Suggestion: ``clearly do not hold
with complement clause and relative clause extraposition, as the following examples by \citep[\page 4, 11]{Keller:94a} and \citet[G.][\page 219]{mueller96:_extrap_and_succes_cyclic} show:''}

\eal
\ex[]{
\gll Planck hat [die Entdeckung \_$_i$ ] gemacht, [daß Licht Teilchennatur hat.]$_i$\footnotemark\\
     Planck has the discovery {} {} made \spacebr{}that light {particle nature} has\\
...
\gll [Daß Licht Teilchennatur hat]$_i$  hat Planck [die Entdeckung \_$_i$ ] gemacht.\footnotemark\\
      \spacebr{}that light {particle nature} has  has Planck the discovery {} {} made \\
...
\gll Was$_i$ hat Hans gesagt, [daß           wir \_$_i$ kaufen sollten]?\\
     what    has Hans said    \spacebr{}that we {} buy should\\
...
\gll [Daß Peter sich auf das Fest \_$_i$ gefreut hat, [das Maria veranstaltet hat,]$_i$ ] hat niemanden gewundert.\footnotemark\\
      \spacebr{}that Peter SELF on the party {} looked.forward has \spacebr{}which Maria organised has {} has noone surprised\\
\footnotetext{(\citealp[\page 11]{wiltschko94:_extrap_in_german}; \citealp[\page 10]{Keller:94a})}
...
\ex[*]{\gll [Daß Peter sich auf das Fest \_$_i$ gefreut hat],  hat niemanden gewundert, [das Maria veranstaltet hat]$_i$\footnotemark\\
      \spacebr{}that Peter SELF on the party {} looked.forward has  has noone surprised \spacebr{}which Maria organised has\\
\footnotetext{(\citealp[\page 11]{wiltschko94:_extrap_in_german}; \citealp[\page 10]{Keller:94a})}}
\zl
...
\inlinetodoobl{JP: So we have Keller's formulation even though we don't use \subcat throughout the
  book? Stefan: Can we use \argst here? Or \comps. \comps would work for \ili{German} in my world.}

\ea
Complement Extraposition Lexical Rule\\

\avmtmp{
[subcat & \1 \+ <[loc & \4 [cat [head & verb $\lor$ prep \\
			         subcat & < > ] ] ] > \+ \2 \\
\punk{nonloc|inher|extra}{\3} ]}
$\mapsto$ \\
\flushright \avmtmp{[subcat & \1 \+ \2 \\
             \punk{nonloc|inher|extra}{\3 $\cup$ \{ \4 \}} ]}
\z
...
\flushright\avmtmp{
[loc|cont \3\\
 nonloc|inher|extra & \1 $\cup$ \{[cat & [head & [\type*{prep $\lor$ rel}
                                                  mod|loc & \2 ] \\
                                   cont & \3 ] ] \} ]
}
\zs
...
i.e.\ the middle of the dependency, is handled by the \isi{Nonlocal Feature Principle}
...
dependencies, which are realised as extraposed daughters.\footnote{We
  ...
  \citet[\page 304--305]{Keller:95} for details. \textsf{loc($x$)} denotes a function
  which takes as $x$ a list of elements of type \type{sign} and
...
the head is regulated by linear precedence statements (see \crossrefchapterw[Section~\ref{sec-id-lp}]{order} on linear
...
extraposition in \ili{German} does not translate to complement extraposition
in equal measure.
...
\gll Man hat [die Frau [des Boten \_$_i$ ]] beschimpft, [der den Befehl überbrachte]$_i$.\footnotemark\\
     one has \spacebr{}the wife \spacebr{}of.the messenger {} {} scolded \spacebr{}who the order delivered\\
...
\inlinetodoobl{JP: Not clear that (\mex{1}b) involves complement extraposition. Stefan: The idea is
  that (\mex{1}a) and (\mex{1}b) are a contrast. Complement in (\mex{1}a) vs. (\mex{1}b). Could be clearer.}
While acceptable examples of complement extraposition from complex NPs
can be found (see below
\inlinetodoobl{JP: Which example is being referenced? Unclear. (\ref{ex-versuch-eines-beweises-der-vermutung})?}), extraposition from adjuncts yields much
...
\glt `He laughed when he left the dungeons.'
}
\z
...
extraction to the right. Finally, no matching effect\inlinetodoobl{Stefan: What is a matching
  effect? It is usually mentioned in connection with free relative clauses. This is also the case in
this paper so far. In nonlocal dependencies there is usually identity between information. The
matching in free relatives is softer. This may be confusing.} can be observed
...
[{V\usebox{\onetwobox}}
   [{V\usebox{\onetwobox}} 
    [{NP\usebox{\onetwobox}} 
       ...
       [{N\usebox{\onetwobox}} 
         [{N\usebox{\onebox}} [Beweis;proof] ] 
         [{NP\usebox{\twobox}} 
           [{D\usebox{\emptybox}} [der;of.the] ]
           [{N\usebox{\twobox}} [Theorie;theory] ] ] ] ]
     [{V\usebox{\emptybox}} [erbracht;produced] ] ] 
  [S [an die$_j$ niemand glaubt;in which nobody believes,roof] ] ]
\end{forest}
}
}
\inlinetodoobl{Stefan: I redid tree with forest, glossed items. Please check and provide caption.}
...
{\gll Ich habe [von [dem Versuch [eines Beweises [der Vermutung \_$_i$ ]]]] gehört, [daß es Zahlen gibt, die die folgenden Bedingungen erfüllen]$_i$.\footnotemark\\
      ...
      fulfil\\
\footnotetext{\citet[St.][223]{mueller_s02lc}}
...
been observed for PP extraction out of NPs \citep{DeKuthy02},\addpages cf. the
...
\ex{
\gll Er hat [ein Buch [über die Theorie \_$_i$ ]] geklaut, [die derzeit kontrovers diskutiert wird]$_i$.\\
...
\inlinetodoobl{Stefan: This is not correct, since not all constructions have a \textsc{hd-dtr}.}
\begin{exe}
...
\noindent
\inlinetodoopt{Stefan: ``the SBCG''? Remove ``the''?}
...
\inlinetodoobl{Stefan: This ends a bit unexpectedly. Are there any pro and cons? Or just one little
  concluding sentence?}
