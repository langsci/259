...
would all be included, while the \changed{eleven} languages mentioned in the first paragraph would
...
%
\ea\label{ARP} 
Argument Realization Principle adapted from \citew[171]{GSag2000a-u}:\\
...
and \compsl. \changed{As \citet[\page 171]{GSag2000a-u} point out, this is not a problem since overt signs that are
...
of Canonicality given in (\mex{1}):
\ea
\isi{Principle of Canonicality} (adapted from \citealt[\page 40]{GSag2000a-u}):\\
\type{sign} \impl \avm{
[ synsem & canonical ]}
\z
...
% \emph{word} \impl
% \avm{
%  	[ss|loc|cat & [subj   & \list*(canonical)\\
%  		       comps  & \list*(canonical) ] ] 
% }
% \z
% %
(\ref{ARP}) and (\mex{0}) together make sure that only the \textit{canonical} feature descriptions
on the \textsc{arg-st} can (and must) appear on the \textsc{subj} and \textsc{comps} lists \added{of heads
that are used in further combinations}.\footnote{
\added{\citet[Section~5.1.3]{GSag2000a-u}  and \citet[\page 50]{AG2007a-u} make use of the fact that
...
This, then, captures the idea that affixal arguments, \added{which are of type \type{non-canonical},} are generally inert in the combinatorics of the syntax proper: they saturate an argument slot and that argument slot is no longer available for other (at least direct) syntactic combination. 

So, returning to the \ili{Macushi} word \textit{ikoneka'p\^{\i}uya} `I made it.' from (\ref{mac a}),
\changed{the relevant partial lexical description is given} in (\ref{made it}):
% 
\ea
\label{made it}
...
no phonological realization). While this analysis has been explored for \ili{Germanic} languages
...
single rule. The rule, which I call the Head-Valent Schema, is given in (\ref{h-v
  ...
  117--118]{borsley95} and \ili{German} \citep[\page 295]{Pollard90a}. As has been a recurring theme throughout this chapter, many
