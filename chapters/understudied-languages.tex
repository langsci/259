...
\ili{German} (ISO: deu) \citep{Crysmann2003b,MuellerLehrbuch3}, 
\ili{Danish} (ISO: dan) \citep{MOeDanish}, 
\ili{Norwegian} (ISO: nor) \citep{HH2004a-u}, 
\ili{French} (ISO: fra) \citep{AG2000a,AG2002b-u,abegod04,ABGT2006a-u}, 
\ili{Spanish} (ISO: spa) \citep{Marimon2013a-u}, 
\ili{Portuguese} (ISO: por) \citep{BC2008a-u}, 
\ili{Mandarin Chinese} (ISO: cmn) \citep{ML2013a,YF2014a-u}, 
\ili{Japanese} (ISO: jpn) \citep{SBB2016a}, and 
...
\begin{samepage}
\begin{exe}
\ex \label{macushi} \ili{Macushi} [mbc] (\citealt{abbott91} via \citealt{siewierska99} and \citealt{corbett03}) \nolistbreak
\begin{xlist}
\ex[]{\label{mac a}
...
\ex \label{lex rule}
\avm{
	[arg-st & \tag{A} <[\type{canonical}] ,	[ \type*{non-canonical}
											ss|loc|cont|ind & \1] >]
	$\mapsto$ \\								 
	[ss|loc|cat|comps & list \+	<[\type*{canonical}
								ss|loc|cont|ind & \1 ]> \\
	arg-st & \tag{A} ]
}
\end{exe}
...
\avm{
	[ss|loc|cont|key &	[\type*{act-und-rel}
						arg1 & \1 \\
						arg2 & \2 ] \\
