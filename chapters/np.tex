...
sm edges
[%
\avmtmp{[head \1 \type{noun}, spr < >, comps < >]}
		[\ibox{2} Det [that]]
		[%
		\avmtmp{[head \1, spr < \2 >, comps < >]} 
			[%
			\avmtmp{[head \1, spr < \2 >, comps < \3 > ]}
				[sister]
			]
...
\begin{exe} 
\ex   \type{substantive}: \avmtmp{[mod & synsem $\vee$ none]}
\end{exe} 
...
\avmtmp{[head \1 \type{noun}, spr < >]}
	...
	\avmtmp{[head \1, spr < \2 >]}
		...
		\avmtmp{[head \4]}
			[%
			\avmtmp{[head \type{adv}]} [very]]
			[%
			\avmtmp{[head \4 [\type{adj} mod \3]]}
				[tall]
			]
		]
		[%
		\avmtmp{\3 [head \1, spr < \2 >]}, before drawing tree={x+=2em} [sister of Leslie, roof, before drawing tree={x+=2em}]]]]
\end{forest}
...
\begin{exe} 
\ex\label{local}  \type{local}: \avmtmp{[category & category \\
...
\begin{exe} 
\ex   \type{determiner}: \avmtmp{[spec & semantic-object]}
\end{exe} 
...
\avmtmp{[head \1 \type{noun}, spr < >]}
	[%
	\avmtmp{\2 [head \3 \type{det}, spr < >]}
		[%
		\avmtmp{\4 [head \type{noun}, spr < >]}
...
sm edges
[%
\avmtmp{[head \1 \type{det}, comps < >]}
	[%
	\avmtmp{[head \1, comps < \4 NP >]}
		[that]
	]
	[%
	\avmtmp{\4 [head \2 \type{noun}, comps < >]}
		[%
		\avmtmp{[head \2, comps < \3 >]}
			[sister]
		]
		[\ibox{3} PP [of Leslie, roof]]]]
\end{forest}
...
sm edges
[%
\avmtmp{[major \1, minor \2]}
	[%
	\avmtmp{[major \1, minor \2 fcompl $+$]}
		[that]
	]
	[%
	\avmtmp{[major \1 [$-$V, $+$N[\type{sing}]], minor|fcompl $-$]}
...
be plausible for such determiners as the \ili{English} \emph{enough} and its 
...
sm edges
[%
\avmtmp{[head \1 \type{noun}, mark \2 \type{marked}]}
	[%
	\avmtmp{[head|sel \4, mark \2]}
		[that]
	]
	[%
	\avmtmp{\4 [head \1, mark \5 \type{unmarked}]}
		[%
		\avmtmp{[head|sel \3, mark \5]}
			[long]
		]
		[%
		\avmtmp{\3 [head \1, mark \5]}
			[bridge]
		]
	]
]
...
heterogeneity of the determiners. The \ili{English} demonstrative \emph{that}, for instance, 
...
sm edges
[%
\avmtmp{[head \1 \type{noun}, mark \2]}
	[%
	\avmtmp{[head \4 \type{noun} [sel \3], mark \2]}
		[%
		\avmtmp{[head|sel \5, mark \2 \type{marked}]}
			[a]
		]
		[%
		\avmtmp{\5 [head \4, mark \type{unmarked}]}
			[hundred]
		]
	]
	[%
	\avmtmp{\3 [head \1, mark \type{unmarked}]}
		[pages]
	]
]
\end{forest}}
...
let us take another look at the attributive adjectives of \ili{Dutch}. 
...
the \ili{English} possessive as a genitive, differently from \citet[\page 199]{SagWasow03}; see
...
sm edges
[%
\avmtmp{[head \1 \type{noun}, mark \2 \type{a}]}
	[%
	\avmtmp{[head|sel \3, mark \2]}
		[a]
	]
	[%
	\avmtmp{\3 [head \1, mark \type{unmarked}]}
...
sm edges
[%
\avmtmp{[head \3 \type{adj}, mark \1 \type{marked}]}
	[%
	\avmtmp{[head|sel \2, mark \1]}
		[so]
	]
	[%
	\avmtmp{\2 [head \3, mark \type{unmarked}]}
		[good]
	]
]
\end{forest}
...
\avmtmp{
 [dtrs &	<[\type*{head-functor-phrase} \\
			synsem|loc|category &	[head & adjective \\
                                 	marking & marked] ], X > \\
head-dtr &	[\type*{regular-nominal-phrase}
...
\avmtmp{[head \1 \type{noun}, mark \2 \type{marked}]}
	[%
	\avmtmp{[head|sel \4, mark \2]} [beaucoup;much]]
	[%
	\avmtmp{\4 [head \1, mark \5 \type{de}]}
		[%
		\avmtmp{[head|sel \3, mark \5]} [de;of]]
		[%
		\avmtmp{\3 [head \1, mark \type{bare}]}
			[farine;flour]
		]
	]
]
...
as in \emph{an employee of a \ili{Japanese} company}, where the 
