...
see also \crossrefchapteralp[Section~\ref{sec-sbcg}]{cxg}). 
...
as unification, inheritance and token-identity (see \crossrefchaptert{formal-background} on formal
foundations and \crossrefchaptert{properties} on basic properties of HPSG).\footnote{This chapter does not treat relative clauses,
...
See \citealp[\page 34]{ps2} and \crossrefchaptert{properties}.
...
definition of type \type{head-adjunct-phrase} \crossrefchapterp{properties}. 
...
\citegen{Kasper97a} work on scopal adjuncts and the discussion of it in \crossrefchaptert[Section~\ref{semantics-sec-adjunct-scope}]{semantics}.}   
...
\begin{forest}
sm edges
[{[\textsc{major} \avmbox{1} [--V, +N[\type{sing}]], \textsc{minor$|$fcompl} +]}
		[{[\textsc{major} \avmbox{1}, \textsc{minor$|$fcompl} +]} [that]]
...
spelled out in (\ref{min}) \citep[312]{Netter94}.\inlinetodostefan{The MINOR features should be shared
  otherwise it is unclear why it is both + at the determiner and at the top node. \citet[\page 136]{Netter96a}
  shared the HEAD features along the head path. MINOR features are shared between det and NP via
  selection. Please change as adviced in earlier mail.}
...
prepositions must, hence, be positively specified for \textsc{fcompl}.\inlinetodostefan{this is not a problem} 
...
An illustration of the functor treatment is given in Figure~\ref{markyy}.\inlinetodostefan{add a more
  telling title. What is the functor? Check my addition to caption}
\begin{figure}
\centering
\oneline{%
\begin{forest}
...
\inlinetodostefan{add a more telling caption. Why is functors in plural? Check my modification of the caption}
...
[\type{phrase}
	[\textsc{headedness}
		[\type{headed-phrase}
			[\type{head-subject-phrase}, name=A]
			[...]]
		[\type{non-headed-phr}]]
	[\textsc{clausality}
		[\type{clause}
			[\type{declarative-clause} [\type{decl-head-subj-cl}, name=B]]
			[...]]
		[\type{non-clause}]]]
...
Its defining properties are spelled out in (\ref{acccx}) \citep[16]{Malouf00}.\inlinetodostefan{head-dtr
  structure sharing inherited? hence redundant?}
...
the English possessive as a genitive, differently from \citet{SagWasow03}.}\inlinetodostefan{head-dtr
  structure sharing inherited? hence redundant?}  
...
\inlinetodostefan{check caption}
\begin{figure}
\centering
\begin{forest}
sm edges
...
\type{intersective-modification}, as defined in (\ref{mononom}).\inlinetodostefan{since intersective-mod
  is a subtype of nominal-parameter, you do not have to mention head-dtr at all}  
...
They are spelled out in (\ref{bigmess2}).\inlinetodostefan{If this is the definition, the structure
  sharing is redundant since it is inherited from the supertype. You should at least add a footnote
  saying that this structure sharing does not have to be given but is here for explanatory purposes
  only.
}
...
whose \textsc{marking} value is of type \type{a}, and the nonhead daughter 
...
shares with the NP as a whole, as spelled out in Figure~\ref{beau}.\inlinetodostefan{add glosses to de in figure} 
...
[{[\textsc{head} \avmbox{1} \type{noun}, \textsc{mark} \avmbox{2} \type{marked}]}
	...
	[{\avmbox{4} [\textsc{head} \avmbox{1}, \textsc{mark} \avmbox{5} \type{de}]}
		[{[\textsc{head$|$sel} \avmbox{3}, \textsc{mark} \avmbox{5}]} [de;gloss]]
...
the surface-oriented monostratal character of HPSG than the DP treatment.\inlinetodostefan{Add:
  While the functor-based analysis has simpler syntactic structures for determinerless structures, additional assumptions have to
  be made regarding the semantics of such structures (not discussed in this chapter).}
