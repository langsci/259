...
The introduction puts the discussion in the broader context of the NP vs.\ DP debate and 
differentiates three HPSG treatments: the specifier treatment, the DP treatment and the functor treatment. 
...
It is first proposed in \citet[\added{\page 106--107, Section~5.7}]{ps} and further developed in \citew[\added{Section~9.4}]{ps2}, 
\citew[\added{189--195}]{GS00}, and \citew{MyPM2021a}. \rep{We}{I} henceforth call it the \emph{specifier treatment}, 
...
and \citet{NerbonneMullen00}. \rep{We}{I} will call it the \emph{DP treatment}. 
...
(\citealp[\added{Section~8.4}]{Sag2012}\added{; on SBCG in general see also} \crossrefchapteralp[Section~\ref{sec-sbcg}]{cxg}). 
\rep{We}{I} will call it the \emph{functor treatment}. This chapter presents the three treatments and 
...
as \del{unification,}\itdobl{unification is not used any longer.} inheritance and token-identity; see \crossrefchaptert{formal-background} 
...
The presentation is mainly based on \citet[\added{Section~9.4}]{ps2} and \citet[\added{189--195}]{GS00}. 
...
is subsumed by the \type{head-complements-phrase} type \crossrefchapterp[Section~\ref{properties:sec-hierarchy-of-phrase-types}]{properties}, 
...
the head of \emph{that sister of Leslie}, the \isi{Head Feature Principle}\footnote{%
...
To model this, \citet[55--57]{ps2} employ\del{s} the feature \textsc{mod(ified)}. 
...
definition of the type \type{head-adjunct-phrase} \crossrefchapterp[Section~\ref{properties:sec-adjuncts}]{properties}. 
...
\citealt*[199]{SagWasow03}) \added{as it is for instance in German (cf.\,\citealt[\page 6--7]{MyPM2021a})}.}
...
In this analysis there is no need for the valence feature \textsc{spr} \added{in the NP}.  
...
{[\textsc{major} \ibox{1}, \textsc{minor} \ibox{2}]}
	...
	[%
	{[\textsc{major} \ibox{1} [$-$V, $+$N[\type{sing}]], \textsc{minor|fcompl} $-$]}
		[sister of Leslie, roof]
	]
]
\end{forest}}
\itdopt{Why is \type{sing} in extra brackets? Remove? Replace by \type{sg}? Suggestion:}
\centerfit{%
...
\itdblue{The genetive NP usually does not select an NP. So one needs a pumping rule and this rule
...
are non-head daughters which lexically select their head sister. \added{For a general comparision of
...
means that the \textsc{spr} feature is dropped.\footnote{Intriguingly, Noam
...
exist and the problems are unformulable.'' \citet[43]{Chomsky13}.\itdopt{This is a very strange
  quote since Chomsky still has them. It is all the stuff that is ``later merged''. By renaming
  things problems do not disappear. I would drop this quote, since it is also not relevant for HPSG.}}  
...
{[\head \ibox{1} \type{noun}, \textsc{mark} \ibox{2}]}
	[%
	{[\head \ibox{4} \changed{[\type{noun}, \textsc{sel} \ibox{3}]}, \textsc{mark} \ibox{2}]}
		[%
...
since it is itself a head-functor-phrase,\itdopt{head-functor phrase or \type{head-functor-phrase},
  that is without a dash or in italics as a type} its \textsc{head} value, which includes 
...
To model this, \citet[\added{\page 167}]{VanEynde06} differentiates between two types 
...
in a monostratal surface-oriented framework than the DP treatment; see also \citew{MuellerHeadless},
\citew{MyPM2021a} and \citew{VanEynde2020}. Of the two NP treatments,
...
\begin{exe}
\ex \added{Lexical rule for gerunds according to \citep[66]{Malouf00}:}\\
\avm{
[head &	[\type*{verb}
		vform & prp] \\
 subj  & < \1 np> \\
...
type hierarchy
  [phrase
    [headedness,partition
      [headed-phrase
        [head-nonargument-phrase
          [head-functor-phr,l sep+=.5\baselineskip
    ...
    [clausality,partition
      [non-clause
        [nominal-parameter
          [intersective-modification,l sep+=.5\baselineskip
...
in \citet[\added{\page 363}]{GS00}. One of the non-clausal phrase types is \type{nominal-parameter}: 
...
\avm{
[\punk{synsem|loc|content|index}{ \1} \\
 dtrs & <[synsem|loc|content|index & \1 ], X > ]
}
\z
\itdopt{This means that the non-head daughter has to be the first daughter. I understood the DTRS
...
\eas
\label{bigmess2} 
\type{big-mess-phr\added{ase}} \impl \\
\avm{
...
the value of \textsc{spec} is of type \type{synsem}, as in \citet[\added{\page 45}]{ps2}, and not of type 
\type{semantic-object}, as in \citet[\added{\page 362}]{GS00}.}  
...
\gll   De sortir un peu te ferait du bien.  \\
       to go.out a bit you would.do {} good \\\hfill\added{(French)}
\trans `Going out a bit would do you some good.'
\itdopt{What is \emph{du}?}
\end{exe}  
...
\gll   Combien as-tu lu [~\trace{} de livres en latin]?  \\
       how.many have-you read {} of books in Latin  \\\hfill\added{(French)} 
\trans `How many books have you read in Latin?' 
\ex\label{gelu}
\gll   Wat zijn dat [~\trace{} voor vreemde geluiden]? \\
       what are that {} for strange noises        \\\hfill\added{(Dutch)} 
