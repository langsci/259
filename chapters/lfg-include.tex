...
\label{intro-sec}
Head-Driven Phrase Structure Grammar is similar in many respects to its cousin framework, Lexical
...
% with an emphasis on contrasts between the two that arise due to differing design principles.  
 We start with a look at the design considerations guiding the development of the two theories (Section~\ref{design-sec}), followed by point by point comparisons of syntactic issues organized by grammatical topic.  Then we turn to the semantic system of LFG,\itdopt{refer to Section 12 here, delete ref to 12.1 below?} beginning with a brief history by way of explaining its motivations (Section~\ref{sec:background}), and continuing with a presentation of the semantic theory itself.  A comparison with HPSG semantics is impractical and will not be attempted here, but see  \crossrefchaptert{semantics} for an overview of HPSG semantics.  
...
\itdopt{The \updown is not aligned with the V and other nodes. It seems to be to far left.}
\zl
...
sm edges without translation
[S$_{f_1}$ 
  [\csn{(\ensuremath{f_1} \LFGfeat{subj}) $=$ \ensuremath{f_2}}{NP$_{f_2}$}
...
\lfgfst{f_1} [ subj & [ pred & \predvall{lion} \\ 
                 ...
                 pers & \vall{3} \\ 
                 prox & $+$ ] \\
        pred  & \predvall{roar $\langle$ (f$_1$\, \LFGfeat{subj}) $\rangle$} \\
        tense & \vall{pres} ]}
\z
...
\lfgfst{f}[ subj & [ pred & \predvall{{pro}} \\ 
                     ...
                     pers & \vall{3} \\ 
                     gend & \vall{fem} ] \\
            pred & \predvall{mad$\langle$ (f\, \LFGfeat{subj}) $\rangle$} \\
            fin  & $+$ ]
}
\itdopt{\emph{f} instead of F?}
\z
...
\ex {\makebox[4em][l]{-{\it s}\/: {\it infl}}\qquad\feqs{
 ...
 \zl
The variant of (\ref{case}a) with \textit{her} as subject is ruled out due to a clash of \LFGfeat{case}  within the \subj{}  f-structure.  The variant with \textit{you} as subject\itdopt{\emph{you} is not among the examples above. Maybe ok.} is ruled out due to a clash of \LFGfeat{pers} features.  This mechanism is essentially the same as in HPSG, where it operates via the valence features.  
...
admire.{\sc 1sg}  {\sc refl}  Miki \\\hfill \added{(\ili{Serbo-Croatian})}
...
\lfgfst{f}[ top  & \rnode{top}{[ ``\textup{Ann}'' ]} \\
            subj & [ ``I'' ] \\
pred &  \predvall{think$\leftangle$ (f\, \LFGfeat{subj}) (f\, \LFGfeat{comp}) $\rightangle$}\\
comp & \lfgfst{g}[  subj & [ ``\vall{he}'' ] \\
		    pred & \predvall{like$\leftangle$ (g\, \LFGfeat{subj})(g\, \LFGfeat{obj}) $\rightangle$}\\
		    obj  & \rnode{obj}{} ] ]
}
\itdobl{G and F should be small, shouldn't they? Maybe define a command for g and f.}
\z
...
{\makebox[8em][l]{{\it wita-jarra-rlu}\/: N}\qquad\feqs{
    (\up \LFGfeat{adj}\ \LFGfeat{pred}) = `small' \\
    (\up \LFGfeat{num}) = \LFGfeat{dual} \\
    (\up \LFGfeat{case}) = \LFGfeat{erg} }}
...
\label{fs-w} 
\avm{
\lfgfst{f}[ pred & \predvall{chase $\leftangle$ (f\, \LFGfeat{subj}) (f\, \LFGfeat{obj}) $\rightangle$}\\
...
Comparing HPSG accounts of nonconfigurationality is instructive.   Two HPSG approaches are described in \crossrefchaptert{order}: the \emph{domain order}\itdopt{Shouldn't this be the \emph{order domain} approach?} approach \citep{DS99a} and the \emph{non-cancellation} approach \citep{Bender2008a}. 
...
\ex 
\word{hope}:  \qquad  $[$ \textsc{arg-st} $\langle$ \ibox{1}\,NP, VP[\textit{inf}, \textsc{subj} $\langle$ \ibox{1} $ \rangle ] \rangle ]$
\end{xlist}
\end{exe}
...
\lfgfst{f}[ subj  & \rnode{top}{[ ``\textup{Pam}'' ]} \\
            pred  & \predvall{hope$\leftangle$ (f\, \LFGfeat{subj}) (f\, \LFGfeat{xcomp}) $\rightangle$}\\
            xcomp & \lfgfst{g}[ pred & \predvall{visit$\leftangle$ (g\, \LFGfeat{subj})(g\, \LFGfeat{obj}) $\rightangle$}\\
				subj & \rnode{obj}{} \\
				obj  & [ ``\textup{Fred}'' ] ] ]
}
...
\itdopt{Remove (\mex{1}c) since it is in (\mex{2})?}
\end{xlist}
\end{exe}
...
\end{description}
\itdopt{Lingo is dead. Removed the link.}
...
\itdopt{Do we need the space before `:'? Shouldn't there be a : at the end of the heading? Consistency with the rest of the volume.}  
    \begin{prooftree}
...
(unlike the logic of Categorial Grammar\added{, see also \crossrefchapteralt{cg} on Categorial Grammar and \crossrefchapteralt[\page \pageref{order-hcs-binary}]{order} on HPSG approaches allowing saturation of elements from the valence lists in arbitrary order}). We could therefore freely
...
\begin{exe}
  \ex\label{ex:sws}
  \begin{minipage}[t]{.99\linewidth}
    Proof 1 (subject wide scope):\medskip\\
    \nopagebreak
\oneline{
...
\ex\label{ex:ows}
\begin{minipage}[t]{.99\linewidth}
    Proof 2 (object wide scope):\medskip\\
    \nopagebreak
    \oneline{
\begin{prooftree}[separation=0.5em]\footnotesize
\hypo{\parbox[b]{\widthof{$\lambda Q.\func{some}(\func{person},Q) :$}}{\raggedright $\lambda Q.\func{some}(\func{person},Q) :$\\$\forall S.(s \multimap S) \multimap S$}}
