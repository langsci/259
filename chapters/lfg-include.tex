...
The rest of this section is organized as follows. In \sectionw~\ref{sec:background} we present some more historical background on semantics for LFG and HPSG.  In \sectionw~\ref{sec:glue-semantics}, we present Glue Semantics (\glue), as a general compositional system in its own right. Then, in \sectionw~\ref{sec:glue-lfg} we look at the syntax--semantics interface with specific reference to an LFG syntax. For further details on semantic composition and the syntax--semantics interface in constraint-based theories of syntax, see \citet{koenig;richter-tv} for semantics for HPSG and \citet{asudeh-lfg-glue} for semantics for LFG.
...
\ex \label{ex:semantics-3}
  \attop{
    ...
    \[
    \[
    \formula{\lambda y.\lambda x.f(y)(x):a \linimp b \linimp c} \pspace[1.5em]
    \formula{[v:a]}^1
    \justifies
    \formula{\lambda x.f(v)(x):b \linimp c} 
    \using \linimpE
    \]    
    \formula{[u:b]}^2
    \justifies
    ...
    \]
    \justifies
    \formula{\lambda v.f(v)(u):a \linimp c}    
    \using \linimpIi{1}
  \]
  \justifies
 \formula{\lambda y.f(y)(u):a \linimp c}    
 \using \Rightarrow_\alpha
 \]
    \justifies
    \formula{\lambda u.\lambda y.f(y)(u):b \linimp a \linimp c}    
    \using \linimpIi{2}
  \]
  \justifies
  \formula{\lambda x.\lambda y.f(y)(x):b \linimp a \linimp c}
  \using \Rightarrow_\alpha 
  \end{prooftree}
}
\end{exe}
...
\sectionw~\ref{sec:glue-lfg}, we will review \lfgglue, as this
is the predominant pairing of syntactic framework and \glue.
% Then, in
% section \sectionw~\ref{sec:glue-hpsg} we briefly review \hpsgglue, following
...
\ex
\begin{avm}
  \fst{c}[pred & \predvall{call}\\
   subj & \fst{b}[pred & \predvall{Blake}]\\
   obj & \fst{a}[pred & \predvall{Alex}]
  ]
\end{avm}
\end{exe}
%
...
\begin{prooftree}
    \[
      \[
        \[
          \formula{\lambda y.\lambda x.\func{call}(y)(x):a \linimp\ b
            \linimp\ c} \hspace*{1em} \formula{\func{alex}:a}
          \justifies \formula{(\lambda y.\lambda
            x.\func{call}(y)(x))(\func{alex}):b \linimp\ c} \using \linimpE
        \]
        \justifies \formula{\lambda x.\func{call}(\func{alex})(x):b \linimp\
          c} \using \betared
      \]
      \formula{\func{blake}:b} \justifies \formula{(\lambda
        x.\func{call}(\func{alex})(x))(\func{blake}):c} \using \linimpE
    \]
    \justifies \formula{\func{call}(\func{alex})(\func{blake}):c} \using \betared
  \end{prooftree}
  }
\end{exe}
%
...
\begin{avm}
  \fst{c}[pred & \predvall{call}\\
   subj & \fst{b}[pred & \predvall{Blake}]\\
   obj & \fst{e}[pred & \predvall{everybody}]
  ]
\end{avm}
\end{exe}
%
...
\hspace*{-.5em}
  ...
  \begin{prooftree}
  \[
    \mformula{\lambda Q.\func{every}(\func{person},Q)}%
    {\forall S.(e \linimp\ S) \linimp\ S}
    \justifies
    \mformula{\lambda Q.\func{every}(\func{person},Q)}%
    {(e \linimp\ c) \linimp\ c} 
  \using \forallE{[c/S]}
...
\hspace*{-0em}
  ...
  \[
  \[
    \mformula{\lambda y.\lambda x.\func{call}(y)(x)}%
    {e \linimp\ b \linimp\ c}
  \hspace*{1em} 
  \formula{[z:e]}^1
  \justifies
  \formula{\lambda x.\func{call}(z)(x): b \linimp\ c}
  \using \linimpE, \betared
  \]
  \formula{\func{blake}:b}
  \justifies
  \formula{\func{call}(z)(\func{blake}):c}
  \using \linimpE, \betared 
  \]
  \justifies
  \formula{\lambda z.\func{call}(z)(\func{blake}):e \linimp\ c}
  \using \linimpIi{1}
  \]
  \justifies
  \formula{\func{every}(\func{person}, \lambda z.\func{call}(z)(\func{blake})):c}
  \using \linimpE, \betared
\end{prooftree}
}
\end{exe}
...
\begin{avm}
  \fst{c}[pred & \predvall{call}\\
   subj & \fst{e}[pred & \predvall{everybody}]\\
   obj & \fst{s}[pred & \predvall{somebody}]
  ]
\end{avm}
\end{exe}
%
...
%
\begin{exe}
\ex \high{Proof 1 (subject wide scope)}\nopagebreak\\
\hspace*{-3em}
\scalebox{0.8}{ 
  \begin{prooftree}
    \[
...
\[
\[
  \mformula{\lambda Q.\func{some}(\func{person},Q)}%
  {\forall S.(s \linimp\ S) \linimp\ S}
  \[
    \[
      \[
\mformula{\lambda y.\lambda x.\func{call}(y)(x)}{s \linimp\ e \linimp\
  c}
\hspace*{1em}
\formula{[v:s]}^1
\justifies
  \formula{\lambda x.\func{call}(v)(x):e \linimp\ c}
\using \linimpE, \betared
\]
\formula{[u:e]}^2
\justifies
\formula{\func{call}(v)(u):c}
\using \linimpE, \betared
\]
\justifies
\formula{\lambda v.\func{call}(v)(u):s \linimp c}
\using \linimpIi{1}
\]
\justifies
  \formula{\func{some}(\func{person},\lambda v.\func{call}(v)(u)):c}
\using \linimpE, \forallE{[c/S]},  \betared
\]
\justifies
\formula{\lambda u.\func{some}(\func{person},\lambda v.\func{call}(v)(u)):e \linimp\ c}
\using \linimpIi{2}
\]
\justifies
\formula{\func{every}(\func{person},\lambda u.\func{some}(\func{person},\lambda v.\func{call}(v)(u)))}
\using \linimpE, \forallE{[c/S]}, \betared
\]
\justifies
\formula{\func{every}(\func{person},\lambda
...
\item \high{Proof 2 (object wide scope)}\nopagebreak\\
\hspace*{-3em}
\scalebox{.9}{
  \begin{prooftree}
    \[
...
\]
\justifies
  \formula{\func{every}(\func{person},\lambda x.\func{call}(v)(x)):c}
\using \linimpE, \forallE{[c/S]},  \betared
\]
\justifies
\formula{\lambda v.\func{every}(\func{person},\lambda
  x.\func{call}(v)(x)):s \linimp\ c}
\using \linimpIi{1}
\]
\justifies
\formula{\func{some}(\func{person},\lambda v.\func{every}(\func{person},\lambda x.\func{call}(v)(x)))}
\using \linimpE, \forallE{[c/S]}, \betared
\]
\justifies
