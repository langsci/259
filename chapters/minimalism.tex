...
formalizing' (see Huybregts and van Riemsdijk 1982: 73\todo{bibkey needed -- RF}), and this view
seems fairly standard within Minimalism. Chomsky and Lasnik (1995: 28)\todo{bibkey needed; 1995? --
...
For HPSG, as the chapters in this volume have illustrated, linguistic expressions have a single relatively simple constituent structure with a minimum of phonologically empty elements.\todo{Potal2010 insert bibkey --RF}\footnote{%
	The relatively simple structures of HPSG are not an automatic consequence of its declarative nature. Postal's Metagraph Grammar framework (formerly known Arc Pair Grammar) is a declarative framework with structures that are similar in complexity to those of Minimalism (see Postal 2010).%
}
...
\citet{Borsley2006a} (2017)\todo{bibkey needed -- RF} discusses Minimalist analyses of relative clauses and \textit{wh}-interrogatives and suggests that at least eight complementizers are necessary. One is optionally realized as \textit{that}, and another is obligatorily realized as \textit{for}. The other six are always phonologically empty. But it has been clear since \citet{Ross67} and Chomsky (1977)\todo{bibkey needed --RF} that relative clauses and \textit{wh}-interrogatives are not the only unbounded dependency constructions. Here are some others:
\eal
...
For a fuller discussion of the issues see \citet{Borsley2006a} (2017)%
}	
...
\zl
A variety of transformational work, including work in Minimalism, has argued for an analysis like (\ref{ex:min-will-kim}) for Welsh finite clauses (see e.g. Jones and Thomas 1977\todo{bibkey needed -- RF}, Sproat 1985\todo{bibkey needed -- RF}, Sadler 1988\todo{bibkey needed -- RF}, Rouveret 1994\todo{bibkey needed -- RF}, and Roberts 2005\todo{bibkey needed -- RF}). But Borsley (2006a)\todo{bibkey needed -- RF} argues that there is no theory-neutral evidence for a structure of this kind. Hence, at least for Welsh, it seems that a simpler flat structure like (\ref{ex:min-john-showed}) is preferable.\footnote{%
	Borsley (2016) argues for a similar flat structure for the Caucasian ergative SOV language Archi.%
}

We turn now to the idea that all structures are headed. For HPSG, and many other approaches, there are headed structures and non-headed structure. Probably the most important example of the latter are coordinate structures such as those in (\ref{ex:min-kim-lee}) (see Sag 2003\todo{bibkey needed -- RF} for an HPSG analysis).
...
\zl 
Such examples have led to HPSG analyses in which coordinate structures have whatever properties are common to the two conjuncts (Sag 2003)\todo{bibkey needed -- RF}. Within Minimalism, one might try to mimic such analyses by proposing that conjunctions have whatever properties are common to their specifier and complement. But a problem arises with an example like (\ref{ex:min-lee}), where the conjuncts are not words but phrases.
...
\zl
Willis (2011)\todo{bibkey needed -- RF} and \citet{Borsley2013a-u},2013)\todo{bibkey needed -- RF} present evidence that Welsh RPs involve the same mechanism as gaps. Within Minimalism, this means that they must involve Move/Internal Merge. But one expects to see a gap where Move/Internal Merge has applied. One Minimalist response suggests that instead of being deleted, the copy left behind by Move/Internal Merge is somehow turned into a pronoun (see McCloskey 2006\todo{bibkey needed -- RF}). Another approach exploits the complexity of Minimalist structures and proposes that there is a gap in the structure somewhere near the RP. See Willis (2011)\todo{bibkey needed -- RF}, \citet{AounChoueiriHornstein2001a-u}, and Boeckx (2003)\todo{bibkey needed -- RF}. For detailed objections to both approaches, see Borsley (2013: section 3)\todo{bibkey needed -- RF}. Within the SLASH-based approach of HPSG, there is no reason to think that there will always be a gap at the bottom of a dependency, and it is not difficult to accommodate RPs. See Borsley (2013)\todo{bibkey needed -- RF} and \citet{crysmann_b10fg},2016)\todo{bibkey needed -- RF} for slightly different approaches.\footnote{%
...
\end{quote}
In recent times some Minimalists have come to a similar view. Thus, Boeckx (2011)\todo{bibkey -- RF} suggests that `some of the most deeply-embedded tenets of the Principles-and-Parameters approach, and in particular the idea of Parameter, have outlived their usefulness.'  It is not really clear what  Chomsky's view of parameters is, but there appears to be no place for them in the version of Minimalism presented in Hauser, Chomsky and Fitch (2002)\todo{bibkey -- RF}, in which the narrow language faculty is just `recursion'. 
