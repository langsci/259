...
\itd{JP: Maybe I am missing something, but lots of the functional heads posited by say Cinque (1999) are dealt with inflectionally (hence through lexical types or lexical rules or relalizational rules) in languages that do things morphologically in HPSG. So, it’s not just phrasal types. But, as I said, maybe I missed the point.}
...
\citew[\page 333]{BG78a}, which is also cited in other mainstream generative work, such as
\citew{GR81}. (\mex{0}b) is from \citew[\page 1068]{HPP2002a-u}, a descriptive grammar of \ili{English}.
...
\citep{Pollock89a-u}.\itd{JP: crossref negation chapter, explicit discussion of Pollock there.}
%% \inlinetodostefan{
...
For actual HPSG analyses dealing with adverb order, see \citew{KM2005a,AG2004b-u}.
...
book-length discussion of \ili{German} clause structure. \crossrefchaptert[Section~4.1]{order} also discusses
...
proposals for NPN in the Minimalist framework: \citet{Travis2003a} and G.\ \citet{GMueller2011a}. G.\ Müller develops a \isi{reduplication} account. He states that reduplication
...
predecessors. While movement-based approaches seem to work well for \isi{SVO} languages like \ili{English}, they
...
\gll Pa	ddyn werthodd Ieuan y ceffyl iddo \emph{fo}?\\
     which man sell.\textsc{pst}.\textsc{3sg} Ieuan the horse to.\textsc{3sgm} he\\
\glt`Which man did Ieuan sell the horse to?'
\ex 
\gll y dyn werthodd Ieuan y ceffyl iddo \emph{fo}\\
the man sell.\textsc{pst}.\textsc{3sg} Ieuan the horse to.\textsc{3sgm} he\\
...
Merge.\footnote{%
  \citet{Rouveret2008a-u} sketches a Minimalist analysis of \ili{Welsh} RPs which does not involve movement. For
...
processes that are involved in linguistic performance.\footnote{
  See \citet{BK82a} for an early argument that an approach which can be readily incorporated into an account of linguistic performance is prefereable to one which cannot.
...
and hypotheses regarding further parts of the utterance are built.\footnote{
  ...
  derivation. Due to the empty elements, there are infinitely many possible numerations\itd{JP:
    really infinitely many? Like for type raising in semantics, where you could type raise ad infinitum, but you choose the simplest type raising that allows you to derive the meaning (I think Partee discusses that in either a 1984 or 1987 paper).} that might be
...
phase, if the derivation is broken up into phases. Moreover, the nature of \ili{English} means that the
