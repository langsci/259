...
\abstract{This paper compares work done under the heading \emph{Minimalist Program} with work done in
  ...
  discuss the differences in complexity of the structures that are assumed. A final section
  discusses psycholinguistic issues like processing and language acquisition.}
...
For further discussion of the issues, see Section~\ref{sec-minimalism-processing} of this paper and e.\,g.\ \citet{PS2001a}, \citet{Postal2003a},
\citeauthor{SW2011a} \citeyearpar{SW2011a,SW2015a}, and \crossrefchaptert{processing}. 
...
claims about the autonomy of syntax. There is a syntactic component and than there are the component
of Phonological Form and Logical Form, in more recent versions of the theory this is the
articulatory"=perceptual system and the conceptual"=intentional system. Syntax was always regarded primary and
PF and LF derived from syntactic representations. This is similar in Minimalism. The problem is that
questions of intonation are connected to semantic and information structural properties. A way
around this is to stipulate syntactic features that can be interpreted by both PF and LF. Another
...
to explain orders of adverbials that seem to hold universally (\eg \citealp[\page 106]{Cinque99a-u} and \citealp[\page 55]{CR2010a}). A functional projection selects for
another functional projection to establish this hierarchy of functional projections and the
respective adverbial phrases can be placed in the specifier of the corresponding functional
projection. \citet[\page 106]{Cinque99a-u} assumes 32 functional projections in the verbal domain.
...
semantics results. If nonlocal dependencies are involved and fillers are combined with gapped
constituents, the result is still a verbal projection. If one believed in Universal Grammar (which
researchers working in HPSG usually do not) and in innately specified constraints on adverb order
...
Having discussed nonlocal dependencies and Cartographic approaches to the left periphery and adverb
...
We look first at binary branching.\footnote{
  ...
  \citep{Partee87a-u}. For respective HPSG analysis see \citew[\page 91--92]{Flickinger2008a} and
  ...
  properties that would be otherwise assigned to the mother node of the unary projection. See for example \citep[\page 370]{Ramchand2005a}. For a
...
and languages like German with freer constituent order).
...
Perhaps recognizing the weaknesses of the ConjP analysis, \citet{Chomsky2013a} sketches a different approach to coordinate structures, in which the first conjunct is the head. This approach has a problem with a simple example like (\mex{1}).
...
\citep{Chomsky99a,Chomsky2008a} are incompatible with psycholinguistic results. Most of Minimalist work assumes that structures are build in a bottom-up fashion. Chomsky
assumes that there are so"=called Phases\is{Phase} and when a Phase is complete it is ``shipped'' to
the interfaces\is{interface} (pronounciation and interpretation). For the example in (\mex{1}) this
would mean that \emph{a liar} is constructed first to be combined with \emph{is}, which is then
combined with \emph{Peter} and \emph{Peter is a liar} is combined with \emph{that}. \emph{that Peter
  is a liar} is shipped to the interfaces and can then be integrated into more complex constituents
like \emph{that Sandy claimed that Peter is a liar}.
\ea
...
Minimalism call themselves ``\isi{Biolinguistics}''), but it is fundamentally against all knowledge
...
world knowledge are used as soon as they are available. Hence, parsing (\mex{0}) is an incremental
process: the hearer hears \emph{Kim} first and as soon as the first sounds of \emph{believe} reach
her the available information is integrated and hypothesis regarding further parts of the utterance
are build.
...
wrong. Of course one could claim that actual processing is performance and Minimalist theories are
dealing with competence but this would beg the question why the vocabulary of performance theories
is used in a competence theory. 

A solution to the problem of compatibility with psycholinguistic results would be to drop the
rhetoric about interfaces. This is possible for many of the works currently published in
Minimalism. One would then get theories with Internal and External Merge that would basically be
variants of Categorial Grammar \citep{BE95a,MuellerUnifying}. As \citet{MuellerUnifying} shows,
Internal Merge corresponds to HPSG's Filler-Head-Schema and External Merge to the Specifier-Head
Schema and the Head-Complement Schema. If statements about Phases and the order of derivations are
avoided, certain Minimalist theories are compatible with constraint-based views on grammar.
