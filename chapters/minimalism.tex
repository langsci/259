...
sociological differences in Section~\ref{sec-sociological-differences}. We go on with the degree of
...
\citealt{ACS2016a-u}; \citealt{SA2016a-u}
...
this process continues for a while an elaborated theoretical edifice results that is not
...
One crucial difference between HPSG and Minimalism is that HPSG allows for the underspecification of
...
dominance schema can have a value of a certain types that subsumes a number of several other
...
the resulting tree \citep{May85a-u,Frey93a,SE2002a}. This is unnecessary in HPSG.\footnote{%
...
nominal phase, for Minimalism a DP, inside the complement of T. Sometimes features like case are not
valued and it is said the probe values the feature at the goal. Both the feature at the probe and
the one at the goal deletes after a successful Agree operation.
Here, and elsewhere, Agree is a
...
As we noted in the last section, Merge combines two expressions to form a larger expression with the same label as one of the expressions. But which of the original expressions provides the label for the larger expression? This issue has been discussed but not satisfactorily.
%%
...
Chomsky defines which label is used in two different cases: the first case defines that the label\is{labeling} is the
...
since otherwise the derivation would ``crash at the interfaces''.\inlinetodostefan{Stefan: check: The
  uninterpretable feature is at the head, but the noun has to be valued. What if the NP has a case
  value from morphology already? It has to be eaten up, hasn't it?} This means that Minimalism claims
...
earlier Chomskyan frameworks pose major problems for acquisition. Furthermore the machinery that is
...
We thank Andreas Pankau and Tom Wasow for discussion and Sebastian Nordhoff and Felix Kopecky for help with Figure~\ref{fig-immunization-MP}.
