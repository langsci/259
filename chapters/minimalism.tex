...
\citep{Bender2000a,Mueller2004e,Borsley99c-u,Borsley2009a-u,Borsley2013a-u}\todostefan{one Borsley
  paper is enough. which one?} even though Ivan Sag argued
against them since 1994 \citep{SF94a} and that defaults\is{default} are not used by all researchers even though they play a
...
are violated:\todostefan{Add footnote on cumulative constraint violation.}
\eal
\ex[]{
...
c-commands\is{c-command}.\footnote{
  It is not necessary to state the c-command restriction since it sis assumed that linguistic objects are built
...
characteristic of Minimalist syntactic structures, as we will see in \hyperref[sec:min-views-structure]{the next section}. 
...
We already mentioned uninterpretable features in the previous subsection. It is assumed in
Minimalism that features like case features are not interpretable and hence have to be consumed
since otherwise the derivation would ``crash at the interfaces''. This means that Minimalism claims
that a case marked NP like \emph{der Mann} `the man' is not interpretable unless it is somehow
stripped of its case information.%% \footnote{
...
that consumes the case features. While this seems cumbersome to most working outside Minimalism,
there are actually deeper problems connected to the consumption of case features. There are
situations in which you need case features more than once. An example for this are free relative
...
will be checked by the governing verb \emph{treffen} `meet'. \emph{wen} will then be a DP without
...
information for whatever reasons. We think that this is the right way to deal with morphological
...
Further complexity arises from the syntactification of semantic
...
However, they have got a way of getting adverbs in the right order and I'm not really sure how we would do that.
}

If one believed in\todostefan{add ``a complex''} Universal Grammar (which researchers working in HPSG usually do not) and in innately specified constraints on adverb order
...
analysis in which the specifier projects its features. She depicts the analysis as in Figure~\ref{fig-coordination-johanesson}.
\begin{figure}
\begin{forest}
[{CoP[X]}
 [X]
...
look at the category of other specifiers rather than their heads? Johannessen's analysis seems to be
equivalent to an HPSG analysis assuming two different head features, which would be highly problematic for the
reasons just given. Furthermore, coordinations are not equivalent to the first conjunct. There are
...
dependencies in passives, unaccusatives, rasing sentences, and scrambling. The following illustrate
...
underlying object has to move to the subject position to Spec,TP where it gets nominative \citep[\page 124]{Chomsky81a}.
\eal
\ex[]{
...
\zl
The analysis assumed in recent Minimalist work differs in detail but is movement-based as its
...
involved in this valence-based analysis of the passive.\todostefan{Stefan: Actually the chapter about order does not deal with SVO vs. SOV and
  \spr vs. \comps. Maybe it should.} The problem of MGG analyses is that they mix two phenomena: passive and subject
...
to discuss scrambling in more detail. Languages like \ili{German} have a more liberal constituent
...
however, then there are two possible readings \citep[\page ]{Frey93a}:\todostefan{add page}
\eal
\ex 
...
\noindent
It turns out that approaches assuming traces run into problems as they predict certain readings for
sentences with multiple traces which do not exist (see \citealp[\page 146]{Kiss2001a} and
...
\subsubsection{Nonloacl dependencies}
...
See also \crossrefchaptert{udc} for a more detailed discussion of nonlocal dependencies.
...
It is sometimes claimed in personal communication\todostefan{references apart from personal
  communication} that current Minimalist theories are better suited to explain production
...
probe/goal relations is. There is feature checking, some features are uninterpretable others are
not, some are strong others are weak. Certain categories are \isi{Phase} boundaries others are not. There
are complex conditions on \isi{Labeling}.
...
formation \citep{MMGRRBW2003a}. Researchers in MGG state that certain ways of expressing things do never occur although
...
\emph{\isi{remnant movement}}, as Hubert Haider (p.\,c.\ 2018) pointed out. The element 5 can move
to the left of 4. The unit containing 4 and 5 can move to the left of 3 and [[5 [4 \trace]] [3
