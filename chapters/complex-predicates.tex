...
\abstract{Complex predicates are constructions in which the head attracts arguments from its
  predicate complement. They characterize auxiliaries, copulas and predicative verbs, certain
  control or raising and causative verbs, and light verbs. This phenomenon has been studied in HPSG in different languages, Romance and Germanic languages, Korean and Persian, which illustrate different aspects. Romance languages show that argument inheritance is distinct from structure: it is compatible with different structures. German and Dutch and Korean show that argument inheritance induces different word order properties, and Persian that it can be preserved by a derivation rule (nominalization from a verb), and, most importantly in that language which has relatively few simplex verbs, that light verb constructions are used to turn a noun into a verb. }
...
\eal 
	...
	\glt `Paul has quickly read it.'}\label{GSexemple1c} 
\zl
...
\ea[]{
	...
	\glt `(someone1) was causing (something) to be eaten.'}
\z
...
\eal
	...
	\gll Lola a   achet\'e des   pommes, et  Alice (a)             cueilli des   p\^eches.\\
	     Lola has bought   some  apples  and Alice \spacebr{}has   picked  some  peaches\\
...
complements, noted here \ibox{2}. On the other hand, the auxiliary is not only a subject raising
...
\eal 
	...
	\glt `Joan wants to eat them.'}\label{GSexemple12f} 
...
also be Non Constituent Coordinations (NCC) (\emph{John gives a book to Maria and discs to her
  brother}). So, the question becomes: why is (\ref{GSexemple23b}) not an acceptable NCC in Spanish?
\cite{AG2010} propose that NCC coordinations are subject to a general constraint in Romance
...
\begin{exe}
        \ex[]{\type{head-complements-phrase} (Romance languages) $\Rightarrow$ \\	
        \begin{avm}
		{\[synsem | loc | cat
		            \[head & \ibox{1} \\
		            comps & \ibox{3}\\
...
\begin{exe}
        \ex[]{\type{head-cluster-phrase}  (Spanish) $\Rightarrow$\\	
        \begin{avm}
		{\[synsem | loc | cat &
		            \[comps & \ibox{1}\\
		            light & $+$\\\]\\
...
The properties of the construction differentiate it clearly from tense auxiliaries and restructuring verbs. For the sake of simplification, we restrict the examples to French, Italian and Spanish. The sequence of the head of the complement with its complements is a constituent, since, for instance, it can be dislocated and pronominalized (\ref{GSexemple31}) (examples (\ref{GSexemple31})-(\ref{GSexemple32}) from \citealt[133-134]{AG2010}).
...
\eal
	...
	\glt `that Karl will read the book.'}\label{GSexemple37c}	
	...
	\glt `because Karl seems to read the book.'}\label{GSexemple37e}
		
	\ex[*]{ 
	\gll Das  ist das Buch das  lesen Karl wird.\\
		 this is  the book that read  Karl will\\
	\glt `This is the book that Karl will read.'}\label{GSexemple37f}
		
	\ex[*]{ 
	\gll Das  ist das Buch das  zu lesen Karl scheint.\\
		 this is  the book that to read  Karl seems\\
	\glt `This is the book that Karl seems to read.'}\label{GSexemple37g}
\zl
...
\begin{exe}
    \ex[] {\type{head-complement-phrase} (German) $\Rightarrow$ \\
    \begin{avm}
...
Following constraint (\ref{GSexemple40}), the head combines with one complement at a time, noted
...
with a complement is [LIGHT -].\footnote{The feature LIGHT is the equivalent of LEX used in German
  ...
  differ from its trace as regards lightness \citep{muller2018clause} (see \crossrefchaptert{udc} for extraction).} The structure of (\ref{GSexemple41}) is exemplified in Figure~\ref{GSfigure10} \citep[22]{muller2018clause}.
...
\begin{exe}
    \ex[] {\type{head-cluster-phrase} (German) $\Rightarrow$ \\
    \begin{avm}
    	...
    	[[\ibox{1} NP [er;he]]
     		[[\ibox{2} NP [das Auto;the car, roof]]
     			...
     			[V \ms{head \ibox{4}, comps \liste{ \ibox{1},\ibox{3},\ibox{2} }}
                		...
                		[V \ms{head \ibox{5}\\
                vform {\normalfont{\emph{bse}}}\\
                comps \liste{\ibox{1},\ibox{3},\ibox{2} }  $\oplus$ \ibox{6}} [sehen;see]]]
     [V \ms{head \ibox{4}\\
                    vform {\normalfont{\emph{fin}}}\\
                    comps \liste{ \ibox{1},\ibox{3},\ibox{2} } $\oplus$ \liste{ \ibox{7} }}, before computing xy={s'-=35pt} [wird;will]]]]]]]
 \end{forest}}    
...
\cite{Mueller2002b} also shows that the copula does not take a saturated AP complement. Contrary to a construction with an incoherent verb, this AP cannot be extraposed (\ref{GSexemple47b}), or pied piped with a relative pronoun (\ref{GSexemple47d}) (from \citealt[70]{Mueller2002b}) (compare with (\ref{GSexemple36c}), (\ref{GSexemple36d})).   
...
\eal
	...
	\glt `Karl was proud of his son.'}\label{GSexemple47b}
	...
	\glt `the son, of whom Karl was proud.'}\label{GSexemple47d}
\zl
...
\eal
	...
	\gll Mia-ka             wul-e               pely-ess-ta.\\ 
		{Mia-\textsc{nom}} {cry-\textsc{part}} {end.up-\textsc{pst}-\textsc{decl}}\\
...
\eal
	...
	\gll Mary-ka            ku             chayk-ul            ilk-ko  	            iss-ta.\\ 
		{Mary-\textsc{nom}} \spacebr{}the  {book-\textsc{acc}} {read-\textsc{part}} {be.in.the.process.of-\textsc{decl}}\\
...
A priori, these data could be explained in two ways: either the auxiliary always takes a VP complement, and scrambling is due to linearization, in which case the domains of the two verbs are unioned \citep{Reape94a}; or there is a complex predicate: the complement of the embedded verb (\emph{the book}) is attracted by the auxiliary verb. 
...
\eal
	...
	\gll Mimi-ka			 sakwa-lul/ka		      mek-ko		      siph-ess-ta.\\ 
		 {Mimi-\textsc{nom}} {apple-\textsc{acc/nom}} {eat-\textsc{part}} {wish-\textsc{pst-decl}}\\
...
\eal
	...
	\gll Mimi-nun			 amwukes-to	     mek-ci		         anh-ass-ta.\\ 
		 {Mimi-\textsc{top}} {anything-also} {eat-\textsc{part}} {not-\textsc{past-decl}}\\
	\glt `Mimi did not eat anything.'}\label{GSexemple53a}
		
	\ex[]{
	\gll Mimi-nun			 amwukes-to	     mek-ko		         siph-ci		      anh-ass-ta.\\ 
		 {Mimi-\textsc{top}} {anything-also} {eat-\textsc{part}} {wish-\textsc{part}} {not-\textsc{past-decl}}\\
	\glt `Mimi did not feel like eating anything.'}\label{GSexemple53b}
	
	\ex[*]{
	\gll Mimi-lul			 amwukes-to	     mek-tolok		     seltukha-ci		      anh-ass-ta.\\ 
		 {Mimi-\textsc{acc}} {anything-also} {eat-\textsc{part}} {persuade-\textsc{part}} {not-\textsc{past-decl}}\\
...
\eal
	...
	\gll Mary-ka    	     John-hanthey        [ku           chayk-ul            ilk-ulako]       seltukha-yess-ta.\\ 
		 {Mary-\textsc{nom}} {John-\textsc{dat}} \spacebr{}the {book-\textsc{acc}} {read-\textsc{part}} {persuade-\textsc{past}-\textsc{decl}}\\
	...
	\gll Ku  chayk-ul             Mary-ka    	     John-hanthey         ilk-ulako            seltukha-yess-ta.\\ 
		 the {book-\textsc{acc}} {Mary-\textsc{nom}} {John-\textsc{dat}}  {read-\textsc{part}} {persuade-\textsc{past-decl}}\\
...
\eal
	...
	\gll Mimi-lul            amwukes-to    an mek-tolok           selkhuta-yess-ta.\\ 
		 {Mimi-\textsc{acc}} anything-also no {eat-\textsc{part}} {persuade-\textsc{past-decl}}\\
...
\eal
	...
	\gll Mary-ka    	     hayekan sakwa-lul            mek-ko 	          iss-ta.\\ 
		 {Mary-\textsc{nom}} anyway  {apple-\textsc{acc}} {eat-\textsc{part}} {be.in.the.process.of-\textsc{decl}}\\
	\glt `Anyway, Mary is eating an apple.'}\label{GSexemple56added-a}

	\ex[*]{
	\gll Mary-ka    	     sakwa-lul            mek-ko              hayekan iss-ta.\\ 
		 {Mary-\textsc{nom}} {apple-\textsc{acc}} {eat-\textsc{part}} anyway  {be.in.the.process.of-\textsc{decl}}\\
...
\ea[]{
	...
	\gll Mimi-nun	         Haha-lul 	         (cengmal)           ttena-tolok	       (cengmal)	    seltukha-yess-ta.\\ 
		 {Mimi-\textsc{top}} {Haha-\textsc{acc}} \spacebr{}really    {leave-\textsc{part}} \spacebr{}really {persuade-\textsc{pst-decl}}\\
	\glt `Mimi (really) persuaded Haha to (really) leave.'}
\z

In addition, there is evidence that the embedded verb and its complement do not form a constituent. While an NP may occur after the head verb in a construction called afterthought (\ref{GSexemple56a}), this is not possible for the embedded verb \emph{mekko} with its complement (\ref{GSexemple56b}) (from \citealt[162]{Chung98a-u}).
...
\begin{exe}
    \ex[] {\type{head-subject-complements-phrase} (Korean) $\Rightarrow$ \\
    \begin{avm}
      {\[synsem | cat & \[
                        subj /\, \< \>\\
                        comps /\, \< \>\\
      ...
      head-dtr | synsem | cat & \[
                            ...
                            comps \ibox{2} \\
                            light $+$\]\\
      non-head-dtrs & \normalfont{(\ibox{1} o \ibox{2}) nonempty list} \]}
    \end{avm}}\label{GSexemple60added}
\end{exe}

The verbal complex is headed by an auxiliary verb, which is [AUX $+$] while other verbs are [AUX $–$].  Thus only auxiliaries can enter this structure, and they must, since the head of the head-subject-complements phrase is specified as [AUX $–$]. The schema for the verbal complex is given in (\ref{GSexemple57}). Auxiliaries attract both the subject (\ibox{1}) and the complements of their verbal complement (list \ibox{3}). The subject value is indicated as \ibox{1}, rather than \la\ibox{1}\ra, because the subject is not always realized in Korean. The verbal complex is [LIGHT $+$], and made up of two verbs, also [LIGHT $+$] (see section \ref{GSsection3.3}). 
...
\newpage
\begin{exe}
    \ex[] {\emph{Verbal-complex-phrase} (Korean) $\Rightarrow$ \\
    \begin{avm}
      {\[synsem | cat & \[head 
                             \[\normalfont{\emph{verb}}\\
                                  aux +\]\\
 ...
 [\ibox{2} NP [ku chayk-ul;the book-\textsc{acc}, roof]]
  [V \ms{aux & $+$\\
    ...
    [V \ms{
        ...
        light & $+$}[iss-ta;be.in.the.process.of-\textsc{part}]]]] \end{forest}}
...
\label{GSexemple61}
    \begin{avm}
		\[synsem \ibox{1}\] \,\, \normalfont{precedes}\,  \[comps\, \< \ldots{} \ibox{1} \ldots \>\,\]
...
of PP complements (headed by \emph{to}, \emph{about}, \ldots), which add semantic information describing the situation. The result is a description of an \emph{soa} which conjoins partial descriptions. Similarly here, the conjunction of the two contents is in effect, but, the content of \emph{lagad} (`kick') being more specialized than that of \emph{zadan}, the content of the complex predicate identifies with it. The complement of the complex predicate may be an NP or a PP headed by \emph{be} (`to') (the preposition is optional).
...
Crucially, the mechanism of argument attraction is not tied with a specific syntactic structure; on
the contrary, it is compatible with different structures: it is shown that the properties of a
...
characterize a language as opposed to another one (Spanish restructuring verbs contrast with Italian
...
the combination of the verb with a class of complements. The description in HPSG can exploit the
hierarchical organization of the lexicon and the mechanism of conjuction of descriptions (informally
referred to as unification) (as with combinations specializing the meaning of the verb in Persian), the richness of the feature structure, appealing to a background feature (as when the noun corresponds to an instrument implied in the action), or to a special feature allowing to point to a specific form (for representing idioms).    
