...
Within a hierarchical organization of the lexicon (chapter ?????), the content of the simplex verb is higher and less specialized than that of the predicative noun. Thus, the content of the complex predicate can be simply that of the noun, if it unifies with it. This is reminiscent of the way \cite{Wechsler95b} represents the import of a PP with a verb like \textit{talk}; the verb content itself is represented as an \textit{soa} with one participant, the talker; the verb can take a number of PP complements (headed by \textit{to, about…}), and its content is then unified with that of the PP. The result is a description of an \textit{soa} with more information than is present in the verb alone. Similarly, here, the content of \textit{lagad} is more specialized than that of \textit{zadan}; hence, it takes over if it is unified with it. The complement of the complex predicate may be an NP or a PP headed by \textit{be} (the preposition is optional). 
