...
structures\itd{Stefan: descriptions?} might allow multiple values for the same feature, but with
...
\citeyear{Fodor83a-u} book, \emph{The Modularity of Mind}.\footnote{Much of the psycholinguistic
  ...
  phonology,\itd{Stefan: morphology?} syntax, semantics, and pragmatics, with the operations of each of these sub-modules
...
The local cooccurrence\inlinetodoopt{JP: for later editions: Maybe a word or two on what local means in this
...
evidence and inferred what types of grammatical information was\itd{Stefan: what types was?} essential for processing.
...
type).\itd{JP: Cite \citew{YKD2019a-u}? Stefan: Cite \citew{MWArgSt} Crossref \crossrefchapterw[Section~\ref{prop:sec-lexicon}]{properties}.}
...
\citealt*{HofmeisterEtal2013}; \itd{JP: Cite
  \citew{CP2020a-u}?}) arguing that certain island phenomena should be attributed entirely
...
\itd{Stefan: \citep{WJKP2014a,WP2011a} tested various theories of complex predicates. They found
  that the lexical one that is assumed in HPSG \citep{Mueller2002b} is to be preferred over the one by Goldberg. I find
  this very interesting and I guess it is worth mentioning.}
...
One such case occurs with pairs like \citegen{Chomsky77a-u}\addpages famous minimal pair in
(\ref{ex:chris-please}). 
