...
command):\is{o-command}\is{obliqueness|(}\is{anaphora|(} the list item that corresponds to the
...
with subjects, objects, and obliques\is{oblique argument} \citep{greenberg:1966}.\is{anaphora|)}\is{obliqueness|)} 
...
translations of (\ref{moro}) (from \citealt[9]{Ackermanetal2017}; \attrib{cl} `noun class'; \attrib{sm}
`subject marker).
...
roles\is{semantic role} in the semantics and their
...
embedded within a \attrib{soa} (state of affairs)\is{feature!soa@\textsc{soa} (State of Affairs)}
...
Ranking them in a\is{thematic hierarchy} \emph{thematic hierarchy}, and labeling each of a predicator's semantic roles (e.g., \attrib{eater} and \attrib{food} for the verb \word{eat}) with a unique thematic role (e.g., \attrib{agent} and \attrib{patient} for \word{eat}), then yields an ordering of roles analogous to the ordering on the \argst list.  Indeed, it would not be difficult to import this kind of system into HPSG, as a means of determining the order of elements on the \argst list.  However, HPSG researchers have generally avoided using a thematic hierarchy, for reasons we now briefly set out.
...
The first approach, which goes back to \citet[\page 215]{pollard+sag:1987}, assumes that passivization targets the first member of a \attrib{subcat} list and either removes it or optionally puts it last on the list, but as a PP.
