...
the lexical rule\footnote{%
Using the symbol $\mapsto$ is a common way to represent lexical rules. The AVM to the left of the symbol is the input to the rule (the \textsc{lex-dtr} in (\ref{pass-lxm}) above) and the AVM on the right-hand side is the output of the lexical rule (the complete linguistic object in (\ref{pass-lxm}) above). For a more detailed discussion of lexical rules see \crossrefchaptert[Section~\ref{lexicon-sec-lexical-rules}]{lexicon}.
}). 
