...
\noindent
Phrase structure rules in the form of immediate dominance schemata identify a certain daughter node as the head daughter (\textsc{head-dtr}) and others as complement daughters (\textsc{comp-dtrs}).  In keeping with the \emph{Subcategorization Principle}, here paraphrased from \cite{pollard+sag:1994}:34, list items are effectively ‘cancelled’ from the \subcat list as complement phrases are joined with the selecting head:
