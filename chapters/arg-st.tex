...
In clauses without implicit or affixal arguments, the \textsc{arg-st} is the concatenation of \feat{subj} and \feat{comps} respectively.  For example, the \subcat list for \word{put} in (\ref{subcats}c) is replaced with the following:
...
The third constraint, in (\ref{emb-act-vb-linking}), links the value of an \attrib{act} attribute embedded within a \attrib{soa} (`state of affairs') \is{feature!soa@\textsc{soa} (State of Affairs)} attribute to an NP that is second on \argst.
