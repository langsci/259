...
those \argst list elements to the elements of the  \val (``valence'') lists, namely \comps
(`complements') and \subj (`subject'; or \spr, `specifier'); we will refer to this second mapping as
...
considering \argst itself (Section~\ref{sec:arg-st}), followed by the mapping from \argst to \val
...
subjects, as complement daughters (\textsc{comp-dtrs}).  In keeping with the \emph{Subcategorization
  Principle},\is{Subcategorization Principle} here paraphrased from \cite[34]{pollard+sag:1994},
...
is shown in this figure only for mapping to \val, which represents locally realized phrasal
...
The \argst and \val lists in this figure contain only arguments linked to participant
...
and \val violates the ARP.\todostefan{Figure~\ref{fig:over} has to be redone.}
...
canonical entry and derives a non-canonical one by removing items from the \val lists, while adding
...
a \val list), or ignored by it (hence expressed in some other way).  See the chapter on the lexicon
for more detail and \citet{MillerandSag1997} for a treatment of \ili{French} clitics as affixes.
...
\val lists.  \argst is directly linked to the semantic \content and is the locus of binding theory,
so the presence of a syntactic argument on the \argst list but not a \val list accounts for null
...
option can be captured in a general way by \val and \argst defaults in the lexical hierarchy for
verbs.

\subsection{The syntax of  \texorpdfstring{\argst}{ARG-ST} and its relation to  \texorpdfstring{\textsc{valence}}{VALENCE}}
\label{argst-sec}
...
D-structure in Government/Binding theory, and the Merge positions of arguments in
Minimalism,\is{Minimalism} assuming in the last two cases the Uniform Thematic Alignment Hypothesis
...
syntactic  ones.  Specifically, the list ordering represents relative syntactic \textit{obliqueness}
of arguments.   The least\isi{oblique argument} is the subject (\subj), followed by the complements
...
between \argst and \val in more detail: syntactic ergativity, exemplified by \ili{Balinese}.

\subsection{Syntactic ergativity}
\label{ergativity}\label{arg-st-sec-ergativity}

\is{ergativity|(} The autonomy of \argst from the \val lists is further illustrated by
...
conclusion, neither \val nor \content provides the right representation for defining binding
...
alternative mapping between \argst and \val include \ili{Tagalog}, Inuit, some Mayan languages,
...
\val or \content, those theories lack any analogue of \argst.  This leads to special problems for
...
\val list items, with relative c"=command in the configurational structure corresponding to relative
...
\citet{AMM2013a}, \citet{Ackermanetal2017} propose a partial ordering \is{ARG-ST!partial ordering
...
%Non-canonical 
Looser relationships between \argst and the \val lists are invoked in accounts of several core
...
in any \val list.  Accounts of syntactic ergativity in HPSG involve variations in the mapping
between \argst and \val lists; in particular, the element of \subj is not, in such languages, the
...
\cite{DavisandKoenig2000b}, who develop a more elaborated representation of lexical semantics, with
...
\citet{Dowty1991}, and \citet{Wechsler1995b}, Davis and Koenig note that many of the pertinent
...
Wechsler's constraints based on partial orderings).  The attribute \feat{key}\isfeat{key} selects
...
\itdopt{Stefan: While working on other chapters, I tried to link here for actor and undergoer. I
  ...
  this.JP told me to use ACTOR and UNDERGOER. So I expected here an explanation why these
...
\feat{content} and their syntactic realizations on \argst based on entailments of the semantic
