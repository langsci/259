...
list.\is{feature!valence@\textsc{valence}!as distinct from \argst}\is{feature!arg-st@\textsc{arg-st}!as distinct from \textsc{valence}} In the other
...
(\ref{fig:replace}) and (\ref{fig:subst}) show the \feat{content} values of these two verbs under this account, where \ibox{a} and \ibox{b} abbreviate the structures in (\ref{fig:place}) and (\ref{fig:remove}).
...
structure-sharing and labels such \add{as} \word{final-product} and \word{source material} are informal and added for clarity.)
...
\gll Gino ha  rimproverato Eva tante   volte da    arrabbiarsi.  \\
     Gino has scolded      Eva so.many times so.as to.get.angry  \\\hfill\added{(\ili{Italian})}
...
\gll Eta kniga byla kuplena Boris-om$_{i}$ dlja sebja$_{i}$.  \\
     this book was bought Boris-\textsc{ins} for self  \\\hfill\added{(\ili{Russian})}
...
\ex
\gll dass er  fischt\\
     that he  fishes\\\hfill\added{(\ili{German})}
...
\gll Johanna a fait manger les enfants. \\
     Johanna has made eat the children \\\hfill\added{(\ili{French})}
...
\ex\label{adj}
 \gll
	...
	my friends me have often helped \\\hfill\added{(\ili{French})}
...
\label{mourir}
\gll Il en est mort. \\
     he of.it is dead.\textsc{pfv.pst} \\\hfill\added{(\ili{French})}
\glt `He died of it.'
\z
...
The entries of the \isi{clause union} verbs are simply stipulated to include on their \argst lists the syntatic arguments of their (lexical) verbal arguments in (\ref{faire-ent}); see \crossrefchaptert{complex-predicates} for details on approaches to complex predicates in HPSG.
...
%See \crossrefchapterw[\pageref{order:rel-shuffle}]{order} for more on complex predicates.}
