...
This principle is of course merely a hypothesis which may turn out to be wrong, but so far it seems correct \citep[494]{MuellerGT-Eng2}, and it is more compatible with HPSG than with the innatist ideas underlying Chomskyan linguistics \citep{Berwick:Chomsky2013a-u}. In WG, it plays an important part because it determines other parts of the theory.
...
\ili{Wambaya} sentence, repeated here as (\ref{ex:19}) \parencites[\page 8]{Bender2008a}{Nordlinger1998}:
...
\node[draw](3rd)[right of=grog]{\strut 3sg.pst};
...
\itd{Stefan: use emph rather than underline?}
...
syntax. For WG, however, the solution is easy: the second N token isa the first, which allows
default inheritance. This example illustrates an idiomatic but generalisable version of the NPN
pattern in which the second N isa the first and the meaning is special; as expected, the pattern is
...
(present)). Because the second noun isa the first, if it has a prepositional complement this is also
