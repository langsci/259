...
\node[draw](sd)[above of=sat, node distance=4cm% lower number in case of path thru triangle
]{(sat,down)};
...
\draw[->] (ci) to[in=north, out=west] node[draw,pos=.25,ellipse,fill=white](d2){d2} (mary);
\draw[->] (cisd) to[in=north, out=west] node[draw,pos=.22,ellipse,fill=white](d3){d3} (-.4,.37);
\draw[dashed,->] (sd) to[in=north, out=west] node[draw,pos=.095, ellipse,solid,fill=white](d4){d4} (-.2,1.5)% here could go the specs of the triangle thingy
to (-.2,.37);
\draw[dashed,->] (sat) to[in=north, out=north] node[draw,pos=.36,ellipse,solid,fill=white](d5){d5}
%(x,y)% triangle specs
(.2,.37);
%triangle "arrows"
...
%todo unterste Reihe überm Strich höher, große Kurve steiler
%%%%%%%%%%%
%below dashed line
%first row
\node[draw](grog) at (0,-2){\strut grog};
%\node[draw,circle,inner sep=.2cm](circ)[above right of=grog, node distance=2cm]{};
...
Moreover, even English offers an easy alternative to the covert verb based on the structure where the verb \textsc{be} is overt. It is reasonably uncontroversial to assume a raising analysis for examples such as (\ref{ex:25}) and (\ref{ex:26}), so (\ref{ex:27}) invites a similar analysis \todo{Müller 2013}.
