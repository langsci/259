...
\IfFileExists{../localcommands.tex}{%hack to check whether this is being compiled as part of a collection or standalone
  ...
  \bibliography{../Bibliographies/stmue,../Bibliographies/dg}
...
\citep{Hudson84a-u,Hudson90a-u,Hudson98a-u,Hudson2007a-u,Hudson2010b-u,Gisborne2010, Duran-Eppler2011,Traugott&Trousdale2013}%
%
\todo{forthcoming???}%
%
...
\todo{Imrényi and Vladár forthcoming}%
%
, is in Figure~\ref{fig:3}. The awkward horizontal braces should not be seen as a nod in the direction of classical PS given that the bracketed words are not even adjacent in the sentence analysed. Kern’s tree in Figure~\ref{fig:4}, on the other hand, is for the German sentence in (\ref{ex:2}).
...
Meanwhile, the European ideas about syntactic structure culminating in Kern’s tree diagram developed rather more slowly. Lucien Tesnière in France wrote the first full theoretical discussion of DS in 1939 but it was not published till 1959 \citep{Tesniere59a-u,Tesniere2015a-u}, complete with stemmas looking like the diagrams produced seventy years earlier by Brassai and Kern. Somewhat later, these ideas were built into theoretical packages in which DS was bundled with various other assumptions about levels and abstractness. Here the leading players were from Eastern Europe, where DS flourished: the Russian Igor Mel’čuk \citep{Melcuk88a-u}, who combined DS with multiple analytical levels, and the Czech linguists Petr Sgall, Eva Hajičová and Jarmila Panevova \citep{SgallHajicováPanevova1986}, who included information structure. My own theory Word Grammar (developed, exceptionally, in the UK), also stems from the 1980s \citep{Hudson84a-u,Hudson90a-u,Hudson2007a-u,Hudson2010b-u,Hudson2017a,Sugayama2002,Gisborne2008,Rosta2008,Gisborne2010,Gisborne2011,Duran-Eppler2011,TraugottTrousdale2013,Duran-EpplerLuescherDeuchar2016,Hudson2016,Hudson2017,Gisborne2019}. This is the theory which I compare below with HPSG, but it is important to remember that other DS theories would give very different answers to some of the questions that I raise.
...
Fortunately for DS, there is a solution: create extra word nodes but treat them as a taxonomy, not a partonomy \citep{Hudson2018}. To appreciate the significance of this distinction, the connection between the concepts “finger” and “hand” is a partonomy, but that between “index finger” and “finger” is a taxonomy; a finger is part of a hand, but it is not a hand, and conversely an index finger is a finger, but it is not part of a finger.
...
%arrows
\draw[->] (came) to[in=north, out=north] node[draw,ellipse,fill=white](d1){d1} (.4,.37);
\draw[->] (ci) to[in=north, out=west] node[draw,pos=.2,ellipse,fill=white](d2){d2} (mary);
\draw[->] (cisd) to[in=north, out=west] node[draw,pos=.205,ellipse,fill=white](d3){d3} (-.4,.37);
%
\draw[dashed,->] (sd) to[in=north, out=west] node[draw,pos=.1, ellipse,solid,fill=white](d4){d4} (-.2,1)% here could go the specs of the triangle thingy
to (-.2,.37);
...
These issues have received detailed attention in WG (\citealt[chap.~5]{Hudson84a-u};\citeyear{Hudson88a}; \citeyear[chap.~14]{Hudson90a-u}; \citeyear{Hudson1995}; \citeyear[175-81, 304-7]{Hudson2010b-u}), but they are peripheral to this chapter.
...
%
\draw[->,dashed] (milk.south) to[in=300,out=225] (3.2,-2.37);
\draw[->,dashed] (give.south) to[in=300, out=225] (3.4,-2.37);
\draw[->,dashed] (mother.south) to[in=300, out=225] (3.6,-2.37);
\draw[->,dashed] (grog.south) to[in=south, out=south] (2.8,-2.37);
%arrow with small circle
\draw[->] (2.8,-1.63) to[in=40, out=135] node[draw,circle,fill=white,inner sep=.2cm](circ){} (.2,-1.63);
%
\draw[->] (3rd.north) to[in=north, out=north] (give);
\draw[->] (7.3,-1.63) to[in=north, out=north] (mother);
\draw[->] (7.7,-1.63) to[in=135, out=40] (milk.north);
\draw[->] (9.7,-1.63) to[in=40, out=135] (-.2,-1.63);
...
\draw[->,dashed] (3.5,3.6) to (1c);
\draw[->,dashed] (one) to (1d);
\draw[->,dashed] (one) to[in=south west, out=south east] (aux);
\draw[->,dashed] (1a) to[in=south east, out=south west] (aux);
\end{tikzpicture}
...
Moreover, even English offers an easy alternative to the covert verb based on the structure where the verb \textsc{be} is overt. It is reasonably uncontroversial to assume a raising analysis for examples such as (\ref{ex:25}) and (\ref{ex:26}), so (\ref{ex:27}) invites a similar analysis%
%
\todo[inline]{(Müller 2013)}%
%
.
