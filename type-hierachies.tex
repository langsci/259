...
Stefan: The AVMs should come out north and the atomic types (case, vform, \ldots) too. You did this in the
second example. This is nice. Can this be done in the type hierarchy style?

What does ``calign children'' do?
...
Stefan: Do you know what one can do about the black bars? The figure is scaled to fit the linewidth,
so there should not be any black bars.
...
type hierarchy
  [head
    ...
    [subst
      [noun
        [c-noun]
        [gerund, edge to=!un] % draw an edge to the node one up (u) and the next siebling (n), which
                              % is "relational"
      ]
...
Stefan: Wow! Could you explain why tier=2 is needed? What does the ``before drawing tree'' magic do?

The lines should not touch the boxes as with the type in other trees.
...
Here I would prefer if the dashed lines would not cross any black nodes. So edges from ``all-''
...
The n children=0 stuff is cool. I turned this into a style. =:-)
...
What does the magic code ``before drawing tree'' do?
...
\begin{forest}
  ...
  [case,
    ...
    [p-nom+acc, calign with current % Align this node with its mother (case).
      [p-nom-acc, delay=no edge]    % p-nom-acc is a fake daughter of p-nom+acc.
    ]
    [nom
...
What does the ``delay'' do? Is it the case that without the ``delay'', ``identify'' would not know
that p-nom-acc is a leaf?
...
What is ``before drawing tree'' doing?
...
identify=!N doesn't seem to work.
...
This works without crossing branches, but maybe it would be nicer if noun-poss-cx would be in the
...
The original did apply a bend to the edge. Would this be possible to avoid the overlap?
\newpage
...
What does ``for siblings'' do? What does ``anchor'' do?
...
What does ``Pp'' mean? Isn't P the previous leaf and p the previous sibling. But the previous leaf
is 1sg-pos and it does not have a previous sibling.
